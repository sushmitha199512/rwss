...
public class RwsRefundsDAO {
		...
		try {
			...
			if (type.equals("1")) {
				query = "select a.work_id,a.work_name,to_char(a.admin_date,'dd/mm/yyyy'),a.sanction_amount ,c.programme_name,"
						...
						+ " and TRUNC(b.bank_send_date)=TO_DATE(?,'dd-MM-yyyy')and a.work_id=b.work_id and a.programme_code=c.programme_code and"
						+ " c.programme_code=d.programme_code and a.subprogramme_code=d.subprogramme_code and substr(a.office_code,4,1)=? ";
				...
				query = "select a.work_id,a.work_name,to_char(a.admin_date,'dd/mm/yyyy'),a.sanction_amount ,c.programme_name,"
						...
						+ " and TRUNC(b.state_send_date)=TO_DATE(?,'dd/MM/yy')and a.work_id=b.work_id and a.programme_code=c.programme_code and "
						+ "c.programme_code=d.programme_code and a.subprogramme_code=d.subprogramme_code and substr(a.office_code,4,1)=? ";
				query += " group by a.work_id,a.work_name,to_char(a.admin_date,'dd/mm/yyyy'),a.sanction_amount ,c.programme_name,d.subprogramme_name,a.office_code";
			...
			System.out.println("Exception in getRefundWorks of RwsRefundsDAO -- " + e);
		} finally {
			try {
				if (stmt != null) {
					stmt.close();
	...
	public void getRefundAmount(String work_id, RefundWork refundWork, String tableName) {
		...
		try {
			...
			System.out.println("Exception in getRefundAmount of RwsRefundsDAO -- " + e);
		} finally {
			try {
				if (stmt1 != null) {
					stmt1.close();
	...
	private void isRefundTotalAvialable(String work_id, RefundWork refundWork, String tableName) {
		...
		try {
			...
			long totAmount = 0;
			String query = "select refund_bill_amount from " + tableName + " where work_id=? and refund_flag='p'";
			stmt1 = conn.prepareStatement(query);
			stmt1.setString(1, work_id);
			rs = stmt1.executeQuery();
			while (rs.next()) {
			...
			System.out.println("Exception in isRefundTotalAvialable of RwsRefundsDAO -- " + e);
		} finally {
			try {
				if (stmt1 != null) {
					stmt1.close();
		...
		try {
			...
			if (!checkIsExists(workId, tableName)) {
				String query = "insert into " + tableName
						+ "(work_id,entered_by,update_date,refund_bill_amount,refund_chq_no,refund_chq_dt,remarks) "
				...
				String uQuery = "update " + tableName + " set update_date=" + "to_date(?,"
						+ "'dd/mm/yyyy'),refund_bill_amount=?,refund_chq_no=?,refund_chq_dt=" + "to_date(?," + "'dd-mm-yyyy'),remarks=?"
			...
			System.out.println("Exception in saveRefund of RwsRefundsDAO -- " + e);
		} finally {
			try {
				if (stmt != null) {
					stmt.close();
	...
	public boolean checkIsExists(String workId, String tableName) {
		...
		try {
			...
			System.out.println("Exception in checkIsExists of RwsRefundsDAO-- " + e);
		} finally {
			try {
				if (stmt != null) {
					stmt.close();
	...
	public ArrayList getReleaseDates(String bankOrPao, String finYear, String circleCode, String divCode) {
		...
		if (bankOrPao.equals("1")) {
			...
			query = "select  distinct to_date(state_send_date,'dd/MM/yy') from rws_bill_pao_bridge_det_tbl   where "
					+ " to_char(state_send_date,'yyyy') in (" + finYear.substring(0, 4) + "," + finYear.substring(5, 9)
		...
		try {
			...
			System.out.println("Exception in getFinancialYears of RwsRefundsDAO-- " + e);
		}
		return releaseDates;
	}
	...
	public ArrayList getFinancialYears(String bankOrPao) {
		...
		String query = "";
		if (bankOrPao.equals("1")) {
			query = "select distinct  substr(to_char(SEND_DATE,'dd/mm/yyyy'),7,4)  from RWS_BILL_bank_SETTLEMENT_TBL  where SEND_DATE is not null order by substr(to_char(SEND_DATE,'dd/mm/yyyy'),7,4) desc ";

		} else if (bankOrPao.equals("2")) {

			query = "select distinct  substr(to_char(SEND_DATE,'dd/mm/yyyy'),7,4)  from RWS_BILL_pao_SETTLEMENT_TBL  where SEND_DATE is not null order by substr(to_char(SEND_DATE,'dd/mm/yyyy'),7,4) desc ";
		}
		Connection conn = null;
		PreparedStatement stmt1 = null;
		ResultSet rs1 = null;
	...
	public int forwardRefund(String work_id, String tableName) {
		...
		try {
			...
			System.out.println("Exception in forwardRefund of RwsRefundsDAO-- " + e);
		} finally {
			try {
				if (stmt != null) {
					stmt.close();
	...
	public ArrayList getPrograms() {
		...
		try {
			...
			System.out.println("Exception in getPrograms of RwsRefundsDAO-- " + e);
		}
	...
	public ArrayList getRefundWorksPrograms(String circleCode, String divoffice, String type, String programCode,
		...
		ArrayList refundList = new ArrayList();
		String tableName = "";
		String yr1 = finYear.substring(0, 4);
		String yr2 = "" + (Integer.parseInt(yr1) + 1);
		...
		try {
			...
			if (type.equals("1")) {
				query = "select a.work_id,a.work_name,to_char(a.admin_date,'dd/mm/yyyy'),a.sanction_amount ,c.programme_name,d.subprogramme_name,a.office_code,sum(BILL_AMOUNT_CENTRAL) from rws_work_admn_tbl a,rws_bill_bank_bridge_det_tbl b,rws_programme_tbl c, rws_subprogramme_tbl d where  substr(a.work_id,5,2)='"
						+ circleCode + "' and to_date(b.bank_send_date)>='01-APR-" + yr1
						+ "' and to_date(b.bank_send_date)<'01-APR-" + yr2
						+ "'and a.work_id=b.work_id and a.programme_code in (" + programCode
						+ ") and a.programme_code=c.programme_code and c.programme_code=d.programme_code and a.subprogramme_code=d.subprogramme_code and substr(a.office_code,4,1)='"

						+ divoffice + "'";
				if (releaseDate != null && !releaseDate.equals("0")) {
					query += "	and TRUNC(b.bank_send_date)=TO_DATE('" + releaseDate + "','dd-MM-yyyy')";

				}
				query += " group by a.work_id,a.work_name,to_char(a.admin_date,'dd/mm/yyyy'),a.sanction_amount ,c.programme_name,d.subprogramme_name,a.office_code";
				tableName = "rws_div_work_bank_refund_tbl";
			} else {
				query = "select a.work_id,a.work_name,to_char(a.admin_date,'dd/mm/yyyy'),a.sanction_amount ,c.programme_name,d.subprogramme_name,a.office_code ,sum(BILL_AMOUT_STATE) from rws_work_admn_tbl a,rws_bill_pao_bridge_det_tbl b,rws_programme_tbl c, rws_subprogramme_tbl d where  substr(a.work_id,5,2)='"
						+ circleCode + "' and to_date(b.state_send_date)>='01-APR-" + yr1
						+ "' and to_date(b.state_send_date)<'01-APR-" + yr2
						+ "'and a.work_id=b.work_id and  a.programme_code in (" + programCode
						+ ") and a.programme_code=c.programme_code and c.programme_code=d.programme_code and a.subprogramme_code=d.subprogramme_code and substr(a.office_code,4,1)='"
						+ divoffice + "'";
				if (releaseDate != null && !releaseDate.equals("0")) {
					query += "	and TRUNC(b.state_send_date)=TO_DATE('" + releaseDate + "','dd-MM-yyyy')";
				}
				query += " group by a.work_id,a.work_name,to_char(a.admin_date,'dd/mm/yyyy'),a.sanction_amount ,c.programme_name,d.subprogramme_name,a.office_code";
				tableName = "rws_div_work_pao_refund_tbl";
			}
			...
			System.out.println("Exception in getRefundWorksPrograms of RwsRefundsDAO-- " + e);
		} finally {
			try {
				if (stmt != null) {
					stmt.close();
