			...
			if (stage != null && stage.equals("01")) {
				...
				while (s1.hasMoreTokens()) {
					...
					while (key1.hasMoreTokens()) {
						...
						String qry = "update rws_work_admn_tbl set admin_no='" + value + "' where work_id='" + val1
								+ "' ";
				...
				while (s1.hasMoreTokens()) {
					...
					while (key1.hasMoreTokens()) {
						String value = key1.nextToken();
						String val1 = key1.nextElement().toString();
						String qry = "insert into rws_contractor_selection_tbl  values('" + value
								+ "','','','','','','','','','','','','','" + val1 + "','','','','','','')";
						count += stmt.executeUpdate(qry);
					}
				...
				while (s1.hasMoreTokens()) {
					...
					while (key1.hasMoreTokens()) {
						String value = key1.nextToken();
						String val1 = key1.nextElement().toString();
						String qry = "insert into rws_work_commencement_tbl  values('" + val1 + "',TO_DATE('" + value
								+ "','dd/MM/yyyy'),'','','','','','','')";
						count += stmt.executeUpdate(qry);
					}
				...
				while (s1.hasMoreTokens()) {
					...
					while (key1.hasMoreTokens()) {
						...
						String qry = "insert into rws_work_expenditure_tbl  values('" + val4 + "','" + value + "','"
								+ val1 + "','" + val2 + "','" + val3 + "','','','')";
						count += stmt.executeUpdate(qry);
					}
				...
				while (s1.hasMoreTokens()) {
					...
					while (key1.hasMoreTokens()) {
						String value = key1.nextToken();
						String val1 = key1.nextElement().toString();
						String qry = "insert into rws_work_completion_tbl  values('" + val1 + "','" + value
								+ "','','','','','','','','','','','','','','','','','','','','','','')";
						count += stmt.executeUpdate(qry);
					}
	...
	public static int insertScData(String habcode, String lccode, String lcname) throws Exception {
		...
		try {
			...
			qry1 = "select hab_code,locality_code,locality_name from RWS_SC_ST_MIN_LOCALITY_TBL where hab_code='"
					+ habcode + "' and locality_code='" + lccode + "' ";
			rs1 = stmt1.executeQuery(qry1);
			if (!rs1.next()) {
				query = " insert into RWS_SC_ST_MIN_LOCALITY_TBL(hab_code,locality_code,locality_name)values('"
						+ habcode + "','" + lccode + "','" + lcname + "')";

			} else {

				query = " update RWS_SC_ST_MIN_LOCALITY_TBL set locality_name='" + lcname + "' where hab_code='"
						+ habcode + "' and locality_code='" + lccode + "'";
			}
	...
	public static int insertAssetData(String habcode, String astcode, String lccode, String popserved)
		...
		try {
			...
			if (!rs1.next()) {
				...
				query = " update RWS_ASSET_LOCALITY_SERVE_TBL set population_served='" + popserved
						+ "' where hab_code='" + habcode + "' and asset_code='" + astcode + "' and locality_code='"
						+ lccode + "'";
			}
			count = stmt.executeUpdate(query);
		} catch (Exception e) {
