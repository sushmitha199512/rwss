...
public class rws_work_releases_ee extends Action {
...
ArrayList programs=null;
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		RwsRefundsDAO refundsDAO = new RwsRefundsDAO();
		...
		if (mode != null && mode.equals("data")) {
			...
			RwsRefundActionForm refundForm = (RwsRefundActionForm) form;
			try {
				dataSource = getDataSource(request);
				...
				if (!(user.getUserId() != null
						&& (user.getUserId().equals(Constants.EE_BUDGET)))) {
				...
				if ((circleCode != null && mode1 != null && mode1
					...
					divisions = offices.getDivisions1(refundForm
							.getCircleCode());
					session.setAttribute("divisions", divisions);

				}
				if (mode1 != null && mode1
						.equalsIgnoreCase("finYears")) {
					session.setAttribute("releaseDates", new ArrayList());
					session.setAttribute("programs", new ArrayList());
				...
				if (mode1 != null && mode1
						.equalsIgnoreCase("releaseDates")) {
					circleCode = refundForm.getCircleCode();
					divisionCode = refundForm.getCmbDivision();
					releaseDates = refundsDAO.getReleaseDates(refundForm.getBankOrPao(),refundForm.getFinYear(),circleCode, Integer
							.toString(refundForm.getCmbDivision()));
				...
				if (mode1 != null && mode1.equalsIgnoreCase("getWorks")) {
					...
					refundWorks = refundsDAO.getRefundWorks(circleCode, Integer
							.toString(refundForm.getCmbDivision()), refundForm
							.getBankOrPao(), releaseDate);
					refundForm.setRefundWorks(refundWorks);
				}
				if (mode1 != null && mode1.equalsIgnoreCase("getWorks1")) {
					refundWorks = refundsDAO.getRefundWorksPrograms(circleCode, Integer
							.toString(refundForm.getCmbDivision()), refundForm
							.getBankOrPao(), refundForm.getProgramCode(),refundForm.getFinYear(),refundForm.getProgramCode(),releaseDate);
					refundForm.setRefundWorks(refundWorks);
				}
				if (mode1 != null && mode1.equalsIgnoreCase("save")) {
					...
					int recno = Integer.parseInt(request
					...
					for (int i = 1; i < recno + 1; i++) {
						if (request.getParameter("check" + i) != null) {
							checking[i] = "1";
							String amount = request.getParameter("refundForm["
									+ (i - 1) + "].refundAmount");
							String chequeDate = (request
									.getParameter("refundForm[" + (i - 1)
											+ "].chequeDate") == null ? "-"
									: request.getParameter("refundForm["
											+ (i - 1) + "].chequeDate"));
							
							
							String chequeNo = (request
									.getParameter("refundForm[" + (i - 1)
											+ "].chequeNo") == null
							...
							recCount += refundsDAO
									.saveRefund(
											...
											chequeNo,
											remarks,
											(refundForm.getBankOrPao().equals(
													"1") ? "rws_div_work_bank_refund_tbl"
					...
					if (recCount >= 1) {
						...
						session.setAttribute("message", "Saved Successfully");
						 if(refundForm.getProgramCode().equals("0")){
						refundWorks = refundsDAO.getRefundWorks(circleCode, Integer
								.toString(refundForm.getCmbDivision()), refundForm
								.getBankOrPao(), releaseDate);
						refundForm.setRefundWorks(refundWorks);
				...
				if (mode1 != null && mode1.equalsIgnoreCase("forwardenc")) {
					...
					for (int i = 1; i < recno + 1; i++) {
						if (request.getParameter("check" + i) != null) {
							...
							recCount += refundsDAO
									.forwardRefund(
											request.getParameter("check" + i),(refundForm.getBankOrPao().equals(
													"1") ? "rws_div_work_bank_refund_tbl"
													: "rws_div_work_pao_refund_tbl"));
				...
				System.out.println("Exception in rws_work_releases_ee -- "+e);
			}
		}
