...
public class RwsWorkLinkDAO {
	...
	public String saveWork(RwsWorkLinkActionForm bean, String userId) {
		...
		try {
			conn = RwsOffices.getConn();
			stmt = conn.createStatement();
			String insertQuery = "insert into rws_work_prog_convert_lnk_tbl values(?,?,?,sysdate)";
			PreparedStatement ps = conn.prepareStatement(insertQuery);
			ps.setString(1,bean.getOldWorkId());
			ps.setString(2,bean.getNewWorkId());
	...
	public String updateWork(RwsWorkLinkActionForm bean, String userId) {
		...
		try {
			conn = RwsOffices.getConn();
			stmt = conn.createStatement();
			String updateQuery = "update  rws_work_prog_convert_lnk_tbl  set WORK_ID_NEW=?,ENTERED_BY=?"
					+ ",ENTERED_ON=sysdate where work_id=?";
			PreparedStatement ps =conn.prepareStatement(updateQuery);
			ps.setString(1,bean.getNewWorkId());
			ps.setString(1,userId);
			ps.setString(3,bean.getOldWorkId());
