...
public class rws_spillwork_releases_ee extends Action {
		...
		if (mode != null && mode.equals("data")) {
			...
			try {
				...
				if (mode1 != null && mode1.equalsIgnoreCase("getWorks1")) {
					...
					refundWorks = refundsDAO.getRefundWorksPrograms(circleCode,
							Integer.toString(refundForm.getCmbDivision()), refundForm.getBankOrPao(),
							refundForm.getProgramCode(), refundForm.getFinYear(), refundForm.getProgramCode(),
							releaseDate);
					refundForm.setRefundWorks(refundWorks);
				}
					...
					if (storedToken.equals(token)) {
						...
						for (int i = 1; i < recno + 1; i++) {
							if (request.getParameter("check" + i) != null) {
								...
								String chequeDate = (request
										.getParameter("refundForm[" + (i - 1) + "].chequeDate") == null ? "-"
												: request.getParameter("refundForm[" + (i - 1) + "].chequeDate"));

								String chequeNo = (request.getParameter("refundForm[" + (i - 1) + "].chequeNo") == null
										|| request.getParameter("refundForm[" + (i - 1) + "].chequeNo").equals(" ")
												? "-"
												: request.getParameter("refundForm[" + (i - 1) + "].chequeNo"));
								...
								recCount += refundsDAO.saveRefund(request.getParameter("check" + i), user.getUserId(),
										amount, chequeDate, chequeNo, remarks, tablename);
						...
						if (recCount >= 1) {
							...
							if (refundForm.getProgramCode().equals("0")) {
								refundWorks = refundsDAO.getRefundWorks(circleCode,
										Integer.toString(refundForm.getCmbDivision()), refundForm.getBankOrPao(),
										releaseDate);
								refundForm.setRefundWorks(refundWorks);
							} else {
								refundWorks = refundsDAO.getRefundWorksPrograms(circleCode,
										Integer.toString(refundForm.getCmbDivision()), refundForm.getBankOrPao(),
										refundForm.getProgramCode(), refundForm.getFinYear(),
										refundForm.getProgramCode(), releaseDate);
								refundForm.setRefundWorks(refundWorks);
							}
						} else {
				...
				System.out.println("Exception in rws_spillwork_releases_ee -- " + e);
			}
		}
