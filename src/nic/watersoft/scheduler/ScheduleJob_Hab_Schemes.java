                    ...
                    String Query="select dcode,mcode,panch_code,COVERAGE_STATUS from RWS_HAB_SCHEME_SRC_DATA_TBL";
                 	...
                 	int roplants1=0,roplants2=0,roplants=0;
                 	 while(rs.next()){
                 		...
                 		String updateROPlants="UPDATE RWS_HAB_SCHEME_SRC_DATA_TBL A SET (NO_OF_ROPLANTS)=? where a.panch_code=?";
             			stmt1=conn.createStatement();
             			PreparedStatement ps=conn.prepareStatement(updateROPlants);
                        ps.setInt(1, roplants);
                        ps.setString(2,rs.getString("panch_code"));
             			ps.executeUpdate();
             			stmt1.close();
                 		...
                 		if(rs.getString("COVERAGE_STATUS").equals("NSS")){
                 		 ...
                 		 resultSet.close();
                 			...
                 			String updateNSSCont="UPDATE RWS_HAB_SCHEME_SRC_DATA_TBL A SET (CONTAMINATION_TYPE)=? where a.panch_code=?";
                 			stmt1=conn.createStatement();
                 			 ps=conn.prepareStatement(updateNSSCont);
                 			 ps.setString(1, typeOfContamination);
                 			 ps.setString(2,rs.getString("panch_code"));
                 			ps.executeUpdate();
                 			stmt1.close();
                 		...
                 		if(resultSet.next()){
                 			String updateDivSubdiv="UPDATE RWS_HAB_SCHEME_SRC_DATA_TBL a SET DIVISION_OFFICE_CODE=?,DIVISION_OFFICE_NAME=?,SUBDIVISION_OFFICE_CODE=?,SUBDIVISION_OFFICE_NAME=? where panch_code=?";
                     		stmt1=conn.createStatement();
                     		ps=conn.prepareStatement(updateDivSubdiv);
                     		ps.setString(1,resultSet.getString("DIVISION_OFFICE_CODE"));
                     		ps.setString(2,resultSet.getString("DIVISION_OFFICE_NAME"));
                     		ps.setString(3,resultSet.getString("SUBDIVISION_OFFICE_CODE"));
                     		ps.setString(4,resultSet.getString("SUBDIVISION_OFFICE_NAME"));
                     		ps.setString(5,rs.getString("panch_code"));
                     		ps.executeUpdate();
                     		stmt1.close();
                 		}
                 		resultSet.close();
