 ...
 * <p>
 * Use Java built-in API in complement of Apache Commons Imaging for format not
 * supported by the built-in API.
 *
...
public class ImageDocumentSanitizerImpl implements DocumentSanitizer {
	...
	public boolean madeSafe(File f) {
		...
		try {
			...
			//System.out.println("f--"+ f.canWrite());
			...
			if ((f != null) && f.exists() && f.canRead() && f.canWrite()) {
				...
				//	System.out.println("in sanitation image reader");
					...
					//		System.out.println("in sanitation has next");
						} else {
					...
					//	System.out.println("in sanitation else");
					}
				}
				...
				//	System.out.println("in sanitation read");
				} else {
					originalImage = Imaging.getBufferedImage(f);
				//	System.out.println("in sanitation buffered");
				}
				...
				Image resizedImage = originalImage.getScaledInstance(originalWidth - 1, originalHeight - 1,
						Image.SCALE_SMOOTH);

				// Resize the resized image by adding 1px on Width and Height - In fact set
				// image to is initial size
				Image initialSizedImage = resizedImage.getScaledInstance(originalWidth, originalHeight,
						Image.SCALE_SMOOTH);
				...
				try (OutputStream fos = Files.newOutputStream(f.toPath(), StandardOpenOption.WRITE)) {
					if (!fallbackOnApacheCommonsImaging) {
						...
						case "PNG": {
						//	System.out.println("in sanitation png");
					...
					//	System.out.println("in sanitation image parser");
					}
				...
				// Set state flag
				safeState = true;
			//	System.out.println("in sanitation "+safeState);
			}
		} catch (Exception e) {
			safeState = false;
	//		LOG.warn("Error during Image file processing !", e);
		}
	//	System.out.println("sanitation result-- " + safeState);
		return safeState;
	}
}
