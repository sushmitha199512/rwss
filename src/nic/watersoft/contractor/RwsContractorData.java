...
public int removetenderScheduleA(String workId,DataSource dataSource) throws Exception
{
	...
	try
	{
		conn = dataSource.getConnection();
		stmt = conn.createStatement();
		String qry="DELETE FROM rws_tender_scheduleA_lnk_tbl WHERE WORK_id=?";
		PreparedStatement ps= conn.prepareStatement(qry);
		ps.setString(1, workId);
		rowCount =ps.executeUpdate();
		 
		if(rowCount>0)
			qry ="DELETE FROM rws_tender_scheduleA_tbl WHERE WORK_id=?";
		ps.setString(1, workId);	
		rowCount=ps.executeUpdate();
	}
	catch (Exception e)
...
public int updateTenderNotification(RwsContractor rwsContractor,String officeCode,DataSource dataSource) throws Exception
{
	...
	try
	{
		...
		String query="update RWS_TENDER_NOTIFICATION_TBL set PACKAGE_WORK=?,TENDER_TYPE=?,TENDER_ISSUE_DATE=to_date(?,'dd/mm/yyyy'),TENDER_CLOSE_DATE =to_date(?,'dd/mm/yyyy'),MODE_OF_EXECUTION=?,COST_OF_TENDER_SCH=?,COST_OF_SALES_TAX=?,EST_CONTRACT_VALUE=? where tender_idno=? and office_code=?";
		//System.out.println("query="+query);
		stmt=conn.prepareStatement(query);
		stmt.setString(1, rwsContractor.getTenderType());
		stmt.setString(2, rwsContractor.getTenderType());
		stmt.setString(3, rwsContractor.getIssueDate());
		stmt.setString(4, rwsContractor.getCloseDate());
		stmt.setString(5, rwsContractor.getModeOfExecution());
		stmt.setString(5, rwsContractor.getCostOfTender());
		stmt.setString(6, rwsContractor.getSalesTax());
		stmt.setString(6,rwsContractor.getTenderIdNo());
