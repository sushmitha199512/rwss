...
public class rws_AssetComplaintAttendedAct_frm extends Action
{

    public rws_AssetComplaintAttendedAct_frm()
    {
    }
        ...
        throws ServletException, IOException
    {
        ...
        if(mode != null && mode.equals("attend"))
        {
            try
            {
                ...
                Debug.println("assetname888" + assetname);
        ...
        if(mode != null && mode.equals("get2"))
        {
            ...
            try
            {
                ...
                Debug.println("headOfficeCode*****user" + headOfficeCode);
                Debug.println("circleOfficeCodeget2*****user" + circleOfficeCode);
                ...
                {
                    Debug.println("adminloginadmin*****");
                    circles = complaintMasterData.getCompCircles(headOfficeCode, getDataSource(request));
                    session.setAttribute("circles", circles);
                    Debug.println("adminloginadmin*************&&&&:"+frm.getCircleOfficeCode()+":");
                    if(frm.getCircleOfficeCode() != null && !frm.getCircleOfficeCode().equals(" "))
                    {
                        ...
                        Debug.println("adminloginadmin*********"+district1);
                        request.setAttribute("district1", district1);
                        frm.setDistrictName(district1);
                        if(frm.getCircleOfficeCode() != null || !frm.getCircleOfficeCode().equals(" ") || !circleOfficeCode.equals("00"))
                        {
                            mandals = complaintMasterData.getCompMandals(frm.getCircleOfficeCode(), getDataSource(request));
                            session.setAttribute("mandals", mandals);
                        } else
                        {
                        ...
                        if(frm.getMandal() != null && !frm.getMandal().equals(""))
                        {
                            panchayats = complaintMasterData.getCompPanchayats(frm.getCircleOfficeCode(), frm.getMandal(), getDataSource(request));
                            session.setAttribute("panchayats", panchayats);
                        } else
                        {
                            session.setAttribute("panchayats", new ArrayList());
                        ...
                        if(frm.getPcode() != null && !frm.getPcode().equals(""))
                        {
                            habs = complaintMasterData.getCompHabs(frm.getCircleOfficeCode(), frm.getMandal(), frm.getPcode(), getDataSource(request));
                            session.setAttribute("habs", habs);
                            //Debug.println(habs.size());
                        } else
                        {
