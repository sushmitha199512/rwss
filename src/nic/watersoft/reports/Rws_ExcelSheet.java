 ...
 */
	public boolean createFirstTable() {
		...
		try {
			...
			while (rs1.next()) {
				...
				ps.setString(3,rs1.getString(4));
				ps.setString(4,rs1.getString(5));
				ps.setString(5,rs1.getString(3));
	...
	public boolean createFourthTable() {
		...
		try {
			...
			while (rs1.next()) {
				...
				Query = "insert into WQM_QUALITYPARAMETER values (?,?,?,?,?,?,?,?)";
				PreparedStatement ps =con.prepareStatement(Query);
		...
		try {
			...
			while (rs1.next()) {
				...
				if (sourcetype.substring(19, 21).equals("HP")) {
					...
					if (rs6.next()) {
						upquery = "update FINALSOURCES_TEMP_TBL t set ASSET_CODE=? where source_code=?";
	...
	public void insertTestidCount() {
		...
		try {
			...
			String insqry = "update WQ_Last_Testid_Temp_Tbl set TestId=? where testtype='1'";
	...
	public void insertFTKTestidCount() {
		...
		try {
			...
			String insqry = "update WQ_Last_Testid_Temp_Tbl set TestId=?  where testtype='2'";
			PreparedStatement ps = con.prepareStatement(insqry);
			ps.setInt(1,testid);
			ps.executeUpdate();
			Debug.println("insert qry............" + insqry);
		...
		try {
			...
			while (rs2.next()) {
				...
				try {
					...
					ps.setString(2,main);
					ps.setString(3,workname);
					ps.setString(4,stateid);
					ps.setString(5,dcode);
					ps.setString(6,schemetypeid);
					ps.setString(7,type);
					ps.setString(8,sancyear);
					ps.setString(9,estcost);
					ps.setString(10,revestcost);
					ps.setString(11,goicost);
					ps.setString(12,statecost);
					ps.setString(13,commcont);
					ps.setDouble(14,popcov);
					ps.setString(15, capacity);
					ps.setString(16, service);
					ps.setString(17, exp);
					ps.setString(18, moreexp);
					ps.setString(19, progCode);
					total += ps.executeUpdate();
		...
		try {
			...
			while (rs1.next()) {
				...
				if (!date_of_comm.equals("")) {
					query = "insert into RWS_SchemeSources_TEMP_TBL(SourcesIdOfState,SchemeIdOfState,SourceTypeId,SourceTypeCategoryId,StateID,DistrictId,HabitationId,Location,DateOfCommissioning)values(?,?,?,'',?,?,?,?,to_date(?,'dd/mm/yyyy'))";
					ps3= con.prepareStatement(query);
					ps3.setString(1, rs1.getString(11));
					ps3.setString(2,rs1.getString(1));
					ps3.setString(3, rs1.getString(2));
					ps3.setString(4,rs1.getString(3));
					ps3.setString(5,rs1.getString(4));
					ps3.setString(6,rs1.getString(8));
					ps3.setString(7, location);
					ps3.setString(8, date_of_comm);
				} else if (date_of_comm.equals("")) {
					query = "insert into RWS_SchemeSources_TEMP_TBL(SourcesIdOfState,SchemeIdOfState,SourceTypeId,SourceTypeCategoryId,StateID,DistrictId,HabitationId,Location)values(?,?,?,'',?,?,?,?)";
					ps3.setString(1, rs1.getString(11));
					ps3.setString(2,rs1.getString(1));
					ps3.setString(3, rs1.getString(2));
					ps3.setString(4,rs1.getString(3));
