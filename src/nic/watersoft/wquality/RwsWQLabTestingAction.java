...
import org.apache.struts.action.ActionMapping;
	...
	public class RwsWQLabTestingAction  extends Action{
	...
	public ActionForward execute(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws ServletException,IOException {
			...
			if(rwsUser.getUserId()!= null && !(rwsUser.getUserId().equals(Constants.ADMIN) || rwsUser.getUserId().equals(Constants.LAKH_USER) || Constants.IsWaterQualityUser(rwsUser.getUserId()) == false  || Constants.IsUser(rwsUser.getUserId()) == false)) {
			...
			waterSample.setParamValue(paramValue);
			if(mode == null)
			{
				mode = mode1; 
			}			
			...
			{ 
				...
				{    
					...
					System.out.println("The Exception in RwsWQLabTestingAction =  "+e.getMessage());
				}				
			}
			...
			{		
				...
				{
					Debug.println("An Exception has occured :"+e);
				}
			}
			if(mode.equalsIgnoreCase("parameters")){
			 try{
                     if(request.getParameter("paramValue").equals("13"))
                     {
                        ArrayList parameters=RwsWQualityData.getWQTestParameterCodes13(getDataSource(request),waterSample.getTestCode(),"1");
                        waterSample.setWqtestParams(parameters);
                     }
                     else
                     {
                         ArrayList parameters=RwsWQualityData.getWQTestParameterCodes(getDataSource(request),waterSample.getTestCode(),"1");
                        waterSample.setWqtestParams(parameters);
                     }
			...
			{		
				...
				{
					System.out.println("The exception in RwsWQLabTestingAction Mode selectSource = "+e.getMessage());
				}				
			}if(mode.equals("delete"))
			{
				...
				{
					System.out.println("The exception in RwsWQLabTestingAction Mode delete = "+e.getMessage());
				}				
			}
			if(mode.equals("Deleteftk"))
			{
				...
				{
					System.out.println("The exception in RwsWQLabTestingAction Mode DeleteFtk = "+e.getMessage());
				}
			...
			{  
				...
				{
					System.out.println("The exception in RwsWQLabTestingAction Mode View = "+e.getMessage());
				}
			...
			{			
				...
				session.setAttribute("prhabs",prhabs);
					...
					System.out.println("The exception in RwsWQLabTestingAction Mode viewSources = "+e.getMessage());
				}
			...
			{		
				...
				session.setAttribute("prhabs",prhabs);
					...
					System.out.println("The exception in RwsWQLabTestingAction Mode viewftkSources = "+e.getMessage());
				}				
				mapping.findForward(mode);
			}
			if(mode.equalsIgnoreCase("save"))
			{   
				System.out.println(storedToken+" Inside Save token"+token);
			     ...
			     {    
						...
						daoMessage=rwsWQLabDAO.insertWQLabSamplesAndResults(rwsWQuality,request,waterSample.getPotable());
				...
				{
					System.out.println("The exception in RwsWQLabTestingAction Mode Save = "+e.getMessage());
				}
			...
			if(mode.equalsIgnoreCase("update"))
			{
			   if (storedToken.equals(token)){					 
					DAOMessage daoMessage=null;
					 daoMessage=rwsWQLabDAO.updtaeWQLabSamplesAndResults(request.getParameter("testId"),request.getParameter("testCode"),request.getParameter("recommendations"),request);
					 message=daoMessage.getMessage();
					 request.setAttribute("message",message);
					 reset(waterSample);
				 }else{
			...
			if(mode.equalsIgnoreCase("get"))
			{
				try 
				 {
					...
					form = buildWaterSampleForm2(request.getParameter("testId"),request);					
					  if(RwsWQualityData.checkIsExists(request.getParameter("testId"))){
					  	mode="close";
					  }
					Debug.println("watch2"); 
					}
				...
				{
					Debug.println("The error in rws_waterSample_frm get is=  "+e);		
				}
			}
			if(mode.equalsIgnoreCase("forwardtolab"))
			{
				...
				{
					Debug.println("The error in rws_waterSample_frm forwardtolab is=  "+e);		
				}
			}
