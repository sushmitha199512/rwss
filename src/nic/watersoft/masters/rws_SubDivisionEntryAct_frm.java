...
public class rws_SubDivisionEntryAct_frm extends Action
{
	protected ActionForm buildSubDivisionForm(String hoc,String coc,String doc,String sdoc,HttpServletRequest request) throws Exception 
	{
		...
		RwsMaster rwsMaster=RwsMasterData.getSubDivision(hoc,coc,doc,sdoc,getDataSource(request));	
			if(rwsMaster!=null)
			{
	...
	public ActionForward execute(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws ServletException,IOException
	{
		...
		if(mode.equalsIgnoreCase("data"))
		{
			try
			{
				...
				if(request.getParameter("doc")!=null)
				{
					...
					{
						request.setAttribute("code",RwsMasterData.getSubDivisionCode("1",request.getParameter("coc"),request.getParameter("doc"),getDataSource(request)));
					}
				}
		...
		if(mode.equalsIgnoreCase("view"))
		{
			...
			subdivisions=RwsMasterData.getSubDivisions(getDataSource(request),coc,doc);
		...
		if(mode.equalsIgnoreCase("getmandals"))
		{
			...
			mandalsubdivisions=RwsMasterData.getMandalSubDivisions(getDataSource(request),request.getParameter("coc"),request.getParameter("doc"),request.getParameter("sdoc"));
			...
			try
			{
				session = request.getSession();
			     ...
			     {	
					...
					ans=RwsMasterData.insertSubDivision(rwsMaster,getDataSource(request));					
		...
		if(mode.equalsIgnoreCase("get"))
		{
			try 
			{
				form = buildSubDivisionForm("1",request.getParameter("coc"),request.getParameter("doc"),request.getParameter("sdoc"),request);
			...
			try
			{
				session = request.getSession();
			     ...
			     {	
						...
						rwsMaster.setSdoc(frm.getSdoc());
						rwsMaster.setSdon(frm.getSdon());				
						ans = RwsMasterData.updateSubDivision(rwsMaster,getDataSource(request));
			     }
			}
			catch (Exception e) 
			{
		...
		if(mode.equalsIgnoreCase("delete"))
		{
			try 
			{	
				int del=RwsMasterData.removeSubDivision("1",request.getParameter("coc"),request.getParameter("doc"),request.getParameter("sdoc"),getDataSource(request));	
				if(del>0)
				{
