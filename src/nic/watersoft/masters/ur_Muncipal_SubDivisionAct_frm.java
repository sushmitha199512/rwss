...
public class ur_Muncipal_SubDivisionAct_frm extends Action
{
	protected ActionForm buildMandalSubDivisionForm(String coc,String doc,String sdoc,String dcode,String mcode,HttpServletRequest request) throws Exception 
	{
		...
		RwsMaster rwsMaster=RwsMasterData.getUrbanMandalSubdivision(coc,doc,sdoc,dcode,mcode,getDataSource(request));
	...
	public ActionForward execute(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws ServletException,IOException
	{
		...
		ArrayList censusDistricts = new ArrayList();
		ArrayList censusSubDistricts = new ArrayList();
		ArrayList censusVillages = new ArrayList();
		...
		if(mode.equalsIgnoreCase("data")|| mode.equalsIgnoreCase("disp"))
		{
			try
			{
				//dists=rwsLocation.getUrbanDistricts();
				circles=rwsOffices.getUrbanCircles("2");
				if(request.getParameter("coc")!=null)
				{
					if(!(request.getParameter("coc")).equals(""))
					{
						divisions = rwsOffices.getUrbanDivisions("2",request.getParameter("coc"));
						dists=rwsLocation.getUrbanDistricts();
						String dist=mandalSubDivisionForm.getDistrict();
					//	System.out.println("district : "+dist);
											//mandalSubDivisionForm.setDistrict(request.getParameter("coc"));
						//mandalSubDivisionForm.setDname(rwsLocation.getUrbanDname(request.getParameter("coc")));
						mandals=rwsLocation.getUrbanMandals(dist);
				...
				if(request.getParameter("doc")!=null)
				{
					...
					{
						subdivisions = rwsOffices.getUrbanSubdivisions("2",request.getParameter("coc"),request.getParameter("doc"));
		...
		if(mode.equalsIgnoreCase("data1"))
		{
			try
			{
				...
				dists=rwsLocation.getUrbanDistricts();
				circles=rwsOffices.getUrbanDistricts();
				if(request.getParameter("coc")!=null)
				{
					if(!(request.getParameter("coc")).equals(""))
					{
						mandals = rwsOffices.getUrbanMandals(request.getParameter("coc"));
					}
				}
				/*if(request.getParameter("doc")!=null)
				{
				...
				if(request.getParameter("doc")!=null)
				{
					if(!(request.getParameter("doc")).equals(""))
					{
						villages = rwsOffices.getUrbanVillages(request.getParameter("coc"),request.getParameter("doc"));
					}
				}
				if(request.getParameter("doc")!=null)
				{
		...
		if(mode.equalsIgnoreCase("showData"))
		{
			...
			{
				mandals = rwsOffices.getUrbanMandals(request.getParameter("coc"));
				habs = rwsOffices.getLocalities(request.getParameter("coc"),request.getParameter("doc"),request.getParameter("mandal"));
				session.setAttribute("mandals",mandals);
		...
		if(mode.equalsIgnoreCase("showMandals"))
		{
			...
			{
				Debug.println("error in showMandals:"+e.getMessage());
			}
		}
		...
		if(mode.equalsIgnoreCase("save"))
		{
			try
			{
				...
				rwsMaster.setMandal(mandalSubDivisionForm.getMandal());
				 ...
				 ans=RwsMasterData.insertUpdateMandalSubDivision(rwsMaster,getDataSource(request));
	...
	if(mode.equalsIgnoreCase("view"))
	{
		...
		{ //System.out.println("In View dcode="+(String)request.getAttribute("dcode"));
		    ...
		    if(mandalSubDivisionForm.getDistrict()!=null && !mandalSubDivisionForm.getDistrict().equals("")){	
		       ...
		       mandalSubdivisions=RwsMasterData.getUpdateMandalSubDivisions(getDataSource(request),mandalSubDivisionForm.getDistrict());
		    }else{
		    	...
		    	if(request.getParameter("dcode")!=null && !request.getParameter("dcode").equals(""))
		    		mandalSubdivisions=RwsMasterData.getUpdateMandalSubDivisions(getDataSource(request),request.getParameter("dcode"));
		    	else
		    		mandalSubdivisions=RwsMasterData.getUpdateMandalSubDivisions(getDataSource(request),(String)request.getAttribute("dcode"));
		    }	
	...
	if(mode.equalsIgnoreCase("get"))
	{
		...
		{   //System.out.println("fdd");
			...
			if(request.getParameter("dcode")!=null)
			{
				...
				{
					mandals=rwsLocation.getUrbanMandals(request.getParameter("dcode"));
				}
			 session.setAttribute("mandal",request.getParameter("mandalCode"));
			}
			else{
				mandals=rwsLocation.getUrbanMandals(mandalSubDivisionForm.getDistrict());
	...
	{
			try
			{    //System.out.println("olddcode="+(String)session.getAttribute("coc")+"old mandal="+(String)session.getAttribute("mandal")+"sdoc="+(String)session.getAttribute("sdoc")+"doc="+(String)session.getAttribute("doc")+"coc="+(String)session.getAttribute("coc"));
				 //System.out.println("coc="+mandalSubDivisionForm.getCoc()+"doc="+mandalSubDivisionForm.getDoc()+"sdoc="+mandalSubDivisionForm.getSdoc()+"dcode="+mandalSubDivisionForm.getDistrict()+"mcode="+mandalSubDivisionForm.getMandal());
                 rwsMaster.setCoc(mandalSubDivisionForm.getCoc());
                 rwsMaster.setDoc(mandalSubDivisionForm.getDoc());
                 rwsMaster.setSdoc(mandalSubDivisionForm.getSdoc());
                 ...
                 ans=RwsMasterData.updateUrbanMandalSubDivision((String)session.getAttribute("mandal"),(String)session.getAttribute("sdoc"),(String)session.getAttribute("doc"),(String)session.getAttribute("coc"),rwsMaster,getDataSource(request));
			}
			...
			try{
				...
				divisions = rwsOffices.getUrbanDivisions("2",mandalSubDivisionForm.getCoc());
			    subdivisions = rwsOffices.getUrbanSubdivisions("2",mandalSubDivisionForm.getCoc(),mandalSubDivisionForm.getDoc());
				session.setAttribute("mandals",mandals);
				session.setAttribute("divisions",divisions);
			    session.setAttribute("subdivisions",subdivisions);
			}catch(Exception e){
	...
	if(mode.equalsIgnoreCase("delete"))
	{
		try 
		{
			...
			int del=RwsMasterData.removeUrbanMandalSubDivision(request.getParameter("coc"),request.getParameter("doc"),request.getParameter("sdoc"),request.getParameter("dcode"),request.getParameter("mandalCode"),getDataSource(request));
			if(del>0)
			{
