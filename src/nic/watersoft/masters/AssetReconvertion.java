...
public class AssetReconvertion extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		AssetReconvertionForm myForm=(AssetReconvertionForm)form;
		boolean reset = true;		
		HttpSession session=request.getSession();
		String storedToken = (String)session.getAttribute("csrfToken");
	    String token = request.getParameter("token");	     
		AssetReConvertDAO hd = new AssetReConvertDAO();
		...
		try{
			...
			if(mode!=null && mode.equals("exist")){
				...
				assets=hd.getAssets(myForm.getHabCode());
				session.setAttribute("assets",assets);			
			}
			...
			if(mode!=null && mode.equals("convert")){
				 if (storedToken.equals(token)){	
					...
					String[] assecodes = myForm.getAssetCode();
					if(habCodeOld==null)
					...
					String message1 = hd.covertOldHabToNewHab(myForm,request);
			...
			System.out.println("The Exception in AssetReconvertion is "+e.getMessage());			
		}
