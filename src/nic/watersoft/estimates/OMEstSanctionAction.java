...
public class OMEstSanctionAction extends Action
{
					...
					HttpServletResponse response) throws Exception 
	{
		...
		//System.out.println("In OMEstSanctionAction ..");
		...
		{	
			...
			if(workIds != null && workIds.length > 0)
			{
				...
				if(isAllowed)
				{
					if(estimateDAO.omSanction(workIds, estimateDTO))
		...
		{	
			...
			if(workIds != null && workIds.length > 0)
			{
				...
				if(isAllowed)
				{
					if(estimateDAO.omSubmit(workIds, estimateDTO))
