...
<table border = 1 cellspacing = 0 cellpadding = 0 width="100%"  bordercolor=#000000 style="border-collapse:collapse" bgcolor="#ffffff" ALIGN=CENTER>
	<caption>
		...
		<font color="#663899" face="Maiandra GD" size="4px"><b>Select Type:<font color="red">*</font>
					<SELECT name="mode" id="mode" style="width: 140px" onchange="this.form.submit()">
					   	<% if(mode != null && !mode.equals("") && mode.equalsIgnoreCase("san4")){ %>
						     <option value="san4" selected><font color="#889900" face="Maiandra GD" size="2px">>4 years</font></option>
						<% } else {%>
							 <option value="san4" ><font color="#889900" face="Maiandra GD" size="2px">>4 years</font></option>
						<% } if(mode != null && !mode.equals("") && mode.equalsIgnoreCase("san3")){ %>
							 <option value="san3" selected>>3 years</option>
						<% } else {%>
							 <option value="san3"><font color="#889900" face="Maiandra GD" size="2px">>3 years</font></option>
						<% } if(mode != null && !mode.equals("") && mode.equalsIgnoreCase("san2")){ %>
						     <option value="san2" selected>>2 years</option>
						 <%} else {%>
						     <option value="san2"><font color="#889900" face="Maiandra GD" size="2px">>2 years</font></option>
						<% } %>
	...
	<tr bgcolor="#8A9FCD" align="center">
		...
		<%if(mode.equals("san4")) {	%>
		Sanctioned > 4 
		<%}else if(mode.equals("san3")) {%> 
		Sanctioned > 3 
		<%}else if(mode.equals("san2")) {%> 
		Sanctioned > 2 
		<%}%> 
	...
	<%try
      ...
      // System.out.println("Programme codes"+prog_codes);
       ...
       qry="SELECT distinct b.dname,d.DIVISION_OFFICE_NAME,a.work_id,WORK_NAME ,to_char(ADMIN_DATE,'dd/mm/yyyy'),to_char(c.GROUNDING_DATE,'dd/mm/yyyy'),SANCTION_AMOUNT,pr.programme_name,to_char(TARGET_DATE_COMP,'dd/mm/yyyy')  FROM RWS_WORK_ADMN_TBL a,rws_district_tbl b,rws_work_commencement_tbl c,rws_division_office_tbl d,rws_programme_tbl pr  where pr.programme_code=Substr(a.work_id,1,2) and pr.programme_code in ("+prog_codes+") and substr(a.work_id,5,2)=b.dcode and substr(a.work_id,5,2)=d.CIRCLE_OFFICE_CODE  and substr(a.office_code,4,1)=d.division_office_code and  a.work_id=c.work_id and work_cancel_dt is null  and a.work_id not in (select work_id from rws_work_completion_tbl) ";
       	if(mode.equals("san4")) {	
    	    qry+=" and sysdate - ADMIN_DATE>4*365   and ADMIN_DATE is not null"; 
		}else if(mode.equals("san3")) {
			qry+=" and sysdate - ADMIN_DATE>3*365   and ADMIN_DATE is not null";
		}else if(mode.equals("san2")) { 
			qry+=" and sysdate - ADMIN_DATE>2*365   and ADMIN_DATE is not null"; 
		}      
