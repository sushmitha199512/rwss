...
<tr bgcolor="#8A9FCD" align="center">
		<td align="center" class="rptHeading" >SVS Works(s) Admin Sanctioned - Not Commenced with >1 year(s) of Time Period </td><td align="right" class="rptHeading" >*** Rupees in Lakhs</td>
</tr>
</table>
<table border =0 cellspacing = 0 cellpadding = 0 width="85%" bordercolor=#000000 style="border-collapse:collapse" bgcolor="#ffffff" align = "center">
<tr align=center>
...
<td class="gridhdbg" ><b>Sanction Amount   </b></td>  
	...
	try
      { int count=1;
       ...
       qry="SELECT  b.dname,d.DIVISION_OFFICE_NAME,a.work_id,WORK_NAME ,to_char(ADMIN_DATE,'dd/mm/yyyy'),nvl(to_char(c.GROUNDING_DATE,'dd/mm/yyyy'),'-'),nvl(SANCTION_AMOUNT,'0'),p.programme_name,sp.subprogramme_name FROM RWS_WORK_ADMN_TBL a,rws_district_tbl  b,rws_work_commencement_tbl c,rws_division_office_tbl d ,rws_programme_tbl p,rws_subprogramme_tbl sp where a.work_id=c.work_id  and substr(a.work_id,5,2)=b.dcode and substr(a.work_id,5,2)=d.CIRCLE_OFFICE_CODE  and substr(a.office_code,4,1)=d.division_office_code and substr(a.work_id,1,2)=p.programme_code and substr(a.work_id,1,2)=sp.programme_code and substr(a.work_id,3,2)=sp.subprogramme_code and ADMIN_DATE is  not null and sysdate - ADMIN_DATE>365  and a.category_code=2  and c.GROUNDING_DATE is null  and  work_cancel_dt is null  group by  b.dname,a.work_id,WORK_NAME ,to_char(ADMIN_DATE,'dd/mm/yyyy'),nvl(to_char(c.GROUNDING_DATE,'dd/mm/yyyy'),'-'),nvl(SANCTION_AMOUNT,'0'),d.DIVISION_OFFICE_NAME ,p.programme_name,sp.subprogramme_name order by b.dname";
