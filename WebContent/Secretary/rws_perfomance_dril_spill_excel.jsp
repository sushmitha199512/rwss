...
String expQuery="SELECT sum(case when to_date(voucher_dt)< '01-Apr-"+yr+"' then voucher_amt else 0 end)||'@'||sum(case when (to_date(voucher_dt)>= '01-Apr-2012' and to_date(voucher_dt)< '01-"+cmonth+"-2012') then voucher_Amt else 0 end) ||'@'||sum(case when (to_date(voucher_dt)>= '01-"+cmonth+"-2012' and to_date(voucher_dt)< '01-"+nextmonth+"-2012') then voucher_Amt else 0 end)   FROM rws_work_exp_voucher_tbl a where a.work_id='"+rs1.getString(3)+"'";
