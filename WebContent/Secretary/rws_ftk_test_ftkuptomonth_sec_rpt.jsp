...
<tr bgcolor="#8A9FCD" ">
		<td align="center" class="rptHeading" colspan=24>Water Quality tests conducted through FTK(<%=session.getAttribute("financialyr")%>)<font color="red"></font></td>	
	</tr>
<tr >
...
String uptoftk="SELECT COUNT(*),substr(source_code,1,2)  FROM RWS_FTK_TESTING_TBL a WHERE to_date(a.TEST_DATE)>='1-APR-"+yr+"' and  to_date(TEST_DATE)<'1-"+prevMonth+"-"+yr+"' " ;

uptoftk+="group by substr(source_code,1,2) order by 1";
...
///System.out.println("uptoftk******"+uptoftk);
	...
	while(rs2.next())
	{
		uptoftkmonth.put(rs2.getString(2),rs2.getString(1));
	}


//during
String duringftkbe="SELECT COUNT(*),substr(source_code,1,2)   FROM RWS_FTK_TESTING_TBL  WHERE to_date(TEST_DATE)>='1-"+prevMonth+"-"+yr+"' and  to_date(TEST_DATE)<'1-"+cmonth+"-"+yr+"' ";

duringftkbe+="  group by substr(source_code,1,2)  order by 1";
...
//System.out.println("uptoftk******"+duringftkbe);
	...
	while(rs2.next())
	{
		duringftkbemt.put(rs2.getString(2),rs2.getString(1));
	}



	//during current
	String duringcurent="SELECT COUNT(*),substr(source_code,1,2)    FROM RWS_FTK_TESTING_TBL  WHERE to_date(TEST_DATE)>='1-"+cmonth+"-"+yr+"' ";
	duringcurent+="  group by substr(source_code,1,2) order by 1";
	...
	//System.out.println("duringcurent******"+duringcurent);
		...
		while(rs2.next())
		{
			duringftkcurent.put(rs2.getString(2),rs2.getString(1));
		}
...
while(rs1.next()){
	...
	String uptoftkmon=(String)uptoftkmonth.get(rs1.getString(2));
	
	String duringbemt=(String)duringftkbemt.get(rs1.getString(2));
	String duringftkcure=(String)duringftkcurent.get(rs1.getString(2));
	//System.out.println("duringbemt"+duringbemt);

	//tot=uptoftkmon+duringbemt+duringftkcure;
...
<td class=rptValue ><%= uptoftkmonth.get(rs1.getString(2))==null?"0":(upto=(Integer.parseInt((String)uptoftkmonth.get(rs1.getString(2))))) %></td>
<td  class=rptValue><%= duringftkbemt.get(rs1.getString(2))==null?"0":(during=(Integer.parseInt((String)duringftkbemt.get(rs1.getString(2))))) %></td>
<td  class=rptValue><%= duringftkcurent.get(rs1.getString(2))==null?"0":(duringaf=(Integer.parseInt((String)duringftkcurent.get(rs1.getString(2))))) %></td>
<%
tot=upto+during+duringaf;
duringaf=0;
