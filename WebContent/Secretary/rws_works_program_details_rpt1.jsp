...
{ %>
			<tr><td align="right" class="gridhdbg"><a href="/pred/home.jsp?loginMode=watersoft">Home|</a></td>
				<td class="gridhdbg"><a href="javascript:onclick=history.go(-1)">Back|</a></td></tr>
<%  } else {%>
	<tr><td align="right" class="gridhdbg"><a href="#" onclick="window.close()">Close</td></tr>
<%} %>
		</table>
	</caption>
	<caption><b>Amount in Rs. lakhs</b></caption>
   <tr><td class="gridhdbg" align="center"><font  color="red" ><b>Note:Based On the Physical  Completion date.</b></font></td></tr>
	</table>
<table cellspacing = 0 cellpadding = 0 height=5% width="100%" bordercolor=#000000 style="border-collapse:collapse" bgcolor="#ffffff" align = "center">
<tr >
...
<tr>
 ...
 <td  class=gridhdbg align="center"  rowspan=2>Releases During <br><%=financialyr %></td>	
		<td  class=gridhdbg align="center" rowspan=2 >Expenditure During <br><%=financialyr %></td>	
		<td  class=gridhdbg align="center"  rowspan=2>No.of.Works Completed<br><%=financialyr %></td>
...
<td  class=gridhdbg align="center"  rowspan=2 >Habs Covered during <br><%=financialyr %></td>	
<td class=gridhdbg align="center"  colspan=3>No.of Works</td>
		<td  class=gridhdbg align="center"  rowspan=2>Est.Cost</td>
<td  class=gridhdbg align="center"  rowspan=2 >Releases During <br><%=financialyr %></td>
		<td  class=gridhdbg align="center"   rowspan=2>Expenditure During <br><%=financialyr %></td>	
...
newWorksCompQuery+="and  w.programme_code in ("+programe+")  and w.work_id  in (select work_id from rws_work_completion_tbl where (to_date(DATE_OF_COMPLETION) > '01-Apr-"+startyr+"') and (to_date(DATE_OF_COMPLETION) < '01-APR-"+endyr1+"')) group by  substr(office_code,2,2)  order by 1";
		stmt3=conn.createStatement();
//System.out.println("New Works"+newWorksCompQuery);
	...
	String newWorksExpQuery="select substr(office_code,2,2),sum(case when to_date(voucher_dt)< '01-Apr-"+startyr+"' then voucher_amt else 0 end)||'@'||sum(case when (to_date(voucher_dt)>= '01-Apr-"+startyr+"' and to_date(voucher_dt)< '01-APR-"+endyr1+"') then voucher_Amt else 0 end) FROM RWS_WORK_ADMN_TBL W ,rws_work_exp_voucher_tbl a  where  (w.ADMIN_DATE >= '01-Apr-"+startyr+"' and w.ADMIN_DATE < '01-Apr-"+endyr1+"') and work_cancel_dt is null ";
 if(scheme!=null && !scheme.equals("")){
...
String newCurRelBankQuery="select substr(W.office_code,2,2),sum(case when (to_date(BANK_SEND_DATE)>= '01-APR-"+startyr+"' and to_date(BANK_SEND_DATE) < '01-APR-"+endyr1+"') then BILL_AMOUNT_CENTRAL else 0 end)  FROM RWS_WORK_ADMN_TBL W ,RWS_bill_bank_bridge_det_TBL a  where  (w.ADMIN_DATE >= '01-Apr-"+startyr+"' and w.ADMIN_DATE < '01-Apr-"+endyr+"') and work_cancel_dt is null ";
 if(scheme!=null && !scheme.equals("")){
...
String newCurRelpaoQuery="select substr(W.office_code,2,2),sum(case when (to_date(STATE_SEND_DATE)>= '01-APR-"+startyr+"' and to_date(STATE_SEND_DATE) < '01-APR-"+endyr1+"') then BILL_AMOUT_STATE else 0 end) FROM RWS_WORK_ADMN_TBL W ,RWS_bill_pao_bridge_det_TBL a  where  (w.ADMIN_DATE >= '01-Apr-"+startyr+"' and w.ADMIN_DATE < '01-Apr-"+endyr1+"') and work_cancel_dt is null ";
 if(scheme!=null && !scheme.equals("")){
...
newWoksCoveredHabs+=" and  w.programme_code in ("+programe+")  and w.work_id in(select work_id from rws_work_completion_tbl where (to_date(DATE_OF_COMPLETION) > '01-Apr-"+startyr+"') and (to_date(DATE_OF_COMPLETION) < '01-APR-"+endyr1+"')) group by  substr(office_code,2,2)  order by 1";
		stmt3=conn.createStatement();
...
if(scheme!=null && !scheme.equals("")){
 ...
 spillOverComp+="and  w.work_id  in (select work_id from rws_work_completion_tbl where (to_date(DATE_OF_COMPLETION) > '01-Apr-"+startyr+"') and (to_date(DATE_OF_COMPLETION) < '01-APR-"+endyr1+"')) group by  substr(office_code,2,2)  order by 1";
...
if(scheme!=null && !scheme.equals("")){
 ...
 spillOverHabsCovered+="and  w.work_id  in (select work_id from rws_work_completion_tbl where (to_date(DATE_OF_COMPLETION) > '01-Apr-"+startyr+"') and (to_date(DATE_OF_COMPLETION) < '01-APR-"+endyr1+"')) group by  substr(office_code,2,2)  order by 1";
...
String spillExpQuery="SELECT substr(office_code,2,2),sum(case when to_date(voucher_dt)< '01-Apr-"+startyr+"' then voucher_amt else 0 end)||'@'||sum(case when (to_date(voucher_dt)>= '01-Apr-"+startyr+"' and to_date(voucher_dt)< '01-APR-"+endyr1+"') then voucher_Amt else 0 end)  FROM RWS_WORK_ADMN_TBL W ,rws_work_exp_voucher_tbl a where to_date(w.ADMIN_DATE) <'01-Apr-"+startyr+"'   and a.work_id=w.work_id    and w.programme_code in ("+programe+") ";
...
String spillRelBankQuery="SELECT substr(W.office_code,2,2),sum(case when (to_date(BANK_SEND_DATE)>= '01-APR-"+startyr+"' and to_date(BANK_SEND_DATE) < '01-APR-"+endyr1+"') then BILL_AMOUNT_CENTRAL else 0 end)  FROM RWS_WORK_ADMN_TBL W ,RWS_bill_bank_bridge_det_TBL a where to_date(w.ADMIN_DATE) <'01-Apr-"+startyr+"'   and a.work_id=w.work_id    and w.programme_code in ("+programe+") ";
...
String spillRelPaoQuery="SELECT substr(W.office_code,2,2),sum(case when (to_date(STATE_SEND_DATE)>= '01-APR-"+startyr+"' and  to_date(STATE_SEND_DATE) < '01-APR-"+endyr1+"') then BILL_AMOUT_STATE else 0 end)  FROM RWS_WORK_ADMN_TBL W ,RWS_bill_pao_bridge_det_TBL a where to_date(w.ADMIN_DATE) <'01-Apr-"+startyr+"'   and a.work_id=w.work_id    and w.programme_code in ("+programe+") ";
...
%>
<td  class="<%=style %>" style="text-align: right;"><%=dname %></td>
<% }
else{
%>
...
<td class="<%=style %>" style="text-align: right;" ><a href="#" onclick="viewWorks('rws_perfomance_dril_spill_rpt1.jsp?cat=2&type=1&ccode=<%=rs1.getString(1) %> ')"><%= spillOverSVS.get(rs1.getString(1))==null?"0":(ssvs=(Integer.parseInt((String)spillOverSVS.get(rs1.getString(1))))) %></a></td>
<td class="<%=style %>" style="text-align: right;" ><a href="#" onclick="viewWorks('rws_perfomance_dril_spill_rpt1.jsp?cat=1&type=1&ccode=<%=rs1.getString(1) %> ')"><%= spillOverMVS.get(rs1.getString(1))==null?"0":(smvs=(Integer.parseInt((String)spillOverMVS.get(rs1.getString(1))))) %></a></td>
<td class="<%=style %>" style="text-align: right;" > <a href="#" onclick="viewWorks('rws_perfomance_dril_spill_rpt1.jsp?type=1&ccode=<%=rs1.getString(1) %> ')"><%= spillOver.get(rs1.getString(1))==null?"0":(tsworks=(Integer.parseInt((String)spillOver.get(rs1.getString(1))))) %></a></td>
...
<td class="<%=style %>" style="text-align: right;"><a href="#" onclick="viewWorks('rws_perfomance_dril_spill_rpt1.jsp?cat=2&type=2&ccode=<%=rs1.getString(1) %>')"><%= newWorksSVS.get(rs1.getString(1))==null?"0":(nsvs=Integer.parseInt((String)newWorksSVS.get(rs1.getString(1)))) %></a></td>
<td class="<%=style %>" style="text-align: right;"><a href="#" onclick="viewWorks('rws_perfomance_dril_spill_rpt1.jsp?cat=1&type=2&ccode=<%=rs1.getString(1) %>')"><%= newWorksMVS.get(rs1.getString(1))==null?"0":(nmvs=Integer.parseInt((String)newWorksMVS.get(rs1.getString(1)))) %></a></td>
<td class="<%=style %>" style="text-align: right;"> <a href="#" onclick="viewWorks('rws_perfomance_dril_spill_rpt1.jsp?type=2&ccode=<%=rs1.getString(1) %>')"><%= newWorks.get(rs1.getString(1))==null?"0":(tnworws=Integer.parseInt((String)newWorks.get(rs1.getString(1)))) %></a></td>
<td class="<%=style %>" style="text-align: right;" ><%= newWorksEst.get(rs1.getString(1))==null?"0":(tsancAmount=Double.parseDouble((String)newWorksEst.get(rs1.getString(1)))) %></td>
...
%>
<tr><td colspan=2 class=gridhdbg >Total:</td>
