...
String divCode=request.getParameter("dcode");
if(divCode!=null){
	session.setAttribute("dcode",divCode);
}
...
String drill=request.getParameter("drill");
 ...
 %>
<form>
<table  cellspacing = 0 cellpadding = 0 width="100%"  bordercolor=#000000 style="border-collapse:collapse" bgcolor="#ffffff" ALIGN=CENTER>
<caption>
...
<tr align="center">
		<td align="center" class="gridhdbg" ></td><td  class="gridhdbg" >*** Rupees in Lakhs</td>
</tr>
</table>
...
<td class=gridhdbg rowspan=2 ><b>Habitation Name </b></td>
...
<td align="center" class=gridhdbg>21</td>
...
String district=request.getParameter("district");
  ...
  hh="select mcode,mname,dcode from rws_mandal_tbl where dcode='"+circleOfficeCode +"'"; 
	...
	/*
	              Habitation Names
	*/
	 String habQuery="select panch_name,panch_code from rws_complete_hab_view ch  where substr(panch_code,1,2)='"+circleOfficeCode +"'"; 
	...
	while(p2.next())
	{
		chabView.put(p2.getString(2),p2.getString(1));
	}
...
if(scheme!=null && !scheme.equals("")){
 ...
 query+=" and  w.work_id not in (select work_id from rws_work_completion_tbl where (to_date(DATE_OF_COMPLETION) < '01-Apr-"+startyr+"')) ";
	...
	query="select d.dname,b.DIVISION_OFFICE_NAME,w.work_id,w.WORK_NAME ,to_char(ADMIN_DATE,'dd/mm/yyyy'),nvl(SANCTION_AMOUNT,'0'),w.category_code,w.AUG_WORKID,p.programme_name,nvl(w.remarks,'-'),nvl(LEAD_HABITATION,'-'),substr(lead_habitation,1,2),substr(lead_habitation,6,2) FROM RWS_WORK_ADMN_TBL W ,rws_division_office_tbl b ,rws_district_tbl  d,rws_programme_tbl p  where substr(w.work_id,5,2)=d.dcode and  substr(w.work_id,1,2)=p.programme_code and (w.ADMIN_DATE >= '01-Apr-"+startyr+"' and w.ADMIN_DATE < '01-"+cmonth+"-"+endyr1+"') and work_cancel_dt is null and substr(w.office_code,2,2)=b.circle_office_code and b.division_office_code=substr(w.office_code,4,1)  and   b.circle_office_code='"+circleOfficeCode +"' ";
 if(scheme!=null && !scheme.equals("")){ query+="and w.type_of_asset in("+scheme+")";  }
 ...
 String sw=(String)hp5.get(rs1.getString(12)+rs1.getString(13));
	...
	String habName=(String)chabView.get(rs1.getString(11));
...
<td class="<%=style %>" style="text-align: left;"><%=sw%> </td>    
...
<td class="<%=style %>" style="text-align: left;" ><%=c %>  </td> 
   <td class="<%=style %>" style="text-align: right;" ><%=ndf.format(explast) %> </td>
		<td class="<%=style %>" style="text-align: right;" > <%=ndf.format(bal=Double.parseDouble(rs1.getString(6))- explast) %></td> 
		<td class="<%=style %>" style="text-align: right;" > <%=ndf.format(explastMonth) %></td> 
	<td class="<%=style %>" style="text-align: right;" > <%=ndf.format(expcurMonth) %></td> 

   
   <td class="<%=style %>" style="text-align: left;" ><%=DateOfGround%>  </td>
<td class="<%=style %>" style="text-align: left;" ><%=DateOfCompl%>  </td>
      		<td class="<%=style %>" style="text-align: right;" > <a href="rws_perfomance_dril_habs_rpt.jsp?workid=<%=rs1.getString(3)%>&type=1"><%=habsSanctioned %></a> </td> 
                    		<td class="<%=style %>" style="text-align: right;" ><a href="rws_perfomance_dril_habs_rpt.jsp?workid=<%=rs1.getString(3)%>&type=2"> <%=habsCovered %></a> </td> 
...
<td class=gridhdbg><%=ndf.format(gsanamt) %></td>
<td align=center>-</td>
<td class=gridhdbg><%=ndf.format(gexplast) %></td>
<td class=gridhdbg><%=ndf.format(gbal) %></td>
<td class=gridhdbg><%=ndf.format(gexplastMonth) %></td>
<td class=gridhdbg><%=ndf.format(gexpcurMonth) %></td>
<td></td>
<td></td>
<td class=gridhdbg><%=ndf.format(ghabsSanctioned) %></td>
<td class=gridhdbg><%=ndf.format(ghabsCovered) %></td>
<tr>
