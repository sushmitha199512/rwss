...
<%String astprocode=(String)session.getAttribute("apid"); 
String index=(String)session.getAttribute("asstindex");
String atype=(String)session.getAttribute("dtype");
// System.out.println("show assets page index............."+index+"atype..."+atype);
String pagetype="";
if(session.getAttribute("pagetype")!=null)
{
pagetype=(String)session.getAttribute("pagetype");
}
else
pagetype="";
...
//	document.forms[0].action="switch.do?prefix=/proposals&page=/AdminstrativeSanction.do?mode=getDivisions1";
    ...
    var asstindex='<%=index%>';
    // alert("1111index"+asstindex);
  
	document.forms[0].action="switch.do?prefix=/drought&page=/AdminstrativeSanction.do?mode=getAssets&someCond1=div&asstindex="+asstindex;
	document.forms[0].submit();
...
{  
    var asstindex='<%=index%>'; 
   //  alert("2222index"+asstindex);
    document.forms[0].action="switch.do?prefix=/drought&page=/AdminstrativeSanction.do?mode=getAssets&someCond1=subdiv&asstindex="+asstindex;
	document.forms[0].submit();
}
function getData()
{   
    var asstindex='<%=index%>';
    //  alert("3333index"+asstindex);
	...
	{
	document.forms[0].action="switch.do?prefix=/drought&page=/AdminstrativeSanction.do?mode=getAssets&someCond=yes&someCond1=yes&asstindex="+asstindex;
	document.forms[0].submit();
	}
}
function initFunc()
{ // alert("44444index"+asstindex);
...
{       
       ...
       //  alert("get parameters....................."+pagetypevalue);
  
       //  alert("5555index"+asstindex);
		var testval=document.getElementById("LABTEST_LIST["+index+"].workid").value;
        // alert("hhhhhhhhhhhhhh"+testval);
		if(that.checked)
		{
			var openerDoc = opener.document;
			var frm = openerDoc.forms[0];
			frm.action = "switch.do?prefix=/drought&page=/AdminstrativeSanction.do?mode=getAssetCode&astcode="+testval+"&asstindex="+asstindex +"&pagetypevalue="+pagetypevalue;
			frm.submit();
			window.close();
		}
	}
...
<jsp:include page="/commons/TableHeader.jsp">
	<jsp:param name="TableName" value="Assets List" />
