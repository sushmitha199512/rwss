...
function viewReport(){
	var url="./masters/rws_bill_selected_to_encpao_rpt.jsp";
...
var mnth=document.forms[0].elements['monthCode'].value;;
 ...
 if(mnth=="0"){
	...
	document.forms[0].action="switch.do?prefix=/fundAccounting&page=/PAOBillAction.do&mode=billsList";	
	document.forms[0].submit();
}

function getDivisions(){
	
	document.forms[0].action="switch.do?prefix=/fundAccounting&page=/PAOBillAction.do&mode=divisions";	
	document.forms[0].submit();
...
function fnCalStatePer(val)
{
	...
	if(parseFloat(document.forms[0].elements["billsList["+val+"].recomandedState"].value)>parseFloat(document.forms[0].elements["billsList["+val+"].recomandedActual"].value))
	{
		document.forms[0].elements["billsList["+val+"].recomandedStatePer"].value = '0';
		document.forms[0].elements["billsList["+val+"].recomandedState"].value='0';
		alert("State Amount of Now Recommended Should be in Less than or equal to Actual Amount of Now Recommended");
	}
	else
	{
		var value = ((parseFloat(document.forms[0].elements["billsList["+val+"].recomandedState"].value))/(parseFloat(document.forms[0].elements["billsList["+val+"].dueActual"].value)))*100;
...
function fnCalState(val)
{
	...
	else
	{
		...
		{
			document.forms[0].elements["billsList["+val+"].recomandedState"].value =  Math.round((parseFloat(value))*Math.pow(10,2))/Math.pow(10,2);
		}
	}
	fnDisable(val);
}	
...
var flag=true;
	...
	for(var k=0;k<length;k++)
		...
		if(document.forms[0].elements["billsList["+k+"].checks"].checked)
		 {
			if(parseFloat(document.forms[0].elements["billsList["+k+"].recomandedState"].value)=="0.0"){
				
				alert("Record("+(k+1)+") Recommended State amount is mandatory or Should be more than 0");
				document.forms[0].elements["billsList["+k+"].recomandedState"].focus=true;
				return;
			}
			if(parseFloat(document.forms[0].elements["billsList["+k+"].recomandedState"].value)>parseFloat(document.forms[0].elements["billsList["+k+"].dueActual"].value)){
				alert("Record("+(k+1)+")     Recommended State Amount should not be more than Bill Amount Due in State.");
				document.forms[0].elements["billsList["+k+"].recomandedState"].focus=true;
				return;
			}
	  ...
	  if(!flag)
	   {
		 alert("Select atleast one check box to save");
		  ...
		  if(confirm("Are You Sure want to Save")) {
			 document.forms[0].action="switch.do?prefix=/fundAccounting&page=/PAOBillAction.do&mode=save&selectedRecords="+workList;
            document.forms[0].submit();
			}
	  }
...
<body>
<html:form  action="PAOBillAction.do" method="post"  >
...
<html:hidden property="token" styleClass="mytext" value="${sessionScope.csrfToken}" />
		...
		<table align=center>
		<tr  align="center">
			<td class="rptHeading"  align="center">PAO - Payment Due and Releases Entry Form</td>
...
<thead class='header'>
			 <tr><td colspan="10" class="rptValue">
				<font color=red>So far Selected Under this District : <b><bean:write name="RwsPAOBillActionForm" property="selectedBillCount" /></b> Bills and its Amount <b style="font-family: Rupee Foradian;">`</b> <b><bean:write name="RwsPAOBillActionForm" property="soFarProcessAmount" /></b></font>
			 ...
			 <tr align="center">
				...
				<td class=gridhdbg rowspan="2">WIP Image</td>
	...
	String style="";
	%>
	<logic:notEmpty name="RwsPAOBillActionForm" property="billsList">
	
	<logic:iterate id="billsList" name="RwsPAOBillActionForm" property="billsList"  indexId="ndx" >
	...
	<tr align="left" id='tr<%=sno%>'  >
		...
		<td class=<%=style %> style="text-align: left;" ><html:hidden name="billsList" property="workId" indexed="true"></html:hidden><bean:write name="billsList" property="workId" /><br><html:text name="billsList" property="workName"  style="width:120px" readonly="true" onmouseover="Tip(this.value,TITLE,'Work Name')"  onmouseout="UnTip()" /></td>
		...
		<bean:write name="billsList" property="billAmtAdmissible" />
		</td>
		...
		<td  class=<%=style %> style="text-align: right;" ><html:text name="billsList" property="dueState" style="width:60px" readonly="true" styleClass="mytext"   /></td>
		...
		<td  class=<%=style %> style="text-align: right;" align="center" onmouseover="Tip('Bill Is Seleted Under BANK Payment.',TITLE,'Alert')" onmouseout="UnTip()"><div class="diva"><span class="spana"><html:checkbox disabled="true" name="billsList" property="checks" styleClass="checka" onclick='<%= "check(" + ndx + ")" %>' indexed="true"/> </span></div></td>
		</logic:equal>
