...
function check()
{
	if (document.forms[0].nvoucher[0].checked || document.forms[0].nvoucher[1].checked)
	{
	}else{
	document.getElementById('selecthab').style.display='none';
	}
	...
	function fnSubmit()
	{
		...
		if(user=="admin" || user=="100000")
		{
			...
			else
			{
				...
				{ 
					...
					if(document.forms[0].nvoucher[0].checked)
					{
					 hvoucher=document.forms[0].nvoucher[0].value;
					}else if(document.forms[0].nvoucher[1].checked)
					 {
					   hvoucher=document.forms[0].nvoucher[1].value;
					...
					document.f1.action="./rws_push_bill_rpt.jsp?&voucher="+hvoucher;
					document.f1.submit();
				}else{
					alert(" Please Select Having Vouchers or Not Having Vouchers ");
				}
		...
		{
			alert("You are not Authorize to do Bill Pushing");
		}
	}
...
<table border = 1 align = center cellspacing = 0 cellpadding = 0 rules = "rows" bordercolor = navy style = "border-collapse:collapse" bgcolor="#ffffff" width=400>		
	...
	<tr bgcolor="#8A9FCD" align="center">
		<td align="center" class="rptHeading" colspan=3>Push Bills form</td>	
	</tr>
...
%>
<tr>
<td class=rptvalue colspan=2><input type="radio" name="nvoucher" value="1"  onclick="show()">Having Voucher Details
<input type="radio" name="nvoucher" value="2" onclick="show1()">Not Having Voucher Details</td>
</tr>
