...
<table border = 1 cellspacing = 0 cellpadding = 0 height=5% width="40%" bordercolor=#000000 style="border-collapse:collapse" bgcolor="#ffffff" align = "center">	
	...
	%>	<tr><td class=rptLabel>Financial Year&nbsp;&nbsp;<font color="red">*</font></td>
		...
		<td class=btext  colspan="5" nowrap>Releases During the Year(01-APR-<%=finyear.substring(0,4)%> To 31-MAR-<%=finyear.substring(5,9)%>)</td>
		<td class=btext rowspan="2" >Expenditure(01-APR-<%=finyear.substring(0,4)%><br>To 31-MAR-<%=finyear.substring(5,9)%>)</td>
		</tr>
		<tr>
		<td class=btext align="center">Central</td>
		<td class=btext align="center" >State</td>
		<td class=btext align="center">Total</td>
		<td class=btext align="center">Offline Release<br>(Upto Now)</td>
		<td class=btext align="center" >Grand Total</td>
		</tr>
		<tr><td align="center" class=btext>1</td><td align="center" class=btext>2</td><td align="center" class=btext>3</td><td align="center" class=btext>4</td><td align="center" class=btext>5</td><td align="center" class=btext>6</td><td align="center" class=btext>7</td><td align="center" class=btext>8</td></tr>
		<%
		String obqry="select sum(BILL_AMT_ADMISSIBLE),substr(b.work_id,1,2) from RWS_work_bill_TBL  a,rws_work_admn_tbl b where a.work_id=b.work_id ";
		...
		String billcqry="select sum(BILL_AMOUNT_CENTRAL ),substr(a.work_id,1,2) from RWS_bill_bank_bridge_det_TBL a,rws_work_admn_tbl b where a.work_id=b.work_id  and bank_send_date>='01-APR-"+yr+"' and bank_send_date<'31-MAR-"+nextyr+"' ";
		...
		String billsqry="select sum(BILL_AMOUT_STATE ),substr(a.work_id,1,2) from RWS_bill_pao_bridge_det_TBL a,rws_work_admn_tbl b where a.work_id=b.work_id  and state_send_date>='01-APR-"+yr+"' and state_send_date<'31-MAR-"+nextyr+"'";
     ...
     String qry="select *	 from rws_programme_tbl  order by 1";
...
%>
		...
		<td  class=rptValue align="right"><%=wbtot%></td>
		<% texp=tot+Double.parseDouble(wbtot);%>
		<td  class=rptValue align="right"><%=ndf.format(texp)%></td>
