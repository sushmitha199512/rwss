...
hoverColor       = "brown"; // darkred";       // TEXT COLOR OF A LINK WHEN YOU HOVER OVER IT
fontStyle        = "12px verdana ,arial, helvetica";           // TEXT STYLE FOR DATES
headingFontStyle = "bold 12px verdana ,arial, helvetica";      // TEXT STYLE FOR WEEKDAY ABBREVIATIONS
...
function setDateFieldFuture(dateField) {
    ...
    setInitialDateFuture();

    // THE CALENDAR FRAMESET DOCUMENTS ARE CREATED BY JAVASCRIPT FUNCTIONS
    calDocTop    = buildTopCalFrameFuture();
    calDocBottom = buildBottomCalFrameFuture();
}


// SET THE INITIAL CALENDAR DATE TO TODAY OR TO THE EXISTING VALUE IN dateField
function setInitialDateFuture() {
    ...
    {
	    var myDate=new Date();
	    myDate.setFullYear(inDate.substring(6,10),inDate.substring(3,5)-1,inDate.substring(0,2));
...
function showCalendarFuture(dateField,ev,new_title) {
    title = "Calendar" ; 
    if(new_title) { 
    	title = new_title ; 
    }	
    calX = screen.availWidth / 3 ; 
    calY = screen.availHeight / 3 ; 
    calW = 180 ; 
    calH = 200 ; 
     
    if (ev) { 
    	if (ev.screenX + calW + 10 > screen.availWidth) {
    	    calX = ev.screenX - calW -10 ;
    	} else { 
    	    calX = ev.screenX + 10 ;	 
    	}          
        if (ev.screenY + calH + 10 > screen.availHeight) {
    	    calY = ev.screenY - calH -10 ;
    	} else { 
    	    calY = ev.screenY + 10; 	 
    	}    
    ...
    winAttr = "dependent=yes,width=" + calW +",height=" + calH + ",titlebar=yes" + 
              ",top=" + calY + ",left=" + calX   
                
    // SET INITIAL VALUE OF THE DATE FIELD AND CREATE TOP AND BOTTOM FRAMES
    setDateFieldFuture(dateField);
    ...
    top.newWin = window.open("javascript:parent.opener.calDocFrameset", "calWin",winAttr);
    top.newWin.focus();
}


// CREATE THE TOP CALENDAR FRAME
function buildTopCalFrameFuture() {
    // CREATE THE TOP FRAME OF THE CALENDAR
    var calDoc =
        "<HTML>" +
        "<HEAD>" +
        "</HEAD>" +
        "<BODY  BGCOLOR='" + topBackground + "' topMargin=0 leftMargin=0 Marginwidth=0 Marginheight=0 >" +
        "<FORM NAME='calControl' onSubmit='return false;'>" +
        "<CENTER>" +
        "<TABLE CELLPADDING=0 CELLSPACING=1 BORDER=0>" +
        "<TR><TD COLSPAN=7>" +
        "<CENTER>" +
        getMonthSelectFuture() +
        "<INPUT NAME='year' VALUE='" + calDate.getFullYear() + "'TYPE=TEXT SIZE=4 MAXLENGTH=4 onChange='parent.opener.setYearFuture()'>" +
        "</CENTER>" +
        "</TD>" +
        "</TR>" +
        "<TR>" +
        "<TD COLSPAN=7>" +
        "<INPUT " +
        "TYPE=BUTTON NAME='previousYear' VALUE='<<'    onClick='parent.opener.setPreviousYearFuture()'><INPUT " +
        "TYPE=BUTTON NAME='previousMonth' VALUE=' < '   onClick='parent.opener.setPreviousMonthFuture()'><INPUT " +
        "TYPE=BUTTON NAME='today' VALUE='Today' onClick='parent.opener.setTodayFuture()'><INPUT " +
        "TYPE=BUTTON NAME='nextMonth' VALUE=' > '   onClick='parent.opener.setNextMonthFuture()'><INPUT " +
        "TYPE=BUTTON NAME='nextYear' VALUE='>>'    onClick='parent.opener.setNextYearFuture()'>" +
        "</TD>" +
        "</TR>" +
        "</TABLE>" +
        "</CENTER>" +
...
function buildBottomCalFrameFuture() {       
...
function setCurrentMonthFuture() {
...
function setPreviousYearFuture() {
...
function setPreviousMonthFuture() {
...
function getMonthSelectFuture() {
    ...
    monthSelect = "<SELECT NAME='month' onChange='parent.opener.setCurrentMonthFuture()'>";
...
function buildCalPartsFuture() {
    ...
    calendarBegin =
        ...
        "A.hover { color: " + hoverColor + "; font: " + fontStyle + "; }\n" +
        ...
        "//A.weekDay:vlink { color: " + dateColor + "; text-decoration: none; font: " + fontStyle + "; }\n" +       
        "A.weekDay:hover { color: " + hoverColor + "; font: " + fontStyle + "; }\n" +
        "//BODY {margin-top: 1px;	margin-left:1px;}\n" +
        "-->" +
        "</STYLE>" +
        "</HEAD>" +
        "<BODY topMargin=0 leftMargin=0 Marginwidth=0 Marginheight=0  BGCOLOR='" + bottomBackground + "' >" +
        "<CENTER>";
...
//var value = document.forms[0].broDate.value;
//alert(value);
function returnDate(inDay)
{
    ...
    // RETURN WEEKDAY 3-DAY ABBREVIATION (UPPERCASE LETTERS)
		...
		//outDate = jsReplace(weekdayAbbrev.toUpperCase(),"WKDY",outDate);
	}
    ...
    //code to call a function (beforeDateWindowClosed()) that handles all the activities before closing  the dateSetting window
    //for this code to work executeBeforeDateWindowClosed should be set to true. It should be a boolean that is true should not be in quotes
    try{
	    if(executeBeforeDateWindowClosed)
	    {
	    	beforeDateWindowClosed(calDateField);
