...
try{
		...
		String qry=" Select PASSWORD  from rws_password_tbl where  user_id='"+userid+"' ";
		...
		while(rs.next())
		{
			if(rs.getString(1).equals(oldpwd))
			{
			   if(newpwd.equals(cpwd))
				{
					query = " UPDATE  rws_password_tbl SET password = ? ,status=? WHERE user_id = ? AND password = ? ";
					pstat = conn.prepareStatement(query);
					pstat.setString(1,newpwd);
					pstat.setString(2,"A");
					pstat.setString(3,userid);
					pstat.setString(4,oldpwd);
					...
					document.location.href="rws_gen_admn_changepass_frm.jsp?special="+special+"&userId="+userid;
					 </script>
					<%
