...
<table border = 1 cellspacing = 0 cellpadding = 0 width=80% height=20%  bordercolor=#000000 style="border-collapse:collapse" bgcolor="#ffffff">
	...
	<tr bgcolor="lavander">
		<td class="textborder" align="center" colspan="8">
			<font color="ffffff">District-StatusWise-Population Report </font></td>
	</tr>
<tr><td valign=top>
...
<%
	try
	{
		...
		query="select distinct d.dcode,d.dname from rws_district_tbl d,rws_habitation_directory_tbl HD where d.dcode=substr(HD.hab_code,1,2) and hd.coverage_status <> 'UI' and hd.coverage_status is not null and d.dcode <> '16'order by d.dcode,d.dname";
		...
		while(rs2.next())
		{
			...
			{    
				...
				//query33="SELECT hd.census_plain_popu,hd.census_sc_popu,hd.census_st_popu from rws_district_tbl d,rws_habitation_directory_tbl HD where d.dcode=substr(HD.hab_code,1,2) and substr(HD.hab_code,1,2)='"+rs2.getString(1)+"' and HD.coverage_status='FC' ";
				query33="SELECT hd.census_plain_popu,hd.census_sc_popu,hd.census_st_popu from rws_district_tbl d,rws_mandal_tbl m,rws_panchayat_tbl p,rws_village_tbl v,rws_panchayat_raj_tbl h,rws_habitation_directory_tbl HD where d.dcode=m.dcode and m.dcode=p.dcode and m.mcode =p.mcode and p.dcode=v.dcode and p.mcode=v.mcode and p.pcode=v.pcode and substr (h.panch_code,1,2) = v.dcode and substr(h.panch_code,6,2) = v.mcode and substr(h.panch_code,13,2) = v.pcode and substr (h.panch_code,8,3) = v.vcode and h.panch_code=hd.hab_code and hd.coverage_status is not null and hd.coverage_status <> 'UI'  and substr(HD.hab_code,1,2)='"+rs2.getString(1)+"' and HD.coverage_status='FC' ";
			}
			if(popRange[j+1].equals("NC") )
			{
				//query44="SELECT hd.census_plain_popu,hd.census_sc_popu,census_st_popu from rws_district_tbl d,rws_habitation_directory_tbl HD where d.dcode=substr(HD.hab_code,1,2)  and substr(HD.hab_code,1,2)='"+rs2.getString(1)+"' and (coverage_status='NC') ";
				query44="SELECT hd.census_plain_popu,hd.census_sc_popu,hd.census_st_popu from rws_district_tbl d,rws_mandal_tbl m,rws_panchayat_tbl p,rws_village_tbl v,rws_panchayat_raj_tbl h,rws_habitation_directory_tbl HD where  d.dcode=m.dcode and m.dcode=p.dcode and m.mcode =p.mcode and p.dcode=v.dcode and p.mcode=v.mcode and p.pcode=v.pcode and substr (h.panch_code,1,2) = v.dcode and substr(h.panch_code,6,2) = v.mcode and substr(h.panch_code,13,2) = v.pcode and substr (h.panch_code,8,3) = v.vcode and h.panch_code=hd.hab_code and hd.coverage_status is not null and hd.coverage_status <> 'UI'  and substr(HD.hab_code,1,2)='"+rs2.getString(1)+"' and HD.coverage_status='NC' ";
			}
			if( popRange[j+2].equals("PC1")){
			//query441="SELECT hd.census_plain_popu,hd.census_sc_popu,census_st_popu from rws_district_tbl d,rws_habitation_directory_tbl HD where d.dcode=substr(HD.hab_code,1,2)  and substr(HD.hab_code,1,2)='"+rs2.getString(1)+"' and (coverage_status='PC1') ";
			query441="SELECT hd.census_plain_popu,hd.census_sc_popu,hd.census_st_popu from rws_district_tbl d,rws_mandal_tbl m,rws_panchayat_tbl p,rws_village_tbl v,rws_panchayat_raj_tbl h,rws_habitation_directory_tbl HD where  d.dcode=m.dcode and m.dcode=p.dcode and m.mcode =p.mcode and p.dcode=v.dcode and p.mcode=v.mcode and p.pcode=v.pcode and substr (h.panch_code,1,2) = v.dcode and substr(h.panch_code,6,2) = v.mcode and substr(h.panch_code,13,2) = v.pcode and substr (h.panch_code,8,3) = v.vcode and h.panch_code=hd.hab_code and hd.coverage_status is not null and hd.coverage_status <> 'UI'  and substr(HD.hab_code,1,2)='"+rs2.getString(1)+"' and HD.coverage_status='PC1' ";
			}
			if(popRange[j+3].equals("PC2"))
			{
				//query55="SELECT hd.census_plain_popu,hd.census_sc_popu,census_st_popu from rws_district_tbl d,rws_habitation_directory_tbl HD where d.dcode=substr(HD.hab_code,1,2)  and substr(HD.hab_code,1,2)='"+rs2.getString(1)+"' and (coverage_status='PC2')";
				query55="SELECT hd.census_plain_popu,hd.census_sc_popu,hd.census_st_popu from rws_district_tbl d,rws_mandal_tbl m,rws_panchayat_tbl p,rws_village_tbl v,rws_panchayat_raj_tbl h,rws_habitation_directory_tbl HD where  d.dcode=m.dcode and m.dcode=p.dcode and m.mcode =p.mcode and p.dcode=v.dcode and p.mcode=v.mcode and p.pcode=v.pcode and substr (h.panch_code,1,2) = v.dcode and substr(h.panch_code,6,2) = v.mcode and substr(h.panch_code,13,2) = v.pcode and substr (h.panch_code,8,3) = v.vcode and h.panch_code=hd.hab_code and hd.coverage_status is not null and hd.coverage_status <> 'UI'  and substr(HD.hab_code,1,2)='"+rs2.getString(1)+"' and HD.coverage_status='PC2' ";
			}
			if(popRange[j+4].equals("PC3"))
			{
			//query551="SELECT hd.census_plain_popu,hd.census_sc_popu,census_st_popu from rws_district_tbl d,rws_habitation_directory_tbl HD where d.dcode=substr(HD.hab_code,1,2)  and substr(HD.hab_code,1,2)='"+rs2.getString(1)+"' and (coverage_status='PC3')";
			query551="SELECT hd.census_plain_popu,hd.census_sc_popu,hd.census_st_popu from rws_district_tbl d,rws_mandal_tbl m,rws_panchayat_tbl p,rws_village_tbl v,rws_panchayat_raj_tbl h,rws_habitation_directory_tbl HD where  d.dcode=m.dcode and m.dcode=p.dcode and m.mcode =p.mcode and p.dcode=v.dcode and p.mcode=v.mcode and p.pcode=v.pcode and substr (h.panch_code,1,2) = v.dcode and substr(h.panch_code,6,2) = v.mcode and substr(h.panch_code,13,2) = v.pcode and substr (h.panch_code,8,3) = v.vcode and h.panch_code=hd.hab_code and hd.coverage_status is not null and hd.coverage_status <> 'UI'  and substr(HD.hab_code,1,2)='"+rs2.getString(1)+"' and HD.coverage_status='PC3' ";
			}
			if( popRange[j+5].equals("PC4"))
			{
			//query552="SELECT hd.census_plain_popu,hd.census_sc_popu,census_st_popu from rws_district_tbl d,rws_habitation_directory_tbl HD where d.dcode=substr(HD.hab_code,1,2)  and substr(HD.hab_code,1,2)='"+rs2.getString(1)+"' and ( coverage_status='PC4')";
			query552="SELECT hd.census_plain_popu,hd.census_sc_popu,hd.census_st_popu from rws_district_tbl d,rws_mandal_tbl m,rws_panchayat_tbl p,rws_village_tbl v,rws_panchayat_raj_tbl h,rws_habitation_directory_tbl HD where  d.dcode=m.dcode and m.dcode=p.dcode and m.mcode =p.mcode and p.dcode=v.dcode and p.mcode=v.mcode and p.pcode=v.pcode and substr (h.panch_code,1,2) = v.dcode and substr(h.panch_code,6,2) = v.mcode and substr(h.panch_code,13,2) = v.pcode and substr (h.panch_code,8,3) = v.vcode and h.panch_code=hd.hab_code and hd.coverage_status is not null and hd.coverage_status <> 'UI'  and substr(HD.hab_code,1,2)='"+rs2.getString(1)+"' and HD.coverage_status='PC4' ";
			}
			if(popRange[j+6].equals("NSS"))
			{
			    //query66="SELECT hd.census_plain_popu,hd.census_sc_popu,census_st_popu from rws_district_tbl d,rws_habitation_directory_tbl HD where d.dcode=substr(HD.hab_code,1,2)  and substr(HD.hab_code,1,2)='"+rs2.getString(1)+"' and coverage_status='NSS'";
