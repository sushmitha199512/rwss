...
int slno=0;
   ...
   %>
	...
	<% if(isModeChange.equalsIgnoreCase("initial")){ %>
		...
		<td ><a href="rws_tanker_movement_rpt_excel.jsp?mode=initial&loggedUser=<%=loggedUser%>">Excel</td>
	<% } else if(isModeChange.equalsIgnoreCase("date")){ %>
	<td ><a href="rws_tanker_movement_rpt_excel.jsp?mode=date&loggedUser=<%=loggedUser%>&fromDate=<%=fromdate%>&toDate=<%=todate%>"">Excel</td>
	<%}else{ %>
	<%} %>
	</tr>  
	</table>
   ...
   %>
	...
	<td class=gridhdbg colspan=1>No. of trips(Completed Trips - Disapproved Trips)</td>
   </tr>
   <!-- <tr>
    			   ...
    			   %>
    				...
    				<% } %>
    				 ...
    				 <%--  <td class="<%=style%> "style="text-align: right;"><%=sourceTrips%></td> --%>
    				  ...
    				  <%if(sourceTrips>0){%>
    				  <td class="<%=style%> "style="text-align: right;"><a href="rws_tanker_movement_drill_rpt.jsp?mode1=mandal&type=<%=val%>&dcode=<%=rst.getString(1)%>&dname=<%=rst.getString(2)%>&fromdate=<%=fromdate%>&todate=<%=todate%>"><%=sourceTrips%></a></td>
    				<%}else{ %>
    	...
    	<%
    			...
    			%>
    			 <tr>
                    ...
                    <td class="btext" style="text-align: right;"><%=totTrips%></td>
                    <!-- <td class="btext" style="text-align: right;"></td> -->
