...
int totalClippings=0;
   %>
	...
	<td ><a href="rws_tanker_movement_drill_rpt_excel.jsp?mode1=hab&type=<%=type%>&dname=<%=dname%>&dcode=<%=dcode%>&mcode=<%=dcode%>&fromdate=<%=fromdate%>&todate=<%=todate%>&mname=<%=dcode%>">Excel%</a></td>
	<% } %>
   ...
   <tr   align=center>
		...
		<table width="45%" border=1 style="border-collapse:collapse"  ALIGN=CENTER >
   <tr   align=center>
		<td class="gridhdbg" colspan="17"><b>Tanker Movement Details for District:<font color="red"><%=dname%> </font> Mandal:<font color="red"><%=mname%> </font>(From:<font color="red"><%=fromdate %></font> To:<font color="red"><%=todate %></font>)</b></td>
		</tr>
		<%} %> --%>
   				 ...
   				 if(rs.next())
   	    		   {
   					...
   					//sourceTrips_total=rs.getInt(4) - rs.getInt(3);
