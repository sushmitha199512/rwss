...
<%
 ...
 //   System.out.println("fromdate$ "+fromdate);
  ...
  <td class="gridhdbg" colspan="7" style=" padding-top:10px; padding-bottom:10px; background-color:#436f8a">District    :    <%=dname%>  </td>
  <td class="gridhdbg" colspan="7" style=" padding-top:10px; padding-bottom:10px; background-color:#436f8a">Habitaion   :    <%=habName %> / <%=habcode%> </td>
   ...
   <!--  <td class=gridhdbg rowspan="2">Distance</td> -->
    ...
    <td class=gridhdbg > Image</td>
	   ...
	   str="select a.TANKER_REG_NO,nvl(to_char(TANKER_TRIP_START_DATE_TIME,'dd/mm/yyyy HH:MI:SS AM'),'--'),nvl(to_char(TANKER_TRIP_END_DATE_TIME,'dd/mm/yyyy HH:MI:SS AM'),'--'),nvl(SOURCE_CODE,'--'),TANKER_SOURCE_LAT,TANKER_SOURCE_LOG,nvl(TANKER_DEST_LAT,0),nvl(TANKER_DEST_LOG,0),nvl(TANKER_TRIP_DISTANCE,0),nvl(TANKER_ARIAL_DISTANCE,0),TRIP_ID,	nvl(dest_habitation,'--'),999  as totalseconds,nvl(to_char(TANKER_TRIP_END_DATE_TIME),'--') as isStarted ,nvl(INSP_MOBILE,'--'),nvl(to_char(INSPECTION_DATE ,'dd/mm/yyyy HH:MI:SS AM'),'--'),nvl(INSP_MOBILE,'--'),c.panch_code,c.panch_name,a.APPROVEL_STATUS "  
			   +",nvl(to_char(TANKER_TRIP_START_DATE_TIME,'dd/mm/yyyy HH24:MI:SS'),'--') as strDate,nvl(to_char(TANKER_TRIP_END_DATE_TIME,'dd/mm/yyyy HH24:MI:SS'),'--') as endDate,TANKER_TRIP_DISTANCE,nvl(APPROVED_BY,'--') as approvedby from RWS_TANKER_MOVEMENT_TBL a,rws_complete_hab_view c  where  a.tanker_reg_no=? and substr(a.dest_habitation,1,16)=c.panch_code   and substr(a.dest_habitation,1,2)=? and substr(a.dest_habitation,6,2)=? and to_date(to_char(TANKER_TRIP_START_DATE_TIME,'dd/mm/yyyy'),'dd/mm/yyyy') >=  to_date(?,'dd/mm/yyyy') and to_date(to_char(TANKER_TRIP_START_DATE_TIME,'dd/mm/yyyy'),'dd/mm/yyyy') <= to_date(?,'dd/mm/yyyy') order by a.tanker_reg_no,a.TANKER_TRIP_START_DATE_TIME desc ";
	   ...
	   pstmt.setString(5,todate );
		...
		//System.out.println("dcode :"+dcode+" mcode:"+mcode+" fromdate:"+fromdate+" todate:"+todate);
    	 ...
    	 {
    		 str="select a.TANKER_REG_NO,nvl(to_char(TANKER_TRIP_START_DATE_TIME,'dd/mm/yyyy HH:MI:SS AM'),'--'),nvl(to_char(TANKER_TRIP_END_DATE_TIME,'dd/mm/yyyy HH:MI:SS AM'),'--'),nvl(SOURCE_CODE,'--'),TANKER_SOURCE_LAT,TANKER_SOURCE_LOG,nvl(TANKER_DEST_LAT,0),nvl(TANKER_DEST_LOG,0),nvl(TANKER_TRIP_DISTANCE,0),nvl(TANKER_ARIAL_DISTANCE,0),TRIP_ID,nvl(dest_habitation,'--'),999  as totalseconds,nvl(to_char(TANKER_TRIP_END_DATE_TIME),'--') as isStarted ,nvl(INSP_MOBILE,'--'),nvl(to_char(INSPECTION_DATE ,'dd/mm/yyyy HH:MI:SS AM'),'--'),nvl(INSP_MOBILE,'--'),c.panch_code,c.panch_name,a.APPROVEL_STATUS "  
    				   +",nvl(to_char(TANKER_TRIP_START_DATE_TIME,'dd/mm/yyyy HH24:MI:SS'),'--') as strDate,nvl(to_char(TANKER_TRIP_END_DATE_TIME,'dd/mm/yyyy HH24:MI:SS'),'--') as endDate,TANKER_TRIP_DISTANCE,nvl(APPROVED_BY,'--') as approvedby from RWS_TANKER_MOVEMENT_TBL a,rws_complete_hab_view c  where  a.tanker_reg_no=? and substr(a.dest_habitation,1,16)=c.panch_code   and substr(a.dest_habitation,1,2)=? and substr(a.dest_habitation,6,2)=?  order by a.tanker_reg_no,a.TANKER_TRIP_START_DATE_TIME desc ";
...
<%
					...
					str1="select panch_name,panch_code  from rws_complete_hab_view where panch_code=?";
					   ...
					   ps1.setString(1, rs.getString(12));
