...
{       
		...
		var chkd=false;
	    for (i=0;i<document.f1.ch.length;i++) {
	   ...
	   return false;
			...
			document.f1.action="./rws_month.jsp";
			document.f1.submit();
...
{%>
<TR> 
	<TD   valign=top  align="center" height="80%" width="30%">
...
<table  bordercolor= navy width="100%" height="100%" border=1 rules="rows" style="border-collapse:collapse" align = center>
	...
	//Debug.println("distrrrrrict"+rDist);
	if(rDist == null || rDist.equals("-1"))
	{
		//Debug.println("IN DISTRICT STARTING...");
...
%>
	...
	// //Debug.println("Rdist   "   +rDist);
    stmt1 = conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
	String Query=null;
	if(fromDate!=null && !rDist.equals("00"))
	Query="select distinct FIN_YEAR from rws_mon_fin_phy_progress_tbl where dcode='"+rDist+"' order by FIN_YEAR DESC";
	else
    Query="select distinct FIN_YEAR from rws_mon_fin_phy_progress_tbl order by FIN_YEAR DESC";
...
<%		//session.setAttribute("dName",null);	
		...
		<tr>
			...
			<%
	         ...
	         //Debug.println("MONTHHHHHHH  "+ Month);
...
<div  style="position:relative;overflow-y:auto;left:0;height:175;width:100%;top:0;">

		<%@ include file="./rws_progress1.jsp"%>
</div>
