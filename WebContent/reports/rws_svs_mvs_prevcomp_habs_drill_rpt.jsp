...
if (type != null && type.equals("1")) {
	if (subType != null && subType.equals("1")) {
		field = " and CATEGORY_CODE='2' ";
		

	}
	//  System.out.println("*****"+field);
	else if (subType != null && subType.equals("2")) {
		field = " and  CATEGORY_CODE='1' ";
	}
}
...
//String SVSHABS="select s.dname,b.division_office_name,w.work_id,w.work_name,nvl(WORK_CATEGORY,'-'),nvl(to_char(ADMIN_DATE),'-'),HAB_CODE,NO_OF_HABS ,nvl(to_char(WORK_CANCEL_DT),'-')  FROM RWS_WORK_ADMN_TBL W ,rws_division_office_tbl b,rws_admn_hab_lnk_tbl c , rws_district_tbl s where b.CIRCLE_OFFICE_CODE =s.DCODE  "+field+" and w.work_id=c.work_id  and work_cancel_dt is null and substr(w.office_code,2,2)=b.circle_office_code and substr(w.office_code,4,1)=b.division_office_code  and substr(w.office_code,2,2)='"+dcode+"' and substr(w.office_code,4,1)='"+divCode+"' ";
String SVSHABS="select s.dname,b.division_office_name,w.work_id,w.work_name,nvl(WORK_CATEGORY,'-'),nvl(to_char(ADMIN_DATE),'-'),HAB_CODE,NO_OF_HABS ,nvl(to_char(WORK_CANCEL_DT),'-') FROM RWS_WORK_ADMN_TBL W ,rws_division_office_tbl b,RWS_WORK_COMP_HAB_LNK_TBL c , rws_district_tbl s where b.CIRCLE_OFFICE_CODE =s.DCODE  "+field+" and w.work_id=c.work_id  and work_cancel_dt is null and w.work_id in (select work_id from rws_work_completion_tbl  where date_of_completion is not null and to_date(date_of_completion)<= '01-Apr-"+yy+"' and to_date(update_date)>= '01-Apr-"+yy+"' ) and substr(w.office_code,2,2)=b.circle_office_code and substr(w.office_code,4,1)=b.division_office_code  and substr(w.office_code,2,2)='"+dcode+"' and substr(w.office_code,4,1)='"+divCode+"' ";
