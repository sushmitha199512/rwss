...
<tr bgcolor="#8A9FCD" align="center">
		<td align="center" class="rptHeading" colspan="14"> HEADWORKS-CLEAR WATER COLLECTION WELL -REPORT </td>
</tr>
</table>
<table border=1 cellpadding=1 cellspacing=1 width="100%">
<tr align=center>
               <td class=btext>Sl.No</td>
			   <td class=btext>Clear water Collection Well Code</td>
			   <td class=btext>Diameter</td>
			   <td class=btext>Depth</td>
			   <td class=btext>Type of Flooring</td>
			   <td class=btext>Location</td>
			   <td class=btext>Capacity</td>
...
try
{
	    ...
	    if(typeassetcode.equals("03"))
	{
		...
		rs1=stmt1.executeQuery("select  cw_collwell_code,diameter,depth,nvl(type_of_flooring,'-'),nvl(location,'-'),capacity,asset_code from rws_cpws_cw_collwell_param_tbl  where asset_code='" + assetcode + "' ");
		while(rs1.next())
    	{
	     				%>
...
<%
			...
			
	{
		...
		stmt1=conn.createStatement();
		rs1=stmt1.executeQuery("select  cw_collwell_code,diameter,depth,nvl(type_of_flooring,'-'),nvl(location,'-'),capacity,asset_code from rws_pws_cw_collwell_param_tbl  where asset_code='"+assetcode+"' ");
		while(rs1.next())
    	{
...
<%
			...
			
	{
		...
		stmt1=conn.createStatement();
		rs1=stmt1.executeQuery("select  cw_collwell_code,diameter,depth,nvl(type_of_flooring,'-'),nvl(location,'-'),capacity,asset_code from rws_mpws_cw_collwell_param_tbl  where asset_code='"+assetcode+"' ");
		while(rs1.next())
    	{
