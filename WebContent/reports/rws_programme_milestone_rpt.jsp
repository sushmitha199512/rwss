...
/////
 ...
 //  System.out.println("Milestone:"+milestonearch);
   ...
   if(dcode!=null)
   {
	   milestone1="SELECT substr(office_code,2,2),substr(office_code,4,1),substr(office_code,5,2),count(m.milestone_code)  FROM RWS_WORK_ADMN_TBL W,rws_MILESTONE_TARGET_ACH_TBL m where   w.work_id=m.work_id and work_cancel_dt is null and substr(work_id,5,2)='"+dcode+"' and w.programme_code in ("+programe+")  and  w.work_id not in (select work_id from rws_work_completion_tbl where date_of_completion is not null and (to_date(DATE_OF_COMPLETION) < '01-Apr-"+startyr+"'))  ";
    if(scheme!=null && !scheme.equals("")){
    	milestone1+=" and w.type_of_asset in("+scheme+") group by substr(office_code,2,2),substr(office_code,4,1),substr(office_code,5,2)  order by 1";
}
   ...
   System.out.println("mileStoneHash:"+milestone1);
    	...
    	while(rs3.next())
    	{
    		mileStoneTargetHash.put(rs3.getString(1)+rs3.getString(2)+rs3.getString(3),rs3.getString(4));
    	}
   ...
   System.out.println("mileStoneHash:"+milestone1);
    	...
    	while(rs3.next())
    	{
    		mileStoneAchievedHash.put(rs3.getString(1)+rs3.getString(2)+rs3.getString(3),rs3.getString(4));
    	}
   ...
   if(dcode!=null)
   {
    ongoing="SELECT substr(office_code,2,2),substr(office_code,4,1),substr(office_code,5,2),count(*)  FROM RWS_WORK_ADMN_TBL W    where  work_cancel_dt is null and substr(work_id,5,2)='"+dcode+"' and w.programme_code in ("+programe+")  and  w.work_id not in (select work_id from rws_work_completion_tbl where date_of_completion is not null and (to_date(DATE_OF_COMPLETION) < '01-Apr-"+startyr+"'))  ";
    if(scheme!=null && !scheme.equals("")){
   ...
   {
	   ongoing="SELECT substr(office_code,2,2),substr(office_code,4,1),substr(office_code,5,2),count(*)  FROM RWS_WORK_ADMN_TBL W   where  work_cancel_dt is null  and w.programme_code in ("+programe+")  and  w.work_id not in (select work_id from rws_work_completion_tbl where date_of_completion is not null and (to_date(DATE_OF_COMPLETION) < '01-Apr-"+startyr+"'))  ";
	   if(scheme!=null && !scheme.equals("")){
	    	ongoing+=" and w.type_of_asset in("+scheme+")group by substr(office_code,2,2),substr(office_code,4,1),substr(office_code,5,2)   order by 1";
	}
   }
...
double gmileStoneTargetValue=0,gmileStoneAchValue=0;;
int gmileStoneTargetValue1=0,gmileStoneAchValue1=0;;
...
//	System.out.println("ss*******"+cricle);
	...
	String mileStoneT=(String)mileStoneTargetHash.get(rs1.getString(3)+rs1.getString(4)+rs1.getString(1));
	...
	String mileStoneA=(String)mileStoneAchievedHash.get(rs1.getString(3)+rs1.getString(4)+rs1.getString(1));
