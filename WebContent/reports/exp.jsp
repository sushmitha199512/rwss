...
ResultSet rs2 = null,r=null,r1=null,r2=null,rr=null,rss=null,rr1=null,rss1=null;
	...
	//System.out.println("divCode issssss"+divCode);
...
try
{
	...
	/*query ="select distinct nvl(w.WORK_NAME,'-'),nvl(w.work_id,'-') from RWS_WORK_ADMN_TBL w,rws_work_admn_prog_lnk_tbl prg where w.work_id(+)=prg.work_id and w.programme_code=prg.programme_code and substr(w.office_code,2,2)='"+dCode+"'" ;*/
      if(dCode!=null&&!dCode.equals("-1")&& divCode.equals("-1") && subdivCode.equals("-1"))
	{
     	query ="select distinct nvl(w.WORK_NAME,'-'),nvl(w.work_id,'-') from RWS_WORK_ADMN_TBL w,rws_work_expenditure_tbl e where w.work_id=e.work_id and substr(w.office_code,2,2)='"+dCode+"' and substr(w.work_id,7,2)='"+schCode+"' and to_char(w.admin_date,'yyyy')='"+yrCode+"'"; //code changed from prepared_on field to admin_date field
    }
	 else 
		if(dCode!=null &&!dCode.equals("-1") && !divCode.equals("-1") && subdivCode.equals("-1"))
	{
		query ="select distinct nvl(w.WORK_NAME,'-'),nvl(w.work_id,'-') from RWS_WORK_ADMN_TBL w,rws_work_expenditure_tbl e where w.work_id=e.work_id and substr(w.office_code,2,2)='"+dCode+"' and substr(w.office_code,4,1)='"+divCode+"' and substr(w.work_id,7,2)='"+schCode+"' and to_char(w.admin_date,'yyyy')='"+yrCode+"'"; //code changed from prepared_on field to admin_date field
	}
	else 
		if(dCode!=null &&!dCode.equals("-1") && !divCode.equals("-1") && !subdivCode.equals("-1"))
	{
		query ="select distinct nvl(w.WORK_NAME,'-'),nvl(w.work_id,'-') from RWS_WORK_ADMN_TBL w,rws_work_expenditure_tbl e where w.work_id=e.work_id and substr(w.office_code,2,2)='"+dCode+"' and substr(w.office_code,4,1)='"+divCode+"' and  substr(w.office_code,5,2)='"+subdivCode+"' and  substr(w.work_id,7,2)='"+schCode+"' and to_char(w.admin_date,'yyyy')='"+yrCode+"'"; //code changed from prepared_on field to admin_date field
...
<tr bgcolor="#8A9FCD" align="center">
		<td align="center" class="rptHeading" colspan=10>Work Expenditure Report</td>	
	</tr>
...
if(dCode!=null&&!dCode.equals("-1")&& divCode.equals("-1") && subdivCode.equals("-1"))
	...
	<tr bgcolor="#edf2f8" align="center">
		<td align="center" class="rptHeading" colspan=12>District: <%=dName%>&nbsp;&nbsp;Scheme: <%=schName%>&nbsp;&nbsp;Year: <%=yrCode%></td>	
	</tr>	
<%
	}
else 
	...
	{%>
	<tr bgcolor="#edf2f8" align="center">
		<td align="center" class="rptHeading" colspan=12>District: <%=dName%>&nbsp;&nbsp;Division:<%=divName%>&nbsp;&nbsp;Scheme: <%=schName%>&nbsp;&nbsp;Year: <%=yrCode%></td>	
	</tr>	
<%
	} else 
		if(dCode!=null &&!dCode.equals("-1") && !divCode.equals("-1") && !subdivCode.equals("-1"))
	...
	%>
	<tr bgcolor="#edf2f8" align="center">
		<td align="center" class="rptHeading" colspan=12>District: <%=dName%>&nbsp;&nbsp;Division:<%=divName%>&nbsp;&nbsp;&nbsp;SubDivision:<%=subdivName%>&nbsp;&nbsp;Scheme: <%=schName%>&nbsp;&nbsp;Year: <%=yrCode%></td>	
	</tr>
	<%
	}%>	
...
<%	
	...
	%>	
		    ...
		    while(rs1.next())
		{
			...
			if(i==1)
			{
			total1=total1+rs1.getFloat(4)+rs2.getFloat(1);
