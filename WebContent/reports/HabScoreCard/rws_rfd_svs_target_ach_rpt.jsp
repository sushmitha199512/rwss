...
function changeLanguage(){
	...
	//var url="switch.do?prefix=/reports&page=/rwssHabScoreCard.do?mode=SCH&langMode="+value;

	var url="switch.do?prefix=/reports&page=/rwssHabScoreCard.do?mode=SVS";
...
<tr><td align="right" class="bwborder" colspan="2">
                    <a href="./public_domain/rws_rfd_svs_target_ach_excel.jsp" target="_new"><bean:message key="app.excel" /></a>|<a href="switch.do?prefix=/reports&page=/rwssHabScoreCard.do?mode=home" ><bean:message key="app.back" /></a></td></tr>       
...
<!-- Grid Table Starts -->                                      
        ...
        <tr>
                                            ...
                                            <td colspan="9" align="right"><html:select  style="width:100px" property="language" onchange="changeLanguage()">
                                            <html:option value="English">English</html:option>
<html:option value="Telugu">Telugu</html:option></html:select></td></tr>                                 
<tr >
		<td align="center" class="gridhdbg" colspan=13><font color="white"><b><bean:message key="app.svsmainheading" />&nbsp;(<%=cYear %>-<%=nYear %>)</b></font></td>	
	</tr>
		<tr >
		<td class="gridhdbg"  rowspan=2 align=center><bean:message key="app.sno" /></td>
		<td class="gridhdbg"  rowspan=2 align=center><bean:message key="app.district" /></td>
<td class="gridhdbg"  align=center colspan=2 nowrap ><bean:message key="app.Target" />&nbsp;(<%=cYear %>-<%=nYear %>) <br></br><font color="red"><b><bean:message key="app.AmountinLakhs" /></b></font></td>
<td class="gridhdbg" colspan=4 align=center ><bean:message key="app.Achievement" />&nbsp;(<%=cYear %>-<%=nYear %>) <br><font color="red"><b><bean:message key="app.AmountinLakhs" /></b></font></br></td>
 <td class="gridhdbg" colspan=3 align=center ><bean:message key="app.PercentageAchieved" /></td>
</tr>
<tr>
<td class="gridhdbg"  align="center"  ><bean:message key="app.HabitationsCoverage" /></td>
 <td  class="gridhdbg" align="center"  ><bean:message key="app.AnticipatedExp" /></td>	
 <td class="gridhdbg"  align="center"  ><bean:message key="app.HabitationsCoverage" /></td>
 <td  class="gridhdbg"  align="center"  ><bean:message key="app.expupto" /> <bean:message key='<%=prevmonth %>' /> </td>
 <td  class="gridhdbg" align="center"  ><bean:message key="app.expduring"/><bean:message key='<%=currentMonth %>' /></td>
 <td  class="gridhdbg"  align="center"  ><bean:message key="app.expduring" />(<%=cYear %>-<%=nYear %>)</td>
 
 <td  class="gridhdbg"  align="center"  ><bean:message key="app.habduring" />(<%=cYear %>-<%=nYear %>)</td>
 <!--  
...
int count=1;
  try{
	  ...
	  String svsHabsQuery="select d.dcode,d.dname,count(*) from rws_work_admn_tbl a,rws_work_comp_hab_lnk_tbl b,rws_district_tbl d,rws_complete_hab_view ch  where ch.panch_code=b.hab_code  and a.work_id in(select work_id from rws_work_completion_tbl where  ((to_date(date_of_completion,'dd/mm/yy')>='01-APR-"+cYear+"') or (to_date(PARTIAL_DATE_OF_COMPLETION,'dd/mm/yy')>='01-APR-"+cYear+"') ) )   and a.WORK_CANCEL_DT is null and a.category_code='2'  and a.type_of_asset in('01','02','03') and a.work_id=b.work_id and d.dcode=substr(a.work_id,5,2)  and substr(a.work_id,1,2) in ("+selectedProgramsPhy+") group by d.dcode,d.dname order by d.dcode";
...
/**********************************************/
	  ...
	  String svsTotExpQuery="select d.dcode,d.dname ,(sum(BILL_AMOUNT_CENTRAL)/100000)  from rws_work_admn_tbl a,rws_district_tbl d, RWS_bill_bank_bridge_det_TBL v  where   a.WORK_CANCEL_DT is null and a.category_code='2'   and v.work_id=a.work_id and d.dcode=substr(a.work_id,5,2)  and substr(a.work_id,1,2) in ("+selectedProgramsFin+") and to_date(v.BANK_SEND_DATE,'dd/mm/yy')>='01-APR-"+cYear+"' and  to_date(v.BANK_SEND_DATE,'dd/mm/yy')<'01-APR-"+nYearStr+"'  group by d.dcode,d.dname order by d.dcode";
	  ...
	  String distQuery="select d.dcode,d.dname,t.SVS_HABS,(t.SVS_EXP*100)  from rws_district_tbl d , rws_work_sch_target_tbl t where t.dcode=d.dcode and FIN_YEAR='2013-2014' and d.dcode<>'16' order by dcode";
...
String deleteQuery2="delete from rws_rfd_cs_rpt_tmp_tbl WHERE RPT_TYPE='SVS'";
...
/******************************************************/
		...
		double targetE=Double.parseDouble(rs3.getString(4));
		 ...
		 String insertQuery1="insert into rws_svs_graph_view_tbl values ('"+rs3.getString(1)+"','"+rs3.getString(2)+"','"+habper+"','"+expper+"','SVS')";
		  sst1.addBatch(insertQuery1);
		 
		  String insertQuery="insert into rws_rfd_cs_rpt_tmp_tbl values ('"+rs3.getString(1)+"','"+rs3.getString(2)+"','"+rs3.getString(3)+"','"+ndf.format(targetE)+".00','"+svsTargetHabs+"','"+d1+"','"+d2+"','"+d3+"','"+habper+"','"+expper+"','SVS')";
	 ...
	 String q1="select *from rws_rfd_cs_rpt_tmp_tbl where rpt_type='SVS' order by HAB_ACH_PER desc ";
	 ...
	 count=1;
	     while(rst.next()){
		  ...
		  <tr><td class="<%=style %>" style="text-align: left;"><%=count++ %></td><td class="<%=style %>"  style="text-align: left;"><bean:message key='<%=rst.getString(2).replaceAll(" ","")%>' /></td>
		  <td class="<%=style %>" style="text-align: left;"><%=rst.getString(3) %></td><td class="<%=style %>" style="text-align: right;" ><%=rst.getString(4) %></td>
		  <td class="<%=style %>" style="text-align: right;"><a href="#" onclick="viewWorks('./public_domain/rws_rfd_svs_target_ach_rpt_drill.jsp?dcode=<%=rst.getString(1) %>&type=2')"><%=rst.getString(5) %></a></td>
 ...
 String colorValueHab="",colorValueExp="";
	  ...
	  %>
	  <tr><td  style="text-align: right;"  class="gridhdbg" colspan="2"><bean:message key="app.total" /> </td><td style="text-align: left;"  class="gridhdbg"  ><%=(int)svsT %></td><td class="gridhdbg" style="text-align: right;"><%=ndf.format(expT)+".00" %></td><td class="gridhdbg"  style="text-align: right;"><%=(int)svsH %></td><td class="gridhdbg" style="text-align: right;" ><%=ndf.format(expUpto) %></td><td class="gridhdbg" style="text-align: right;" ><%=ndf.format(expCurrent) %></td><td class="gridhdbg" style="text-align: right;" > <%=ndf.format(expTot) %></td>
 ...
 %>
				    ...
				    <bean:message key="app.graph" /><a href="<rws:context page='/public_domain/rws_rfd_svs_target_ach_graph.jsp' />" target="_new"><img src="<rws:context page='/images/chart.png' />"  width="40" height="40"/></a></td>
				</tr>
</table>
