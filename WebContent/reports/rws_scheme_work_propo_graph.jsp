...
<%	    int total=0;
	   for(int m=0;m<count;m++) {%>
			...
			<td class="mystyle1"><a href="./rws_sche_work_pro_list_rpt.jsp?district=<%=district%>&mandal=<%=mandal%>&panchayat=<%=panchayat%>&scheme[]=<%=districts[m]%>&FDate=<%=fdate%>&TDate=<%=tdate%>"><%=data[m]%></a></td>
		</tr>
 <%
		   total=total+data[m];
	   }%>
