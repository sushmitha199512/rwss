<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">
  ...
  try
 {
     String sqlquery=null,distcode=null,mcode=null,pcode=null;
	 int srlno=0;
   ...
   sqlquery ="select distinct v.vname,v.vcode,count(h.hp_code),SUM(Case When h.status='W' then 1 Else 0 End) as W, SUM(Case When h.status='N' then 1 Else 0 End) as N,SUM(Case When status='D' then 1 Else 0 End) as D,SUM(Case When h.status='S' then 1 Else 0 End) AS S,SUM(Case When h.status='C' then 1 Else 0 End) AS C,count(h.quality_affected),count(h.flouride),count(h.brakish),count(h.iron) from rws_hp_subcomp_param_tbl h, rws_village_tbl v where v.dcode=substr(h.hp_code,1,2) and v.mcode=substr(h.hp_code,6,2) and v.pcode=substr(h.hp_code,13,2)and v.vcode=substr(h.hp_code,8,3) and substr(h.hp_code,1,2)=? and substr(h.hp_code,6,2)=? and substr(h.hp_code,13,2)=?  group by v.vcode,v.vname";
...
<table border = 1 cellspacing = 0 cellpadding = 0 height=5% width="100%" bordercolor=#000000 style="border-collapse:collapse" bgcolor="#ffffff" align = "center">	
		...
		<tr  bgcolor="#8A9FCD" align="center">
			<td align="center" class="rptHeading" colspan=12>Village-Borewell-Status-Report</td>	
		</tr>
...
<% 
	...
	<tr>
		...
		<td><a href="rws_habitation_rpt.jsp?vcode=<%=rs.getString(2)%>&pcode=<%=pcode%>&mcode=<%=mcode%>
		&dcode=<%=distcode%>"><%=rs.getString(1)%></a></td>
		<td><a href="rws_borewelldetails_rpt.jsp?vcode=<%=rs.getString(2)%>&pcode=<%=pcode%>&mcode=<%=mcode%>
		&dcode=<%=distcode%>"><%=rs.getInt(3)%></a></td>
