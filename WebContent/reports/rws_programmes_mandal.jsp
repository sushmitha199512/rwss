  ...
  <% try
	...
	String  q1="";
	   ...
	   for(int b=0;b<programcodes.length;b++)
	     ...
	     str="SELECT distinct m.mname as MANDAL,count(*) as TOTAL"+q1+",m.mcode FROM RWS_PRAJAPADAM_TBL pr,RWS_PRAJAPADAM_EXIST_SCH_TBL E,rws_mandal_tbl m WHERE pr.HAB_CODE=E.HAB_CODE and substr(pr.hab_code,1,2)=m.dcode and substr(pr.hab_code,6,2)=m.mcode and substr(pr.hab_code,1,2)='"+distcode+"'  group by m.mcode,m.mname order by m.mcode";
	...
	<tr align="center">
  <td align="center" class="rptHeading" colspan=<%=Cnt%> >&nbsp;&nbsp;&nbsp;&nbsp;District:<%=distname%>&nbsp;&nbsp;Program:<%=programname%>
	  &nbsp;&nbsp;</td>
  </tr>
...
%>        
		...
		<td class=rptValue><a href="rws_programmes_panchyat.jsp?district=<%=circlecode%>&program=<%=programcode%>&dcode=<%=distcode%>&mancode=<%=rs.getString("mcode")%>&distname=<%=distname%>&manname=<%=rs.getString(1)%>"><%=rs.getString(1)%></a></td>
		...
		if(record[z]!=null && !record[z].equals("0"))
			{%>
			<td class=rptValue><a href="./rws_prajapath_rpt1.jsp?district=<%=circlecode%>&program=<%=programcode%>&dcode1=<%=distcode%>&prog=<%=programcodes[z]%>"><%=record[z]%></a></td>
		<%
            }
            else{%>
...
<td class=btext align="center"><%=grandtotal%></td>
	...
	String str2= "select distinct p.programme_code,p.programme_name FROM rws_PROGRAMME_tbl p order by p.programme_code";
	   ...
	   str="SELECT distinct m.mname as MANDAL,count(*) AS TOTAL"+q1+",m.mcode FROM RWS_PRAJAPADAM_TBL pr,RWS_PRAJAPADAM_EXIST_SCH_TBL E,rws_mandal_tbl m WHERE pr.HAB_CODE=E.HAB_CODE and substr(pr.hab_code,1,2)=m.dcode and m.mcode=substr(pr.hab_code,6,2) and substr(pr.hab_code,1,2)='"+distcode+"' and pr.programme_code='"+programcode+"' group by m.mname,m.mcode  order by m.mcode ";
		 ...
		 int grandtotal=0,slno=0,total=0;%>
		 <tr bgcolor="#8A9FCD" align="center">
		<td align="center" class="rptHeading" colspan=<%=Cnt%>>Programme Wise-Works Report</font></td>	
	</tr>
	<tr align="center">
...
%>        
        <tr align=center>
		<td class=rptValue><%=slno%></td>
		<td class=rptValue><a href="rws_programmes_panchyat.jsp?district=<%=circlecode%>&program=<%=programcode%>&dcode=<%=distcode%>&distname=<%=distname%>&mancode=<%=rs.getString("mcode")%>&manname=<%=rs.getString(1)%>"><%=rs.getString(1)%></a></td>
		<td class=btext><%=rs.getString("TOTAL")%></td>
		<%
		String[] record = null;
	    record = new String[Cnt-1];
		...
		&dcode=<%=distcode%>&mcode=<%=rs.getString("mcode")%>"><%=record[z]%></a></td>
			<%
            }
            else{%>
            <td class=rptValue><%=record[z]%></td>
...
%>        
        <tr align=center>
		<td class=rptValue><%=slno%></td>
		<td class=rptValue><a href="rws_programmes_panchyat.jsp?district=<%=circlecode%>&program=<%=programcode%>&dcode=<%=distcode%>&mancode=<%=rs.getString("mcode")%>&manname=<%=rs.getString(1)%>"><%=rs.getString(1)%></a>
		</td>
		<td class=btext><%=rs.getString("Total")%></td>
		<%
		String[] record = null;
		...
		if(record[z]!=null && !record[z].equals("0"))
			{%>
			<td class=rptValue><a href="./rws_prajapath_rpt1.jsp?district=<%=circlecode%>&program=<%=programcode%>&prog=<%=programcodes[z]%>&mcode=<%=rs.getString("mcode")%>"><%=record[z]%></a></td>
			<% 
			}
			 else{%>
            <td class=rptValue><%=record[z]%></td>
...
%>        
        <tr align=center>
		<td class=rptValue><%=slno%></td>
		<td class=rptValue><a href="rws_programmes_panchyat.jsp?district=<%=circlecode%>&program=<%=programcode%>&mancode=<%=rs.getString("mcode")%>&dcode=<%=distcode%>&manname=<%=rs.getString(1)%>"><%=rs.getString(1)%></a>
		</td>
		<td class=btext><%=rs.getString("Total")%></td>
		<%
		String[] record = null;
		...
		if(record[z]!=null && !record[z].equals("0"))
			{%>
		<td class=rptValue><a href="./rws_prajapath_rpt1.jsp?district=<%=circlecode%>&program=<%=programcode%>
		&mcode=<%=rs.getString("mcode")%>"><%=record[z]%></a></td>
		<% 
			}
			 else{%>
            <td class=rptValue><%=record[z]%></td>
