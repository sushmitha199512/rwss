...
try
	{
      String qry="",qry1="",qry2="",qry3="",qry4="",qry5="",qry6="",qry7="",qry8="",qry9="",qry10="",qry11="",qry12="",qry13="",qry14="",qry15="";
        ...
        String mycolor="";
	         ...
	         qry15="SELECT distinct substr(a.work_id,5,2),count(a. work_id) FROM RWS_WORK_ADMN_TBL a,rws_work_commencement_tbl c where a.TYPE_OF_ASSET in("+scheme+") and a.work_id=c.work_id and work_cancel_dt is null  and a.work_id not in (select work_id from rws_work_completion_tbl) and sysdate - grounding_date>5*365 and substr(a.work_id,1,2) in ("+program+")  group by substr(a.work_id,5,2)";
           ...
           qry14="SELECT distinct substr(a.work_id,5,2),count(a. work_id) FROM RWS_WORK_ADMN_TBL a,rws_work_commencement_tbl c where a.TYPE_OF_ASSET in("+scheme+") and a.work_id=c.work_id and work_cancel_dt is null  and a.work_id not in (select work_id from rws_work_completion_tbl) and sysdate - grounding_date<5*365  and sysdate - grounding_date>4*365 and substr(a.work_id,1,2) in ("+program+")  group by substr(a.work_id,5,2)";
	       ...
	       // System.out.println("qry14 "+qry14);
	        ...
	        qry13="SELECT distinct substr(a.work_id,5,2),count(a. work_id) FROM RWS_WORK_ADMN_TBL a,rws_work_commencement_tbl c where a.TYPE_OF_ASSET in("+scheme+") and  a.work_id=c.work_id and work_cancel_dt is null  and a.work_id not in (select work_id from rws_work_completion_tbl) and sysdate - grounding_date<4*365  and sysdate - grounding_date>3*365 and substr(a.work_id,1,2) in ("+program+")  group by substr(a.work_id,5,2)";
	       ...
	       qry12=" SELECT distinct substr(a.work_id,5,2),count(a. work_id) FROM RWS_WORK_ADMN_TBL a,rws_work_commencement_tbl c where a.TYPE_OF_ASSET in("+scheme+") and  a.work_id=c.work_id and work_cancel_dt is null  and a.work_id not in (select work_id from rws_work_completion_tbl) and sysdate - grounding_date<3*365  and sysdate - grounding_date>2*365 and substr(a.work_id,1,2) in ("+program+")  group by substr(a.work_id,5,2)";
	      ...
	      // System.out.println("qry12 "+qry12);
	       ...
	       qry11=" SELECT distinct substr(a.work_id,5,2),count(a. work_id) FROM RWS_WORK_ADMN_TBL a,rws_work_commencement_tbl c where a.TYPE_OF_ASSET in("+scheme+") and  a.work_id=c.work_id and work_cancel_dt is null  and a.work_id not in (select work_id from rws_work_completion_tbl) and sysdate - grounding_date<2*365  and sysdate - grounding_date>365 and substr(a.work_id,1,2) in ("+program+")  group by substr(a.work_id,5,2)";
	     ...
	     //  System.out.println("qry11 "+qry11);
	        ...
	        qry10="SELECT distinct substr(a.work_id,5,2),count(a. work_id) FROM RWS_WORK_ADMN_TBL a,rws_work_commencement_tbl c where a.TYPE_OF_ASSET in("+scheme+") and  a.work_id=c.work_id and work_cancel_dt is null  and a.work_id not in (select work_id from rws_work_completion_tbl) and sysdate - grounding_date>365 and substr(a.work_id,1,2) in ("+program+")  group by substr(a.work_id,5,2)";
	        ...
	        qry9="SELECT distinct substr(a.work_id,5,2),count(a. work_id) FROM RWS_WORK_ADMN_TBL a,rws_work_commencement_tbl c where a.TYPE_OF_ASSET in("+scheme+") and  a.work_id=c.work_id and work_cancel_dt is null  and a.work_id not in (select work_id from rws_work_completion_tbl) and sysdate - grounding_date<365 and sysdate - grounding_date>180 and substr(a.work_id,1,2) in ("+program+") group by substr(a.work_id,5,2)";
	       ...
	       // System.out.println("qry9 "+qry9);
	        ...
	        qry8="SELECT distinct substr(a.work_id,5,2),count(a. work_id) FROM RWS_WORK_ADMN_TBL a,rws_work_commencement_tbl c where a.TYPE_OF_ASSET in("+scheme+") and  a.work_id=c.work_id  and work_cancel_dt is null  and a.work_id not in (select work_id from rws_work_completion_tbl) and sysdate - grounding_date<180 and sysdate - grounding_date>90 and substr(a.work_id,1,2) in ("+program+")  group by substr(a.work_id,5,2)";
	        ...
	        qry7="SELECT distinct substr(a.work_id,5,2),count(a.work_id) FROM RWS_WORK_ADMN_TBL a,rws_new_est_tbl b where a.TYPE_OF_ASSET in("+scheme+") and  a.work_id=b.work_id and work_cancel_dt is null and  a.work_id not in(select work_id from rws_work_completion_tbl)  and a.work_id not in (select work_id from rws_work_commencement_tbl ) and sysdate - ts_date>365 and  substr(a.work_id,1,2) in ("+program+") group by substr(a.work_id,5,2)";
	      ...
	      //  System.out.println("qry7 "+qry7);
	        ...
	        qry6="SELECT distinct substr(a.work_id,5,2),count(a.work_id) FROM RWS_WORK_ADMN_TBL a,rws_new_est_tbl b where a.TYPE_OF_ASSET in("+scheme+") and  a.work_id=b.work_id and work_cancel_dt is null and  a.work_id not in(select work_id from rws_work_completion_tbl)  and a.work_id not in (select work_id from rws_work_commencement_tbl ) and sysdate - ts_date<365 and sysdate - ts_date>180 and  substr(a.work_id,1,2) in ("+program+") group by substr(a.work_id,5,2)";
        ...
        qry5="SELECT distinct substr(a.work_id,5,2),count(a.work_id) FROM RWS_WORK_ADMN_TBL a,rws_new_est_tbl b where a.TYPE_OF_ASSET in("+scheme+") and  a.work_id=b.work_id and work_cancel_dt is null and  a.work_id not in(select work_id from rws_work_completion_tbl)  and a.work_id not in (select work_id from rws_work_commencement_tbl ) and sysdate - ts_date<180 and sysdate - ts_date>90 and  substr(a.work_id,1,2) in ("+program+") group by substr(a.work_id,5,2)";
       ...
       // System.out.println("qry5 "+qry5);
        ...
        qry4="SELECT distinct substr(a.work_id,5,2),count(a.work_id) FROM RWS_WORK_ADMN_TBL a  where a.TYPE_OF_ASSET in("+scheme+") and  work_cancel_dt is null and  a.work_id not in(select work_id from rws_work_completion_tbl)  and  a.work_id not in(select work_id from rws_new_est_tbl) and sysdate - admin_date>365 and  substr(a.work_id,1,2) in ("+program+") group by substr(a.work_id,5,2)";
      ...
      //  System.out.println("qry1 "+qry1);
        ...
        qry2="SELECT substr(a.work_id,5,2),count(distinct a.work_id) FROM RWS_WORK_ADMN_TBL a where a.TYPE_OF_ASSET in("+scheme+") and  work_cancel_dt is null and  a.work_id not in(select work_id from rws_work_completion_tbl) and substr(a.work_id,1,2) in ("+program+") group by substr(a.work_id,5,2)";
