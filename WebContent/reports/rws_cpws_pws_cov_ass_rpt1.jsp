...
if (finYear.toString().substring(0, 4).equals(s.toString())) {
	...
	<table border=0 cellspacing=0 cellpadding=0 width="80%" 	style="border-collapse: collapse" ALIGN=CENTER>
		...
		Set othersSet=new HashSet();
				if (session.getAttribute("cpwsSet") == null) {
					String popCalculateQuery = "select distinct  b.hab_code,type_of_asset_code from rws_asset_mast_tbl a,rws_asset_hab_tbl b,"
							+habTableName+ " c where a.asset_code=b.asset_code and b.hab_code=c.hab_code  and a.DATE_CREATION<=to_date('31-03-"+ye+"','DD-MM-YYYY')  and c.act_coverage_status is not null and c.act_coverage_status <> 'UI'  order by type_of_asset_code";
				...
				//cpwsquery = "select count(distinct a.asset_code),substr(c.hab_code,1,2) from rws_asset_mast_tbl a,"+habTableName+" c where a.hab_code=c.hab_code and type_of_asset_code='03'  and a.DATE_CREATION<=to_date('31-03-"+ye+"','DD-MM-YYYY') and  c.act_coverage_status is not null and c.act_coverage_status <> 'UI'  group by substr(c.hab_code,1,2)";
				
				//////cpwsquery = "select count(distinct a.asset_code),substr(c.hab_code,1,2) from rws_asset_mast_tbl a,"+habTableName+" c where a.hab_code=c.hab_code and type_of_asset_code='03'  and a.DATE_CREATION<=to_date('31-03-"+ye+"','DD-MM-YYYY') and  c.act_coverage_status is not null and c.act_coverage_status <> 'UI'  group by substr(c.hab_code,1,2)";
				cpwsquery = "select count(distinct a.asset_code),substr(c.hab_code,1,2) from rws_asset_mast_tbl a,"+habTableName+" c where a.hab_code=c.hab_code and type_of_asset_code='03'  and a.DATE_CREATION<=to_date('31-03-"+ye+"','DD-MM-YYYY')  and substr(a.asset_code,5,2)=substr(c.hab_code,1,2)  and  c.act_coverage_status is not null and c.act_coverage_status <> 'UI'  group by substr(c.hab_code,1,2)";
				...
				//				pwscount = "select count(distinct a.asset_code),substr(c.hab_code,1,2) from rws_asset_mast_tbl a,rws_asset_hab_tbl b,"+habTableName+" c where a.asset_code=b.asset_code and b.hab_code=c.hab_code and type_of_asset_code='01'  and a.DATE_CREATION<=to_date('31-03-"+ye+"','DD-MM-YYYY')  and c.act_coverage_status is not null and c.act_coverage_status <> 'UI'  group by substr(c.hab_code,1,2)";

				pwscount = "select count(distinct a.asset_code),substr(c.hab_code,1,2) from rws_asset_mast_tbl a,rws_asset_hab_tbl b,"+habTableName+" c where a.asset_code=b.asset_code and b.hab_code=c.hab_code and type_of_asset_code='01'  and a.DATE_CREATION<=to_date('31-03-"+ye+"','DD-MM-YYYY')  and substr(a.asset_code,5,2)=substr(c.hab_code,1,2) and c.act_coverage_status is not null and c.act_coverage_status <> 'UI'  group by substr(c.hab_code,1,2)";
				//System.out.println("@@@@#"+pwscount);
				...
				//				String mpwscount = "select count(distinct a.asset_code),substr(c.hab_code,1,2) from rws_asset_mast_tbl a,rws_asset_hab_tbl b,"+habTableName+" c where a.asset_code=b.asset_code and b.hab_code=c.hab_code and type_of_asset_code='02'  and a.DATE_CREATION<=to_date('31-03-"+ye+"','DD-MM-YYYY')  and c.act_coverage_status is not null and c.act_coverage_status <> 'UI'  group by substr(c.hab_code,1,2)";

				String mpwscount = "select count(distinct a.asset_code),substr(c.hab_code,1,2) from rws_asset_mast_tbl a,rws_asset_hab_tbl b,"+habTableName+" c where a.asset_code=b.asset_code and b.hab_code=c.hab_code and type_of_asset_code='02'  and a.DATE_CREATION<=to_date('31-03-"+ye+"','DD-MM-YYYY')  and substr(a.asset_code,5,2)=substr(c.hab_code,1,2) and c.act_coverage_status is not null and c.act_coverage_status <> 'UI'  group by substr(c.hab_code,1,2)";
				...
				//				String hpwscount = "select count(distinct a.asset_code),substr(c.hab_code,1,2) from rws_asset_mast_tbl a,rws_asset_hab_tbl b,"+habTableName+" c where a.asset_code=b.asset_code and b.hab_code=c.hab_code and type_of_asset_code in ('04','05')  and a.DATE_CREATION<=to_date('31-03-"+ye+"','DD-MM-YYYY')  and c.act_coverage_status is not null and c.act_coverage_status <> 'UI'  group by substr(c.hab_code,1,2)";

				String hpwscount = "select count(distinct a.asset_code),substr(c.hab_code,1,2) from rws_asset_mast_tbl a,rws_asset_hab_tbl b,"+habTableName+" c where a.asset_code=b.asset_code and b.hab_code=c.hab_code and type_of_asset_code in ('04','05')  and a.DATE_CREATION<=to_date('31-03-"+ye+"','DD-MM-YYYY')  and substr(a.asset_code,5,2)=substr(c.hab_code,1,2) and c.act_coverage_status is not null and c.act_coverage_status <> 'UI'  group by substr(c.hab_code,1,2)";
				...
				//				String dpwscount = "select count(distinct a.asset_code),substr(c.hab_code,1,2) from rws_asset_mast_tbl a,rws_asset_hab_tbl b,"+habTableName+" c where a.asset_code=b.asset_code and b.hab_code=c.hab_code and type_of_asset_code='09'  and a.DATE_CREATION<=to_date('31-03-"+ye+"','DD-MM-YYYY')  and c.act_coverage_status is not null and c.act_coverage_status <> 'UI'  group by substr(c.hab_code,1,2)";

				String dpwscount = "select count(distinct a.asset_code),substr(c.hab_code,1,2) from rws_asset_mast_tbl a,rws_asset_hab_tbl b,"+habTableName+" c where a.asset_code=b.asset_code and b.hab_code=c.hab_code and type_of_asset_code='09'  and a.DATE_CREATION<=to_date('31-03-"+ye+"','DD-MM-YYYY') and substr(a.asset_code,5,2)=substr(c.hab_code,1,2) and c.act_coverage_status is not null and c.act_coverage_status <> 'UI'  group by substr(c.hab_code,1,2)";
				...
				//				String openwellsCount = "select count(distinct a.asset_code),substr(c.hab_code,1,2) from rws_asset_mast_tbl a,rws_asset_hab_tbl b,"+habTableName+" c where a.asset_code=b.asset_code and b.hab_code=c.hab_code and type_of_asset_code='06'  and a.DATE_CREATION<=to_date('31-03-"+ye+"','DD-MM-YYYY')  and c.act_coverage_status is not null and c.act_coverage_status <> 'UI'  group by substr(c.hab_code,1,2)";

				String openwellsCount = "select count(distinct a.asset_code),substr(c.hab_code,1,2) from rws_asset_mast_tbl a,rws_asset_hab_tbl b,"+habTableName+" c where a.asset_code=b.asset_code and b.hab_code=c.hab_code and type_of_asset_code='06'  and a.DATE_CREATION<=to_date('31-03-"+ye+"','DD-MM-YYYY') and substr(a.asset_code,5,2)=substr(c.hab_code,1,2) and c.act_coverage_status is not null and c.act_coverage_status <> 'UI'  group by substr(c.hab_code,1,2)";
				...
				//				String othersCount = "select count(distinct a.asset_code),substr(c.hab_code,1,2) from rws_asset_mast_tbl a,rws_asset_hab_tbl b,"+habTableName+" c where a.asset_code=b.asset_code and b.hab_code=c.hab_code and type_of_asset_code in ('07','08','10','11','12')  and a.DATE_CREATION<=to_date('31-03-"+ye+"','DD-MM-YYYY')  and c.act_coverage_status is not null and c.act_coverage_status <> 'UI'  group by substr(c.hab_code,1,2)";

				String othersCount = "select count(distinct a.asset_code),substr(c.hab_code,1,2) from rws_asset_mast_tbl a,rws_asset_hab_tbl b,"+habTableName+" c where a.asset_code=b.asset_code and b.hab_code=c.hab_code and type_of_asset_code in ('07','08','10','11','12')  and a.DATE_CREATION<=to_date('31-03-"+ye+"','DD-MM-YYYY') and substr(a.asset_code,5,2)=substr(c.hab_code,1,2) and c.act_coverage_status is not null and c.act_coverage_status <> 'UI'  group by substr(c.hab_code,1,2)";
				...
				while (rs1.next()) {
					...
					String popQuery = "select distinct b.hab_code,nvl(ACT_PLAIN_POPU,0) ,nvl(ACT_SC_POPU,0) , nvl(ACT_ST_POPU,0),type_of_asset_code   from rws_asset_mast_tbl a,rws_asset_hab_tbl b,"+habTableName+" c where a.asset_code=b.asset_code and b.hab_code=c.hab_code and type_of_asset_code in ('01','02','03','04','09','05','06','07','08','10','11','12') and substr(c.hab_code,1,2)='"
							+ rs1.getString(1) + "'  and a.DATE_CREATION<=to_date('31-03-"+ye+"','DD-MM-YYYY')  and c.act_coverage_status is not null and c.act_coverage_status <> 'UI'  order by type_of_asset_code";
		...
		<tr>
			...
			<td class=<%=style %> style="text-align: right;"><a href="#"
				onclick="viewReport('rws_cpws_pws_cov_ass_rpt_drill1.jsp?name=cpws&type=asset&dcode=<%=rs1.getString(1) %>&finYear=<%=finYear%>')"><%=ast%></a></td>
			<td class=<%=style %> style="text-align: right;"><a href="#"
				onclick="viewReport('rws_cpws_pws_cov_ass_rpt_drill1.jsp?name=cpws&type=hab&dcode=<%=rs1.getString(1) %>&finYear=<%=finYear%>')"><%=cpwsCount %></a></td>
			<td class=<%=style %> style="text-align: right;"><%=cpwsPop %></td>
			<td class=<%=style %> style="text-align: right;"><a href="#"
				onclick="viewReport('rws_cpws_pws_cov_ass_rpt_drill1.jsp?name=pws&type=asset&dcode=<%=rs1.getString(1) %>&finYear=<%=finYear%>')"><%=ast3%></a></td>
			<td class=<%=style %> style="text-align: right;"><a href="#"
				onclick="viewReport('rws_cpws_pws_cov_ass_rpt_drill1.jsp?name=pws&type=hab&dcode=<%=rs1.getString(1) %>&finYear=<%=finYear%>')"><%=pwsCount %></a></td>
			<td class=<%=style %> style="text-align: right;"><%=pwsPop %></td>
			<td class=<%=style %> style="text-align: right;"><a href="#"
				onclick="viewReport('rws_cpws_pws_cov_ass_rpt_drill1.jsp?name=mpws&type=asset&dcode=<%=rs1.getString(1) %>&finYear=<%=finYear%>')"><%=ast6%></a></td>
			<td class=<%=style %> style="text-align: right;"><a href="#"
				onclick="viewReport('rws_cpws_pws_cov_ass_rpt_drill1.jsp?name=mpws&type=hab&dcode=<%=rs1.getString(1) %>&finYear=<%=finYear%>')"><%=mpwsCount %></a></td>
			<td class=<%=style %> style="text-align: right;"><%=mpwsPop %></td>
			<td class=<%=style %> style="text-align: right;"><a href="#"
				onclick="viewReport('rws_cpws_pws_cov_ass_rpt_drill1.jsp?name=dp&type=asset&dcode=<%=rs1.getString(1) %>&finYear=<%=finYear%>')"><%=ast12%></a></td>
			<td class=<%=style %> style="text-align: right;"><a href="#"
				onclick="viewReport('rws_cpws_pws_cov_ass_rpt_drill1.jsp?name=dp&type=hab&dcode=<%=rs1.getString(1) %>&finYear=<%=finYear%>')"><%=dpCount %></a></td>
			<td class=<%=style %> style="text-align: right;"><%=dpPop %></td>
			<td class=<%=style %> style="text-align: right;"><a href="#"
				onclick="viewReport('rws_cpws_pws_cov_ass_rpt_drill1.jsp?name=hp&type=asset&dcode=<%=rs1.getString(1) %>&finYear=<%=finYear%>')"><%=ast9%></a></td>
			<td class=<%=style %> style="text-align: right;"><a href="#"
				onclick="viewReport('rws_cpws_pws_cov_ass_rpt_drill1.jsp?name=hp&type=hab&dcode=<%=rs1.getString(1) %>&finYear=<%=finYear%>')"><%=hpCount %></a></td>
			<td class=<%=style %> style="text-align: right;"><%=hpPop %></td>
			<td class=<%=style %> style="text-align: right;"><a href="#"
				onclick="viewReport('rws_cpws_pws_cov_ass_rpt_drill1.jsp?name=ow&type=asset&dcode=<%=rs1.getString(1) %>&finYear=<%=finYear%>')"><%=ast13 %></a></td>
			<td class=<%=style %> style="text-align: right;"><a href="#"
				onclick="viewReport('rws_cpws_pws_cov_ass_rpt_drill1.jsp?name=ow&type=hab&dcode=<%=rs1.getString(1) %>&finYear=<%=finYear%>')"><%=openWellCount  %></a></td>
			<td class=<%=style %> style="text-align: right;"><%=oWPop %></td>
			<td class=<%=style %> style="text-align: right;"><a href="#"
				onclick="viewReport('rws_cpws_pws_cov_ass_rpt_drill1.jsp?name=o&type=asset&dcode=<%=rs1.getString(1) %>&finYear=<%=finYear%>')"><%=ast14 %></a></td>
			<td class=<%=style %> style="text-align: right;"><a href="#"
				onclick="viewReport('rws_cpws_pws_cov_ass_rpt_drill1.jsp?name=o&type=hab&dcode=<%=rs1.getString(1) %>&finYear=<%=finYear%>')"><%=oCount %></a></td>
