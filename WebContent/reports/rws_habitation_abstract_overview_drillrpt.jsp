		...
		<table border=0 cellspacing=0 cellpadding=0 height=5% width="100%" bordercolor=#000000 style="border-collapse:collapse"	bgcolor="#ffffff" align="center">
			...
			<tbody>
						...
						if (lpcdrange.toString().equals("55")) {
							if (poptype.toString() != null && !poptype.toString().equals("") && !poptype.toString().equals("ALL")) {
								if (poptype.toString().equals("SC")) {
									query += " and (0.4)*(nvl(hd.CENSUS_PLAIN_POPU,'0')+nvl(hd.CENSUS_SC_POPU,'0')+nvl(hd.CENSUS_ST_POPU,'0')+nvl(hd.CENSUS_MINORITY_POPU,'0'))<= hd.CENSUS_SC_POPU and (0.4)*(nvl(hd.CENSUS_PLAIN_POPU,'0')+nvl(hd.CENSUS_SC_POPU,'0')+nvl(hd.CENSUS_ST_POPU,'0')+nvl(hd.CENSUS_MINORITY_POPU,'0'))> hd.CENSUS_ST_POPU  ";
								} else if (poptype.toString().equals("ST")) {
									query += "  AND (0.4)*(NVL(HD.CENSUS_PLAIN_POPU,'0')+NVL(HD.CENSUS_SC_POPU,'0')+NVL(HD.CENSUS_ST_POPU,'0')+NVL(HD.CENSUS_MINORITY_POPU,'0'))<= HD.CENSUS_ST_POPU ";
								} else {
									query += " AND (0.4)*(NVL(HD.CENSUS_PLAIN_POPU,'0')+NVL(HD.CENSUS_SC_POPU,'0')+NVL(HD.CENSUS_ST_POPU,'0')+NVL(HD.CENSUS_MINORITY_POPU,'0'))> HD.CENSUS_SC_POPU AND  (40/100)*(NVL(HD.CENSUS_PLAIN_POPU,'0')+NVL(HD.CENSUS_SC_POPU,'0')+NVL(HD.CENSUS_ST_POPU,'0')+NVL(HD.CENSUS_MINORITY_POPU,'0'))> HD.CENSUS_ST_POPU  ";
								}
							}
						}else{
							if (poptype.toString() != null && !poptype.toString().equals("") && !poptype.toString().equals("ALL")) {
				...
				<tr>
					...
					<td class="<%=style%>"><%=rs1.getInt(6)+rs1.getInt(7)+rs1.getInt(8)+rs1.getInt(9)+rs1.getInt(10)%></td>
