...
<%
if(assettype.equals("01") || assettype.equals("02") || assettype.equals("03"))
{
%>
		<td align="center" class="rptHeading" colspan="14"> Assets  <%=assetName%> - With out Sources , Reservoirs  Report</td>
<%}
  else {%>
<td align="center" class="rptHeading" colspan="14"> Assets  <%=assetName%> - Without Sources  Report</td>
<%} %>
...
%>
			   ...
			   <td class=btext><%=assetName %> No</td>
			   <td class=btext><%=assetName %> Without Sources</td>
			   <td class=btext><%=assetName %> Without Reservoirs</td>
<%} else if(assettype.equals("09")){
...
try
{
	String query = "";
	 ...
	 if(dcode.equals("51"))
    	...
    	while(rs1.next())
          ...
          <% 
            String query11s="select count(*),type_of_asset_code  from rws_asset_mast_tbl a,rws_asset_scheme_tbl b,rws_complete_hab_view p where  p.panch_code=a.hab_code and substr(hab_code,1,2)='"+rs1.getString(1)+"' and type_of_asset_code='"+assettype+"' and a.asset_code=b.asset_code group by type_of_asset_code"; 
		 	...
		 	rs8=stmt8.executeQuery(query11s);
	       ...
	       %><td class=rptValue align="right"><%=rs8.getInt(1)%></td><%
	     	...
	     	if(pwssource!=0){
						%>
                <td class=rptValue align="right"><a href="rws_sources_nodetails_mandal_rpt.jsp?district=<%=rs1.getString(1)%>&assettype=<%=assettype2%>"><%=pwssource%></a></td>
                <%}
                else{
                 %>
                 <td class=rptValue align="right"><%=pwssource%></td>
                 <%}
		 	...
		 	rs4=stmt4.executeQuery(query122);
			 while(rs4.next())
			 {
			 	pwsreservoirs++;
				...
				gpwsreservoirs=pwsreservoirs+gpwsreservoirs;
	...
	if(assettype.equals("01") || assettype.equals("02") || assettype.equals("03"))
	{
	     	if(pwsreservoirs>0)
	     		{
						%>
                <td class=rptValue align="right"><a href="rws_reservoirs_nodetails_mandal_rpt.jsp?district=<%=rs1.getString(1)%>&assettype=<%=assettype2%>"><%=pwsreservoirs%></a></td>
                <%}
                else
                {
                 %>
                 <td class=rptValue align="right"><%=pwsreservoirs%></td>
                 <%} }%>
...
{
%>
     <td class="btext" align="right"><%=gpwsreservoirs%></td>
<%} %>
