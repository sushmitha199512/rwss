...
<td class=btext  align=center colspan=11 style="background-color:lightyellow">Spill Over Works<font color="red"><b>---(<%=subProg %>)</b></font></td>
<td class=btext colspan=9 align=center style="background-color:#d2b48c">New Works <font color="red"><b>---(<%=subProg %>)</b></font></td>
</tr>
<tr>
...
String popQuery="select a.hab_code,nvl(a.CENSUS_PLAIN_POPU,0),nvl(a.CENSUS_SC_POPU,0),nvl(a.CENSUS_ST_POPU,0) from rws_habitation_directory_tbl a where a.COVERAGE_STATUS <>'UI' ";
	...
	String newWorksCompQuery="select b.circle_office_code,b.division_office_code,count(*) FROM RWS_WORK_ADMN_TBL W ,rws_division_office_tbl b where  (w.ADMIN_DATE >= '01-Apr-"+startyr+"' and w.ADMIN_DATE < '01-Apr-"+endyr+"') and work_cancel_dt is null and substr(w.office_code,2,2)=b.circle_office_code and substr(w.office_code,4,1)=b.division_office_code and  w.category_code='1' ";
 if(scheme!=null && !scheme.equals("")){
...
String newWorksCurExpQuery="select b.circle_office_code,b.division_office_code,sum(case when (to_date(voucher_dt)>= '01-APR-"+endyr+"' and to_date(voucher_dt)< '01-APR-"+endyr+"') then voucher_Amt else 0 end) FROM RWS_WORK_ADMN_TBL W ,rws_division_office_tbl b,rws_work_exp_voucher_tbl a  where  (w.ADMIN_DATE >= '01-Apr-"+startyr+"' and w.ADMIN_DATE < '01-Apr-"+endyr+"') and work_cancel_dt is null and  w.category_code='1'";
 if(scheme!=null && !scheme.equals("")){
...
String spillExpQuery="SELECT b.circle_office_code,b.division_office_code,w.work_id,sum(case when to_date(voucher_dt)< '01-Apr-"+startyr+"' then voucher_amt else 0 end)||'@'||sum(case when (to_date(voucher_dt)>= '01-Apr-"+startyr+"' and to_date(voucher_dt)< '01-APR-"+endyr+"') then voucher_Amt else 0 end)  FROM RWS_WORK_ADMN_TBL W ,rws_division_office_tbl b,rws_work_exp_voucher_tbl a where to_date(w.ADMIN_DATE) <'01-Apr-"+startyr+"'   and a.work_id=w.work_id  and  w.category_code='1'  and substr(w.office_code,2,2)=b.circle_office_code and substr(w.office_code,4,1)=b.division_office_code  and w.programme_code in ("+programe+") ";
...
String spillRelBankQuery="SELECT b.circle_office_code,b.division_office_code,w.work_id,sum(case when (to_date(BANK_SEND_DATE)>= '01-APR-"+startyr+"' and to_date(BANK_SEND_DATE) < '01-APR-"+endyr+"') then BILL_AMOUNT_CENTRAL else 0 end)  FROM RWS_WORK_ADMN_TBL W ,rws_division_office_tbl b,RWS_bill_bank_bridge_det_TBL a where to_date(w.ADMIN_DATE) <'01-Apr-"+startyr+"'   and a.work_id=w.work_id  and substr(w.office_code,2,2)=b.circle_office_code and  w.category_code='1' and substr(w.office_code,4,1)=b.division_office_code  and w.programme_code in ("+programe+") ";
...
////System.out.println("spillRelBankQuery"+spillRelBankQuery);
 ...
 double sbankUpto1=0;
       if(srelban!=null){    
  sbankUpto1=Double.parseDouble(srelban)/100000;
 }
...
if(srelpao!=null){
 double spaoUpto1=Double.parseDouble(srelpao)/100000;
...
if(scheme!=null && !scheme.equals("")){
 ...
 spHabsCurrent+="and  w.work_id  in (select work_id from rws_work_completion_tbl where (to_date(DATE_OF_COMPLETION) >= '01-APR-"+endyr+"') and (to_date(DATE_OF_COMPLETION) < '01-"+nextmonth+"-"+endyr+"')) and b.circle_office_code='"+rs1.getString(3)+"' and b.division_office_code='"+rs1.getString(4)+"' "; 
...
////System.out.println("New Works"+newWorksEstCostQuery);
	...
	String newWorksExpQuery="select b.circle_office_code,b.division_office_code,w.work_id,sum(case when to_date(voucher_dt)< '01-Apr-"+startyr+"' then voucher_amt else 0 end)||'@'||sum(case when (to_date(voucher_dt)>= '01-Apr-"+startyr+"' and to_date(voucher_dt)< '01-APR-"+endyr+"') then voucher_Amt else 0 end) FROM RWS_WORK_ADMN_TBL W ,rws_division_office_tbl b,rws_work_exp_voucher_tbl a  where  (w.ADMIN_DATE >= '01-Apr-"+startyr+"' and w.ADMIN_DATE < '01-Apr-"+endyr+"') and work_cancel_dt is null and  w.category_code='1'";
 if(scheme!=null && !scheme.equals("")){
...
String newCurRelBankQuery="select b.circle_office_code,b.division_office_code,w.work_id,sum(case when (to_date(BANK_SEND_DATE)>= '01-APR-"+startyr+"' and to_date(BANK_SEND_DATE) < '01-APR-"+endyr+"') then BILL_AMOUNT_CENTRAL else 0 end)||'@'||sum(case when (to_date(BANK_SEND_DATE)>='01-APR-"+endyr+"' and to_date(BANK_SEND_DATE)< '01-"+nextmonth+"-"+endyr+"') then BILL_AMOUNT_CENTRAL else 0 end) FROM RWS_WORK_ADMN_TBL W ,rws_division_office_tbl b,RWS_bill_bank_bridge_det_TBL a  where  (w.ADMIN_DATE >= '01-Apr-"+startyr+"' and w.ADMIN_DATE < '01-Apr-"+endyr+"') and work_cancel_dt is null and  w.category_code='1'";
 if(scheme!=null && !scheme.equals("")){
...
String newCurRelpaoQuery="select b.circle_office_code,b.division_office_code,w.work_id,sum(case when (to_date(STATE_SEND_DATE)>= '01-APR-"+startyr+"' and to_date(STATE_SEND_DATE) < '01-APR-"+endyr+"') then BILL_AMOUT_STATE else 0 end)||'@'||sum(case when (to_date(STATE_SEND_DATE)>='01-APR-"+endyr+"' and to_date(STATE_SEND_DATE)< '01-"+nextmonth+"-"+endyr+"') then BILL_AMOUT_STATE else 0 end) FROM RWS_WORK_ADMN_TBL W ,rws_division_office_tbl b,RWS_bill_pao_bridge_det_TBL a  where  (w.ADMIN_DATE >= '01-Apr-"+startyr+"' and w.ADMIN_DATE < '01-Apr-"+endyr+"') and work_cancel_dt is null and  w.category_code='1'";
 if(scheme!=null && !scheme.equals("")){
...
String newWorksHabs="select distinct w.work_id FROM RWS_WORK_ADMN_TBL W ,rws_division_office_tbl b,rws_admn_hab_lnk_tbl c  where w.work_id=c.work_id and  (w.ADMIN_DATE >= '01-Apr-"+startyr+"' and w.ADMIN_DATE < '01-Apr-"+endyr+"') and work_cancel_dt is null and substr(w.office_code,2,2)=b.circle_office_code and substr(w.office_code,4,1)=b.division_office_code and  w.category_code='1'";
if(scheme!=null && !scheme.equals("")){
...
String newWoksCoveredHabs="select distinct w.work_id FROM RWS_WORK_ADMN_TBL W ,rws_division_office_tbl b,RWS_WORK_COMP_HAB_LNK_TBL c  where w.work_id=c.work_id and  (w.ADMIN_DATE >= '01-Apr-"+startyr+"' and w.ADMIN_DATE < '01-Apr-"+endyr+"') and work_cancel_dt is null and substr(w.office_code,2,2)=b.circle_office_code and substr(w.office_code,4,1)=b.division_office_code and  w.category_code='1' ";
if(scheme!=null && !scheme.equals("")){
          newWoksCoveredHabs+="and w.type_of_asset in("+scheme+")";
}

newWoksCoveredHabs+=" and  w.programme_code in ("+programe+")  and w.work_id in(select work_id from rws_work_completion_tbl where (to_date(DATE_OF_COMPLETION) > '01-Apr-"+startyr+"') and (to_date(DATE_OF_COMPLETION) < '01-APR-"+endyr+"')) and   b.circle_office_code='"+rs1.getString(3)+"' and b.division_office_code='"+rs1.getString(4)+"'  order by 1";
		stmt4=conn.createStatement();
...
<td class=btext align=right><%=ndf.format(gnbal) %></td>
