	...
	<table  border=0 rules=none style="border-collapse:collapse" align = "right">
		<tr align="right">
			<td class="bwborder">
				...
				<a href="./rws_dist_popcate_xls.jsp" target="_new">Excel</a>
	  ...
	  // int count3=0;
       ...
       if(popRange[j].equals("SC"))
	   {
			...
			st3=conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
			 //System.out.println("query3 is0:"+query3);
       ...
       //System.out.println("after query is:"+query3);
		rs3 = st3.executeQuery(query3);
		while(rs3.next())
		{
			if (rs3.getInt(1)<=rs3.getInt(2))
			{
    		 if (rs3.getInt(1)<=rs3.getInt(3))
    		 {
         	  //min=x;
				...
				if(((rs3.getInt(1)+rs3.getInt(2)+rs3.getInt(3))*0.5) == 0)
				 {
			   		count++;
...
%>
		...
		<tr>	
			 ...
			 <td class=btext align="right"><%=count+count1+count2%></td>
			 <td class=btext align="right"><%=plaintot+sctot+sttot%></td>
		     <td class=rptValue align="right"><%=count%></td>
			 <td class=rptValue align="right"><%=plaintot%></td>
			 <td class=rptValue align="right"><%=count1%></td>
			 <%////System.out.println(count1+" sctot "+sctot);%>
			 <td class=rptValue align="right"><%=sctot%></td>
			 <td class=rptValue align="right"><%=count2%></td>
			 <%////System.out.println(count1+" sttot "+sttot);%>
			 <td class=rptValue align="right"><%=sttot%></td>
		</tr>
<%
			}
			catch(Exception e)
			...
			%><tr>
		<td align="center" class=btext colspan=2>Grand Totals</td>	
		<td class=btext align="right"><%=count3%>	</td>
		<td class=btext align="right"><%=pop%></td>	
	    <td class=rptValue align="right"><%=counttot%></td>
	    <td class=rptValue align="right"><%=grplantot%></td>
	    <td class=rptValue align="right"><%=counttot1%></td>
	    <td class=rptValue align="right"><%=grsctot%></td>
	    <td class=rptValue align="right"><%=counttot2%></td>
	    <td class=rptValue align="right"><%=gesttot%></td>
 		</tr>
		 <%
		}
		catch(Exception e)
