...
<%		
	...
	String dname=null;
	 ...
	 {
	 	String q="select dname from rws_district_tbl where dcode='"+district+"' ";
...
<form name=f1 method="post">
 ...
 <table border = 1 cellspacing = 0 cellpadding = 0 width=100% height=20%  bordercolor=#000000 style="border-collapse:collapse" bgcolor="#ffffff">
	...
	<tr>
	    <td class="btext" rowspan=1>S.NO.</td>
		<td class="btext" rowspan=1>MANDALS</td>
...
%>
	...
	<tr>
        ...
        <td class="mystyle1"><font color="#6D839C"><%=rs1.getString(2)%></font></td>
<%	 //PWS
	    String q="SELECT COUNT(*) FROM RWS_ASSET_MAST_TBL M,RWS_ASSET_SCHEME_TBL S,rws_mandal_tbl m WHERE " +
	   		     "M.ASSET_CODE=S.ASSET_CODE AND SUBSTR(SCHEME_CODE,17,3) LIKE '6%' AND M.TYPE_OF_ASSET_CODE='01' and substr(m.hab_code,1,2)=m.dcode and substr(m.hab_code,6,2)=m.mcode and substr(m.hab_code,1,2)='"+dcode+"' and  substr(m.hab_code,6,2)='"+rs1.getString(1)+"' ";
...
<%		}
		...
		String q1="SELECT COUNT(*) FROM RWS_ASSET_MAST_TBL M,RWS_ASSET_SCHEME_TBL S,rws_mandal_tbl m WHERE "+
	   		      "M.ASSET_CODE=S.ASSET_CODE AND SUBSTR(SCHEME_CODE,17,3) LIKE '4%' AND M.TYPE_OF_ASSET_CODE='02' and substr(m.hab_code,1,2)=m.dcode and substr(m.hab_code,6,2)=m.mcode and substr(m.hab_code,1,2)='"+dcode+"' and  substr(m.hab_code,6,2)='"+rs1.getString(1)+"' ";
...
<%
	    ...
	    String q2="SELECT COUNT(*) FROM RWS_ASSET_MAST_TBL M,RWS_ASSET_SCHEME_TBL S,rws_mandal_tbl m WHERE "+
   		" M.ASSET_CODE=S.ASSET_CODE AND SUBSTR(SCHEME_CODE,17,3) LIKE '8%' AND M.TYPE_OF_ASSET_CODE='03' and substr(m.hab_code,1,2)=m.dcode and substr(m.hab_code,6,2)=m.mcode and substr(m.hab_code,1,2)='"+dcode+"' and  substr(m.hab_code,6,2)='"+rs1.getString(1)+"' ";
