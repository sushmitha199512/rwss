...
<%
	...
	/* String pQry = "and b.programme_code in (01,02,18,23,32,48,49,50,51,58,59,60,72)"; */
	String nrdwpProgrammeCodes=Constants.NRDWP_RELATED;
	String pQry = "and b.programme_code in ("+nrdwpProgrammeCodes+")";
try
{
	...
	String q = "select sum(OP_CL_BAL),substr(office_code,2,3) from RWS_GRANT_OP_CL_BAL_TBL where programme_code in ("+nrdwpProgrammeCodes+") group by substr(office_code,2,3)";
	...
	String q1 = "select nvl(sum(b.LOC_AMOUNT),0),substr(office_to_code,2,3) from rws_loc_master_tbl a,rws_loc_details_tbl b,rws_bro_master_tbl c where a.loc_id=b.loc_id and a.bro_id=b.bro_id and b.bro_id=c.bro_id and   a.bro_id=c.bro_id and fin_year='2010-2011' "+pQry+" group by substr(office_to_code,2,3)";
	...
	String q3 = "select nvl(sum(voucher_amt),0),substr(b.office_code,2,3) from rws_work_exp_voucher_tbl a,rws_work_admn_tbl b where a.work_id=b.work_id and BILL_SL_NO <> '0' "+pQry+" group by substr (b.office_code,2,3)";
	...
	String q5 = "select nvl(sum(BILL_AMT_CONTRACTOR),0),nvl(sum(BILL_AMT_ADMISSIBLE),0),substr(b.office_code,2,3) from rws_work_bill_tbl a,rws_work_admn_tbl b where a.work_id=b.work_id and bill_sl_no<>'0' "+pQry+" group by substr(b.office_code,2,3)";
...
<tr  class="alternateRow">
			...
			<td class="rptValue" align="right"><a href="#" onClick="wopen('rws_ob_releases_rpt.jsp?dcode=00&divcode=0&dnamee=Hyderabad&divname=Head Office','popup', 600, 300)"><%=ndf.format(Double.parseDouble(ob))%></a></td>
			<%}else{%>
	...
	<%
		  int countt = 0;
		while(rs.next()){
		ob = ""+ht.get(rs.getString(1)+rs.getString(2));
		if(ob!=null && !ob.equals("null")){}else{ob="0";}
		...
		String bill = ""+ht3.get(rs.getString(1)+rs.getString(2));
		...
		<!-- OB -->
		<%if(ob!=null && !ob.equals("0")){%>
		<td class="rptValue" align="right"><a href="#" onClick="wopen('rws_ob_releases_rpt.jsp?dcode=<%=rs.getString(1)%>&divcode=<%=rs.getString(2)%>&dnamee=<%=rs.getString(3)%>&divname=<%=rs.getString(4)%>&special=Y','popup', 900, 300)"><%=ndf.format(Double.parseDouble(ob))%></a></td>
		<%}else{%>
		<td class="rptValue" align="right"><%=ndf.format(Double.parseDouble(ob))%></td>
		<%}%>
		<!-- Releases -->
		<%if(release!=null && !release.equals("0")){%>
		<td class="rptValue" align="right"><a href="#" onClick="wopen('rws_releases_rpt.jsp?dcode=<%=rs.getString(1)%>&divcode=<%=rs.getString(2)%>&dnamee=<%=rs.getString(3)%>&divname=<%=rs.getString(4)%>&special=Y','popup', 900, 300)"><%=ndf.format(Double.parseDouble(release)*100000)%></a></td>
		<%}else{%>
		...
		<td class="rptValue" align="right"><a href="#" onClick="wopen('rws_exp_vouhcers_rpt.jsp?dcode=<%=rs.getString(1)%>&divcode=<%=rs.getString(2)%>&dnamee=<%=rs.getString(3)%>&divname=<%=rs.getString(4)%>&special=Y','popup', 900, 300)"><%=ndf.format(Double.parseDouble(exp))%></a></td>
		<%}else{%>
		...
		<td class="rptValue" align="right"><a href="#" onClick="wopen('rws_bill_rpt.jsp?dcode=<%=rs.getString(1)%>&divcode=<%=rs.getString(2)%>&dnamee=<%=rs.getString(3)%>&divname=<%=rs.getString(4)%>&special=Y','popup', 900, 300)"><%=ndf.format(Double.parseDouble(bill))%></a></td>
		<%}else{%>
	  ...
	  %>
		<tr class="normalRow">
			...
			<td nowrap class="text2bold1" align="right"><%=ndf.format(col4+col5)%></td>
			<td nowrap class="text2bold1" align="right"><%=ndf.format(col7)%></td>
			<td nowrap class="text2bold1" align="right"><%=ndf.format((col4+col5)-col7)%></td>
			<td nowrap class="text2bold1" align="right"><%=ndf.format(col9)%></td>
			<td nowrap class="text2bold1" align="right"><%=ndf.format(col10)%></td>
			<td nowrap class="text2bold1" align="right"><%=ndf.format((col4+col5)-col10)%></td>
			<td>&nbsp;</td>
	  </tr>
	  <tr class="alternateRow">
		<td align="center"  class="locked" colspan=3>Total(In Lakhs.)</td>
			...
			<td nowrap class="text2bold1" align="right"><%=ndf.format((col4+col5)/100000)%></td>
			<td nowrap class="text2bold1" align="right"><%=ndf.format(col7/100000)%></td>
			<td nowrap class="text2bold1" align="right"><%=ndf.format(((col4+col5)-col7)/100000)%></td>
			...
			<td nowrap class="text2bold1" align="right"><%=ndf.format(((col4+col5)-col10)/100000)%></td>
			<td>&nbsp;</td>
	  </tr>
	</tbody>
	</table>
...
<table align="center" cellpadding="0" cellspacing="0">
  <col width="311">
  <tr height="17">
    <td height="17" width="500"><span class="style1">20% ARWS, ARWS, ARWS (Scp), ARWS BHARAT NIRMAN</span></td>
  </tr>
  <tr height="17">
    <td height="17"><span class="style1">Bharat Nirman, NRDWP-COV, NRDWP-QA, SMP, TRIBAL and NRDWP-O AND M</span></td>
  </tr>
</table>
