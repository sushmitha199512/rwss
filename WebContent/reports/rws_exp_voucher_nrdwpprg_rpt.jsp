			...
			<table  border=0 rules=none style="border-collapse:collapse" align = "right">		
				<tr>
					...
					<a href="../home.jsp">Home</a>|
						<a href="./rws_exp_voucher_nrdwpprg_mrpt.jsp?fyear=<%=fyear%>&fyear1=<%=fyear1%> ">Back</a>
					</td>
				</tr>
			</table>
		</caption>
		<tr  align="center">
			<td align="centre" class="rptHeading" style="width:950">Releases Vs Expenditures Vs Payment Dues Report for NRDWP Programmes Financial Year Wise<br><font face=verdana size=2 color=red><%=trial%> &nbsp;and&nbsp;<%=trial1%>
...
String q5="select nvl(sum(BILL_AMT_CONTRACTOR),0),nvl(sum(BILL_AMT_ADMISSIBLE),0),substr(b.office_code,2,3)  from rws_work_bill_tbl a,rws_work_admn_tbl b,rws_programme_tbl c where a.work_id=b.work_id and EE_FWD_DATE is not null and bill_sl_no<>'0' and substr(a.work_id,1,2)= c.programme_code and c.programme_code in ('02','01','48','32','23','49','51','18','37') and (to_char(BILL_DATE,'yy')='"+fyear+"' and substr(to_char(BILL_DATE,'dd-mm-yy'),4,2)>='03'or to_char(BILL_DATE,'yy')='"+fyear1+"' and substr(to_char(BILL_DATE,'dd-mm-yy'),4,2)<='04') group by substr(b.office_code,2,3)";
...
String q3="select nvl(sum(voucher_amt),0),substr(b.office_code,2,3) from rws_work_exp_voucher_tbl a,rws_work_admn_tbl b,rws_programme_tbl c where a.work_id=b.work_id and BILL_SL_NO <>'0' and c.programme_code in ('02','01','48','32','23','49','51','18','37','60') and substr(a.work_id,1,2)=c.programme_code and (to_char(VOUCHER_DT,'yy')='"+fyear+"' and substr(to_char(VOUCHER_DT,'dd-mm-yy'),4,2)>='03' or to_char(VOUCHER_DT,'yy')='"+fyear1+"' and substr(to_char(VOUCHER_DT,'dd-mm-yy'),4,2)<='04') group by substr(b.office_code,2,3)";
...
String q1="select nvl(sum(BILL_AMOUNT_CENTRAL),0),substr(a.office_code,2,3) from rws_bill_bank_bridge_det_tbl a ,rws_programme_tbl c where a.bill_sl_no<>'0' and a.office_code is not null and c.programme_code in ('02','01','48','32','23','49','51','18','37','60') and substr(a.work_id,1,2)=c.programme_code and  (to_char(BANK_SEND_DATE,'yy')='"+fyear+"' and substr(to_char(BANK_SEND_DATE,'dd-mm-yy'),4,2)>='03'or to_char(BANK_SEND_DATE,'yy')='"+fyear1+"' and substr(to_char(BANK_SEND_DATE,'dd-mm-yy'),4,2)<='04') group by substr (a.office_code,2,3)";
...
String q6="select nvl(sum(BILL_AMOUT_STATE),0),substr(a.office_code,2,3) from rws_bill_pao_bridge_det_tbl a ,rws_programme_tbl c where a.bill_sl_no<>'0' and a.office_code is not null and  c.programme_code in ('02','01','48','32','23','49','51','18','37','60') and substr(a.work_id,1,2)=c.programme_code and  (to_char(STATE_SEND_DATE,'yy')='"+fyear+"' and substr(to_char(STATE_SEND_DATE,'dd-mm-yy'),4,2)>='03'or to_char(STATE_SEND_DATE,'yy')='"+fyear1+"' and substr(to_char(STATE_SEND_DATE,'dd-mm-yy'),4,2)<='04') group by substr (a.office_code,2,3)";
...
String q8="select nvl(sum(a.op_cl_bal),0),substr(a.office_code,2,3) from rws_grant_op_cl_bal_tbl a ,rws_programme_tbl b where office_code is not null and b.programme_code in ('02','01','48','32','23','49','51','18','37','60') and a.programme_code=b.programme_code and  (to_char(a.OP_BAL_DT,'yy')='"+fyear+"' and substr(to_char(a.OP_BAL_DT,'dd-mm-yy'),4,2)>='03' or to_char(a.OP_BAL_DT,'yy')='"+fyear1+"' and substr(to_char(a.OP_BAL_DT,'dd-mm-yy'),4,2)<='04') group by substr (a.office_code,2,3)";
...
String q9="select nvl(sum(a.op_cl_bal),0),substr(a.office_code,2,3) from rws_grant_op_cl_bal_tbl a ,rws_programme_tbl b where office_code is not null and b.programme_code in ('02','01','48','32','23','49','51','18','37','60') and a.programme_code=b.programme_code and  (to_char(a.OP_BAL_DT,'yy')='"+fyear4+"' and substr(to_char(a.OP_BAL_DT,'dd-mm-yy'),4,2)>='03' or to_char(a.OP_BAL_DT,'yy')='"+fyear+"' and substr(to_char(a.OP_BAL_DT,'dd-mm-yy'),4,2)<='04') group by substr (a.office_code,2,3)";
...
<div id="tbl-container22">
	...
	<tr align="center" style="position: relative; top: expression(this.offsetParent.scrollTop-1);"> 
		...
		<td style="background-color: #2B3856" width="190"><font face=verdana size=2 color=white>&nbsp;&nbsp;Total(OB+Bank+Pao)&nbsp;&nbsp;</font></td>
...
<%}
       ...
       <%if(bamt!=null && !bamt.equals("0"))
			{
			%>
			 <td class="rptValue" align="right">  <a href="#" onClick="wopen('rws_nrdwpbankreleases_rpt.jsp?dcode=<%=rs.getString(1)%>&divcode=<%=rs.getString(2)%>&dnamee=<%=rs.getString(3)%>&divname=<%=rs.getString(4)%>','popup', 900, 300)"> <%=ndf.format(Double.parseDouble(bamt))%> </td>
<%}
		 else
		 {%>
      ...
      <%if(pamt!=null && !pamt.equals("0"))
			...
			%>
			 ...
			 <td class="rptValue" align="right" ><a href="#" onClick="wopen('rws_nrdwppaoreleases_rpt.jsp?dcode=<%=rs.getString(1)%>&divcode=<%=rs.getString(2)%>&dnamee=<%=rs.getString(3)%>&divname=<%=rs.getString(4)%>','popup', 900, 300)"> <%=ndf.format(Double.parseDouble(pamt))%></td>
...
<%if(exp!=null && !exp.equals("0"))
			...
			%>
			 ...
			 <td class="rptValue" align="right"><a href="#" onClick="wopen('rws_nrdwpvoucherreleases_rpt.jsp?dcode=<%=rs.getString(1)%>&divcode=<%=rs.getString(2)%>&dnamee=<%=rs.getString(3)%>&divname=<%=rs.getString(4)%>','popup', 900, 300)">
...
Note********: In this Report the following programmes are related to <b>NRDWP</b></font>
<table align="center" cellpadding="0" cellspacing="0">
  <col width="311">
  <tr height="17">
    <bean:message key="app.nrdwprelatedfin"/>
