...
<%
	...
	String rDistrict, rConstituency, rMandal, rPanchayat, rVillage, distvalue, distname, convalue, conname, manvalue, manname = null,panname=null,vilname=null;
...
<caption>
	...
	if(rDistrict != null && rDistrict.equals("00"))
	{
		...
		query2="SELECT DISTINCT m.mcode,M.MNAME,SUM(Case When HD.coverage_status='NSS' then 1 Else 0 End) as NSS FROM RWS_MANDAL_TBL M,RWS_CONSTITUENCY_TBL C, RWS_HABITATION_DIRECTORY_TBL HD,RWS_PANCHAYAT_RAJ_TBL RAJ where HD.HAB_CODE=RAJ.PANCH_CODE AND SUBSTR(HD.HAB_CODE,1,2) = M.DCODE AND M.DCODE = '" +distvalue+ "' AND SUBSTR(HD.HAB_CODE,6,2) = M.MCODE AND C.DCODE = M.DCODE AND SUBSTR(HD.HAB_CODE,3,3) = C.CONSTITUENCY_CODE AND C.CONSTITUENCY_CODE = '" +convalue+ "' and hd.coverage_status is not null and hd.coverage_status!='UI' and hd.coverage_status='NSS' GROUP BY mcode,mname";
	}
...
<%
		...
		{			
%>
		<td class=rptValue><a href="./rws_NSS_panch_habitations_rpt.jsp?distvalue=<%=distvalue%>&distname=<%=distname%>&convalue=<%=convalue%>&conname=<%=conname%>&manvalue=<%=rs2.getString(1)%>&manname=<%=rs2.getString(2)%>&rDistrict=<%=rDistrict%>&rMandal=<%=rMandal%>&rPanchayat=<%=rPanchayat%>&rVillage=<%=rVillage%>&rConstituency=<%=rConstituency%>"><%=rs2.getString(2)%></a></td>
<%
		}
