...
<%
	...
	//String rDistrict1=request.getParameter("distvalue");
	System.out.println("contituency report rDistrict in main "+rDistrict );	
	...
	<caption>
	 <table border=0 rules=none style="border-collapse:collapse" align = "right">			
			<tr align="right">
				...
				<td class="bwborder"><a href="./rws_const_coverage_status_constituency_xls.jsp?rDistrict=<%=rDistrict%>&district=<%=distvalue%>&distname=<%=distname%>&constituency=<%=rConstituency%>&mandal=<%=rMandal%>&panchayat=<%=rPanchayat%>&village=<%=rVillage%>" target="_new">Excel</a></td>
...
<%
		if(!rDistrict.equals(null) && rDistrict.equals("00") && rConstituency.equals("-1") && rMandal.equals("-1") && rPanchayat.equals("-1") && rVillage.equals("-1"))
		{			
%>
		<td class=rptValue><a href="./rws_const_coverage_status_mandal_rpt.jsp?distvalue=<%=distvalue%>&distname=<%=distname%>&convalue=<%=rs2.getString(1)%>&conname=<%=rs2.getString(2)%>&rDistrict=<%=rDistrict%>&rMandal=<%=rMandal%>&rPanchayat=<%=rPanchayat%>&rVillage=<%=rVillage%>&rConstituency=<%=rConstituency%>"><%=rs2.getString(2)%></a></td>
<%
		}
		%>
