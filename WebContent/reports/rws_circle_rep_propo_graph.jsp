...
<%! 
	  ...
	  String dist[],districts[],distnames[],codes[],codes1[];
	 double data1[],data2[];
...
//ystem.out.println(e+"Couldn't create Pool");
	}
...
while(r6.next())
{
	codes[count]=r6.getString(1);
	dist[count]=r6.getString(2);
	data[count]=r6.getInt(3);
	count++;
}
   ...
   if(count!=0){
	...
	// ch.addTitle("Circles - Representative Proposal ", "times new roman Bold Italic",12);
	 
	ch.setPlotArea(230, 40, 300, 50, Chart.Transparent, Chart.Transparent,
...
%>
  ...
  for(j=0;j<count;j++){
	...
	if(x.equals("b"))
   {
	...
	ch.xAxis().setTitle("Circles / Divisions / Sub Divisions");
...
if(x.equals("p"))
 {
    ...
    // ch.addTitle2(Chart.Top, "Circles - Representative Proposal  ", "Times New Roman Bold Italic",13);
	ch.set3D();
...
<%//if(dCode.equals("51"))
if(district.equals("51") || (!district.equals("51") && mandal.equals("0")))
{ 
%>
...
<%}//else if(!dCode.equals("51") && mCode.equals("5200"))
...
<%	 }
	   ...
	   for(int m=0;m<count;m++) {
		   ...
		   %>
			...
			<td class="mystyle1"><a href="./rws_pro_list_rpt.jsp?code=<%=codes[m]%>&district=<%=district%>&mandal=<%=mandal%>&panchayat=<%=panchayat%>&FDate=<%=fdate%>"><%=data[m]%></a></td>
		</tr>
 <%
		   total=total+data[m];}%>
 <tr>
			<td colspan=2 class="btext">Total:</td>
			<td class="btext"><a href="./rws_pro_list_rpt.jsp?district=<%=district%>&mandal=<%=mandal%>&panchayat=<%=panchayat%>&FDate=<%=fdate%>"><%=total %></td>
	</tr>
 </table>
