...
try
{
   ...
   if(source.equals("source")){
	   ...
	   qry="select SOURCE_CODE,nvl(SOURCE_NAME,'-'),nvl(LOCATION,'-') ,nvl(SAFE_LPD,0),nvl(UNSAFE_LPD,0),nvl(FLOURIDE,0),nvl(BRAKISH,0),nvl(IRON,0),nvl(NITRATE,0),nvl(LATITUDE,0),nvl(longitude,0),nvl(WAY_POINT,0),dname,mname,pname,vname,panch_code,panch_name from rws_source_tbl a,rws_complete_hab_view b where dcode='"+dcode+"' and ((safe_lpd is not null and nvl(safe_lpd,0)>0) or (unsafe_lpd is not null and nvl(unsafe_lpd,0)>0)) and substr(SOURCE_CODE,1,16)=b.panch_code ";
