...
<% 
System.out.println(" hi in jasps");
...
<tr bgcolor="#8A9FCD" align="center">
	...
	<%int cnt=1;
	String nqry="select count(*),category_code FROM RWS_WORK_ADMN_TBL W where w.ADMIN_DATE >= '01-Apr-"+yr+"' and work_cancel_dt is null "+planQry+" ";
	if(dcode!=null && !dcode.equals("")){
		nqry+=" and substr(w.work_id,5,2)='"+dcode+"'";
	}
	nqry+=" group by category_code order by 2";
	...
	String sqry="SELECT count(*),category_code  FROM RWS_WORK_ADMN_TBL W where w.ADMIN_DATE <'01-Apr-"+yr+"'  and work_cancel_dt is null "+planQry+"  and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < '01-Apr-"+yr+"')) ";
	if(dcode!=null && !dcode.equals("")){
		sqry+=" and substr(w.work_id,5,2)='"+dcode+"'";
	}
	sqry+=" group by category_code order by 2";
	...
	String hqry="select count(distinct hab_code),category_code from rws_admn_hab_lnk_tbl a,rws_work_admn_tbl b  where a.work_id=b.work_id and work_cancel_dt is null "+planQry+"  and  a.work_id not  in(select work_id from rws_work_completion_tbl where date_of_completion <'01-Apr-"+yr+"')  ";
	if(dcode!=null && !dcode.equals("")){
		hqry+=" and substr(b.work_id,5,2)='"+dcode+"'";
	}
	hqry+=" group by category_code";
	...
	String nsqry="select sum(sanction_amount),category_code  from rws_work_admn_tbl  where  admin_date>='01-Apr-"+yr+"'  "+planQry+"   and work_cancel_dt is null    ";
	if(dcode!=null && !dcode.equals("")){
		nsqry+=" and substr(work_id,5,2)='"+dcode+"'";
	}
	nsqry+=" group by category_code order by 2";
	...
	String ssqry="SELECT sum(sanction_amount),category_code FROM RWS_WORK_ADMN_TBL W where w.ADMIN_DATE <'01-Apr-"+yr+" ' and work_cancel_dt is null  "+planQry+"  and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < '01-Apr-"+yr+"'))  ";
	if(dcode!=null && !dcode.equals("")){
		ssqry+=" and substr(w.work_id,5,2)='"+dcode+"'";
	}
	ssqry+=" group by category_code order by 2";
   ...
   String expqry="select  sum(case when year<> '"+financialyr+"' then total else 0 end)||'@'||sum(case when year='"+financialyr+"' then total else 0 end),category_code  from rws_exp_view a, rws_work_admn_tbl  b where a.work_id=b.work_id and work_cancel_dt is null  "+planQry+"  and  a.work_id not in (select work_id from rws_work_completion_tbl where date_of_completion <'01-Apr-"+yr+"')   ";
   if(dcode!=null && !dcode.equals("")){
	   expqry+=" and substr(b.work_id,5,2)='"+dcode+"'";
	}
   expqry+=" group by category_code  order by 2";
   Statement	stmt6=conn.createStatement();
	...
	String tsqry="SELECT distinct SUM(CASE WHEN E.TS_NO IS NOT NULL THEN 1 ELSE 0 END),category_code FROM RWS_NEW_EST_TBL E,RWS_WORK_ADMN_TBL W WHERE W.WORK_ID=E.WORK_ID and work_cancel_dt is null  "+planQry+"  and w.work_id not in( select work_id from rws_work_completion_tbl where date_of_completion <'01-Apr-"+yr+"')  ";
	if(dcode!=null && !dcode.equals("")){
		tsqry+=" and substr(w.work_id,5,2)='"+dcode+"'";
		}
	tsqry+=" group by category_code  order by 2";
	...
	String gqry="SELECT distinct SUM(CASE WHEN G.GROUNDING_DATE IS NOT NULL THEN 1 ELSE 0 END),category_code  FROM RWS_WORK_COMMENCEMENT_TBL G,RWS_WORK_ADMN_TBL W WHERE W.WORK_ID=G.WORK_ID and work_cancel_dt is null  "+planQry+"  and  w.work_id not in( select work_id from rws_work_completion_tbl where date_of_completion <'01-Apr-"+yr+"')   ";
	if(dcode!=null && !dcode.equals("")){
		gqry+=" and substr(w.work_id,5,2)='"+dcode+"'";
		}
	gqry+=" group by category_code  order by 2";
	...
	if(stmt8!=null)stmt8.close();
	 String wcqry="select sum(case when date_of_completion>='01-Apr-"+yr+"' and date_of_completion<'01-Apr-"+(Integer.parseInt(yr)+1)+"' then 1 else 0 end),category_code from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id  and work_cancel_dt is null  "+planQry+" ";
	 if(dcode!=null && !dcode.equals("")){
			wcqry+=" and substr(w.work_id,5,2)='"+dcode+"'";
			}
		wcqry+=" group by category_code  order by 2";
	...
	String chqry="select sum(w.no_of_habs),category_code from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and work_cancel_dt is null  "+planQry+"  and date_of_completion>='01-Apr-"+yr+"' and date_of_completion<'01-Apr-"+(Integer.parseInt(yr)+1)+"'  ";
	 if(dcode!=null && !dcode.equals("")){
			chqry+=" and substr(w.work_id,5,2)='"+dcode+"'";
			}
		chqry+=" group by category_code  order by 2";
   ...
   String covqry="Select count(hab_code),category_code  from RWS_WORK_COMP_HAB_LNK_TBL  a,rws_work_admn_tbl b where a.work_id =b.work_id and work_cancel_dt is null  "+planQry+"  and  a.work_id in (select work_id from  rws_work_completion_tbl where date_of_completion>='01-Apr-"+yr+"'  and date_of_completion<'01-Apr-"+(Integer.parseInt(yr)+1)+"' )  ";
   if(dcode!=null && !dcode.equals("")){
	   covqry+=" and substr(b.work_id,5,2)='"+dcode+"'";
		}
   covqry+=" group by category_code  order by 2";
   ...
   double sptot=0.0,nptot=0.0,totall=0.0,exprev=0.0,expdur=0.0,totexp=0.0;
	...
	while(rs.next())
	{
	 ...
	 <tr>
		...
		<td class=btext><a href="rws_work_gen_rpt.jsp?catCode=<%=rs.getString(1)%>&planNonplan=<%=planNonplan%>&district=00"><%=rs.getString(2)%></a></td>
