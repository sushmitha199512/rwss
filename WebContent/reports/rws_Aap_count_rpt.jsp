...
<tr bgcolor="#8A9FCD" align="center">
	...
	<tr bgcolor="#8A9FCD" align="center">
		<td align="center" class="btext" colspan=17><font color="red"> <%=catName%> Works</font></td>	
	</tr>
	...
	<%int cnt=1;
	String sqry="SELECT count(*),substr(w.work_id,5,2)  FROM RWS_WORK_ADMN_TBL W,rws_aap_selected_tbl c where w.work_id=c.work_id and w.ADMIN_DATE <'01-Apr-"+yr+"'   "+catQry+"    ";
    ...
    String nqry="select count(*),substr(w.work_id,5,2) FROM RWS_WORK_ADMN_TBL W,rws_aap_selected_tbl c where w.work_id=c.work_id and w.ADMIN_DATE >= '01-Apr-"+yr+"'   "+catQry+" ";
    ...
    String hqry="select count(distinct hab_code),substr(a.work_id,5,2) from rws_admn_hab_lnk_tbl a,rws_work_admn_tbl b,rws_aap_selected_tbl c  where a.work_id=b.work_id and a.work_id=c.work_id and c.work_id=b.work_id   "+catQry+"   ";
	...
	String nsqry="select sum(sanction_amount),substr(a.work_id,5,2)  from rws_work_admn_tbl a,rws_aap_selected_tbl b  where   a.work_id=b.work_id and admin_date>='01-Apr-"+yr+"'   "+catQry+"   and work_cancel_dt is null   ";
	nsqry+=" group by substr(a.work_id,5,2)  order by 2 ";
    ...
    String ssqry="SELECT sum(sanction_amount),substr(W.work_id,5,2) FROM RWS_WORK_ADMN_TBL W,rws_aap_selected_tbl b where w.work_id=b.work_id and w.ADMIN_DATE <'01-Apr-"+yr+" '    "+catQry+"   ";
  ...
  // System.out.println("ssqry:"+ssqry);
   ...
   String expqry="select  sum(case when year<> '"+yr+"-"+(""+(nextYear)).substring(2,4)+"' then total else 0 end)||'@'||sum(case when year='"+yr+"-"+(""+(nextYear)).substring(2,4)+"' then total else 0 end),substr(b.work_id,5,2)  from rws_exp_view a, rws_work_admn_tbl  b,rws_aap_selected_tbl c where a.work_id=b.work_id and  a.work_id=c.work_id and  c.work_id=b.work_id and work_cancel_dt is null  "+catQry+"  ";
   ...
   String tsqry="SELECT distinct SUM(CASE WHEN E.TS_NO IS NOT NULL THEN 1 ELSE 0 END),substr(W.work_id,5,2) FROM RWS_NEW_EST_TBL E,RWS_WORK_ADMN_TBL W,rws_aap_selected_tbl c WHERE W.WORK_ID=E.WORK_ID and W.WORK_ID=c.WORK_ID and c.WORK_ID=E.WORK_ID    "+catQry+"   ";
 ...
 // System.out.println("tsqry:"+tsqry);
	...
	String gqry="SELECT distinct SUM(CASE WHEN G.GROUNDING_DATE IS NOT NULL THEN 1 ELSE 0 END),substr(W.work_id,5,2)  FROM RWS_WORK_COMMENCEMENT_TBL G,RWS_WORK_ADMN_TBL W,rws_aap_selected_tbl c WHERE W.WORK_ID=G.WORK_ID and  W.WORK_ID=c.WORK_ID and C.WORK_ID=G.WORK_ID    "+catQry+"   ";
	...
	if(stmt8!=null)stmt8.close();
	 ...
	 String wcqry="select sum(case when  date_of_completion<'01-Apr-"+(Integer.parseInt(yr)+1)+"' then 1 else 0 end),substr(w.work_id,5,2) from rws_work_completion_tbl wc, rws_work_admn_tbl w,rws_aap_selected_tbl c where w.work_id=wc.work_id  and w.work_id=c.work_id and c.work_id=wc.work_id and work_cancel_dt is null   "+catQry+"  ";
    wcqry+="group by substr(w.work_id,5,2)  order by 2 ";
	//date_of_completion>='01-Apr-"+yr+"' and
	...
	String chqry="select count(hab_code),substr(w.work_id,5,2) from rws_work_completion_tbl wc, rws_admn_hab_lnk_tbl w,rws_aap_selected_tbl c,rws_work_admn_tbl d where w.work_id=wc.work_id and  w.work_id=c.work_id and c.work_id=wc.work_id  and  w.work_id=d.work_id  and  work_cancel_dt is null   "+catQry+"   and date_of_completion<'01-Apr-"+(Integer.parseInt(yr)+1)+"'  ";
    chqry+="group by substr(w.work_id,5,2)  order by 2 ";
	//and date_of_completion>='01-Apr-"+yr+"'
   ...
   String covqry="Select count(hab_code),substr(b.work_id,5,2)  from RWS_WORK_COMP_HAB_LNK_TBL  a,rws_work_admn_tbl b,rws_aap_selected_tbl c  where a.work_id =b.work_id and   a.work_id =c.work_id and  b.work_id =c.work_id and work_cancel_dt is null   "+catQry+"  and  a.work_id in (select work_id from  rws_work_completion_tbl where date_of_completion<'01-Apr-"+(Integer.parseInt(yr)+1)+"' )  ";
   // date_of_completion>='01-Apr-"+yr+"'  and
...
//System.out.println("covqry:"+covqry);
	...
	if(loggedUser!=null && (loggedUser.equals("admin")||loggedUser.equals("100000")))
		...
		qry=" select dcode,dname from rws_district_tbl where dcode<>'16' order by dcode";
			...
			qry=" select dcode,dname from rws_district_tbl where dcode='"+loggedUser.substring(1,3)+"' order by dcode";
		}
	...
	while(rs.next())
	{
		...
		<td class=rptvalue title=Click nowrap><%=rs.getString(2)%><br><a href="rws_aap_count_div_rpt.jsp?dist=<%=rs.getString(1)%>&year=<%=yr%>&catCode=<%=catCode%>&dname=<%=rs.getString(2)%>">Division</a>|<a href="rws_aap_count_man_rpt.jsp?dist=<%=rs.getString(1)%>&year=<%=yr%>&catCode=<%=catCode%>&dname=<%=rs.getString(2)%>" >Mandal</a></td>
