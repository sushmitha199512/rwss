...
while (rs2.next())
 	...
 	if(fdate.substring(7,11).equals(currentYear))
	{
		 ...
		 " substr(h.hab_code,1,2)='"+dCode+"' and substr(h.hab_code,6,2)='"+mCode+ "' and  	 substr(h.hab_code,13,2)='"+pCode+"' and  substr(h.hab_code,8,3)='"+vCode+"' and  status_date ='"+fdate+"' and h.coverage_status is not null and h.coverage_status <> 'UI' and h.hab_code=pr.panch_code   order by d.dcode,m.mcode,p.pcode,v.vcode,pr.panch_code";
...
<%if(fdate!=null && !fdate.equals("0") )
	{%>
<td class="btext">
<font size=2>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Status As On : </font><font color="navy"> <%=fdate %>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </font> 
	<% }%>
	<% if(!dCode.equals("51")) {%>
...
<font color="navy">Village: <%=r6.getString(8)%></font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp
...
if(Count!=0){
	...
	%>
		<tr align=left bgcolor="#ffffff">
			...
			<td class="rptValue"><%=r6.getString(11) %> </td> 
			<td class="rptValue"><%=r6.getString(10) %> </td>
			<td class="rptValue"><%=r6.getString(15) %> </td>
			<% 
 ...
 //// System.out.println("QU2:"+qu2);
		...
		while(r7.next())
		{
			...
			{
			 %>
			<td class="rptValue" align="right"><a href="./rws_assets_resource_rpt.jsp?dcode=<%=dCode%>&mcode=<%=mCode%>&pcode=<%=pCode%>&hcode=<%=r6.getString(11)%>&hname=<%=r6.getString(9)%>&code=<%="Assets"%>"><%=astcount%></a>
			</td>
...
if(workid>0)
{ %>
<td class="rptValue"  align="right"><a href="./rws_assets_resource_rpt.jsp?code=<%="Works"%>&dcode=<%=dCode%>&mcode=<%=mCode%>&pcode=<%=pCode%>&hcode=<%=r6.getString(11)%>&hname=<%=r6.getString(9)%>"><%=workid%>
</a></td>
...
String costqry="select (select  sum(b.asset_cost) from rws_asset_mast_tbl b,rws_asset_type_tbl a,rws_panchayat_raj_tbl d,rws_asset_scheme_tbl s,rws_habitation_directory_tbl h where d.panch_code=h.hab_code and b.asset_code=s.asset_code  and   d.panch_code=substr(scheme_code,1,16) and substr(d.panch_code,1,2)='"+dCode+"' and substr(d.panch_code,6,2)='"+mCode+"' and substr(d.panch_code,13,2)='"+pCode+"' and substr(d.panch_code,8,3)='"+vCode+"' and substr(d.panch_code,1,16)='"+r6.getString(11)+"' and  b.type_of_asset_code=a.type_of_asset_code) a,(select sum(sanction_amount) from rws_work_admn_tbl w,rws_panchayat_raj_tbl pr,rws_habitation_directory_tbl h where h.hab_code=pr.panch_code and w.lead_habitation=pr.panch_code and substr (pr.panch_code,1,2)='"+dCode+"'  and substr (pr.panch_code,6,2)='"+mCode+"' and substr(pr.panch_code,13,2)='"+pCode+"' and substr (pr.panch_code,8,3)='"+vCode+"'  and w.lead_habitation='"+r6.getString(11)+"' and w.work_id not in (select asset_code from rws_asset_mast_tbl))b from dual";
	...
	<td class="rptValue"  align="right"><a href="./rws_assets_resource_rpt.jsp?dcode=<%=dCode%>&mcode=<%=mCode%>&pcode=<%=pCode%>&hcode=<%=r6.getString(11)%>&hname=<%=r6.getString(9)%>&code=<%="Assets"%>"><%=rs.getString(1)%></a></td>
	<%}else {%>
		<td class="rptValue"  align="right">-</td>
	<%}if(rs.getString(2)!=null && !rs.getString(2).equals("0")) {%>
	<td class="rptValue"  align="right"><a href="./rws_assets_resource_rpt.jsp?code=<%="Works"%>&dcode=<%=dCode%>&mcode=<%=mCode%>&pcode=<%=pCode%>&hcode=<%=r6.getString(11)%>&hname=<%=r6.getString(9)%>"><%=rs.getString(2)%></a></td>
	<%}else {%>
<td class="rptValue"  align="right">-</td>
...
while(rs2.next()) 
{
	...
	%>
	<td class="rptValue"  align="right"><a href="./rws_assets_resource_rpt.jsp?dcode=<%=dCode%>&mcode=<%=mCode%>&pcode=<%=pCode%>&hcode=
