...
<%
   ...
   Statement stmt1=null,stmt2=null;
    ...
    String rDistrict = null,rDistrict1 = null;;
	String rMandal = null;
	String rPanch = null;
	...
	String dcode="";
	if(loggedUser!=null && loggedUser.substring(0,3).equals("col")){
...
<script language="JavaScript">
	function fnSubmit()
	{
	 ...
	 if(document.forms[0].habtype[0].checked)
	 {
	  ...
	  /* if(dist=="00")
	   {
	     document.forms[0].action="./rws_dist_status_resource_rpt.jsp";
	    document.forms[0].submit(); 
	   }
	   if(dist!="00" && mand=="-1" && panch=="-1" && vill=="-1" && hab=="-1")
	   {
	    //alert("any dist selected");
	     document.forms[0].action="./rws_mandal_status_resource_rpt.jsp";
         document.forms[0].submit(); 
	   }
	   //alert(mand);
	    if(dist!="00" && mand!="-1" && panch=="-1" && vill=="-1" && hab=="-1")
	   {
	    //alert("in mandal select ");
	     document.forms[0].action="./rws_panch_status_resource_rpt.jsp";
		 document.forms[0].submit();
	   }
	    if(dist!="00" && mand!="-1" && panch!="-1" && vill=="-1" && hab=="-1")
	    {
	     //alert("in panch select ");
	     document.forms[0].action="./rws_village_status_resource_rpt.jsp";
		 document.forms[0].submit();
	    }
	   if(dist!="00" && mand!="-1" && panch!="-1" && vill!="-1" && hab=="-1")
	    {
	     //alert("in vill select ");
	    document.forms[0].action="./rws_panch_only_status_resource_rp.jsp";
		document.forms[0].submit();
	    }
	    if(dist!="00" && mand!="-1" && panch!="-1" && vill!="-1" && hab!="-1")
	    {
	     //alert("in Hab select ");*/
	    document.forms[0].action="./rws_hab_only_status_resource_home_rpt.jsp";
		document.forms[0].submit();
	    }
	 }
	  ...
	  function fnSubmit1()
	  {
	   ...
	   {
	    alert(" Minimum 3 chars required");
	   }
	   else{
        var url="./rws_hab_status_resource_home_rpt1.jsp";
        // alert(url);
...
<%} %>
	...
	<tr bgcolor="#8A9FCD" align="center">
		<td align="center" class="rptHeading" colspan=3>Habitation Status- Water Resource Report</td>	
	</tr>
...
<%     		session.setAttribute("districtCode", "All");
				...
				query = "SELECT DISTINCT D.DCODE, D.DNAME FROM RWS_DISTRICT_TBL D, RWS_HABITATION_DIRECTORY_TBL HD WHERE SUBSTR(HD.HAB_CODE,1,2)= D.DCODE and ?=? ";
...
<%
				...
				query = "SELECT DISTINCT m.mcode, m.mname FROM RWS_mandal_TBL m, RWS_HABITATION_DIRECTORY_TBL HD WHERE SUBSTR(HD.HAB_CODE,1,2)= m.DCODE and SUBSTR(HD.HAB_CODE,6,2)= m.mCODE and SUBSTR(HD.HAB_CODE,1,2)=? order by m.mname";
				pstmt1 = conn.prepareStatement(query);
				pstmt1.setString(1,rDistrict);
...
<%
				...
				query = "SELECT DISTINCT p.pcode, p.pname FROM RWS_PANCHAYAT_TBL p, RWS_HABITATION_DIRECTORY_TBL HD WHERE SUBSTR(HD.HAB_CODE,1,2)= p.DCODE and SUBSTR(HD.HAB_CODE,6,2)= p.mCODE and SUBSTR(HD.HAB_CODE,13,2)= p.PCODE and SUBSTR(HD.HAB_CODE,1,2)=? and SUBSTR(HD.HAB_CODE,6,2)=?  order by p.pname";
				// // System.out.println("query for panch:"+query);
				//rs = stmt.executeQuery(query);
				pstmt2 = conn.prepareStatement(query);
				pstmt2.setString(1,rDistrict);
				pstmt2.setString(2,rMandal);
...
<%
	...
	<tr>
		<td class = btext> 
				<font color=navy><b>Village</b><font color="red">*</font> </font>
		<td class=btext>
...
<%
				...
				query = "SELECT distinct v.vcode, v.vname from rws_village_tbl v  where v.dcode=? and v.mcode=? and v.pcode=? order by v.vname";
				//rs = stmt.executeQuery(query);
				pstmt3 = conn.prepareStatement(query);
				pstmt3.setString(1,rDistrict);
				pstmt3.setString(2,rMandal);
				pstmt3.setString(3,rPanch);
...
<%                 session.setAttribute("villageCode", rVillage);
 ...
 <tr>
		<td class = btext> 
				<font color=navy><b>Habitation</b><font color="red">*</font> </font>
		<td class=btext>
...
<%
				...
				//stmt = conn.createStatement();
				query = "SELECT distinct h.panch_code, h.panch_name from rws_panchayat_raj_tbl h  where substr(panch_code,1,2)=? and substr(panch_code,6,2)=? and substr(panch_code,13,2)=? and substr(panch_code,8,3)=? order by h.panch_name";
				//rs = stmt.executeQuery(query);
				pstmt4 = conn.prepareStatement(query);
				pstmt4.setString(1,rDistrict);
				pstmt4.setString(2,rMandal);
				pstmt4.setString(3,rPanch);
				pstmt4.setString(4,rVillage);
...
<table border = 1 align =center cellspacing = 0 cellpadding = 0 rules = "rows" bordercolor = navy style = "border-collapse:collapse" bgcolor="#ffffff" width=350>	
<%
 if(rDistrict1 == null || rDistrict1.equals("00"))
	{
%>
<td class=rptLabel >District<font color="red">*</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
		<td class=btext>
...
%>     
		...
		<%
				...
				pstmt5.setDouble(1,rand1);
				pstmt5.setDouble(2,rand1);
		        rs = pstmt5.executeQuery();
				while(rs.next())
				{
					if(rDistrict1 != null && rDistrict1.equals(rs.getString(1)))
					{					
%>
				<option value="<%=rs.getString(1)%>" SELECTED><%=rs.getString(2)%></option>
<%
...
%>
		...
		<tr>
           <td class="rptLabel" colspan=2 align="left">Enter&nbsp;Your&nbsp;Search&nbsp;String:<font color="red">*</font><input type="text" name="searchstring" /><br></td>
  	        </tr>
  	         <tr bgcolor="#8A9FCD" align="center">
		<td colspan=3 align=center>
