...
<%
   ...
   // nic.watersoft.commons.Debug.println("financialYearTo:"+financialYearTo);
	...
	planCode = (String)session.getAttribute("planNonplan");
	String planNonplan = planCode;
	String planQry = "",planQry1 = "";
	if(planCode.equals("1"))
	{
		planQry = "and plan_code = '1'";
		planQry1 = "and a.plan_code = '1'";
	}
	else if(planCode.equals("2"))
	{
		planQry = "and plan_code = '2'";
		planQry1 = "and a.plan_code = '2'";
	}
...
<table border = 1 cellspacing = 0 cellpadding = 0 height=5% width="100%" bordercolor=#000000 style="border-collapse:collapse" bgcolor="#ffffff" align = "center">
	...
	////System.out.println("First Drill");
	   ...
	   %>
	   	...
	   	<%@ include file="./rws_worksmonitoring_gen_label.jsp"%>
	   	<%String qry1=null;
		
		qry1="select * from rws_work_category_tbl";
		////System.out.println("Test Qry is::"+qry1);
		stmt1 = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
		rs1=stmt1.executeQuery(qry1);
		String view1="Create or Replace view spillworks? as SELECT work_id,office_code,no_of_habs,category_code,plan_code FROM RWS_WORK_ADMN_TBL W where w.ADMIN_DATE < ? and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < ?))";
		String view2="Create or Replace view newworks? as SELECT work_id,office_code,no_of_habs,category_code,plan_code FROM RWS_WORK_ADMN_TBL W where w.ADMIN_DATE >= ?";
		...
		<%if(rs1.getInt(1)==1){%>
		<td class=rptValue title=Click><b><a href="rws_work_proj_abstract_rpt1.jsp?district=<%=rs1.getString(1)%>&planNonplan=<%=planNonplan%>&catCode=1&finyear=<%=financialyr%>&distname=<%=rs1.getString(2)%>"><%=rs1.getString(2)%></a></td>
		<%}
		else if(rs1.getInt(1)==2){%>
		<td class=rptValue title=Click><b><a href="rws_work_proj_abstract_rpt1.jsp?district=<%=rs1.getString(1)%>&planNonplan=<%=planNonplan%>&catCode=2&finyear=<%=financialyr%>&distname=<%=rs1.getString(2)%>"><%=rs1.getString(2)%></a></td>
		<%}
		else if(rs1.getInt(1)==3){%>
		<td class=rptValue title=Click><b><a href="rws_work_proj_abstract_rpt1.jsp?district=<%=rs1.getString(1)%>&planNonplan=<%=planNonplan%>&catCode=3&finyear=<%=financialyr%>&distname=<%=rs1.getString(2)%>"><%=rs1.getString(2)%></a></td>
		<%}
		else%>
	   	<% 
	   	String qry2=null;Statement stmt2=null;
		    ...
		    String qry21="select (select count(*) from spillworks"+session.getId().substring(1,10)+" where category_code='"+rs1.getString(1)+"' "+planQry+")a,(select sum(no_of_habs) from spillworks"+session.getId().substring(1,10)+" where category_code='"+rs1.getString(1)+"' "+planQry+")b,			(select count(*) from spillworks"+session.getId().substring(1,10)+"  a,rws_work_admn_prog_lnk_tbl b where a.work_id=b.work_id and category_code='"+rs1.getString(1)+"' "+planQry+")c  from dual";	
			String qry22 = "select (select count(*) from newworks"+session.getId().substring(1,10)+" where category_code='"+rs1.getString(1)+"' "+planQry+")a,(select sum(no_of_habs) from newworks"+session.getId().substring(1,10)+" where category_code='"+rs1.getString(1)+"' "+planQry+")b,(select count(*) from newworks"+session.getId().substring(1,10)+"  a,rws_work_admn_prog_lnk_tbl b where a.work_id=b.work_id and category_code='"+rs1.getString(1)+"' "+planQry+")c  from dual";
			//String qry22 ="select count(*) from newworks"+session.getId().substring(1,10)+" where substr (work_id,5,2) ='"+rs1.getString(1)+"'";
	   	//////System.out.println("query21"+qry21);
	   //////System.out.println("query22"+qry22);
	   	...
	   	while(rs21.next()&& rs22.next())
            ...
            qry44 = "select (select sum(a.sanction_amount) from rws_work_admn_tbl a,spillworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and a.category_code='"+rs1.getString(1)+"' "+planQry1+")a,(select sum(a.sanction_amount) from rws_work_admn_prog_lnk_tbl a,spillworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and category_code='"+rs1.getString(1)+"' "+planQry+")b from dual";
            ...
            rs44=stmt44.executeQuery(qry44);
  			...
  			qry45 = "select (select sum(a.sanction_amount) from rws_work_admn_tbl a,newworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and a.category_code='"+rs1.getString(1)+"' "+planQry1+")a,(select sum(a.sanction_amount) from rws_work_admn_prog_lnk_tbl a,newworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and category_code='"+rs1.getString(1)+"' "+planQry+")b from dual";
      ...
      if(rs44!=null){rs44.close();}
       if(stmt44!=null){stmt44.close();}
         ...
         // ArrayList progs=worksData.getAdminPrograms(reque,fb.getWorkId());
          ...
          String work="";
			...
			String aa = "select sum(expenditure_upto_the_pre_year) from RWS_WORK_EXP_UPTO_PRE_YEAR_TBL where work_id in (select work_id from spillworks"+session.getId().substring(1,10)+" where category_code='"+rs1.getString(1)+"' "+planQry+" union select work_id from newworks"+session.getId().substring(1,10)+" where category_code='"+rs1.getString(1)+"' "+planQry+")";
       ...
       if(stmt8!=null){stmt8.close();}
         ...
         String qry9=null;Statement stmt9=null;
			...
			qry9 = "select sum(case when date_of_completion<'"+fixYear+"' then 1 else 0 end),sum(case when date_of_completion>='"+fixYear+"' and date_of_completion<'"+fixNextYear+"' then 1 else 0 end) from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and category_code='"+rs1.getString(1)+"' "+planQry+"";
       ...
       String qry4=null;
			...
			qry4 = "select (select sum(w.no_of_habs) from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and category_code='"+rs1.getString(1)+"' "+planQry+" and date_of_completion<'"+fixYear+"')as a, (select sum(w.no_of_habs) from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and category_code='"+rs1.getString(1)+"' "+planQry+" and date_of_completion>='"+fixYear+"' and date_of_completion<'"+fixNextYear+"')as b from dual";
			//////System.out.println(qry4);
       ...
       if(stmt4!=null){stmt4.close();}
        ...
        String qry10=null; Statement stmt10=null;
           ...
           // qry10="Select count(e.hab_code) from RWS_WORK_COMP_HAB_LNK_TBL e where e.work_id in (select work_id from spillworks"+session.getId().substring(1,10)+" where substr(work_id,5,2)='"+rs1.getString(1)+"' union select work_id from newworks"+session.getId().substring(1,10)+" where substr(work_id,5,2)='"+rs1.getString(1)+"')";
            ...
            qry10="select (Select count(e.hab_code) from RWS_WORK_COMP_HAB_LNK_TBL e where e.work_id in (select w.work_id from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and category_code='"+rs1.getString(1)+"' "+planQry+" and date_of_completion<'"+fixYear+"'))a, (Select count (e.hab_code) from RWS_ADMN_HAB_LNK_TBL e where e.work_id in (select w.work_id from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and category_code='"+rs1.getString(1)+"' "+planQry+" and date_of_completion>='"+fixYear+"' and date_of_completion<'"+fixNextYear+"'))b from dual";
			//////System.out.println("qry10:"+qry10);
...
<td class="btext" align="right"><%=ghabcovn1%></td>
       ...
       Statement stmtdv1=null,stmtdv2=null;
		...
		String dropqry2="drop VIEW newworks?";
		 ...
		 pss2.setString(2,  session.getId().substring(1,10));
		pss1.executeUpdate();
		pss2.executeUpdate();
		}
	...
	////System.out..println("First Drill");
	   ...
	   %>
		...
		String view1="Create or Replace view spillworks? as SELECT work_id,office_code,no_of_habs FROM RWS_WORK_ADMN_TBL W where w.plan_code ='1' and w.category_code='2' and w.ADMIN_DATE < ? and SUBSTR (w.work_id,5,2)=? and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION >= ? or DATE_OF_COMPLETION is null))";
		String view2="Create or Replace view newworks? as SELECT work_id,office_code,no_of_habs FROM RWS_WORK_ADMN_TBL W where w.plan_code ='1' and w.category_code='2' and w.ADMIN_DATE >= ? and SUBSTR (w.work_id,5,2)=?";
		////System.out..println("View1:"+view1);
		////System.out..println("View2:"+view2);
		Statement stmtv1= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
	   	Statement stmtv2= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
		...
		ps3.setString(2,  fixYear);
		ps3.setString(3,rDistrict);
		ps3.setString(4,  fixYear);
		ps4.setString(1,session.getId().substring(1,10));
		ps4.setString(2,fixYear);
		ps4.setString(3,rDistrict);
	   	int vcount=ps3.executeUpdate();
 	   	ps4.executeUpdate();   	
...
<td class="btext" align="right"><%=ghabcovn1%></td>
       ...
       Statement stmtdv1=null,stmtdv2=null;
		...
		String dropqry1="drop VIEW spillworks?";
		String dropqry2="drop VIEW newworks?";
		PreparedStatement ps13=conn.prepareStatement(dropqry1);
		PreparedStatement ps14=conn.prepareStatement(view2);
		 ps13.setString(1,session.getId().substring(1,10));
	     ps14.setString(2,session.getId().substring(1,10));
		ps13.executeUpdate();
		ps14.executeUpdate();
		}
		catch(SQLException se)
	...
	////System.out..println("Second Drill");
	   ...
	   %>
		...
		stmt1 = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
		rs1=stmt1.executeQuery(qry1);
		String view1="Create or Replace view spillworks? as SELECT work_id,office_code,no_of_habs FROM RWS_WORK_ADMN_TBL W where w.plan_code ='1' and w.category_code='2' and w.ADMIN_DATE < ?  and SUBSTR (w.office_code,2,2)=? and SUBSTR(w.office_code,4,1)=? and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION >= ? or DATE_OF_COMPLETION is null))";
		String view2="Create or Replace view newworks?  as SELECT work_id,office_code,no_of_habs FROM RWS_WORK_ADMN_TBL W where w.plan_code ='1' and w.category_code='2' and w.ADMIN_DATE >= ? and SUBSTR (w.office_code,2,2)=? and SUBSTR(w.office_code,4,1)=?";
		Statement stmtv1= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
	   	Statement stmtv2= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
