...
try
{
	...
	query = " SELECT P.PNAME, PR.PANCH_NAME, H.DISTRISC_CODE, H.DIS_LEN, H.DIS_COVE, H.DIS_LN, H.DIS_TLC, H.DIS_BN, H.DIS_MAT_CODE, H.NO_OF_PIPES FROM RWS_ASSET_MAST_TBL A, RWS_ASSET_SCHEME_TBL S, RWS_PWS_DISTRI_SC_PARAM_TBL H, RWS_ASSET_TYPE_TBL T, RWS_PANCHAYAT_TBL P, RWS_PANCHAYAT_RAJ_TBL PR WHERE A.ASSET_CODE = S.ASSET_CODE AND A.ASSET_CODE = H.ASSET_CODE AND A.TYPE_OF_ASSET_CODE = T.TYPE_OF_ASSET_CODE AND T.TYPE_OF_ASSET_NAME = 'PWS' AND A.TYPE_OF_ASSET_CODE='" +rAssetType+ "' AND SUBSTR(H.DISTRISC_CODE,1,2) = '" +rCircle+ "' AND SUBSTR(H.DISTRISC_CODE,6,2) = '" +rMandal+ "' AND SUBSTR(H.DISTRISC_CODE,13,2) = '" +rPanchayat+ "' AND SUBSTR(H.DISTRISC_CODE,8,3) = '" +rVillage+ "' AND SUBSTR(H.DISTRISC_CODE,17,3) LIKE '%6%' AND SUBSTR(H.DISTRISC_CODE,1,2) = P.DCODE AND SUBSTR(H.DISTRISC_CODE,6,2) = P.MCODE AND SUBSTR(H.DISTRISC_CODE,13,2) = P.PCODE AND SUBSTR(H.DISTRISC_CODE,1,16) = PR.PANCH_CODE ORDER BY H.DISTRISC_CODE " ;
	}
	...
	session.setAttribute("distribution", AR);
	rs.close();
	stmt.close();
}
catch(Exception e)
{
	//System.out.println("The Error in assetdistric_rpt.jsp = " + e);
}
%>
<%@ include file="./rws_asset_distric_temp_rpt.jsp"%>