...
<script language="JavaScript">
	...
	document.f1.action="./progrmsched.jsp";
		document.f1.submit();		
	}
...
<table border = 1 align = center cellspacing = 0 cellpadding = 0 rules = "rows" bordercolor = navy style = "border-collapse:collapse" bgcolor="#ffffff" width=350>		
	...
	<tr bgcolor="#8A9FCD" align="center">
		<td align="center" class="rptHeading" colspan=3>Programme Schedule Report</font></td>	
	</tr>
<%				
try
{		
...
<%
	...
	query = "SELECT distinct d.circle_office_code,d.circle_office_name from rws_circle_office_tbl d,rws_work_admn_tbl w,RWS_work_commencement_tbl work where w.work_id=work.work_id and d.circle_office_code=substr(work.office_code,2,2) order by d.circle_office_name ";
...
<%
	...
	stmt = conn.createStatement();
		...
		query = "select distinct d.division_office_code,d.division_office_name from rws_division_office_tbl d,rws_work_admn_tbl w,RWS_work_commencement_tbl work where w.work_id=work.work_id and d.circle_office_code=substr(work.office_code,2,2) and d.division_office_code=substr(work.office_code,4,1) and substr(work.office_code,2,2)='"+rDist+"'";
		if(rDist!=null && !rDist.equals(""))
		{
		rs = stmt.executeQuery(query);
		while(rs.next())
...
<%
	...
	try
	{
		stmt = conn.createStatement();
		/*query = "SELECT distinct d.circle_office_code,d.circle_office_name  from rws_circle_office_tbl d,rws_work_admn_tbl w where d.circle_office_code=substr(w.office_code,2,2) order by  d.circle_office_code";*/
		query = "select distinct sd.subdivision_office_code,sd.subdivision_office_name from rws_subdivision_office_tbl sd,rws_work_admn_tbl w,rws_work_commencement_tbl work where w.work_id=work.work_id and sd.circle_office_code=substr (work.office_code,2,2) and sd.division_office_code=substr(work.office_code,4,1) and sd.subdivision_office_code=substr(work.office_code,5,2) and substr(work.office_code,2,2)='"+rDist+"' and substr(work.office_code,4,1)='"+rDiv+"'";
		rs = stmt.executeQuery(query);
		if(rDist!=null && !rDist.equals(""))
		{
		while(rs.next())
...
<%
	...
	stmt = conn.createStatement();
	queryplan = "select distinct w.PLAN_CODE,decode(w.PLAN_CODE,'1','Plan',2,'Non Plan','-') as planName from RWS_WORK_ADMN_TBL w,rws_work_commencement_tbl work where w.work_id=work.work_id  "; 
	if(rDist!=null && !rDist.equals(""))
	{	 
...
{	
stmt = conn.createStatement();
String querywork = "select distinct w.CATEGORY_CODE,c.CATEGORY_NAME from RWS_WORK_ADMN_TBL w,rws_work_category_tbl c,rws_work_commencement_tbl work where w.work_id=work.work_id and c.CATEGORY_CODE=w.CATEGORY_CODE "; 
if(rDist == null || rDist.equals(""))
{
%>
	<tr>
...
{	
stmt = conn.createStatement();
String queryscheme = "select distinct a.type_of_asset_code,a.type_of_asset_name from RWS_WORK_ADMN_TBL w,rws_asset_type_tbl a ,rws_work_commencement_tbl work where w.work_id=work.work_id  and  substr(w.work_id,7,2)=a.type_of_asset_code"; 
...
stmt = conn.createStatement();
 ...
 queryaug = "select distinct w.AUG_NEW_CODE,decode(w.AUG_NEW_CODE,'1','MainScheme','2','Augmentation','3','Maintenance','-') as Aug  from RWS_WORK_ADMN_TBL w,rws_work_commencement_tbl work where w.work_id=work.work_id   "; 
...
{	
stmt = conn.createStatement();
String queryprog = "select distinct p.PROGRAMME_CODE ,p.PROGRAMME_NAME from RWS_WORK_ADMN_TBL w,rws_programme_tbl p,rws_work_commencement_tbl work where w.work_id=work.work_id and substr(w.work_id,1,2)=p.PROGRAMME_CODE  "; 
//select distinct a.type_of_asset_code,a.type_of_asset_name from RWS_WORK_ADMN_TBL w,rws_asset_type_tbl a where  substr(w.office_code,2,2)='"+rDist+"' and substr(w.office_code,4,1)='"+rDiv+"' and substr(w.office_code,5,2)='"+rSubDiv+"' and substr(w.work_id,7,2)=a.type_of_asset_code"; 
if(rDist == null || rDist.equals(""))
{
%>
...
String queryyear=" select distinct to_char(admin_date,'yyyy') from RWS_WORK_ADMN_TBL w,rws_work_commencement_tbl work where w.work_id=work.work_id  and to_char(admin_date,'yyyy') is not null ";
//select distinct a.type_of_asset_code,a.type_of_asset_name from RWS_WORK_ADMN_TBL w,rws_asset_type_tbl a where  substr(w.office_code,2,2)='"+rDist+"' and substr(w.office_code,4,1)='"+rDiv+"' and substr(w.office_code,5,2)='"+rSubDiv+"' and substr(w.work_id,7,2)=a.type_of_asset_code"; 
if(rDist == null || rDist.equals(""))
{
%>
