...
<tr>
<td class=rptvalue><a href="rws_payment_Excess_xls.jsp" target="_new">Excel</a>|<a href="../home.jsp">Close</a></td></tr>
...
rs.next();
String prevWorkid="";
String  workid=rs.getString(9);
rs=stmt.executeQuery(qry);
while(rs.next())
{
	 workid=rs.getString(9);
	 String color="green";
...
<%}else{%>
<td class=rptvalue colspan=3></td>
<%}%>
...
<%
String checkqry="select (select  sum(PERCENT_COMPLETED_VAL)  from  rws_milestone_target_ach_tbl  where work_id='"+workid+"')a, (Select sum(bill_amt_admissible) from rws_work_bill_tbl where work_id='"+workid+"')b from dual ";
 ResultSet crs=cstmt.executeQuery(checkqry);
 if(crs.next())
	{
         //System.out.println("1:"+crs.getDouble(1) +":2:"+crs.getDouble(2) );
       if(crs.getDouble(1) >= crs.getDouble(2))
		{%>
			<td class=rptvalue align="right">rectification Action Taken</td>
...
<%
prevWorkid=rs.getString(9);	

} 
}catch(Exception e)
{
 //System.out.println("Exception in Payment Excees Report:"+e);
}
%>
