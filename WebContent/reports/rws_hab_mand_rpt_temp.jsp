...
<tr >
		<td class="textborder" align="center">
				<font size=2>Status As On: </font><font color=#F2A80D> <%=FDATE %> 
	</tr>
	<% }%>
</tr>
</table>
...
<% //if(session.getAttribute("mName")==null){ %>
		<td class="mystyle1">
		<a href="rws_hab_panch_status_rpt.jsp?mcode=<%=record1[0]%>&mname=<%=record1[1]%>">
			<%=record1[1]%></a></td> 
	<% //} %>


				<% 		////System.out.println("333333333330   "+labelNames.length);
...
<%	
System.out.println("sree"+labelNames.length);
...
<% 
	...
	//pgcnt=1;
		...
		{
		%>
		<a href="rws_hab_mand_rpt_temp.jsp?start=<%=r1%>&slno=<%=slno%>&pgcnt=<%=s1%>"><font   color=blue>Next </font></a>
		<%}
