...
<tr bgcolor="#8A9FCD" align="center">
		<td align="center" class="rptHeading" ></td><td align="right" class="rptHeading" colspan=11>*** Rupees in Lakhs</td>
</tr>
</table>
...
<td align="center" class=btext>11</td>
   ...
   query="SELECT distinct d.dname,b.DIVISION_OFFICE_NAME,w.work_id,w.WORK_NAME ,p.programme_name,nvl(SANCTION_AMOUNT,'0'),to_char(ADMIN_DATE,'dd/mm/yyyy'),extract(YEAR from ADMIN_DATE)  FROM RWS_WORK_ADMN_TBL W ,rws_division_office_tbl b,rws_district_tbl  d,rws_programme_tbl p  where  w.category_code='"+type+"'  and w.work_cancel_dt is null  and w.work_id in (select work_id from rws_work_completion_tbl  where date_of_completion is not null and to_date(date_of_completion)<'01-Apr-"+yy+"' and (to_date(update_date)>= '01-Apr-"+prevyr+"' and to_date(update_date)<'01-Apr-"+yy+"')) and substr(w.work_id,1,2)=p.programme_code and substr(w.work_id,5,2)=d.dcode  and substr(w.office_code,2,2)=b.circle_office_code and substr(w.office_code,4,1)=b.division_office_code  and  b.circle_office_code='"+circleOfficeCode +"' and b.division_office_code='"+divCode+"' order by 8  ";
   ...
   while(rs1.next())
       {
        ...
        %>
             ...
             <tr>
                ...
                <td class=rptValue align="right"><%=expAmt.get(rs1.getString(3))==null?"0":(explast=(Double.parseDouble((String)expAmt.get(rs1.getString(3)))))%></td> 
