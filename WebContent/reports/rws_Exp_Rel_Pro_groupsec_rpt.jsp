...
<%@ include file="financialYear.jsp" %>
		...
		<td class=btext rowspan="2">Programme</td>
			...
			<%if(finyear.equals("2011-2012")){%>
		<td class=btext  colspan="13" nowrap>Central Releases During the Year(01-APR-<%=finyear.substring(0,4)%> To 31-MAR-<%=finyear.substring(5,9)%>)<br>State Releases During the Year(17-APR-<%=finyear.substring(0,4)%> To 31-MAR-<%=finyear.substring(5,9)%>)</td>
		<%}else{%>
		<td class=btext  colspan="13" nowrap>Central Releases During the Year(01-APR-<%=finyear.substring(0,4)%> To 31-MAR-<%=finyear.substring(5,9)%>)<br>State Releases During the Year(01-APR-<%=finyear.substring(0,4)%> To 31-MAR-<%=finyear.substring(5,9)%>)</td>
		<%}%>
	...
	<!-- 	<td class=btext align="center">Offline Release(Upto 31-MAR-<%=finyear.substring(5,9)%>)</td>
	 ...
	 -->	</tr>
		...
		if(dcode!=null && !dcode.equals("All")) billcqry += " and substr(a.office_code,2,2)='"+dcode+"' and  not in('01','02','18','23','32','48','49','51','53','58','59','60')";
		...
		String billc="";
		//
		billc="select sum(BILL_AMOUNT_CENTRAL ),substr(a.work_id,1,2) from RWS_bill_bank_bridge_det_TBL a,rws_work_admn_tbl b where a.work_id=b.work_id  and bank_send_date>='01-APR-"+yr+"' and bank_send_date<'31-MAR-"+nextyr+"' and txid is not null ";
		if(dcode!=null && !dcode.equals("All")) billc += " and substr(a.office_code,2,2)='"+dcode+"' and substr(work_id,1,2) in('01','02','18','23','32','48','49','51','53','58','59','60') ";
		billc += " group by substr(a.work_id,1,2) ";
		//
		Statement stmt55=conn.createStatement();
		System.out.println("sourcesample:"+billc);
		ResultSet rs55=stmt55.executeQuery(billc);
     ...
     String qry="select programme_name,programme_code from rws_programme_tbl where programme_code not in('00') order by 1";
	...
	while(rs.next())
	{
		 programcode=rs.getString(2);
		
		if( programcode.equals("01") || programcode.equals("02") || programcode.equals("18")|| programcode.equals("23")|| programcode.equals("32")|| programcode.equals("48")|| programcode.equals("49")|| programcode.equals("51")|| programcode.equals("53")|| programcode.equals("58")|| programcode.equals("60") )
		{
			pname="NRDWP";
			String censum=(String)sourceh.get(rs.getString(2));
			if(censum!=null && !censum.equals("") && !censum.equals("null"))
			{
			p=Double.parseDouble(censum);
			total=total+p;
			System.out.println("!***************pppp"+p);
			System.out.println("!***************88"+total);
	  ...
	  //  if(wbtot!=null && !wbtot.equals("") && !wbtot.equals("null")){}else{wbtot="0";}
		 String cenamt=""+h2.get(rs.getString(2));
...
%>
		...
		<td  class=rptValue nowrap><%=pname%><%=programcode%><%=rs.getString(1)%></td>
