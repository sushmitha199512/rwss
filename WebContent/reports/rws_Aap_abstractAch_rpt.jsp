...
function fnHab(status,dcode)
{
	...
	var left = parseInt((screen.availWidth/2)-(width/2));
	var top = parseInt((screen.availHeight/2)-(height/2));
	var url = "rws_habcov_detailsAch.jsp?status="+status+"&dcode="+dcode+"&catCode="+catCode;
	var properties = "width="+width+",height="+height+",screenX="+left+",screenY="+top+",toolbar=no,"
					   + "status=no,menubar=no,location=no,directories=no,scrollbars=yes,copyhistory=no,"
...
<caption>
			<table  border=0 rules=none style="border-collapse:collapse" align = "right">		
				<tr>
					...
					<a href="../home.jsp">Home</a> |<a href="./rws_Aap_abstractAch_xls.jsp?catCode=<%=catCode%>&catName=<%=catName%>" target=_new>Excel</a></td>
				</tr>
			</table>
			</caption>
</table>
...
<tr>
    ...
    <td class=btext align="center"   colspan="13">Achievement</td>
</tr>
<tr>
 ...
 //System.out.println("catName111111::");
		...
		{
			 qry2="SELECT distinct substr(w.work_id,5,2) ,dname FROM RWS_WORK_ADMN_TBL W,rws_aap_selected_tbl b  ,rws_district_tbl d where w.work_id=b.work_id  and substr(w.work_id,5,2)=d.dcode   order by 1";
		}else
		{
			 qry2="SELECT distinct substr(w.work_id,5,2) ,dname FROM RWS_WORK_ADMN_TBL W,rws_aap_selected_tbl b  ,rws_district_tbl d where w.work_id=b.work_id  and substr(w.work_id,5,2)='"+loggedUser.substring(1,3)+"'  and  d.dcode=substr(w.work_id,5,2)  order by 1";
		}
...
while(rs2.next())
{
			...
			if(loggedUser!=null && (loggedUser.equals("admin")||loggedUser.equals("100000")))
		{
			qry="select  (SELECT count(w.work_id)  FROM RWS_WORK_ADMN_TBL W,rws_aap_selected_tbl b  where w.work_id=b.work_id  and substr(b.work_id,5,2)='"+rs2.getString(1)+"' "+cat+" and work_cancel_dt is null  and b.work_id  in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < '01-Apr-"+yr+"')) )as b ,( SELECT sum(w.sanction_amount)  FROM RWS_WORK_ADMN_TBL W,rws_aap_selected_tbl b  where w.work_id=b.work_id  and substr(b.work_id,5,2)='"+rs2.getString(1)+"'   "+cat+"   and work_cancel_dt is null  and b.work_id  in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < '01-Apr-"+yr+"')) )as c ,( SELECT count(distinct w.hab_code)  FROM RWS_WORK_COMP_HAB_LNK_TBL  W,rws_aap_selected_tbl b ,rws_work_admn_tbl c where w.work_id=b.work_id  and substr(b.work_id,5,2)='"+rs2.getString(1)+"'  "+cat+" and c.work_id=b.work_id and b.work_id  in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < '01-Apr-"+yr+"'))  )as d from dual ";

		}else
		{
			qry="select  (SELECT count(w.work_id)  FROM RWS_WORK_ADMN_TBL W,rws_aap_selected_tbl b  where w.work_id=b.work_id  and substr(b.work_id,5,2)='"+loggedUser.substring(1,3)+"'  "+cat+"  and work_cancel_dt is null  and b.work_id  in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < '01-Apr-"+yr+"')) )as b ,( SELECT sum(w.sanction_amount)  FROM RWS_WORK_ADMN_TBL W,rws_aap_selected_tbl b  where w.work_id=b.work_id  and substr(b.work_id,5,2)='"+loggedUser.substring(1,3)+"'  "+cat+"   and work_cancel_dt is null  and b.work_id  in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < '01-Apr-"+yr+"'))  )as c ,( SELECT count(distinct w.hab_code)  FROM RWS_WORK_COMP_HAB_LNK_TBL  W,rws_aap_selected_tbl b ,rws_work_admn_tbl c where w.work_id=b.work_id  and substr(b.work_id,5,2)='"+loggedUser.substring(1,3)+"'  "+cat+" and c.work_id=b.work_id and c.work_id=b.work_id and b.work_id  in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < '01-Apr-"+yr+"')) )as d from dual   ";
		}
...
<%if(rs.getInt(1)>0)
	       {%>
		   <td class=rptValue align="right"><a href="#" onclick="wopen('rws_aap_work_detailsAch.jsp?dname=<%=rs2.getString(2)%>&dcode=<%=rs2.getString(1)%>&cat=<%=catCode%>','popup', 900, 300)"><%=rs.getInt(1)%></a></td>
     	<%}
	else{%>
...
<%if(rs.getInt(3)>0)
	       {%>
		   <td class=rptValue align="right"><a href="#" onclick="wopen('rws_aap_habsAch_rpt.jsp?dname=<%=rs2.getString(2)%>&dcode=<%=rs2.getString(1)%>&cat=<%=catCode%>','popup', 900, 300)"><%=rs.getInt(3)%></a></td>
     	<%}
	else{%>
	<td class=rptvalue align="right">0</td>
	<%}%>
<%String qry1="select  sum(case when coverage_status='FC' then 1 else 0 end)a,sum(case when coverage_status='PC4' then 1 else 0 end) b,sum(case when coverage_status='PC3' then 1 else 0 end) c,sum(case when coverage_status='PC2' then 1 else 0 end) d,sum(case when coverage_status='PC1' then 1 else 0 end) e,sum(case when coverage_status='NC' then 1 else 0 end) f,sum(case when coverage_status='NSS' then 1 else 0 end)g,sum(case when coverage_status is not null  then 1 else 0 end)  h from rws_AAP_selected_tbl a ,rws_complete_hab_view v,RWS_WORK_COMP_HAB_LNK_TBL  b,rws_work_admn_tbl c  where a.work_id=b.work_id and v.panch_code=b.hab_code  and a.work_id=c.work_id and b.work_id=c.work_id  and work_cancel_dt is null  and a.work_id  in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < '01-Apr-"+yr+"')) ";
