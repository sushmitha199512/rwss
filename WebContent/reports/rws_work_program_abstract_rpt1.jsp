...
<%
   ...
   if(financialyr!=null && financialyr.equals("2011-12")){    
	...
	//System.out.println("fix:"+fixYear);
	...
	if(request.getParameter("progCode")!=null)
	{
		session.setAttribute("progCode",request.getParameter("progCode"));
	}
...
<table border = 1 cellspacing = 0 cellpadding = 0 height=5% width="100%" bordercolor=#000000 style="border-collapse:collapse" bgcolor="#ffffff" align = "center">
	<caption>
		<table  border=0 rules=none style="border-collapse:collapse" align = "right">			
			<tr align="right">
				...
				<td class="bwborder"><a href="./rws_work_program_abstract_excel.jsp" target=_new">Excel|</a></td>
	...
	////System.out.println("First Drill  000000");
	 ...
	 <%
		...
		String view1="Create or Replace view spillworks? as SELECT work_id,office_code,no_of_habs,programme_code FROM RWS_WORK_ADMN_TBL W where  work_cancel_dt is null and  w.ADMIN_DATE < ? and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < ? ))";
		String view2="Create or Replace view newworks?  as SELECT work_id,office_code,no_of_habs,programme_code FROM RWS_WORK_ADMN_TBL W where work_cancel_dt is null  and w.ADMIN_DATE >= ?";
		...
		<td class=rptValue title=Click><b><a href="rws_work_program_abstract_rpt1.jsp?progCode=<%=rs1.getString(1)%>&planNonplan=<%=planCode%>&drill=11&catCode=<%=catCode%>&distname=<%=rs1.getString(2)%>"><%=rs1.getString(2)%></a></td>
	   	...
	   	String qry2=null;Statement stmt2=null;
			...
			String qry22 = "select (select count(*) from newworks"+session.getId().substring(1,10)+" where programme_code='"+rs1.getString(1)+"')a,(select sum(no_of_habs) from newworks"+session.getId().substring(1,10)+" where programme_code ='"+rs1.getString(1)+"')b,(select count(*) from newworks"+session.getId().substring(1,10)+"  a,rws_work_admn_prog_lnk_tbl b where a.work_id=b.work_id and  a.programme_code='"+rs1.getString(1)+"')c from dual";
	   	  ...
	   	  // //System.out.println("query in prog 21:"+qry21);
           ////System.out.println("query in prog 22:"+qry22);
  ...
  try
  {
      ...
      if(rs44!=null){rs44.close();}
       ...
       {
         //System.out.println("exception in drill :"+e1);
		 e1.printStackTrace();
       ...
       if(stmt8!=null){stmt8.close();}
         ...
         String qry9=null;Statement stmt9=null;
			...
			qry9 = "select sum(case when date_of_completion<'"+fixYear+"' then 1 else 0 end),sum(case when date_of_completion>='"+fixYear+"' and date_of_completion<'"+fixNextYear+"' then 1 else 0 end) from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and  programme_code='"+rs1.getString(1)+"' "+planQry+" "+catQry+" ";
       ...
       String qry4=null;
			...
			qry4 = "select (select sum(w.no_of_habs) from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and programme_code='"+rs1.getString(1)+"' and date_of_completion<'"+fixYear+"' "+planQry+"  "+catQry+" )as a, (select sum(w.no_of_habs) from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and programme_code='"+rs1.getString(1)+"'  and date_of_completion>='"+fixYear+"' and date_of_completion<'"+fixNextYear+"'  "+planQry+" "+catQry+" )as b from dual";
 ...
 if(drill.equals("11"))
	...
	// //System.out.println("First Drill 111111");
	   ...
	   %>
		...
		qry1="SELECT DISTINCT D.DCODE AS A_CODE,D.DNAME AS A_NAME FROM RWS_WORK_ADMN_TBL A,RWS_ADMN_HAB_LNK_TBL AL,RWS_DISTRICT_TBL D WHERE A.WORK_ID=AL.WORK_ID AND D.DCODE=SUBSTR (AL.HAB_CODE,1,2) and D.DCODE=SUBSTR(A.WORK_ID,5,2)  and  work_cancel_dt is null  GROUP BY D.DCODE,D.DNAME";
		////// //System.out.println("Test Qry is::"+qry1);
		stmt1 = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
		rs1=stmt1.executeQuery(qry1);
		String view1="Create or Replace view spillworks?  as SELECT work_id,office_code,no_of_habs,programme_code FROM RWS_WORK_ADMN_TBL W where  work_cancel_dt is null  and  programme_code=? and w.ADMIN_DATE < ? and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < ? ))";
		String view2="Create or Replace view newworks?  as SELECT work_id,office_code,no_of_habs,programme_code FROM RWS_WORK_ADMN_TBL W where work_cancel_dt is null  and  programme_code=? and w.ADMIN_DATE >= ? ";
		 //System.out.println("view1"+view1);
		 //System.out.println("view2"+view2);
		Statement stmtv1= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
	   	Statement stmtv2= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
		 PreparedStatement ps1=conn.prepareStatement(view1);
	       PreparedStatement ps2=conn.prepareStatement(view2);
	        ps1.setString(1,session.getId().substring(1,10));
	        ps1.setString(2,request.getParameter("progCode")+" "+catQry+"  "+planQry);
	        ps1.setString(3,fixYear);
	        ps1.setString(4,fixYear);
			ps2.setString(1,session.getId().substring(1,10));
			ps2.setString(2,request.getParameter("progCode")+" "+catQry+"  "+planQry);
...
// // //System.out.println("qry44aaaaaaaaa"+qry44);
      ...
      if(rs44!=null){rs44.close();}
       ...
       String qry4=null;
			...
			//qry4="Select count(e.hab_code) from RWS_admn_HAB_LNK_TBL e where e.work_id in (select work_id from spillworks"+session.getId().substring(1,10)+" where substr(work_id,5,2)='"+rs1.getString(1)+"' union select work_id from newworks"+session.getId().substring(1,10)+" where substr(work_id,5,2)='"+rs1.getString(1)+"')";
			////// //System.out.println(qry4);
			qry4 = "select (select sum(w.no_of_habs) from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rs1.getString(1)+"' and substr(w.work_id,1,2)='"+request.getParameter("progCode")+"' and date_of_completion<'"+fixYear+"' "+planQry+" "+catQry+" )as a, (select sum(w.no_of_habs) from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rs1.getString(1)+"'  and substr(w.work_id,1,2)='"+request.getParameter("progCode")+"' and date_of_completion>='"+fixYear+"' and date_of_completion<'"+fixNextYear+"' "+planQry+" "+catQry+" )as b from dual";
		...
		String dropqry2="drop VIEW newworks?";
		 PreparedStatement ps11=conn.prepareStatement(dropqry1);
	       PreparedStatement ps12=conn.prepareStatement(dropqry2);
			...
			ps12.setString(1,session.getId().substring(1,10));
		ps11.executeUpdate();
		ps12.executeUpdate();
		}
		catch(SQLException se)
	...
	excelnav = "1";
	 // //System.out.println("First Drill");
	   ...
	   %>
		...
		String view1="Create or Replace view spillworks? as SELECT work_id,office_code,no_of_habs FROM RWS_WORK_ADMN_TBL W where  w.ADMIN_DATE < ? and SUBSTR (w.work_id,5,2)=? and substr(w.work_id,1,2)=? and  work_cancel_dt is null  and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < ? ))";
		String view2="Create or Replace view newworks?  as SELECT work_id,office_code,no_of_habs FROM RWS_WORK_ADMN_TBL W where w.ADMIN_DATE >= ? and SUBSTR (w.work_id,5,2)=?  and substr(w.work_id,1,2)=? and work_cancel_dt is null   ";
		 //System.out.println("View1:"+view1);
		 //System.out.println("View2:"+view2);
		Statement stmtv1= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
	   	Statement stmtv2= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
	       ...
	       PreparedStatement ps2=conn.prepareStatement(view2);
	        ...
	        ps1.setString(4, request.getParameter("progCode"));
	        ps1.setString(5, fixYear);
	        ps2.setString(1,session.getId().substring(1,10));
	        ps2.setString(2, fixYear+" "+catQry+"  "+planQry);
	        ps2.setString(3, rDistrict);
	        ps2.setString(4, request.getParameter("progCode"));
       ...
       if(stmt4!=null){stmt4.close();}
        ...
        String qry10=null; Statement stmt10=null;
          ...
          //  qry10="Select count(e.hab_code) from RWS_WORK_COMP_HAB_LNK_TBL e where e.work_id in (select work_id from spillworks"+session.getId().substring(1,10)+" where SUBSTR(office_code,4,1)='"+rs1.getString(1)+"' union select work_id from newworks"+session.getId().substring(1,10)+" where SUBSTR(office_code,4,1)='"+rs1.getString(1)+"')";
            ////// //System.out.println("qry10:"+qry10);
			qry10="select (Select count(e.hab_code) from RWS_WORK_COMP_HAB_LNK_TBL e where e.work_id in (select w.work_id from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rDistrict+"' and substr(w.work_id,1,2)='"+request.getParameter("progCode")+"' and SUBSTR (office_code,4,1) ='"+rs1.getString(1)+"' and date_of_completion<'"+fixYear+"' "+planQry+" "+catQry+" ))a, (Select count (e.hab_code) from RWS_ADMN_HAB_LNK_TBL e where e.work_id in (select w.work_id from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rDistrict+"' and substr(w.work_id,1,2)='"+request.getParameter("progCode")+"' and SUBSTR (office_code,4,1) ='"+rs1.getString(1)+"' and date_of_completion>='"+fixYear+"' and date_of_completion<'"+fixNextYear+"' "+planQry+" "+catQry+" ))b from dual";
			////// //System.out.println("qry10"+qry10);
            stmt10= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
             rs10=stmt10.executeQuery(qry10);
            while(rs10.next())
 ...
 --><td class="btext" align="right"><%=ghabcovn1%></td>
		...
		String dropqry2="drop VIEW newworks?";
		 PreparedStatement psm1=conn.prepareStatement(dropqry1);
	       PreparedStatement psm2=conn.prepareStatement(dropqry2);
			...
			psm2.setString(1,session.getId().substring(1,10));
		psm1.executeUpdate();
		psm2.executeUpdate();
		}
		catch(SQLException se)
	...
	////// //System.out.println("Second Drill");
	   ...
	   %>
		...
		stmt1 = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
		rs1=stmt1.executeQuery(qry1);
		String view1="Create or Replace view spillworks? as SELECT work_id,office_code,no_of_habs FROM RWS_WORK_ADMN_TBL W where w.ADMIN_DATE < ? and SUBSTR (w.office_code,2,2)=? and substr(w.work_id,1,2)=? and SUBSTR(w.office_code,4,1)=? and work_cancel_dt is null  and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION <? ))";
		String view2="Create or Replace view newworks?  as SELECT work_id,office_code,no_of_habs FROM RWS_WORK_ADMN_TBL W where w.ADMIN_DATE >= ? and SUBSTR (w.office_code,2,2)=?  and work_cancel_dt is null   and substr(w.work_id,1,2)=?  and SUBSTR(w.office_code,4,1)=?";
		Statement stmtv1= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
	   	Statement stmtv2= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
 	   ////// //System.out.println(view1);
	       ...
	       PreparedStatement ps2=conn.prepareStatement(view2);
	        ...
	        ps1.setString(5, request.getParameter("progCode"));
	        ps1.setString(6, fixYear);
	        ps2.setString(1,session.getId().substring(1,10));
	        ps2.setString(2, fixYear+" "+catQry+"  "+planQry);
	        ps2.setString(3, rDistrict);
	        ps2.setString(4,divisionCode);
	        ps2.setString(5, request.getParameter("progCode"));
 	   ps1.executeUpdate();
 	   	ps2.executeUpdate();   	
