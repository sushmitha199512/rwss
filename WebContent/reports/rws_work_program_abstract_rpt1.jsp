...
<table border = 1 cellspacing = 0 cellpadding = 0 height=5% width="100%" bordercolor=#000000 style="border-collapse:collapse" bgcolor="#ffffff" align = "center">
	...
	////System.out.println("First Drill  000000");
	 ...
	 <%
		...
		String view1="Create or Replace view spillworks"+session.getId().substring(1,10)+" as SELECT work_id,office_code,no_of_habs,programme_code FROM RWS_WORK_ADMN_TBL W where  work_cancel_dt is null and  w.ADMIN_DATE < '"+fixYear+"' "+catQry+"  "+planQry+" and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < '"+fixYear+"' ))";
		String view2="Create or Replace view newworks"+session.getId().substring(1,10)+" as SELECT work_id,office_code,no_of_habs,programme_code FROM RWS_WORK_ADMN_TBL W where work_cancel_dt is null  and w.ADMIN_DATE >= '"+fixYear+"' "+catQry+"  "+planQry+"";
 ...
 if(drill.equals("11"))
	...
	// //System.out.println("First Drill 111111");
	   ...
	   %>
		...
		stmt1 = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
		rs1=stmt1.executeQuery(qry1);
		String view1="Create or Replace view spillworks"+session.getId().substring(1,10)+" as SELECT work_id,office_code,no_of_habs,programme_code FROM RWS_WORK_ADMN_TBL W where  work_cancel_dt is null  and  programme_code='"+request.getParameter("progCode")+"' "+catQry+"  "+planQry+" and w.ADMIN_DATE < '"+fixYear+"' and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < '"+fixYear+"' ))";
		String view2="Create or Replace view newworks"+session.getId().substring(1,10)+" as SELECT work_id,office_code,no_of_habs,programme_code FROM RWS_WORK_ADMN_TBL W where work_cancel_dt is null  and  programme_code='"+request.getParameter("progCode")+"' "+catQry+"  "+planQry+" and w.ADMIN_DATE >= '"+fixYear+"' ";
		 //System.out.println("view1"+view1);
		 //System.out.println("view2"+view2);
		Statement stmtv1= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
	   	Statement stmtv2= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
	...
	excelnav = "1";
	 // //System.out.println("First Drill");
	   ...
	   %>
		...
		String view1="Create or Replace view spillworks"+session.getId().substring(1,10)+" as SELECT work_id,office_code,no_of_habs FROM RWS_WORK_ADMN_TBL W where  w.ADMIN_DATE < '"+fixYear+"' "+catQry+"  "+planQry+" and SUBSTR (w.work_id,5,2)='"+rDistrict+"' and substr(w.work_id,1,2)='"+request.getParameter("progCode")+"' and  work_cancel_dt is null  and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < '"+fixYear+"' ))";
		String view2="Create or Replace view newworks"+session.getId().substring(1,10)+" as SELECT work_id,office_code,no_of_habs FROM RWS_WORK_ADMN_TBL W where w.ADMIN_DATE >= '"+fixYear+"' "+catQry+"  "+planQry+" and SUBSTR (w.work_id,5,2)='"+rDistrict+"'  and substr(w.work_id,1,2)='"+request.getParameter("progCode")+"' and work_cancel_dt is null   ";
		 //System.out.println("View1:"+view1);
		 //System.out.println("View2:"+view2);
		Statement stmtv1= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
	   	Statement stmtv2= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
 ...
 --><td class="btext" align="right"><%=ghabcovn1%></td>
	...
	////// //System.out.println("Second Drill");
	   ...
	   %>
		...
		stmt1 = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
		rs1=stmt1.executeQuery(qry1);
		String view1="Create or Replace view spillworks"+session.getId().substring(1,10)+" as SELECT work_id,office_code,no_of_habs FROM RWS_WORK_ADMN_TBL W where w.ADMIN_DATE < '"+fixYear+"'  "+catQry+"  "+planQry+" and SUBSTR (w.office_code,2,2)='"+rDistrict+"' and substr(w.work_id,1,2)='"+request.getParameter("progCode")+"' and SUBSTR(w.office_code,4,1)='"+divisionCode+"' and work_cancel_dt is null  and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION <'"+fixYear+"' ))";
		String view2="Create or Replace view newworks"+session.getId().substring(1,10)+" as SELECT work_id,office_code,no_of_habs FROM RWS_WORK_ADMN_TBL W where w.ADMIN_DATE >= '"+fixYear+"' "+catQry+"  "+planQry+" and SUBSTR (w.office_code,2,2)='"+rDistrict+"'  and work_cancel_dt is null   and substr(w.work_id,1,2)='"+request.getParameter("progCode")+"'  and SUBSTR(w.office_code,4,1)='"+divisionCode+"'";
		Statement stmtv1= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
	   	Statement stmtv2= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
