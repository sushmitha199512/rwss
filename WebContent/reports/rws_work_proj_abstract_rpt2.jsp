...
<table border = 1 cellspacing = 0 cellpadding = 0 height=5% width="100%" bordercolor=#000000 style="border-collapse:collapse" bgcolor="#ffffff" align = "center">
	<caption>
		<table  border=0 rules=none style="border-collapse:collapse" align = "right">			
			<tr align="right">
			    <td class="bwborder"><a href="http://aprwss.gov.in/index.html">Home|</a></td>
	...
	////System.out..println("First Drill");
	   ...
	   ////System.out.println("view2"+view2);
		Statement stmtv1= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
	   	...
	   	PreparedStatement ps3=conn.prepareStatement(view1);
	   	PreparedStatement ps4=conn.prepareStatement(view2);
	   	        ps3.setString(1,session.getId().substring(1,10));
	   			ps3.setString(2,catCode);
	   			ps3.setString(3,fixYear);
	   			ps4.setString(4,fixYear);
	   			ps4.setString(1,session.getId().substring(1,10));
	   			ps4.setString(2,catCode);
	   			ps4.setString(3,fixYear);
	   	ps3.executeUpdate();
 	   	ps4.executeUpdate();   	
		...
		<td class=rptValue title=Click><b><a href="rws_work_proj_abstract_rpt2.jsp?district=<%=rs1.getString(1)%>&planNonplan=<%=planCode%>&drill=1&catCode=<%=catCode%>&distname=<%=rs1.getString(2)%>"><%=rs1.getString(2)%></a></td>
...
<td class="btext" align="right"><%=ghabcovn1%></td>
       ...
       Statement stmtdv1=null,stmtdv2=null;
		...
		PreparedStatement ps1=conn.prepareStatement(dropqry1);
		PreparedStatement ps2=conn.prepareStatement(dropqry2);
	...
	////System.out..println("First Drill");
	   ...
	   %>
		...
		<tr onmouseover="this.style.background='#E3E4FA'" onmouseout="this.style.background='WHITE'">
        <td class=rptValue><%=++slno%></td>
		<td class=rptValue title=Click><b><a href="rws_work_proj_abstract_rpt2.jsp?district=<%=rDistrict%>&planNonplan=<%=planCode%>&drill=2&distname=<%=districtName%>&catCode=<%=catCode%>&division=<%=rs1.getString(1)%>&divname=<%=rs1.getString(2)%>"><%=rs1.getString(2)%></a></td>
...
//            qry44="SELECT SUM(CASE WHEN w.ADMIN_DATE <  '"+fixYear+"' and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION >= '01-Apr- 2008' or DATE_OF_COMPLETION is null)) AND E.TS_AMOUNT IS NOT NULL THEN TS_AMOUNT ELSE 0 END) AS EST_AMT FROM RWS_WORK_ADMN_TBL W, RWS_NEW_EST_TBL E where w.plan_code ='"+planCode+"' and w.category_code='"+catCode+"' and substr(w.office_code,2,2)='"+rs1.getString(1)+"' and W.WORK_ID=E.WORK_ID AND W.PROGRAMME_CODE=SUBSTR(E.WORK_ID,1,2)";
            qry44 = "select sum(a.sanction_amount) from rws_work_admn_tbl a,spillworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and SUBSTR(b.office_code,4,1)='"+rs1.getString(1)+"'";
////System.out.println("qry44bbbbbbbb"+qry44);
            stmt44 = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
            rs44=stmt44.executeQuery(qry44);
//          qry45="SELECT SUM(CASE WHEN w.ADMIN_DATE >= '"+fixYear+"' AND E.TS_AMOUNT IS NOT NULL THEN TS_AMOUNT ELSE 0 END) AS EST_AMT FROM RWS_WORK_ADMN_TBL W, RWS_NEW_EST_TBL E where w.plan_code ='"+planCode+"' and w.category_code='"+catCode+"' and substr(w.office_code,2,2)='"+rs1.getString(1)+"' and W.WORK_ID=E.WORK_ID AND W.PROGRAMME_CODE=SUBSTR(E.WORK_ID,1,2)";
  			qry45 = "select sum(a.sanction_amount) from rws_work_admn_tbl a,newworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and SUBSTR(b.office_code,4,1)='"+rs1.getString(1)+"'";
           ////System.out..println("qry45"+qry45);
            stmt45 = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
            rs55=stmt45.executeQuery(qry45);
            while(rs44.next()&& rs55.next())
	...
	////System.out..println("Second Drill");
	  ...
	  ////System.out..println("query"+qry22);
	   	...
	   	while(rs21.next()&& rs22.next())
            ...
            {
//            qry44="SELECT SUM(CASE WHEN w.ADMIN_DATE <  '"+fixYear+"' and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION >= '01-Apr- 2008' or DATE_OF_COMPLETION is null)) AND E.TS_AMOUNT IS NOT NULL THEN TS_AMOUNT ELSE 0 END) AS EST_AMT FROM RWS_WORK_ADMN_TBL W, RWS_NEW_EST_TBL E where w.plan_code ='"+planCode+"' and w.category_code='"+catCode+"' and substr(w.office_code,2,2)='"+rs1.getString(1)+"' and W.WORK_ID=E.WORK_ID AND W.PROGRAMME_CODE=SUBSTR(E.WORK_ID,1,2)";
            qry44 = "select sum(a.sanction_amount) from rws_work_admn_tbl a,spillworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and  SUBSTR(b.office_code,5,2)='"+rs1.getString(1)+"'";
           ////System.out.println("qry44ssssssssssss"+qry44);
            stmt44 = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
            rs44=stmt44.executeQuery(qry44);
//          qry45="SELECT SUM(CASE WHEN w.ADMIN_DATE >= '"+fixYear+"' AND E.TS_AMOUNT IS NOT NULL THEN TS_AMOUNT ELSE 0 END) AS EST_AMT FROM RWS_WORK_ADMN_TBL W, RWS_NEW_EST_TBL E where w.plan_code ='"+planCode+"' and w.category_code='"+catCode+"' and substr(w.office_code,2,2)='"+rs1.getString(1)+"' and W.WORK_ID=E.WORK_ID AND W.PROGRAMME_CODE=SUBSTR(E.WORK_ID,1,2)";
  			qry45 = "select sum(a.sanction_amount) from rws_work_admn_tbl a,newworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and SUBSTR(b.office_code,5,2)='"+rs1.getString(1)+"'";
           ////System.out..println("qry45"+qry45);
            stmt45 = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
            rs55=stmt45.executeQuery(qry45);
            while(rs44.next()&& rs55.next())
