...
<table border = 0>	
		...
		<tr  align="center">
			<td align="center" class="rptHeading" style="width:350">Divison Wise Report Under Program:<%=pname%></td>
...
try
{
	...
	String q = "select sum(OP_CL_BAL),substr(office_code,2,3) from RWS_GRANT_OP_CL_BAL_TBL where programme_code='"+pcode+"' group by substr(office_code,2,3)";
	...
	String q3 = "select nvl(sum(voucher_amt),0),substr(b.office_code,2,3) from rws_work_exp_voucher_tbl a,rws_work_admn_tbl b where a.work_id=b.work_id and BILL_SL_NO <> '0' and b.programme_code='"+pcode+"' group by substr (b.office_code,2,3)";
	...
	String q5 = "select nvl(sum(BILL_AMT_CONTRACTOR),0),nvl(sum(BILL_AMT_ADMISSIBLE),0),substr(b.office_code,2,3) from rws_work_bill_tbl a,rws_work_admn_tbl b where a.work_id=b.work_id and bill_sl_no<>'0' and b.programme_code='"+pcode+"' group by substr(b.office_code,2,3)";
...
<tr  class="alternateRow">
			...
			<td class="rptValue" align="right"><a href="#" onClick="wopen('rws_ob_releases_rpt1.jsp?dcode=00&divcode=0&dnamee=Hyderabad&divname=Head Office&pcode=<%=pcode%>&pname=<%=pname%>','popup', 600, 300)"><%=ndf.format(Double.parseDouble(ob))%></a></td>
			<%}else{%>
	...
	<%
		...
		<!-- OB -->
		<%if(ob!=null && !ob.equals("0")){%>
		<td class="rptValue" align="right"><a href="#" onClick="wopen('rws_ob_releases_rpt1.jsp?dcode=<%=rs.getString(1)%>&divcode=<%=rs.getString(2)%>&dnamee=<%=rs.getString(3)%>&divname=<%=rs.getString(4)%>&pcode=<%=pcode%>&pname=<%=pname%>','popup', 900, 300)"><%=ndf.format(Double.parseDouble(ob))%></a></td>
		<%}else{%>
		<td class="rptValue" align="right"><%=ndf.format(Double.parseDouble(ob))%></td>
		<%}%>
		<!-- Releases -->
		<%if(release!=null && !release.equals("0")){%>
		<td class="rptValue" align="right"><a href="#" onClick="wopen('rws_releases_rpt1.jsp?dcode=<%=rs.getString(1)%>&divcode=<%=rs.getString(2)%>&dnamee=<%=rs.getString(3)%>&divname=<%=rs.getString(4)%>&pcode=<%=pcode%>&pname=<%=pname%>','popup', 900, 300)"><%=ndf.format(Double.parseDouble(release)*100000)%></a></td>
		<%}else{%>
		...
		<td class="rptValue" align="right"><a href="#" onClick="wopen('rws_exp_vouhcers_rpt1.jsp?dcode=<%=rs.getString(1)%>&divcode=<%=rs.getString(2)%>&dnamee=<%=rs.getString(3)%>&divname=<%=rs.getString(4)%>&pcode=<%=pcode%>&pname=<%=pname%>','popup', 900, 300)"><%=ndf.format(Double.parseDouble(exp))%></a></td>
		<%}else{%>
		...
		<td class="rptValue" align="right"><a href="#" onClick="wopen('rws_bill_rpt1.jsp?dcode=<%=rs.getString(1)%>&divcode=<%=rs.getString(2)%>&dnamee=<%=rs.getString(3)%>&divname=<%=rs.getString(4)%>&pcode=<%=pcode%>&pname=<%=pname%>','popup', 900, 300)"><%=ndf.format(Double.parseDouble(bill))%></a></td>
		<%}else{%>
