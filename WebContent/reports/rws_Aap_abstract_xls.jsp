...
while(rs2.next())
{
			...
			if(loggedUser!=null && (loggedUser.equals("admin")||loggedUser.equals("100000")))
		{
			qry="select  (SELECT count(w.work_id)  FROM RWS_WORK_ADMN_TBL W,rws_aap_selected_tbl b  where w.work_id=b.work_id  and substr(b.work_id,5,2)='"+rs2.getString(1)+"' "+cat+")as b ,( SELECT sum(w.sanction_amount)  FROM RWS_WORK_ADMN_TBL W,rws_aap_selected_tbl b  where w.work_id=b.work_id  and substr(b.work_id,5,2)='"+rs2.getString(1)+"'   "+cat+")as c ,( SELECT count(distinct w.hab_code)  FROM RWS_ADMN_hab_lnk_TBL W,rws_aap_selected_tbl b ,rws_work_admn_tbl c where w.work_id=b.work_id  and substr(b.work_id,5,2)='"+rs2.getString(1)+"'  "+cat+" and c.work_id=b.work_id)as d from dual ";

		}else
		{
			qry="select  (SELECT count(w.work_id)  FROM RWS_WORK_ADMN_TBL W,rws_aap_selected_tbl b  where w.work_id=b.work_id  and substr(b.work_id,5,2)='"+loggedUser.substring(1,3)+"'  "+cat+" )as b ,( SELECT sum(w.sanction_amount)  FROM RWS_WORK_ADMN_TBL W,rws_aap_selected_tbl b  where w.work_id=b.work_id  and substr(b.work_id,5,2)='"+loggedUser.substring(1,3)+"'  "+cat+")as c ,( SELECT count(distinct w.hab_code)  FROM RWS_ADMN_hab_lnk_TBL W,rws_aap_selected_tbl b ,rws_work_admn_tbl c where w.work_id=b.work_id  and substr(b.work_id,5,2)='"+loggedUser.substring(1,3)+"'  "+cat+" and c.work_id=b.work_id)as d from dual   ";
		}
