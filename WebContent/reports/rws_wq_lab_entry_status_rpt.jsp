...
<caption>
		...
		<td class=rptValue colspan="11"><b><font color="blue">WaterQuality Sources Report-Lab Wise</font></b></td>
		</tr>
        <tr align=center  align=center>
...
<td class=btext>Total No.of Sources</td>
       ...
       <tr align=center>
        ...
        <td class=btext>11</td><td class=btext>12</td>
		</tr>
	<%
       try{  slno=1;
          ...
          String notFoundContaminated="select count(*),substr(hd.panch_code,1,2),c.division_office_code  from rws_ftk_testing_tbl c,rws_complete_hab_view hd  where hd.panch_code=substr(c.source_code,1,16) and  (c.ARSENIC is null and NITRATE is null and FLORIDE is null and  SALINITY is null and  IRON is null and TURBIDITY is null and CHLORIDES is null and MANGANESE is null and SULPHATES is null and COPPER is null and  CALCIUM is null and  TDS is null and  MAGNESIUM is null and  HARDNESS is null) and c.TEST_DATE=(SELECT MAX(B.TEST_DATE)  FROM RWS_FTK_TESTING_TBL B WHERE B.SOURCE_CODE=c.SOURCE_CODE ) ";
if(month!=null && !month.equals("") && !month.equals("0") &&  !month.equals("51"))
			 {
...
//srinivas
            ...
            String notFoundContaminatedb="select count(*),substr(hd.panch_code,1,2),c.division_office_code  from rws_ftk_testing_tbl c,rws_complete_hab_view hd  where hd.panch_code=substr(c.source_code,1,16) and  ( COLIFORM is null ) and c.TEST_DATE=(SELECT MAX(B.TEST_DATE)  FROM RWS_FTK_TESTING_TBL B WHERE B.SOURCE_CODE=c.SOURCE_CODE ) ";
          ...
          //chemical sample
           ...
           //lab samples res
			...
			String query= "select a.circle_office_code,a.circle_office_name,b.division_office_code, b.division_office_name,c.LAB_NAME, c.LAB_CODE  from rws_water_quality_lab_tbl  c ,rws_circle_office_tbl a,rws_division_office_tbl b where a.circle_office_code=b.circle_office_code";
			       query+=" and a.circle_office_code=substr(c.office_code,2,2)  and b.division_office_code=substr(c.office_code,4,1) and c.status<>'UI' and b.status<>'UI' order by circle_office_code,b.division_office_code ";
             
			  stmt=conn.createStatement();
			  rs=stmt.executeQuery(query);
             System.out.println("main qry:"+query);
               boolean flag=false;        String divname="";
               int j=0;
			  while(rs.next())
		     {
               flag=true;
...
<%} %>
			<td class=rptValue><%=rs.getString(5)%></td>		
				...
				<td class=rptValue align="right"><%=notFoundsources%></td>
				<td class=rptValue align="right"><%=notFoundsourcesb%></td>
...
<tr align=center  align=center>
	...
	<%
       try{  slno=1;
		 ...
		 String notFoundContaminated="select count(*),substr(hd.panch_code,1,2),c.division_office_code  from rws_ftk_testing_tbl c,rws_complete_hab_view hd  where hd.panch_code=substr(c.source_code,1,16) and  (ARSENIC is null and NITRATE is null and FLORIDE is null and  SALINITY is null and  IRON is null and TURBIDITY is null and CHLORIDES is null and MANGANESE is null and SULPHATES is null and COPPER is null and  CALCIUM is null and  TDS is null and  MAGNESIUM is null and  HARDNESS is null) and c.TEST_DATE=(SELECT MAX(B.TEST_DATE)  FROM RWS_FTK_TESTING_TBL B WHERE B.SOURCE_CODE=c.SOURCE_CODE ) ";
if(month!=null && !month.equals("") && !month.equals("0") &&  !month.equals("51"))
			 {
			  notFoundContaminated+=" and to_char(test_date,'MON')='"+month+"' ";
			 }
...
//end
          ...
          String qryc1="select count(*),substr(hd.panch_code,1,2),c.division_office_code from rws_water_sample_collect_tbl c,rws_complete_hab_view hd ,RWS_WQ_TEST_RESULTS_TBL r where r.test_code=1 and hd.panch_code=c.hab_code and c.test_id is not null and r.test_id=c.test_id ";
           ...
           //lab samples res
			...
			String query= "select a.circle_office_code,a.circle_office_name,b.division_office_code, b.division_office_name,c.LAB_NAME, c.LAB_CODE  from rws_water_quality_lab_tbl  c, rws_circle_office_tbl a,rws_division_office_tbl b where a.circle_office_code=b.circle_office_code";
			       query+=" and a.circle_office_code=substr(c.office_code,2,2)  and b.division_office_code=substr(c.office_code,4,1) and a.circle_office_code='"+circlecode+"'";
                   query+=" and a.circle_office_code=substr(c.office_code,2,2)  and b.division_office_code=substr(c.office_code,4,1)  order by circle_office_name ";
