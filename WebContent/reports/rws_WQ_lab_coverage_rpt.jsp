...
System.out.println("yr***********************"+yr);
 ...
 //System.out.println("yr3***********************"+yr3);
 //System.out.println("yr***********************"+yr1);
...
<tr>
<td class="gridhdbg" colspan=3 align=center ><b>Water Quality Laboratory Testing Coverage </b></td>
</tr>
...
/*
String q3="select substr(d.office_code,2,2),count(*) from rws_wq_test_results_tbl a,RWS_WATER_QUALITY_LAB_TBL d where  to_date(a.prepared_on)>='1-"+cmonth+"-"+yr1+"'  and a.test_id in(select test_id from rws_wq_test_results_lnk_tbl)  and a.lab_code=d.lab_code group by substr(d.office_code,2,2)";
...
stmt=conn.createStatement();
		...
		rss=stmt.executeQuery(q3);
			...
			while(rss.next())
			{
			  currentHash.put(rss.getString(1),rss.getString(2));
			}
...
/*String cur=(String)currentHash.get(rs.getString(1));	
 if(cur==null){
cur="0";
}else{
...
<td class="<%=style %>" style="text-align:right;"><a href="#" onclick="viewWorks('rws_WQ_lab_coverage_drill_rpt.jsp?dcode=<%=rs.getString(1)%>&dname=<%=rs.getString(2)%>')"><%=upTo%></td>




<td class="<%=style %>" style="text-align:right;"><a href="#" onclick="viewWorks('rws_WQ_lab_coverage_drilltestd_rpt.jsp?dcode=<%=rs.getString(1)%>&dname=<%=rs.getString(2)%>')"><%=prev%></td>
<td class="<%=style %>" style="text-align:right;"><%=nothab %></td>
