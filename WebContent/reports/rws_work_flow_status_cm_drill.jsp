          ...
          <%
              ...
              try {
                  ...
                  if(mode.equals("admin_sactioned")){
                      query = "select distinct a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP,'dd/mm/yyyy'),'--') as TARGET_DATE_COMP from RWS_WORK_ADMN_TBL a"
                    		  +" where WORK_CANCEL_DT is null and a.work_id not in "
                  ...
                  else if(mode.equals("tech_sanctioned")){
                        query="select distinct a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP,'dd/mm/yyyy'),'--') as TARGET_DATE_COMP"
                        		+" from rws_new_est_tbl b,RWS_WORK_ADMN_TBL a where a.work_id not in" 
                        		+"(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and  DATE_OF_COMPLETION<'"+yearStart+"' )"
                        		+ " and WORK_CANCEL_DT is null and substr(a.work_id,7,2) in ("	+ schemes+ ") and a.work_id=b.work_id and PROGRAMME_CODE in ("+ programme+") "
                        		+" and  TARGET_DATE_COMP>='"+ yearStart+"' and  ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '') and a.CORE_DASHBOARD_STATUS='Y'";
                              }
                  else if(mode.equals("entrusted")){
                      query="select distinct a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP,'dd/mm/yyyy'),'--') as TARGET_DATE_COMP"
                    	   +" from RWS_CONTRACTOR_SELECTION_TBL b, RWS_WORK_ADMN_TBL a,rws_new_est_tbl e where a.work_id not in "
                           +" (select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and DATE_OF_COMPLETION<'"+yearStart+"' ) " 
                           +" and WORK_CANCEL_DT is null and substr(a.work_id,7,2) in ("	+ schemes+ ") and a.work_id=b.work_id and a.work_id=e.work_id and "
                           +" a.PROGRAMME_CODE in ("+programme+") and TARGET_DATE_COMP>='"+ yearStart+"' and ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '') and  a.CORE_DASHBOARD_STATUS='Y'";
                  }
                   else if(mode.equals("grounded")){
                       query="select distinct a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP,'dd/mm/yyyy'),'--') as TARGET_DATE_COMP"
                    		   +" from RWS_WORK_COMMENCEMENT_TBL b,RWS_WORK_ADMN_TBL a,rws_new_est_tbl e,RWS_CONTRACTOR_SELECTION_TBL c,RWS_SOURCE_IDEN_FIN_TBL s where a.work_id not in "
		                       +" (select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and DATE_OF_COMPLETION<'"+yearStart+"') " 
                    ...
                    else if(mode.equals("under_progress")){
                        query="select distinct a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP,'dd/mm/yyyy'),'--') as TARGET_DATE_COMP"
                        	+" from RWS_WORK_ADMN_TBL a,RWS_WORK_COMMENCEMENT_TBL b,rws_new_est_tbl e,RWS_CONTRACTOR_SELECTION_TBL c,RWS_SOURCE_IDEN_FIN_TBL s where a.work_id not in "
                          +" (select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and DATE_OF_COMPLETION < '"+yearEnd+"') " 
                          +" and WORK_CANCEL_DT is null and substr(a.work_id,7,2) in ("+ schemes+ ") and  a.work_id=b.work_id and a.work_id=e.work_id and a.work_id=c.work_id and "
                          +" a.CORE_DASHBOARD_STATUS='Y' and a.work_id=s.work_id and a.PROGRAMME_CODE in ("+ programme+") and TARGET_DATE_COMP>='"+ yearStart+"' and ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '')";
                    }
                    ...
                    /*   query="select distinct a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP,'dd/mm/yyyy'),'--') as TARGET_DATE_COMP"
                	  ...
                	  query="select distinct a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP,'dd/mm/yyyy'),'--') as TARGET_DATE_COMP"
                    		  +" from RWS_WORK_ADMN_TBL a,RWS_WORK_COMMENCEMENT_TBL b,rws_new_est_tbl e,RWS_CONTRACTOR_SELECTION_TBL c,RWS_SOURCE_IDEN_FIN_TBL s where a.work_id in "
	                          +"(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and DATE_OF_COMPLETION>'31-MAR-"+ys+ "') " 
