...
<table border=1 cellspacing=0 cellpadding=0 width="100%"
	...
	<table border=0 rules=none style="border-collapse:collapse"
		...
		<tr align="right">
			...
			<td class="bwborder"><a href="../reports/rws_work_expenditure_xls.jsp?district=<%=dCode%>&division=<%=divCode%>&subdivision=<%=subdivCode%>&work=<%=workccode%>&augmentation=<%=aCode%>&plan=<%=planCode%>&scheme=<%=schCode%>&programme=<%=pCode%>&subprogramme=<%=spCode%>&year=<%=yearCode%>&dName=<%=dName%>&divName=<%=divName%>&subdivName=<%=subdivName%>&workcName=<%=workcName%>&augName=<%=aName%>&PlanName=<%=planName%>&schName=<%=schName%>&progName=<%=pName%>&sprogName=<%=spName%>&yrName=<%=adyear%>"
