...
String qry=" select dname,mname,pname,vname,panch_code,panch_name from rws_complete_hab_view a,rws_admn_hab_lnk_tbl b,rws_work_admn_tbl c,rws_aap_selected_tbl d where a.panch_code=b.hab_code and c.work_id=b.work_id and d.work_id=b.work_id  and coverage_status='"+request.getParameter("status")+"' and work_cancel_dt is null  and c.work_id  in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < '01-Apr-"+yr+"')) ";
