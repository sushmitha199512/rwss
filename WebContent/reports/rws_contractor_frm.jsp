...
<script language="JavaScript">
	...
	function fnSubmit()
	{
		...
		{
			alert("Year(AGREEMENT_DATE) Mandatory...");
			return false;
		}		
				
		document.f1.action="./contract.jsp";
		document.f1.submit();	
	}
...
<table border = 1 align = center cellspacing = 0 cellpadding = 0 rules = "rows" bordercolor = navy style = "border-collapse:collapse" bgcolor="#ffffff" width=350>		
	<caption>
		...
		<td align="center" class="rptHeading" colspan=3>Contractor Management Report</td>	
	</tr>
<%				
try
{		
...
<%
	...
	query="SELECT distinct d.circle_office_code,d.circle_office_name from rws_circle_office_tbl d,RWS_WORK_ADMN_TBL w,RWS_CONTRACTOR_SELECTION_TBL con where w.work_id=con.work_id  and d.circle_office_code=substr(w.office_code,2,2) order by d.circle_office_name";
...
<%
	...
	stmt = conn.createStatement();
		...
		query="select distinct d.division_office_code,d.division_office_name from rws_division_office_tbl d,RWS_WORK_ADMN_TBL w,RWS_CONTRACTOR_SELECTION_TBL con where w.work_id=con.work_id and d.circle_office_code=substr(w.office_code,2,2) and d.division_office_code=substr(w.office_code,4,1) and substr(w.office_code,2,2)='"+rDist+"' ";
		rs = stmt.executeQuery(query);
		while(rs.next())
		{
...
<%
	...
	try
	{
		...
		query="select distinct sd.subdivision_office_code,sd.subdivision_office_name from rws_subdivision_office_tbl sd,RWS_WORK_ADMN_TBL w,RWS_CONTRACTOR_SELECTION_TBL con where w.work_id=con.work_id and sd.circle_office_code=substr (w.office_code,2,2) and sd.division_office_code=substr(w.office_code,4,1) and sd.subdivision_office_code=substr(w.office_code,5,2) and substr(w.office_code,2,2)='"+rDist+"' and substr(w.office_code,4,1)='"+rDiv+"' ";
		rs = stmt.executeQuery(query);
		while(rs.next())
		{
...
<%
	...
	stmt = conn.createStatement();
	queryplan = "select distinct w.PLAN_CODE,decode(w.PLAN_CODE,'1','Plan',2,'Non Plan','-') as planName from RWS_WORK_ADMN_TBL w,RWS_CONTRACTOR_SELECTION_TBL con where w.work_id=con.work_id  "; 
	if(rDist!=null && !rDist.equals(""))
	{	 
...
{	
stmt = conn.createStatement();
String querywork = "select distinct w.CATEGORY_CODE,c.CATEGORY_NAME from RWS_WORK_ADMN_TBL w,rws_work_category_tbl c,RWS_CONTRACTOR_SELECTION_TBL con where w.work_id=con.work_id and c.CATEGORY_CODE=w.CATEGORY_CODE "; 
if(rDist == null || rDist.equals(""))
{
%>
	<tr>
...
{	
stmt = conn.createStatement();
String queryscheme = "select distinct a.type_of_asset_code,a.type_of_asset_name from RWS_WORK_ADMN_TBL w,rws_asset_type_tbl a,RWS_CONTRACTOR_SELECTION_TBL con where w.work_id=con.work_id and substr(w.work_id,7,2)=a.type_of_asset_code"; 
if(rDist == null || rDist.equals(""))
{
%>
	<tr>
...
stmt = conn.createStatement();
 ...
 queryaug = "select distinct w.AUG_NEW_CODE,decode(w.AUG_NEW_CODE,'1','MainScheme','2','Augmentation','3','Maintenance','-') as Aug  from RWS_WORK_ADMN_TBL w,RWS_CONTRACTOR_SELECTION_TBL con where w.work_id=con.work_id "; 
...
{	
stmt = conn.createStatement();
String queryprog = "select distinct p.PROGRAMME_CODE ,p.PROGRAMME_NAME from RWS_WORK_ADMN_TBL w,rws_programme_tbl p,RWS_CONTRACTOR_SELECTION_TBL con where w.work_id=con.work_id and substr(w.work_id,1,2)=p.PROGRAMME_CODE  "; 
//select distinct a.type_of_asset_code,a.type_of_asset_name from RWS_WORK_ADMN_TBL w,rws_asset_type_tbl a where  substr(w.office_code,2,2)='"+rDist+"' and substr(w.office_code,4,1)='"+rDiv+"' and substr(w.office_code,5,2)='"+rSubDiv+"' and substr(w.work_id,7,2)=a.type_of_asset_code"; 
if(rDist == null || rDist.equals(""))
{
%>
...
{	
stmt = conn.createStatement();
//String queryyear=" select distinct to_char(admin_date,'yyyy') from RWS_WORK_ADMN_TBL w where to_char(admin_date,'yyyy') is not null ";
String queryyear="select distinct to_char(con.TENDER_APPROVED_ON,'yyyy') from RWS_WORK_ADMN_TBL w,RWS_CONTRACTOR_SELECTION_TBL con where con.work_id=w.work_id and to_char(con.TENDER_APPROVED_ON,'yyyy') is not null ";

if(rDist == null || rDist.equals(""))
{
%>
	<tr>
		<td class=rptLabel nowrap>Year(AGREEMENT_DATE)<font color="red">*</font></td>
		<td class=btext>
