...
<%	
	...
	{  
		...
		if(rDistrict != null && rDistrict.equals("00"))
		   {
			rs5 =stmt5.executeQuery("select distinct to_char(update_date,'YYYY') from rws_habitation_directory_tbl where update_date is not null order by 1 desc");
			}else{
		rs5 =stmt5.executeQuery("select distinct to_char(update_date,'YYYY') from rws_habitation_directory_tbl where substr(hab_code,1,2)='"+rDistrict+"' and update_date is not null order by 1 desc");
	}}
...
<%
	...
	if(request.getParameter("type")!=null && !Type.equals("-1") && Type.equals("b"))
	{
		...
		if(rDistrict != null && rDistrict.equals("00"))
		   {
			rs1=stmt1.executeQuery("select distinct to_char(hd.update_date,'dd/mm/yy') as a1 from rws_habitation_directory_tbl hd where hd.update_date is not null and  to_CHAR(hd.update_date,'YYYY')='"+Year+"' order by substr(a1,7,8) desc,substr(a1,4,2) desc,substr(a1,1,2) desc");
			}else{
    	rs1=stmt1.executeQuery("select distinct to_char(hd.update_date,'dd/mm/yy') as a1 from rws_habitation_directory_tbl hd where hd.update_date is not null and  to_CHAR(hd.update_date,'YYYY')='"+Year+"' and substr(hd.hab_code,1,2)='"+rDistrict+"' order by substr(a1,7,8) desc,substr(a1,4,2) desc,substr(a1,1,2) desc");
	    }}
...
<%                    
	...
	if(request.getParameter("type")!=null && !request.getParameter("type").equals("-1") && request.getParameter("type").equals("b"))
	{
       ...
       // System.out.println("fdate-1"+rDistrict);
		if(rDistrict != null && rDistrict.equals("00"))
		   {
            String qry="select distinct to_char(hd.update_date,'dd/mm/yy') as a1 from rws_habitation_directory_tbl hd where to_date(hd.update_date,'dd/mm/yy') >= to_date('"+fromDate+"','dd/mm/yy')  order by substr(a1,7,8) desc,substr(a1,4,2) desc,substr(a1,1,2) desc";
			//System.out.println("Todate query"+qry);
			rs1=stmt1.executeQuery(qry);
			
           } else{
		rs1=stmt1.executeQuery("select distinct to_char(hd.update_date,'dd/mm/yy') as a1 from rws_habitation_directory_tbl hd where to_date(hd.update_date,'dd/mm/yy') >= to_date('"+fromDate+"','dd/mm/yy')  and substr(hd.hab_code,1,2)='"+rDistrict+"' order by substr(a1,7,8) desc,substr(a1,4,2) desc,substr(a1,1,2) desc");
