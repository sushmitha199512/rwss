...
function getWorks()
	{
	  ...
	  alert("Please Select Circle Code");
		return false;
      //document.monitoringForm.action="switch.do?prefix=/works&page=/worksDelete.do&mode=data";
	  //document.monitoringForm.submit();		
	  }
	  else if(document.forms[0].cmbDivision.value==''){
	  alert("Please Select Division Code");
		return false;
     // document.monitoringForm.action="switch.do?prefix=/works&page=/worksDelete.do&mode=data&mode1=programmes";
	 // document.monitoringForm.submit();		
	  }
	 else if(document.forms[0].workCat.value==''){
	  alert("Please Select WorkCategory ");
		return false;
     // document.monitoringForm.action="switch.do?prefix=/works&page=/worksDelete.do&mode=data&mode1=workcategory";
	 // document.monitoringForm.submit();		
	  } else if(document.forms[0].plan[0].checked==false && document.forms[0].plan[1].checked==false){
	  alert("Please Check Plan / Non Plan ");
		return false;
     // document.monitoringForm.action="switch.do?prefix=/works&page=/worksDelete.do&mode=data&mode1=getWorks";
	 // document.monitoringForm.submit();		
	  } 
	  else {
		document.forms[0].action="switch.do?prefix=/works&page=/worksDelete.do&mode=data&mode1=getWorks";
...
<table bgcolor="#DEE3E0" bordercolor= "#8A9FCD" rules="none" width="100%" border=1 style="border-collapse:collapse;" >
	...
	<td>Programme</td>
	  ...
	  <%if(!subusercode.equals("00")){ %>
			<td  ><input type=submit name=GetData value=GetData onClick="return getWorks();" disabled></td> 
			<%} else if(subusercode.equals("00")){%>
			<td  ><input type=submit name=GetData value=GetData onClick="return getWorks();" ></td> 
			<%} %>
...
<%}
	else
	{
		...
		%>
			<tr><td colspan="9" align="center" ><input type=button value=Delete onClick="fnchkdelete();" ></td></tr>
		<%
		}
		else 
		{
	%>
			<tr><td colspan="9" align="center" ><input type=button value=Delete onClick="fnchkdelete();" disabled></td></tr>
		<%
		}
	}
%>
