...
<%
    ...
    while (rsCmDB.next()) {
    	if(rsCmDB.getString(1).equals("89")){//for JJM above 5 laks, below 5 laks condition
...
<BODY>
		...
		<table border=1 cellspacing=0 cellpadding=0 height=5% width="1050px"
            ...
            <%
				...
				try {
					...
					String amtCondition = "";
					String admin_Query = "",tech_Query="",Entrusted_Query = "",Grounded_Query = "",under_Query="",completed_Query="";
					for (int i = 1; i <= programmeList.size(); i++) 
					{
						...
						if (programme.length() > 2) {
							...
							if (resultSet1.next()) {
								if(programme.equals("89")){
									System.out.println("jjmcondition : "+jjmcondition);
									if(jjmcondition.equals("below")){
									programmeName = resultSet1.getString("PROGRAMME_NAME")+" (Below 5 Lakhs)";
									amtCondition = " and SANCTION_AMOUNT <=5 ";
									jjmcondition = "above";
									}else{ 
										programmeName = resultSet1.getString("PROGRAMME_NAME")+" (Above 5 Lakhs)";
										amtCondition = " and SANCTION_AMOUNT >=5 ";
									}
								}else	{							
						...
						admin_Query="select count(distinct a.work_id) from RWS_WORK_ADMN_TBL a,rws_programme_tbl p where WORK_CANCEL_DT is null and a.work_id not in "
			                      ...
			                      +"and a.programme_code=p.PROGRAMME_CODE and substr(a.work_id,7,2) in ("	+ schemes+ ") and a.PROGRAMME_CODE in ("+programme+") and TARGET_DATE_COMP>='01-APR-"+ yearStart+"' and ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '')  "+amtCondition;
			                      
						tech_Query="select count(distinct a.work_id) from rws_new_est_tbl b,RWS_WORK_ADMN_TBL a where a.work_id not in" 
		                           +"(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and  DATE_OF_COMPLETION<'01-APR-"+yearStart+"' ) and a.CORE_DASHBOARD_STATUS='Y'"
		                           + " and WORK_CANCEL_DT is null and substr(a.work_id,7,2) in ("+ schemes+ ") and a.work_id=b.work_id and PROGRAMME_CODE in ("+ programme+") and  TARGET_DATE_COMP>='01-APR-"+ yearStart+"' and  ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '') "+amtCondition;
		                           
						Entrusted_Query="select count(distinct a.work_id) from RWS_CONTRACTOR_SELECTION_TBL b, RWS_WORK_ADMN_TBL a,rws_new_est_tbl e where a.work_id not in "
                               +"(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and DATE_OF_COMPLETION<'01-APR-"+yearStart+"')  and a.CORE_DASHBOARD_STATUS='Y'" 
                               +" and WORK_CANCEL_DT is null and substr(a.work_id,7,2) in ("+ schemes+ ") and a.work_id=b.work_id and a.work_id=e.work_id and "
                               +" a.PROGRAMME_CODE in ("+programme+") and TARGET_DATE_COMP>='01-APR-"+ yearStart+"' and ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '') "+amtCondition;
                               
						Grounded_Query="select count (distinct a.work_id) from RWS_WORK_COMMENCEMENT_TBL b,RWS_WORK_ADMN_TBL a,rws_new_est_tbl e,RWS_CONTRACTOR_SELECTION_TBL c,RWS_SOURCE_IDEN_FIN_TBL s where a.work_id not in "
						 ...
						 under_Query="select count (distinct a.work_id) from RWS_WORK_ADMN_TBL a,RWS_WORK_COMMENCEMENT_TBL b,rws_new_est_tbl e,RWS_CONTRACTOR_SELECTION_TBL c,RWS_SOURCE_IDEN_FIN_TBL s where a.work_id not in "
						 ...
						 completed_Query="select count (distinct a.work_id) from RWS_WORK_ADMN_TBL a,RWS_WORK_COMMENCEMENT_TBL b,rws_new_est_tbl e,RWS_CONTRACTOR_SELECTION_TBL c,RWS_SOURCE_IDEN_FIN_TBL s where a.work_id in "
	                                +"(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and DATE_OF_COMPLETION>='01-APR-"+yearStart+ "') and b.GROUNDING_DATE is not null " 
						...
						stmt4= conn.createStatement();                                                                                    // System.out.println("Grounded_Query"+Grounded_Query);
		...
		%>		
			<tr>
				...
				<td class="<%=style%>" style="text-align: left;" align="left"><a href="./rws_work_flow_dist_mandal_cm_rpt.jsp?mode=districts&programme=<%=programme%>&finyear=<%=finyear%>&programmeName=<%=programmeName%>"><%=programmeName%></a></td>
