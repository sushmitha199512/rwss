...
//.out.println("circleofficecode************"+circleofficecode);
...
//before querry
//query.append("select distinct dname,mname,pname,vname,panch_name,panch_code from rws_wq_test_results_tbl b,rws_complete_hab_view c where  substr(b.source_code,1,16)=c.panch_code  and C.COVERAGE_STATUS<>'UI' and  to_date(prepared_on)>='1-APR-"+yr.toString()+"' and  to_date(prepared_on)<'31-MAR-"+yr3.toString()+"' and dcode='"+dcode.toString()+"'  order by dname");

q1.delete(0,q1.length());
if(typeofsource!=null && typeofsource.equals("01"))
{
q1.append("select dname,mname,pname,vname,panch_name,panch_code ,c.division_office_code,c.source_code   from rws_ftk_testing_tbl c,rws_complete_hab_view hd  where hd.panch_code=substr(c.source_code,1,16) and H2S_VIAL ='Y' and substr(c.source_code,20,2)='SO' and dcode='"+circleofficecode+"' and c.division_office_code='"+divisionofficecode+"'");
}
else
{
	q1.append("select dname,mname,pname,vname,panch_name,panch_code ,c.division_office_code,c.source_code   from rws_ftk_testing_tbl c,rws_complete_hab_view hd  where hd.panch_code=substr(c.source_code,1,16) and H2S_VIAL ='Y' and dcode='"+circleofficecode+"' and c.division_office_code='"+divisionofficecode+"'");
	}
			...
			q1.append(" group by dname,mname,pname,vname,panch_name,panch_code ,c.division_office_code,c.source_code   order by 2 ");
...
if(!rs3.next()){
%>
<tr><td class="gridbg1" style="text-align: left; text-align:center;" align="center" colspan="10">No Records Available</td></tr>
<%}
else{
do{
