...
<%
	...
	%>      <TR>
		<td class=rptValue align="left"><%=++slno%></td>
        <td class=rptValue align="left"><a href="./rws_wrk_progress_resource_out_home_rpt.jsp?work_id=<%=rs1.getString(1)%>&dcode=<%=dist%>&scheme=<%=rs1.getString(7)%>"><%=rs1.getString(2)%><%="-"+rs1.getString(1)%></a>
		</td>
		<td class=rptValue align="left"><%=rs1.getString(3)%></td>
		<td class=rptValue align="left"><%=rs1.getString(4)%></TD>
