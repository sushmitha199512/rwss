...
<tr bgcolor="#8A9FCD" align="center">
		<td align="center" class="rptHeading" colspan="14">HEADWORKS-RAWWATERCOLLECTIONWELL-REPORT </td>
</tr>
</table>
<table border=1 cellpadding=1 cellspacing=1 width="100%">
<tr align=center>
               <td class=btext>Sl.No</td>
			   <td class=btext>Raw water Collection Well Code</td>
...
try
{
	    ...
	    if(typeassetcode.equals("03"))
	{
		...
		rs1=stmt1.executeQuery("select  rw_collwell_code,diameter,depth,nvl(type_of_flooring,'-'),nvl(location,'-'),capacity,asset_code from rws_cpws_rw_collwell_param_tbl  where asset_code='"+assetcode+"'  ");
		while(rs1.next())
    	{
	     				%>
...
<%
			...
			
	{
		...
		stmt1=conn.createStatement();
		rs1=stmt1.executeQuery("select  rw_collwell_code,diameter,depth,nvl(type_of_flooring,'-'),nvl(location,'-'),capacity,asset_code from rws_pws_rw_collwell_param_tbl  where asset_code='"+assetcode+"' ");
		while(rs1.next())
    	{
...
<%
			...
			
	{
		...
		stmt1=conn.createStatement();
		rs1=stmt1.executeQuery("select  rw_collwell_code,diameter,depth,nvl(type_of_flooring,'-'),nvl(location,'-'),capacity,asset_code from rws_mpws_rw_collwell_param_tbl  where asset_code='"+assetcode+"' ");
		while(rs1.next())
    	{
