...
function fnSubmit()
{
	...
	document.forms[0].action="rws_bankwise_state_rpt.jsp";
...
<table><tr><td class=btext>Bill Wise Releases - State </td></tr></table>
<table border = 1 align = center cellspacing = 0 cellpadding = 0 rules = "rows" bordercolor = navy style = "border-collapse:collapse" bgcolor="#ffffff" width=292>
 <tr><td class=rptLabel>Date of Payment:&nbsp;&nbsp;<font color="red">*</font></td>
		...
		<%
	     ...
	     String  query="select  distinct to_char(SEND_DATE,'dd/mm/yyyy') from RWS_BILL_PAO_SETTLEMENT_TBL where send_date is not null and TXID!='2011000001'";
		if(loggedUser!=null && (loggedUser.equals("admin")||loggedUser.equals("100000") ||loggedUser.equals("guest"))){}else
		{
	...
	if(data!=null && !data.equals(""))
	 ...
	 String qry1="select  distinct division_office_code,division_office_name,circle_office_code from rws_division_office_tbl  a, RWS_BILL_PAO_SETTLEMENT_TBL  b where substr(office_code,2,2)=a.circle_office_code and substr(office_code,4,1)=a.division_office_code  and to_char(SEND_DATE,'dd/MON/yyyy')= '"+da+"/"+mn1+"/"+ya+"'  order by circle_office_code  "; 
