...
while(r6.next())
{
   ...
   if(count!=0){
	...
	ch.addTitle("Representative Proposal ", "times new roman Bold Italic",12);
...
if(x.equals("p"))
 {
    ...
    ch.addTitle2(Chart.Top, "Representative Proposal ", "Times New Roman Bold Italic",13);
	ch.set3D();
...
<%}  int j=0,s=0;%>
   ...
   <table border = 1  align="center" cellspacing = 0 cellpadding = 0 width="45%"   style="border-collapse:collapse">
   <%	
	   for(int l=0;l<colors1.length;l++)
	   {
	    %>
		<tr>
			<td bgcolor="<%=colors1[l]%>"><%="_"%></td>
			<td><%=++s%></td>
			<td><%=districts[l]%></td>
		</tr>
		<%}%>
