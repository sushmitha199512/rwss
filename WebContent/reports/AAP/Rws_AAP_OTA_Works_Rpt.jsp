...
//////System.out.println("Changing");
   ...
   int prevyear=0,prevmon=0;String fyear,fyear1;
	...
	String pNames="";
	String progNameQuery="select programme_name,NRDWP_RELATED from rws_programme_tbl where programme_code in("+programe+") order by NRDWP_RELATED desc";
	System.out.println(progNameQuery);
	Statement statement=conn.createStatement();
	ResultSet resultSet=statement.executeQuery(progNameQuery);
	while(resultSet.next()){
		if(resultSet.getString("NRDWP_RELATED").equals("Y")){
		pNames="NRDWP-Related";
		}
		else{
			pNames+=", "+resultSet.getString("programme_name");
		}
	}
	resultSet.close();
	statement.close();
...
<table border=1 cellspacing=0 cellpadding=0 width="60%"
	...
	align="center"><tr >
			<td align="center" class="gridhdbg" colspan=27>AAP Abstract during <%=financialstartyr %> and Programme: <%=pNames %></td>	
	...
	<!-- <td  class="gridhdbg">Expenditure(Ongoing)</td> -->
	...
	try{
		...
		double totOngoingBalEstCost=0,totTargetBalEstCost=0,totCompExp=0,totCompBalEstCost=0,tottobecompBalEstCost=0,totfinalOngoingBalEstCost=0,totfinalOngoingExp=0,totexpong=0;
		
		
		String distQuery="";
		if(dcode.equals("00")){
			distQuery="select dcode,dname from rws_district_tbl order by dcode";
		}
		else{
			distQuery="select dcode,dname from rws_district_tbl where dcode='"+dcode+"' order by dcode";
		}
		stmt2=conn.createStatement();
		rs2=stmt2.executeQuery(distQuery);
		while(rs2.next()){
		...
		rs.close();
			...
			<td class="<%=style%>" style="text-align: left;" align="left"><a href="./Rws_AAP_OTA_Works_Programme_Rpt.jsp?mode=programme&district=<%=dcode%>&program=<%=programe%>&dname=<%=rs2.getString(2)%>&finyear=<%=financialstartyr%>&scheme=<%=scheme%>"><%=rs2.getString(2)%></a></td>
			<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./Rws_AAP_OTA_Works_drill.jsp?mode=ongoingWorks&dcode=<%=dcode%>&programmeCode=<%=programe%>&dname=<%=rs2.getString(2)%>&finyear=<%=financialstartyr%>&scheme=<%=scheme%>')"><%=ongoingWorks%></a></td>
			<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./Rws_AAP_OTA_Works_drill.jsp?mode=ongoingHabs&dcode=<%=dcode%>&programmeCode=<%=programe%>&dname=<%=rs2.getString(2)%>&finyear=<%=financialstartyr%>&scheme=<%=scheme%>')"><%=ongoingHabs%></a></td>
		...
		rs.close();
			...
			%>
			<%if(targetWorks!=0){ %>
			<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./Rws_AAP_OTA_Works_drill.jsp?mode=targetWorks&dcode=<%=dcode%>&programmeCode=<%=programe%>&dname=<%=rs2.getString(2)%>&finyear=<%=financialstartyr%>&scheme=<%=scheme%>')"><%=targetWorks%></a></td>
			<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./Rws_AAP_OTA_Works_drill.jsp?mode=targetHabs&dcode=<%=dcode%>&programmeCode=<%=programe%>&dname=<%=rs2.getString(2)%>&finyear=<%=financialstartyr%>&scheme=<%=scheme%>')"><%=targetHabs%></a></td>
			<%} else{%>
		...
		%>
		<%if(worksComp!=0){ %>
		<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./Rws_AAP_OTA_Works_drill.jsp?mode=compWorks&dcode=<%=dcode%>&programmeCode=<%=programe%>&dname=<%=rs2.getString(2)%>&finyear=<%=financialstartyr%>&scheme=<%=scheme%>')"><%=worksComp%></a></td>
		<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./Rws_AAP_OTA_Works_drill.jsp?mode=compHabs&dcode=<%=dcode%>&programmeCode=<%=programe%>&dname=<%=rs2.getString(2)%>&finyear=<%=financialstartyr%>&scheme=<%=scheme%>')"><%=habsCov%></a></td>
		<%} else{%>
	...
	rs.close();
		...
		%>
		<%-- <%if(tobecompWorks!=0){ %>
		<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./Rws_AAP_OTA_Works_drill.jsp?mode=tobecompWorks&dcode=<%=dcode%>&programmeCode=<%=programe%>&dname=<%=rs2.getString(2)%>&finyear=<%=financialstartyr%>&scheme=<%=scheme%>')"><%=tobecompWorks%></a></td>
		<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./Rws_AAP_OTA_Works_drill.jsp?mode=tobecompHabs&dcode=<%=dcode%>&programmeCode=<%=programe%>&dname=<%=rs2.getString(2)%>&finyear=<%=financialstartyr%>&scheme=<%=scheme%>')"><%=tobecompHabs%></a></td>
		<%} else{%>
		...
		stmt1.close();
		rs1.close();
		double finalongoingbankPao=(finalongoingbankLastYear + finalongoingpaoLastYear) / 100000;
		...
		<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./Rws_AAP_OTA_Works_drill.jsp?mode=totOngoingWorks&dcode=<%=dcode%>&programmeCode=<%=programe%>&dname=<%=rs2.getString(2)%>&finyear=<%=financialstartyr%>&scheme=<%=scheme%>')"><%=finalongoingWorks%></a></td>
		<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./Rws_AAP_OTA_Works_drill.jsp?mode=totOngoingHabs&dcode=<%=dcode%>&programmeCode=<%=programe%>&dname=<%=rs2.getString(2)%>&finyear=<%=financialstartyr%>&scheme=<%=scheme%>')"><%=finalongoingHabs%></a></td>
		<%} else{%>
		...
		totCompExp=totCompExp+compbankPao;
		...
		<tr>
			...
			<td class="gridhdbg" style="text-align: right;"><%=ndf.format(totexpong)%></td>
