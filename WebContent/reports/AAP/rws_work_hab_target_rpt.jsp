...
session.setAttribute("schemes",schemes);
  ...
  //  System.out.println("endyr"+endyr);
    int startyr1=Integer.parseInt(startyr);
    int endddyr1=Integer.parseInt(endyr);
    
    
    int nextyr=startyr1+1;
    int nextendyear=endddyr1+1;
  //  System.out.println("nextyr"+nextyr);
    //System.out.println("nextyr end"+nextendyear);
...
String divCode=request.getParameter("divCode");
  ...
  String currentyr=(Integer.parseInt(sysyear))+"-"+(Integer.parseInt(sysyear)+1);
	
	String targetyr1=(Integer.parseInt(sysyear)+1)+"-"+(Integer.parseInt(sysyear)+2);
	
	
	String targetnextyr=(Integer.parseInt(sysyear)+2)+"-"+(Integer.parseInt(sysyear)+3);

	
	String nexttonectnextyr=(Integer.parseInt(sysyear)+3)+"-"+(Integer.parseInt(sysyear)+3);
...
if(financialstartyr.equals(targetyr1)){
...
{ %>
			<tr><td align="right" class="bwborder"><a href="/pred/home.jsp?loginMode=watersoft">Home|</a></td>
				<td class="bwborder"><a href="./rws_work_hab_target_frm.jsp">Back|</a><a href="./rws_work_hab_target_excel.jsp">Excel</a></td></tr>
<%  } else {%>
...
<table border=1 cellspacing=0 cellpadding=0 width="80%"
	...
	align="center"><tr >
			<td align="center" class="gridhdbg" colspan=33>AAP <%=financialstartyr %>- List of SVS/MVS Works & Habitations targeted for completion during the year <%=financialstartyr %> under various programs</td>	
...
String sno="",snamnt="",remarks="",tempdist="";
  try{
	  ...
	  System.out.println("grounddtqry 2 anupama");
	  String sourceqry="select d.WORK_ID,a.source_code,b.source_name,(case when location is null then '-' else location end) as location,c.SOURCE_TYPE_NAME from RWS_WORK_ADMN_TBL d,RWS_SOURCE_IDEN_FIN_TBL a,RWS_SOURCE_TBL b,RWS_SOURCE_TYPE_TBL c where work_cancel_dt is null  and d.work_id not in (select work_id from rws_work_completion_tbl where (to_date(DATE_OF_COMPLETION) < '01-Apr-"+startyr+"')) and d.work_id=a.work_id and a.source_code=b.source_code and b.SOURCE_TYPE_CODE=c.SOURCE_TYPE_CODE ";
		...
		//System.out.println("sourceqry 1"+sourceqry);
		stmt4=conn.createStatement();
		rs4=stmt4.executeQuery(sourceqry);
		//System.out.println("sourceqry 1 anupama"+sourceqry);
		...
		//System.out.println("grounddtqry 2"+grounddtqry);
		...
		// System.out.println("estcost 3 "+estcost);
		...
		while(rs5.next()){
			estcost_det.put(rs5.getString(1),rs5.getString(2));
		}
		...
		//System.out.println("estcost 4 "+direstcost);
		...
		while(rs5.next()){
			direstcost_det.put(rs5.getString(1),rs5.getString(2));
		}
		...
		//System.out.println("MPsanctionamnt 5"+MPsanctionamnt);
		stmt4=conn.createStatement();
	  ...
	  String probexp="",probexp1="",probexp2="",probexp3="";
		...
		//System.out.println("habqry 6"+habqry);
		int styleCount=0;
		String style="";
