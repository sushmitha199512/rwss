	...
	<caption>
		<table  border=0 rules=none style="border-collapse:collapse" align = "right">			
			<tr align="right">
				...
				<td class="bwborder"><a href="./rws_const_coverage_status_mandal_xls.jsp?distvalue=<%=distvalue%>&distname=<%=distname%>&convalue=<%=convalue%>&conname=<%=conname%>&district=<%=rDistrict%>&constituency=<%=rConstituency%>&mandal=<%=rMandal%>&panchayat=<%=rPanchayat%>&village=<%=rVillage%>" target="_new">Excel</a></td>
...
<%
		...
		{			
%>
		<td class=rptValue><a href="./rws_const_coverage_status_panchayat_rpt.jsp?distvalue=<%=distvalue%>&distname=<%=distname%>&convalue=<%=convalue%>&conname=<%=conname%>&manvalue=<%=rs2.getString(1)%>&manname=<%=rs2.getString(2)%>&rDistrict=<%=rDistrict%>&rMandal=<%=rMandal%>&rPanchayat=<%=rPanchayat%>&rVillage=<%=rVillage%>&rConstituency=<%=rConstituency%>"><%=rs2.getString(2)%></a></td>
<%
		}
		if(rDistrict != null && !rDistrict.equals("00") && rConstituency.equals("-1") && rMandal.equals("-1") && rPanchayat.equals("-1") && rVillage.equals("-1")) 
		{
...
%>
		<td class=rptValue><a href="./rws_const_coverage_status_panchayat_rpt.jsp?rDistrict=<%=rDistrict%>&rMandal=<%=rMandal%>&rPanchayat=<%=rPanchayat%>&rVillage=<%=rVillage%>&rConstituency=<%=rConstituency%>&distvalue=<%=distvalue%>&distname=<%=distname%>&conname=<%=conname%>&convalue=<%=convalue%>&manvalue=<%=rs2.getString(1)%>&manname=<%=rs2.getString(2)%>"><%=rs2.getString(2)%></a></td>
<%
		}
		...
		{			
%>
		<td class=rptValue><a href="./rws_const_coverage_status_panchayat_rpt.jsp?rDistrict=<%=rDistrict%>&rMandal=<%=rMandal%>&rPanchayat=<%=rPanchayat%>&rVillage=<%=rVillage%>&rConstituency=<%=rConstituency%>&distvalue=<%=distvalue%>&distname=<%=distname%>&manvalue=<%=rs2.getString(1)%>&manname=<%=rs2.getString(2)%>&conname=<%=conname%>&convalue=<%=convalue%>"><%=rs2.getString(2)%></a></td>
<%
		}
