...
int count=0;
String qry = "";
//qry="select  distinct  work_id,bill_sl_no,to_char(bank_send_date,'dd-mm-yyyy'), nvl(BILL_AMOUNT_CENTRAL,0) from rws_bill_bank_bridge_det_tbl a where a.bill_sl_no<>'0' and a.office_code is not null and substr(office_code,2,3)='"+dcode+""+divcode+"'";  

//qry="select  distinct  work_id,bill_sl_no,to_char(bank_send_date,'dd-mm-yyyy'), nvl(BILL_AMOUNT_CENTRAL,0) from rws_bill_bank_bridge_det_tbl a where a.bill_sl_no<>'0' and a.office_code is not null and substr(office_code,2,3)='"+dcode+""+divcode+"' and (to_char(bank_send_date,'yy')='"+fyear+"' and substr(to_char(bank_send_date,'dd-mm-yy'),4,2)>='03' or to_char(bank_send_date,'yy')='"+fyear1+"' and substr(to_char(bank_send_date,'dd-mm-yy'),4,2)<='04')";
 qry="select  distinct  a.work_id,b.work_name,p.programme_name,sp.subprogramme_name,a.bill_sl_no,to_char(bank_send_date,'dd-mm-yyyy'), nvl(BILL_AMOUNT_CENTRAL,0) from rws_bill_bank_bridge_det_tbl a ,rws_work_admn_tbl b ,rws_programme_tbl p, rws_subprogramme_tbl sp where b.programme_code=p.programme_code and (b.programme_code=sp.programme_code and b.subprogramme_code=sp.subprogramme_code) and a.work_id=b.work_id  and a.office_code is not null and substr(a.office_code,2,3)='"+dcode+""+divcode+"' and (to_char(bank_send_date,'yy')='"+fyear+"' and substr(to_char(bank_send_date,'dd-mm-yy'),4,2)>='03' or to_char(bank_send_date,'yy')='"+fyear1+"' and substr(to_char(bank_send_date,'dd-mm-yy'),4,2)<='04')";
