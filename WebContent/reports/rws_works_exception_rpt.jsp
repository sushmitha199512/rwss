...
<font color="blue" face=verdana size=2><b>District:</b></font><font color="black" face=verdana size=2><%=dname%>&nbsp;&nbsp;for <%=pname.substring(0,pname.length()-1)%>  Works</font>&nbsp; </p>
...
{ %>
			<tr><td align="right" class="bwborder"><a href="/pred/home.jsp?loginMode=watersoft">Home</a>&nbsp;|&nbsp;</td>
				...
				<td class="bwborder"><a href="rws_works_exception_rpt_xls.jsp" target="_new">Excel</a></td></tr>
<%  } else {%>
...
try
	{
        ...
        String mycolor="";
            ...
            qry15="    SELECT distinct substr(a.work_id,1,2),count(a. work_id) FROM RWS_WORK_ADMN_TBL a,rws_work_commencement_tbl c where a.work_id=c.work_id and work_cancel_dt is null  and a.work_id not in (select work_id from rws_work_completion_tbl) and sysdate - grounding_date>5*365 and substr(a.work_id,1,2) in ("+programe+") and substr(a.work_id,5,2)='"+rDistrict+"'  group by substr(a.work_id,1,2)";
           ...
           qry14="   SELECT distinct substr(a.work_id,1,2),count(a. work_id) FROM RWS_WORK_ADMN_TBL a,rws_work_commencement_tbl c where a.work_id=c.work_id and work_cancel_dt is null  and a.work_id not in (select work_id from rws_work_completion_tbl) and sysdate - grounding_date<5*365 and sysdate - grounding_date>4*365 and substr(a.work_id,1,2) in ("+programe+") and substr(a.work_id,5,2)='"+rDistrict+"'  group by substr(a.work_id,1,2)";
	        ...
	        qry13="  SELECT distinct substr(a.work_id,1,2),count(a. work_id) FROM RWS_WORK_ADMN_TBL a,rws_work_commencement_tbl c where a.work_id=c.work_id and work_cancel_dt is null  and a.work_id not in (select work_id from rws_work_completion_tbl) and sysdate - grounding_date<4*365 and sysdate - grounding_date>3*365 and substr(a.work_id,1,2) in ("+programe+") and substr(a.work_id,5,2)='"+rDistrict+"'  group by substr(a.work_id,1,2)";
	       ...
	       qry12="  SELECT distinct substr(a.work_id,1,2),count(a. work_id) FROM RWS_WORK_ADMN_TBL a,rws_work_commencement_tbl c where a.work_id=c.work_id and work_cancel_dt is null  and a.work_id not in (select work_id from rws_work_completion_tbl) and sysdate - grounding_date<3*365 and sysdate - grounding_date>2*365 and substr(a.work_id,1,2) in ("+programe+") and substr(a.work_id,5,2)='"+rDistrict+"' group by substr(a.work_id,1,2)";
	       ...
	       qry11="  SELECT distinct substr(a.work_id,1,2),count(a. work_id) FROM RWS_WORK_ADMN_TBL a,rws_work_commencement_tbl c where a.work_id=c.work_id and work_cancel_dt is null  and a.work_id not in (select work_id from rws_work_completion_tbl) and sysdate - grounding_date<2*365 and sysdate - grounding_date>365 and substr(a.work_id,1,2) in ("+programe+") and substr(a.work_id,5,2)='"+rDistrict+"'  group by substr(a.work_id,1,2)";
	      ...
	      // System.out.println("qry11 "+qry11);
	       ...
	       qry10=" SELECT distinct substr(a.work_id,1,2),count(a. work_id) FROM RWS_WORK_ADMN_TBL a,rws_work_commencement_tbl c where a.work_id=c.work_id and work_cancel_dt is null  and a.work_id not in (select work_id from rws_work_completion_tbl) and sysdate - grounding_date>365 and substr(a.work_id,1,2) in ("+programe+") and substr(a.work_id,5,2)='"+rDistrict+"'  group by substr(a.work_id,1,2)";
	     ...
	     //  System.out.println("qry10 "+qry10);
	        ...
	        qry9=" SELECT distinct substr(a.work_id,1,2),count(a. work_id) FROM RWS_WORK_ADMN_TBL a,rws_work_commencement_tbl c where a.work_id=c.work_id and work_cancel_dt is null  and a.work_id not in (select work_id from rws_work_completion_tbl) and sysdate - grounding_date<365 and sysdate - grounding_date>180 and substr(a.work_id,1,2) in ("+programe+") and substr(a.work_id,5,2)='"+rDistrict+"' group by substr(a.work_id,1,2)";
	     ...
	     //  System.out.println("qry9 "+qry9);
	        ...
	        qry8=" SELECT distinct substr(a.work_id,1,2),count(a. work_id) FROM RWS_WORK_ADMN_TBL a,rws_work_commencement_tbl c where a.work_id=c.work_id  and work_cancel_dt is null  and a.work_id not in (select work_id from rws_work_completion_tbl) and sysdate - grounding_date<180 and sysdate - grounding_date>90 and substr(a.work_id,1,2) in ("+programe+") and substr(a.work_id,5,2)='"+rDistrict+"'  group by substr(a.work_id,1,2)";
	      ...
	      System.out.println("qry8 "+qry8);
	       ...
	       qry7=" SELECT distinct substr(a.work_id,1,2),count(a.work_id) FROM RWS_WORK_ADMN_TBL a,rws_new_est_tbl b where a.work_id=b.work_id and work_cancel_dt is null and  a.work_id not in(select work_id from rws_work_completion_tbl)  and a.work_id not in (select work_id from rws_work_commencement_tbl ) and sysdate - ts_date>365 and  substr(a.work_id,1,2) in ("+programe+") and substr(a.work_id,5,2)='"+rDistrict+"' group by substr(a.work_id,1,2)";
	       ...
	       // System.out.println("qry7 "+qry7);
	        ...
	        qry6 ="SELECT distinct substr(a.work_id,1,2),count(a.work_id) FROM RWS_WORK_ADMN_TBL a,rws_new_est_tbl b where a.work_id=b.work_id and work_cancel_dt is null and  a.work_id not in(select work_id from rws_work_completion_tbl)  and a.work_id not in (select work_id from rws_work_commencement_tbl ) and sysdate - ts_date<365 and sysdate - ts_date>180 and  substr(a.work_id,1,2) in ("+programe+") and substr(a.work_id,5,2)='"+rDistrict+"' group by substr(a.work_id,1,2)";
        ...
        qry5=" SELECT distinct substr(a.work_id,1,2),count(a.work_id) FROM RWS_WORK_ADMN_TBL a,rws_new_est_tbl b where a.work_id=b.work_id and work_cancel_dt is null and  a.work_id not in(select work_id from rws_work_completion_tbl)  and a.work_id not in (select work_id from rws_work_commencement_tbl ) and sysdate - ts_date<180 and sysdate - ts_date>90 and  substr(a.work_id,1,2) in ("+programe+") and substr(a.work_id,5,2)='"+rDistrict+"' group by substr(a.work_id,1,2)";
        ...
        qry4=" SELECT distinct substr(a.work_id,1,2),count(a.work_id) FROM RWS_WORK_ADMN_TBL a  where work_cancel_dt is null and  a.work_id not in(select work_id from rws_work_completion_tbl)  and  a.work_id not in(select work_id from rws_new_est_tbl) and sysdate - admin_date>365 and  substr(a.work_id,1,2) in ("+programe+") and substr(a.work_id,5,2)='"+rDistrict+"' group by substr(a.work_id,1,2)";
      ...
      // System.out.println("qry4 "+qry4);
        ...
        qry3=" SELECT distinct substr(a.work_id,1,2),count(a.work_id) FROM RWS_WORK_ADMN_TBL a  where  work_cancel_dt is null and  a.work_id not in(select work_id from rws_work_completion_tbl)  and  a.work_id not in(select work_id from rws_new_est_tbl) and sysdate - admin_date<365 and sysdate - admin_date>180  and  substr(a.work_id,1,2) in ("+programe+") and substr(a.work_id,5,2)='"+rDistrict+"' group by substr(a.work_id,1,2)";
     ...
     //  System.out.println("qry3 "+qry3);
        ...
        qry2=" SELECT substr(a.work_id,1,2),count(distinct a.work_id) FROM RWS_WORK_ADMN_TBL a where work_cancel_dt is null and  a.work_id not in(select work_id from rws_work_completion_tbl) and substr(a.work_id,1,2) in ("+programe+") and substr(a.work_id,5,2)='"+rDistrict+"' group by substr(a.work_id,1,2)";
     ...
     // System.out.println("qry2 "+qry2);
	   ...
	   qry1=" SELECT distinct substr(a.work_id,1,2),count(a.work_id) FROM RWS_WORK_ADMN_TBL a  where work_cancel_dt is null and  a.work_id not in(select work_id from rws_work_completion_tbl)  and  a.work_id not in(select work_id from rws_new_est_tbl) and sysdate - admin_date<180 and sysdate - admin_date>90  and  substr(a.work_id,1,2) in ("+programe+") and substr(a.work_id,5,2)='"+rDistrict+"' group by substr(a.work_id,1,2)";
        ...
        qry="select distinct programme_code,programme_name from rws_programme_tbl where programme_code in("+programe+") order by programme_name";
