...
session.setAttribute("year1",y1.toString());
...
try	
  {query.append("select substr(habcode,1,2),count(*),sum(nvl(TRANSPORT_EST,'0')) from rws_drought_habitation_tbl where TRANSPORTATION ='Y' and year='"+y1+"' group by substr(habcode,1,2)");
	...
	if(stmt2!=null)stmt2.close();
     ...
     query.append("select substr(habcode,1,2),count(*),sum(nvl(HIRING_OF_SOURCES,'0')),sum(nvl(HIRING_EST,'0')) from rws_drought_habitation_tbl where HIRING_OF_SOURCES>0 and year='"+y1+"' group by substr(habcode,1,2)");
	 Statement stmt3=conn.createStatement();
//System.out.println("hireqry**********"+query);	
...
session.setAttribute("hireList",hireList);
	...
	query.delete(0,query.length());
	query.append("select substr(habcode,1,2),sum(nvl(FLUSHING,0)),sum(nvl(FLUSHING_EST,'0')) from rws_drought_habitation_tbl where FLUSHING>0 and year='"+y1+"' group by substr(habcode,1,2)");
	Statement stmt4=conn.createStatement();
  //  System.out.println("flushqry**********"+query);	
	...
	session.setAttribute("flushList",flushList);
	...
	query.delete(0,query.length());
	query.append("select substr(habcode,1,2),sum(nvl(DEEPENING,0)) ,sum(nvl(DEEPENING_EST,'0')) from rws_drought_habitation_tbl where DEEPENING>0 and year='"+y1.toString()+"' group by substr(habcode,1,2)");
		Statement stmt5=conn.createStatement();
	//    System.out.println("deepqry**********"+query);	
	...
	query.delete(0,query.length());
	query.append("select substr(habcode,1,2),sum(nvl(DEEP_OW_NUM,0)),sum(nvl(DEEP_OW_EST,'0')) from rws_drought_habitation_tbl where DEEP_OW_NUM>0 and year='"+y1+"' group by substr(habcode,1,2)");
	Statement stmt6=conn.createStatement();
   // System.out.println("owdeepqry**********"+query);	
	...
	session.setAttribute("owdeepList",owdeepList);
	...
	query.delete(0,query.length());
	query.append("select substr(habcode,1,2),sum(nvl(SST_NUM,0)),sum(nvl(SST_EST,'0')) from rws_drought_habitation_tbl where SST_NUM>0 and year='"+y1.toString()+"' group by substr(habcode,1,2)");
		Statement stmt7=conn.createStatement();
	//    System.out.println("sstqry**********"+query);	
	...
	session.setAttribute("sstList",sstList);
	...
	query.append("select dcode,count(*) from rws_drought_mandal_tbl  where year='"+y1.toString()+"' and drought_mandal='Y' group by dcode");
	 ...
	 //   System.out.println("mancntqry**********"+query);	
	   	ResultSet rs8=stmt8.executeQuery(query.toString());
	 //   System.out.println("mancntqry---exe**********"+query);
	...
	session.setAttribute("mancntList",mancntList);


///////////////////////////////////////////////////////////////////
	if(dcode.toString().equals("00"))
  {
	//	System.out.println("if-00-dcode:***"+dcode.toString());
		query.delete(0,query.length());
		query.append("select   substr(habcode,1,2),d.dname,count(*) from rws_district_tbl d,rws_drought_habitation_tbl h where substr(h.habcode,1,2)=d.dcode and year='"+y1.toString()+"' group by substr(habcode,1,2),d.dname order by 2");
	//	System.out.println("if-00***"+query);
  }
  else
	{
	  query.delete(0,query.length());
	...
	//	System.out.println("else-00***"+query);
	}
...
//String List4=""+deepList.get(rs1.getString(1));
 ...
 //System.out.println("List7 &&&&&&&&"+List4);
...
List5.delete(0,List5.length());
 List5.append(""+owdeepList.get(rs1.getString(1)));     
  ...
  //  System.out.println("List7 &&&&&&&&"+List7);
    ...
    <td class="rptValue"><a href="#" onclick="viewWorks('rws_drought_contin_drill_rpt.jsp?dcode=<%=rs1.getString(1)%> ')"><%=rs1.getString(2)%></td>
