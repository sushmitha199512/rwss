...
<%
	...
	ARDis = (ArrayList) session.getAttribute("distribution");
	int limitPerPage = 20;
	String [] S1 = new String [ARDis.size()];
%>
</head>
<body bgcolor="#edf2f8">
...
%>
	...
	<tr align=center>
		...
		<td class=rptLabel colspan=5>Distribution</td>
		<td class=rptLabel rowspan=2>Material Code</td>
		<td class=rptLabel rowspan=2>No Of Pipe Lines</td>
	</tr>
	<tr>
		<td class=rptLabel>Length</td>				
		<td class=rptLabel>Covered</td>
		<td class=rptLabel>Length Needed</td>
		<td class=rptLabel>Length Covered</td>
		<td class=rptLabel>Balance</td>
	</tr>

<%
	int totalpgs = Math.round(ARDis.size() / limitPerPage);
	...
	if(request.getParameter("pr") != null)
	{
	...
	else
		{
			...
			//System.out.println("AR Size = " + ARDis.size());				
					for(int i = pr; i<limitPerPage + pg && i<ARDis.size(); i++)	
					{					
					S1 = (String[])ARDis.get(i);					
%>	
	<tr>
...
%>
			...
			<a href="rws_asset_distric_temp_rpt.jsp?pr=0">[First Page]</a> 
			<a href="rws_asset_distric_temp_rpt.jsp?pr=<%=pr-limitPerPage%>">[Previous Page]</a> 
<%
		}
			if (pr < (totalpgs * limitPerPage) - limitPerPage) 
			{
%>
				<a href="rws_asset_distric_temp_rpt.jsp?pr=<%=pr+limitPerPage%>">[Next Page]</a> 
				<a href="rws_asset_distric_temp_rpt.jsp?pr=<%=(totalpgs * limitPerPage) - limitPerPage%>">[Last Page]</a> 
<%
			}
...
{
	//System.out.println("The Error in assetdistricrpt.jsp = " + e);
}
%>
		</td>
	</tr>
