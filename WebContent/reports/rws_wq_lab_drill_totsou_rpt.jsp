...
String nmonth=(String)session.getAttribute("nmonth");
...
String dcode=request.getParameter("dcode");
   ...
   String s1=(String)session.getAttribute("y");
...
<td class=btext ><b>Prepared On   </b></td>
	...
	try
      {
		String wqtest="";
		wqtest="select lab_code,test_id,test_code,source_code,to_char(prepared_on,'dd/mm/yyyy'),MNAME,PNAME,VNAME,PANCH_CODE,PANCH_NAME  from RWS_WQ_TEST_RESULTS_TBL a,rws_complete_hab_view b where  substr(source_code,1,2)=b.dcode and substr(source_code,6,2)=b.mcode and substr(source_code,13,2)=b.pcode and substr(source_code,8,3)=b.vcode and substr(source_code,1,16)=b.panch_code and substr(source_code,1,2)='"+dcode+"'  and substr(test_id,1,3)='"+labCode+"' and substr(test_id,4,2)='"+s1+"' and to_date(prepared_on,'dd/mm/yy')>='01-JAN-"+ye+"' and to_date(prepared_on,'dd/mm/yy')<'01-"+nmonth+"-"+ye+"' ";
...
String query="";
if(testCode!=null && testCode!=""){
query="select  source_code  from RWS_WQ_TEST_RESULTS_TBL where  substr(source_code,1,2)='"+dcode+"' and substr(test_id,1,3)='"+labCode+"' and substr(test_id,4,2)='"+s1+"' and to_date(prepared_on,'dd/mm/yy')>='01-JAN-"+ye+"' and to_date(prepared_on,'dd/mm/yy')<'01-"+nmonth+"-"+ye+"' and test_code='"+testCode+"'";
}
else{
	query="select  source_code,count(distinct source_code)  from RWS_WQ_TEST_RESULTS_TBL where  substr(source_code,1,2)='"+dcode+"' and substr(test_id,1,3)='"+labCode+"' and substr(test_id,4,2)='"+s1+"' and to_date(prepared_on,'dd/mm/yy')>='01-JAN-"+ye+"' and to_date(prepared_on,'dd/mm/yy')<'01-"+nmonth+"-"+ye+"' group by  source_code";
}
