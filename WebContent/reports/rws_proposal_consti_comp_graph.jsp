...
<%  
	...
	distnames =new String[Count];
	 data2 =new double[Count];

while(r6.next())
{
	dist[count]=r6.getString(1);
	data2[count]=r6.getDouble(2);
	count++;
}
   ...
   int p=0;
	 ...
	 String names[] = request.getParameterValues("consq");
   ...
   if(count!=0){
	 ...
	 sr2=new String[count];
 	for(int m=0;m<count;m++) {

	   districts[m]=dist[m];
	   data1[m]=data2[m];
	   colors1[m]=colors[m];
	   sr2[m] = names[m].substring(names[m].lastIndexOf("-")+1, names[m].length());
	...
	TextBox textbox = ch.xAxis().setLabels(sr2);
...
%>
  ...
  for(j=0;j<count;j++){
	...
	if(x.equals("b"))
   {
	...
	//ch.addTitle("Contituency Wise Estimate Cost Graph Report  ", "times new roman Bold Italic",12);
    ch.yAxis().setTitle("Total Estimate Cost (Rs. in Lakhs)");
	ch.xAxis().setTitle("Constituency Name ");
...
<%}  int j=0;%>
  ...
  <table border = 1 cellspacing = 0 cellpadding = 0 width="65%"  bordercolor=#000000 style="border-collapse:collapse" bgcolor="#ffffff">
	<tr>
				...
				<td class="btext" >Constituency Name </td> 
				<td class="btext" >Total Estimate Cost (Rs. in Lakhs)</td> 
			</tr>
			<tr>
<%	 
	   double total=0;
	   for(int m=0;m<count;m++) {%>
		...
		<td class="mystyle1"><a href="./rws_const_est_cost_rpt.jsp?const[]=<%=names1[m]%>&assetType=<%=aType%>&finYear=<%=FYEAR[0]%><%=FYEAR[FYEAR.length-1]%>"><%=sr2[m]%></a></td>
			<td class="mystyle1" align=right><%=data2[m]%> </td>
		</tr>
  <%		
		   total=total+data2[m];
   }%>
