...
<table border = 1 cellspacing = 0 cellpadding = 0 width="100%"  bordercolor=#000000 style="border-collapse:collapse" bgcolor="#ffffff" align = "center">
	<caption>
		<table  border=0 rules=none style="border-collapse:collapse" align = "right">			
			<tr align="right">
				...
				<td class="bwborder"><a href="./rws_tech_status_xls.jsp?district=<%=dCode%>&division=<%=divCode%>&subdivision=<%=subdivCode%>&work=<%=workccode%>&augmentation=<%=aCode%>&plan=<%=planCode%>&scheme=<%=schCode%>&programme=<%=pCode%>&subprogramme=<%=spCode%>&year=<%=yearCode%>&dName=<%=dName%>&divName=<%=divName%>&subdivName=<%=subdivName%>&workcName=<%=workcName%>&augName=<%=aName%>&PlanName=<%=planName%>&schName=<%=schName%>&progName=<%=pName%>&sprogName=<%=spName%>&yrName=<%=year%>"
	...
	<tr bgcolor="#8A9FCD" align="center">
		<td align="center" class="rptHeading" colspan=17>Technical Sanction Report Details</font></td>	
	</tr>
...
<%	}%> 
	...
	<table border = 1 cellspacing = 0 cellpadding = 0 width="100%"  bordercolor=#000000 style="border-collapse:collapse" bgcolor="#ffffff" align = "center">
		...
		<%-- <td class="btext" nowrap>Plan/<br>Non-Plan</td>
...
//System.out.println("in revised technical::"+rscheme.getString(1));
String q1="";
String q2 ="";
     if(rscheme.getString(1).equals("11"))
    {
 ...
 if(rscheme.getString(1).equals("11"))
	...
	else
	{
	   q2 ="select distinct pr.panch_code,nvl(pr.panch_name,'-'),nvl(CENSUS_PLAIN_POPU,'0')+nvl(CENSUS_SC_POPU,'0')+nvl(CENSUS_ST_POPU,'0') AS TOTAL,nvl(coverage_status,'-') AS STATUS from RWS_WORK_ADMN_TBL w,RWS_NEW_REVISED_EST_TBL e,RWS_REV_EST_NEW_HAB_LNK_TBL ln,RWS_PANCHAYAT_RAJ_TBL PR,rws_habitation_directory_tbl h where e.work_id=ln.work_id and ln.hab_code=pr.panch_code(+) and ln.hab_code=h.hab_code(+) and ln.work_id='"+rs.getString(2)+"' AND H.COVERAGE_STATUS IS NOT NULL AND H.COVERAGE_STATUS<>'UI' order by pr.panch_code";
		...
		while(r2.next())
		{
	//	////System.out.println(" in revised "+r2.getString(2));
%>
	<tr>
	<td></td><%
	 if(rscheme.getString(1).equals("11"))
