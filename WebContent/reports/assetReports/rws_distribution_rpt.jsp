...
<caption>
		<table  border=0 rules=none style="border-collapse:collapse" align = "right">			
			<tr align="right">
				<td class="bwborder"><a href="rws_graph_frm1.jsp"><a href="javascript:history.go(-1)">Back |</a></td>
...
<td align="center" class="rptHeading" colspan="14"> Assets - DISTRIBUTION </td>
</tr>
</table>
...
<tr align=center>
               ...
               <td class=btext>No. Of Pipes Required</td>
               <td class=btext>No Of Air Valves</td>
               <td class=btext>Total Length Needed</td>
               
               <td class=btext>No. Of Sluice Valves</td>
               <td class=btext>Total Length Provided</td>
               <td class=btext>No. Of Scour Valves</td>
               
               <td class=btext>Full Villages Covered</td>
               <td class=btext>No. of Private House Connections</td>
               <td class=btext>Balance Needed in Mts</td>
...
//String query="select  dis_len,nvl(dis_cove,'-'),dis_ln,dis_tlc,dis_bn,distrisc_code,nvl(dis_mat_code,'-'),no_of_pipes,no_airvalves,no_scourvalves,no_sluicevalves,no_pr_hc,nvl(location,'-'),no_standposts,asset_code from rws_ast_distri_sc_param_tbl   where asset_code='"+assetcode+"' and substr(distrisc_code,1,2)='"+districtcode+"' and substr(distrisc_code,6,2)='"+mandalcode+"' and substr(distrisc_code,13,2)='"+panchayatcode+"' and substr(distrisc_code,8,3)='"+villagecode+"' ";
//
//String query="select  dis_len,nvl(dis_cove,'-'),dis_ln,dis_tlc,dis_bn,distrisc_code,nvl(dis_mat_code,'-'),no_of_pipes,no_airvalves,no_scourvalves,no_sluicevalves,no_pr_hc,nvl(location,'-'),no_standposts,asset_code from rws_ast_distri_sc_param_tbl   where asset_code='"+assetcode+"' ";
String query="select  DISTRISC_CODE,nvl(NO_OF_PIPES,0),nvl(NO_AIRVALVES,0),nvl(DIS_LN,0),nvl(NO_SLUICEVALVES,0),nvl(DIS_TLC,0),nvl(NO_SCOURVALVES,0),decode(DIS_COVE,'Y','Yes','N','No')village,nvl(NO_PR_HC,0),nvl(DIS_BN,0),nvl(NO_STANDPOSTS,0)  from rws_ast_distri_sc_param_tbl where asset_code='"+assetcode+"'";
