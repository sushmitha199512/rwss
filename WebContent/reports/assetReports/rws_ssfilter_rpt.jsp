...
<tr bgcolor="#8A9FCD" align="center">
		<td align="center" class="rptHeading" colspan="14"> Assets - SS FILTERS </td>
</tr>
</table>
...
%>
       ...
       <tr align=center>
			   ...
			   <td class=btext>No. Of Hours<br>
			    of Usage</td>
			   <td class=btext>Size Of Units(Sq. Mts)</td>
               <td class=btext>No. Of StandBy Units</td>
               <td class=btext>Shelter Existing</td>
			  
			   <td class=btext>General Performance</td>
			   <td class=btext>Extension Scope</td>
			   <td class=btext>Sedimentation Tank</td>
			   <td class=btext>Horizontal Roughing Filter</td>
			   <td class=btext>Vertical Filter</td>
              
			   </tr>
<%
  String  query=null;
//			  query="select wtpsc_code,nvl(location,'-'),nvl(use_capacitymld,'0'),nvl(units_no,0),nvl(rate_of_filtration,0),nvl(wtp_size,'0'),nvl(units_standby,0),decode(shelter_existing,'Y','Yes','N','No','','-')result1,decode(performance,'G','Good','B','Bad','','-')result,decode(extention_scope,'Y','Yes','N','No','','-')result2,substr(chk_filter,0,2),substr(chk_filter,2,4),substr(chk_filter,4,6),asset_code	  from RWS_AST_WTP_SC_PARAM_TBL   where asset_code='" + assetcode + "' and substr(wtpsc_code,1,2)='" + districtcode + "' and substr(wtpsc_code,6,2)='" + mandalcode + "' and substr(wtpsc_code,13,2)='" + panchayatcode + "' and substr(wtpsc_code,8,3)='" + villagecode + "' and substr(wtpsc_code,20,2)='"+type+"' ";
			  query="select wtpsc_code,nvl(location,'-'),nvl(use_capacitymld,'0'),nvl(units_no,0),nvl(rate_of_filtration,0),nvl(wtp_size,'0'),nvl(units_standby,0),decode(shelter_existing,'Y','Yes','N','No','','-')result1,decode(performance,'G','Good','B','Bad','','-')result,decode(extention_scope,'Y','Yes','N','No','','-')result2,substr(chk_filter,0,2),substr(chk_filter,2,4),substr(chk_filter,4,6)	  from RWS_AST_WTP_SC_PARAM_TBL   where asset_code='" + assetcode + "'  and substr(wtpsc_code,20,2)='"+type+"' ";
			 
		stmt1=conn.createStatement();
		rs1=stmt1.executeQuery(query);
		while(rs1.next())
    	{
			if(rs1.getString(11)!=null && rs1.getString(11).equals("01"))
			{
				sedtank="Yes";
			}
			else
			...
			if(rs1.getString(12)!=null && rs1.getString(12).equals("02"))
						{
							horfil="Yes";
						}
						else
						...
						if(rs1.getString(13)!=null && rs1.getString(13).equals("03"))
						{
							verfil="Yes";
						}
						else
...
%>
				...
				<td class=rptValue><%=horfil%></td>
				<td class=rptValue><%=verfil%></td>
