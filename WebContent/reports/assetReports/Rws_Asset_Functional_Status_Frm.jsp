...
function unview()
{
	...
	function fnSubmit()
	{
	var frommonth=document.f1.fromDate.value.split("-");
	var tomonth=document.f1.toDate.value.split("-");
			 ...
			 if(document.f1.finyear.value=='0' && (document.f1.fromDate.value=='' && document.f1.toDate.value==''))
			{
			 ...
			 else if(document.f1.fromDate.value!='' && document.f1.toDate.value!=''  && frommonth[1]!=tomonth[1]){
				 
				 alert("Please select with in the month");
				 return false;
			 }
			else
		{
				if(document.f1.finyear.value!='0' &&  (document.f1.fromDate.value=='' && document.f1.toDate.value=='')){
					var finanYear=document.f1.finyear.value.split("-");
					document.f1.startyear.value =finanYear[0];
					document.f1.endyear.value=finanYear[1];
				}else{
					if (document.f1.finyear.value == '0' && document.f1.fromDate.value != ''	&& document.f1.toDate.value != '') {
						if (frommonth[1] >= 4) {
							document.f1.startyear.value = frommonth[2];
							document.f1.endyear.value = parseInt(frommonth[2]) + 1;
						} else {
							document.f1.startyear.value = parseInt(frommonth[2]) - 1;
							document.f1.endyear.value = frommonth[2];
						}
				  }
			 }
			document.f1.action = "./Rws_Asset_Functional_Status_Rpt.jsp";
			document.f1.submit();
		}
	}
</script>
...
<table align="center"><tr><td><font color="#FF0000">Note:</font>&nbsp;&nbsp;Select 'From Date' and 'To Date' in the same financial year.</td></tr>
	</table>
