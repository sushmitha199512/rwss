...
<tr align="right">
	...
	<tr >
	<td align="center" class="rptHeading" >District Wise HandPumps Status Report</td>
	</tr>
...
try
{
	 ...
	 {
query="select distinct substr(hp_code,1,2),dname,sum(case when Status='W' then 1 else 0 end ),sum(case when status='N' then 1 else 0 end ),sum(case when status='D' then 1 else 0 end ),sum(case when status='S' then 1 else 0 end ),sum(case when status='C' then 1 else 0 end ),sum(case when status is null then 1 else 0 end) From rws_hp_subcomp_param_tbl a,RWS_COMPLETE_HAB_VIEW b   WHERE substr(hp_code,1,16)=b.panch_code  group by substr(hp_code,1,2),dname ";
	 }
	 else
	 {
		 query="select distinct substr(hp_code,1,2),dname,sum(case when Status='W' then 1 else 0 end ),sum(case when status='N' then 1 else 0 end ),sum(case when status='D' then 1 else 0 end ),sum(case when status='S' then 1 else 0 end ),sum(case when status='C' then 1 else 0 end ),sum(case when status is null then 1 else 0 end) From rws_hp_subcomp_param_tbl a,RWS_COMPLETE_HAB_VIEW b   WHERE substr(hp_code,1,16)=b.panch_code and dcode='"+rDistrict+"'  group by substr(hp_code,1,2),dname "; 
	 }
System.out.println("query**********" +query);
	...
	%>
	   ...
	   <td class="<%=style %>" style="text-align: right;"> <%=rs8.getInt(7)%></td>
	      <td class="<%=style %>" style="text-align: right;"><a href="#" onclick="viewWorks('rws_hp_notHaving_status_drill.jsp?dcode=<%=rs8.getString(1)%>')"><%=rs8.getInt(8)%></a></td>
