...
pname1=request.getParameter("pname1");
typeassetcode=request.getParameter("typeassetcode");
typeassetcode=(String)session.getAttribute("typeassetcode");
...
<tr bgcolor="#edf2f8" align="center">
		<td class=rptValue colspan=3><font color="navy">Panchayat:<%=pname1%>&nbsp;Asset:<%=typeassetname%></td>
	</tr>
<tr bgcolor="#8A9FCD" align="center">
		<td align="center" class="rptHeading" colspan="14"> Assets - Village Wise</td>
...
try
{
	...
	String query="select  count(*),d.vname,d.vcode from rws_asset_mast_tbl b,rws_asset_type_tbl a,rws_village_tbl d,rws_asset_scheme_tbl s where  a.type_of_asset_code='" + typeassetcode + "' and b.asset_code=s.asset_code  and  substr(s.scheme_code,1,2)=d.dcode and d.dcode='" + districtcode + "' and substr(s.scheme_code,6,2)=d.mcode and d.mcode='" + mandalcode + "' and  substr(s.scheme_code,13,2)=d.pcode and d.pcode='"+ panchayatcode + "' and b.type_of_asset_code=a.type_of_asset_code and substr(s.scheme_code,8,3)=d.vcode group by d.vname,d.vcode order by d.vcode ";
   ...
   while(rs1.next())

	{
		...
		<td class=rptValue><a href="rws_habitation_rpt.jsp?villagecode=<%=rs1.getString(3)%>&districtname=<%=districtname%>&mandalname=<%=mandalname%>&panchayatname=<%=pname1%>&typeassetcode=<%=typeassetcode%>&districtcode=<%=districtcode%>&mandalcode=<%=mandalcode%>&panchayatcode=<%=panchayatcode%>&villagename=<%=rs1.getString(2)%>&typeassetname=<%=typeassetname%>"><%=vname%></a></td>
