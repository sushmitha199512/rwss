	...
	var loggedUser = '<%=loggedUser%>';
		...
		else
		{
			if(loggedUser=="admin"||loggedUser=="100000" ||loggedUser=="guest")
			{
				...
				//document.f1.constituency.value = document.f1.constituency.options[document.f1.constituency.selectedIndex].text;
			...
			document.f1.action="./rws_asset_status_det_rpt.jsp";
			document.f1.submit();
		}	
	}
		</script>
...
<table border = 1 align = center cellspacing = 0 cellpadding = 0 rules = "rows" bordercolor = navy style = "border-collapse:collapse" bgcolor="#ffffff" width=350>		
	...
	<tr bgcolor="#8A9FCD" align="center">
		<td align="center" class="rptHeading" colspan=3>BaseLine Survey Government Building in Gram Panchayat - Report</td>	
	</tr>
	<%
try
{
...
%>
	...
	<tr><td class=rptLabel>Constituency&nbsp;&nbsp;</td>
		<td class=btext>
		<SELECT name="constituency" id="combo" class="mycombo" style="width:150px"  onchange="this.form.submit()">
				
				<option value="00">-Select-</option>
            <%if(dcode!=null && !dcode.equals("")) {
			Statement stmt2 = conn.createStatement();
			query="select  distinct CONSTITUENCY_CODE,CONSTITUENCY_NAME from RWS_CONSTITUENCY_TBL   where dcode='"+dcode+"' order by CONSTITUENCY_CODE";
			// System.out.println("qqq:"+query);
			ResultSet rs2 = stmt2.executeQuery(query);
			while(rs2.next())
			{
				 if(rs2.getString(1).equals(constituency)){
	%>
