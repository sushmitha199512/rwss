...
<tr bgcolor="#8A9FCD" align="center">
	<td align="center" class="rptHeading" colspan="14"> Assets - CHLORNATION </td>
</tr>
</table>
<table border=1 cellpadding=1 cellspacing=1 width="100%">
<tr align=center>
               <td class=btext>Sl.No</td>
			   <td class=btext>CL CODE</td>
			   <td class=btext>LOCATION </td>
			   <td class=btext>WORKING </td>
			   <td class=btext>BLEACHING </td>
			   <td class=btext>LIQUID CHILORINE </td>
			   <td class=btext>GAS CHILORINE </td>
			   <td class=btext>ONLINE DOSE </td>
...
//String query="select  CL_CODE,nvl(LOCATION,'-'),nvl(WORKING,'  '),nvl(BLEACHING,'   '),nvl(BLEACHING_WORKING,'  '),"+"nvl(LIQUID_CHILORINE,'  '),nvl(LIQUID_CHILORINE_WORKING,'  '),nvl(GAS_CHILORINE,'  '),GAS_CHILORINE_WORKING,"+"nvl(ONLINE_DOSE,'  '),ONLINE_DOSE_WORKING,nvl(REMARKS,'  '),ASSET_CODE from RWS_AST_CHLORINATION_TBL  where "+"asset_code='" + assetcode + "' and substr(cl_code,1,2)='" + districtcode + "' and "+"substr(cl_code,6,2)='" + mandalcode + "' and substr(cl_code,13,2)='" + panchayatcode + "' and "+"substr(cl_code,8,3)='" + villagecode + "'  ";	 
String query="select  CL_CODE,nvl(LOCATION,'-'),decode(WORKING,'Y','YES','N','NO',null,'-'),decode(BLEACHING_WORKING,'Y','YES','N','NO',null,'-') BLEACHING ,decode(LIQUID_CHILORINE_WORKING,'Y','YES','N','NO',null,'-') LIQUIDCHLORINE,decode(GAS_CHILORINE_WORKING,'Y','YES','N','NO',null,'-') GASCHLORINE ,decode(ONLINE_DOSE_WORKING,'Y','YES','N','NO',null,'-') ONLINEDOSE,nvl(REMARKS,'-') from RWS_AST_CHLORINATION_TBL  where asset_code='" + assetcode + "'";	 
