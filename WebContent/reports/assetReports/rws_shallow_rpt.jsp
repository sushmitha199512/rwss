...
<td align="center" class="rptHeading" colspan="14"> Assets - SHALLOW HANDPUMPS </td>
</tr>
</table>
...
<tr align=center>
			   ...
			   <td class=btext>Diameter(Mts)</td>
			   <td class=btext>Depth(Mts)</td>
			   <td class=btext>Hygiene</td>
			   <td class=btext>Average No. Of Hours Working per day</td>
...
//String query="select  shallowhp_code,nvl(location,'-'),nvl(diameter,0),nvl(depth,0),nvl(hygiene,0),nvl(average_hours,0),nvl(average_time,0),nvl(yield,0),water_quality,nvl(FLORIDE_PER,0),nvl(brakish_per,0),nvl(iron_per,0),nvl(others_per,0),nvl(FLORIDE,0),nvl(brakish,0),nvl(iron,0),nvl(others,0) from RWS_SHALLOWHANDPUMPS_TBL  where  substr(hab_code,1,2)='"+districtcode+"' and substr(hab_code,6,2)='"+mandalcode+"' and substr(hab_code,13,2)='"+panchayatcode+"' and substr(hab_code,8,3)='"+villagecode+"' and  asset_code='"+assetcode+"' ";
//String query="select  shallowhp_code,nvl(location,'-'),nvl(diameter,0),nvl(depth,0),nvl(hygiene,0),nvl(average_hours,0),nvl(average_time,0),nvl(yield,0),water_quality,nvl(FLORIDE_PER,0),nvl(brakish_per,0),nvl(iron_per,0),nvl(others_per,0),nvl(FLORIDE,0),nvl(brakish,0),nvl(iron,0),nvl(others,0) from RWS_SHALLOWHANDPUMPS_TBL  where  asset_code='"+assetcode+"' ";
  String query="select shallowhp_code,nvl(location,'-'),nvl(diameter,0),nvl(depth,0),nvl(hygiene,'-'),nvl(average_hours,0),nvl(yield,0),decode(water_quality,'SAFE SOURCE','SAFE SOURCE','NSS','NSS')waterquality from RWS_SHALLOWHANDPUMPS_TBL where asset_code='"+assetcode+"' "; 
