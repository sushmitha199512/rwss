...
<body style="margin-top: 10%">
	<table border="0" cellspacing="0" cellpadding="0" width="80%" style="border-collapse:collapse" align="center">
		<thead>
			<tr>
				<th align="center" class="gridhdbg" colspan="25">Inspection Abstract Report For Schemes ( District :- <%=Dname %>	
					<%if  (Mname != null){ %>
						, Mandal :- <%=Mname %>  				
					<%} if (Pname != null){ %> 
						, Panchyat :- <%=Pname %>
	...
	<%		String query1="",query2="",query3="",query4="",query5="",query6="", query7="";
			...
			try{	
				...
				if (Mode != null && Mode.equals("oneMonth")){
					query2="SELECT A.TYPE_OF_ASSET_CODE, D.TYPE_OF_ASSET_NAME, A.ASSET_NAME, A.ASSET_CODE, C.MNAME, C.PNAME, C.PANCH_NAME, C.PANCH_CODE, A.ASSET_STATUS, NVL(E.INSPECTED_BY,'-'), TO_CHAR(E.UPDATE_DATE,'DD-MM-YYYY HH:MI:SS AM') FROM RWS_ASSET_MAST_TBL A, RWS_COMPLETE_HAB_VIEW C, RWS_ASSET_TYPE_TBL D, RWS_ASSET_IMAGE_TBL E WHERE A.ASSET_CODE=E.ASSET_CODE AND A.HAB_CODE=C.PANCH_CODE AND A.TYPE_OF_ASSET_CODE=D.TYPE_OF_ASSET_CODE AND A.TYPE_OF_ASSET_CODE IN ("+SchemeType+") AND C.DCODE='"+Dcode+"' AND (to_date(E.UPDATE_DATE) > '"+format.format(d1)+"')";
				}
				if (Mode != null && Mode.equals("twoMonth")){
					query2="SELECT A.TYPE_OF_ASSET_CODE, D.TYPE_OF_ASSET_NAME, A.ASSET_NAME, A.ASSET_CODE, C.MNAME, C.PNAME, C.PANCH_NAME, C.PANCH_CODE, A.ASSET_STATUS, NVL(E.INSPECTED_BY,'-'), TO_CHAR(E.UPDATE_DATE,'DD-MM-YYYY HH:MI:SS AM') FROM RWS_ASSET_MAST_TBL A, RWS_COMPLETE_HAB_VIEW C, RWS_ASSET_TYPE_TBL D, RWS_ASSET_IMAGE_TBL E WHERE A.ASSET_CODE=E.ASSET_CODE AND A.HAB_CODE=C.PANCH_CODE AND A.TYPE_OF_ASSET_CODE=D.TYPE_OF_ASSET_CODE AND A.TYPE_OF_ASSET_CODE IN ("+SchemeType+") AND C.DCODE='"+Dcode+"' AND (to_date(E.UPDATE_DATE) > '"+format.format(d2)+"' AND to_date(E.UPDATE_DATE) <= '"+format.format(d1)+"')";
				}
				if (Mode != null && Mode.equals("threeMonth")){
					query2="SELECT A.TYPE_OF_ASSET_CODE, D.TYPE_OF_ASSET_NAME, A.ASSET_NAME, A.ASSET_CODE, C.MNAME, C.PNAME, C.PANCH_NAME, C.PANCH_CODE, A.ASSET_STATUS, NVL(E.INSPECTED_BY,'-'), TO_CHAR(E.UPDATE_DATE,'DD-MM-YYYY HH:MI:SS AM') FROM RWS_ASSET_MAST_TBL A, RWS_COMPLETE_HAB_VIEW C, RWS_ASSET_TYPE_TBL D, RWS_ASSET_IMAGE_TBL E WHERE A.ASSET_CODE=E.ASSET_CODE AND A.HAB_CODE=C.PANCH_CODE AND A.TYPE_OF_ASSET_CODE=D.TYPE_OF_ASSET_CODE AND A.TYPE_OF_ASSET_CODE IN ("+SchemeType+") AND C.DCODE='"+Dcode+"' AND (to_date(E.UPDATE_DATE) > '"+format.format(d3)+"' AND to_date(E.UPDATE_DATE) <= '"+format.format(d2)+"')";
				}
				if (Mode != null && Mode.equals("greaterThan3Month")){
					query2="SELECT A.TYPE_OF_ASSET_CODE, D.TYPE_OF_ASSET_NAME, A.ASSET_NAME, A.ASSET_CODE, C.MNAME, C.PNAME, C.PANCH_NAME, C.PANCH_CODE, A.ASSET_STATUS, NVL(E.INSPECTED_BY,'-'), TO_CHAR(E.UPDATE_DATE,'DD-MM-YYYY HH:MI:SS AM') FROM RWS_ASSET_MAST_TBL A, RWS_COMPLETE_HAB_VIEW C, RWS_ASSET_TYPE_TBL D, RWS_ASSET_IMAGE_TBL E WHERE A.ASSET_CODE=E.ASSET_CODE AND A.HAB_CODE=C.PANCH_CODE AND A.TYPE_OF_ASSET_CODE=D.TYPE_OF_ASSET_CODE AND A.TYPE_OF_ASSET_CODE IN ("+SchemeType+") AND C.DCODE='"+Dcode+"' AND (to_date(E.UPDATE_DATE) > '"+format.format(d6)+"' AND to_date(E.UPDATE_DATE) <= '"+format.format(d3)+"')";
				}	
				if (Mode != null && Mode.equals("greaterThan6Month")){
					query2="SELECT A.TYPE_OF_ASSET_CODE, D.TYPE_OF_ASSET_NAME, A.ASSET_NAME, A.ASSET_CODE, C.MNAME, C.PNAME, C.PANCH_NAME, C.PANCH_CODE, A.ASSET_STATUS, NVL(E.INSPECTED_BY,'-'), TO_CHAR(E.UPDATE_DATE,'DD-MM-YYYY HH:MI:SS AM') FROM RWS_ASSET_MAST_TBL A, RWS_COMPLETE_HAB_VIEW C, RWS_ASSET_TYPE_TBL D, RWS_ASSET_IMAGE_TBL E WHERE A.ASSET_CODE=E.ASSET_CODE AND A.HAB_CODE=C.PANCH_CODE AND A.TYPE_OF_ASSET_CODE=D.TYPE_OF_ASSET_CODE AND A.TYPE_OF_ASSET_CODE IN ("+SchemeType+") AND C.DCODE='"+Dcode+"' AND (to_date(E.UPDATE_DATE) > '"+format.format(d12)+"' AND to_date(E.UPDATE_DATE) <= '"+format.format(d6)+"')";
				}
			...
			%>
				<tr>
	 				...
	 				<td class="<%=style%>" style="text-align: center;"> <a href="#" onclick="viewWorks1('./rws_abstract_report_for_schemes_drill2.jsp?mode=TestSafe&dCode=<%=Dcode %>&dName=<%=Dname %>&mCode=<%=Mcode%>&mName=<%=Mname%>&pCode=<%=Pcode%>&pName=<%=Pname%>&vCode=<%=Vcode%>&vName=<%=Vname%>&hCode=<%=Hcode%>&hName=<%=Hname%>&schemeType=<%=rs2.getString(2)%>&assetName=<%=rs2.getString(3)%>&assetCode=<%=rs2.getString(4)%>&status=<%=AssetStatus%>&schemeType=<%=rs2.getString(2)%>')"><%=TestSafe%></a> </td>
	 				<td class="<%=style%>" style="text-align: center;"> <a href="#" onclick="viewWorks1('./rws_abstract_report_for_schemes_drill2.jsp?mode=TestUnsafe&dCode=<%=Dcode %>&dName=<%=Dname %>&mCode=<%=Mcode%>&mName=<%=Mname%>&pCode=<%=Pcode%>&pName=<%=Pname%>&vCode=<%=Vcode%>&vName=<%=Vname%>&hCode=<%=Hcode%>&hName=<%=Hname%>&schemeType=<%=rs2.getString(2)%>&assetName=<%=rs2.getString(3)%>&assetCode=<%=rs2.getString(4)%>&status=<%=AssetStatus%>&schemeType=<%=rs2.getString(2)%>')"><%=TestUnsafe%></a> </td>
	 				<td class="<%=style%>" style="text-align: left;"> <%=AssetStatus%> </td>	 				
	 				<td class="<%=style%>" style="text-align: center;"> <%if (ACompCount != null && !ACompCount.equals("0")){ %><a href="#" onclick="viewWorks1('./rws_abstract_report_for_schemes_drill2.jsp?mode=CompnentDetails&dCode=<%=Dcode %>&dName=<%=Dname %>&mCode=<%=Mcode%>&mName=<%=Mname%>&pCode=<%=Pcode%>&pName=<%=Pname%>&vCode=<%=Vcode%>&vName=<%=Vname%>&hCode=<%=Hcode%>&hName=<%=Hname%>&schemeType=<%=rs2.getString(2)%>&assetName=<%=rs2.getString(3)%>&assetCode=<%=ACode%>&status=<%=AssetStatus%>&schemeType=<%=rs2.getString(2)%>')"><%=ACompCount%></a><%}else{ %><%=ACompCount%><%} %> </td>
