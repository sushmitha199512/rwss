...
<% DecimalFormat ndf = new DecimalFormat("####.##");
    ...
    Date d12 = c.getTime();
     ...
     String dcode="";
     if(Constants.IsUser(user.getUserId()) && !user.getUserId().equals(Constants.LAKH_USER))
    	 dcode=user.getCircleOfficeCode();
...
<body>
	<table border="0" cellspacing="0" cellpadding="0" width="80%" style="border-collapse:collapse" align="center">
		<thead>
			...
			<tr>
				<th align="center" class="gridhdbg" colspan="15">Inspection Abstract Report For Schemes(Repeated Inspections)</th>
			</tr>
			<tr>
		...
		<tbody>		
			<%	if(typeOfAssetCode != null && !typeOfAssetCode.equals("0")){
			  ...
			  query2="SELECT COUNT(*),C.DCODE FROM RWS_ASSET_MAST_TBL A, RWS_COMPLETE_HAB_VIEW C WHERE A.HAB_CODE=C.PANCH_CODE AND TYPE_OF_ASSET_CODE IN ("+SchemeType+") ";
			  if(!dcode.equals("") )
				  query2+="and c.dcode='"+dcode+"' " ;
			  
			  query2+= " GROUP BY C.DCODE";
			  ...
			  //System.out.println(query2);
				...
				rs2.close();
				ps2.close();
				query3="SELECT COUNT(*),C.DCODE FROM RWS_ASSET_MAST_TBL A, RWS_COMPLETE_HAB_VIEW C WHERE A.HAB_CODE=C.PANCH_CODE AND A.ASSET_STATUS=1 AND TYPE_OF_ASSET_CODE IN ("+SchemeType+")"  ;
				
						if(!dcode.equals(""))
							query3+=" and c.dcode='"+dcode+"' " ;
						  
						query3+= " GROUP BY C.DCODE";
				...
				rs3.close();
				ps3.close();
				query4="SELECT COUNT(*),C.DCODE FROM RWS_ASSET_MAST_TBL A, RWS_COMPLETE_HAB_VIEW C WHERE A.HAB_CODE=C.PANCH_CODE AND A.ASSET_STATUS=2 AND TYPE_OF_ASSET_CODE IN ("+SchemeType+") ";
						if(!dcode.equals(""))
							query4+=" and c.dcode='"+dcode+"' " ;
						  
						query4+= " GROUP BY C.DCODE";
				...
				rs4.close();
				ps4.close();
				query5="SELECT DCODE, DNAME FROM RWS_DISTRICT_TBL " ;
						if(!dcode.equals(""))
							query5+="  where dcode='"+dcode+"' " ;
						  
						query5+= " order by DCODE";
				...
				while(rs5.next()){
					...
					int count = 0;					
				 	...
				 	query7="SELECT to_date(D.inspect_date), A.ASSET_CODE FROM RWS_ASSET_MAST_TBL A, RWS_COMPLETE_HAB_VIEW C, RWS_ASSET_INSPECTION_TBL D WHERE A.ASSET_CODE=D.ASSET_CODE AND A.HAB_CODE=C.PANCH_CODE AND TYPE_OF_ASSET_CODE IN ("+SchemeType+")  AND C.DCODE=? GROUP BY to_date(D.inspect_date), A.ASSET_CODE";
				 	////System.out.println(rs5.getString(1)+"         "+query7);
			...
			%>
				<tr>
	 				...
	 				<td class="<%=style%>" style="text-align: left;"> <a href="rws_abstract_report_his_drill.jsp?mode=Mandal&dCode=<%=rs5.getString(1)%>&dName=<%=rs5.getString(2)%>&typeOfAsset=<%=SchemeType%>"><%=rs5.getString(2)%></a> </td>
	 				<td class="<%=style%>" style="text-align: center;"> <a href="#" onclick="viewWorks('./rws_abstract_report_his_drill1.jsp?mode=Total&dCode=<%=rs5.getString(1)%>&dName=<%=rs5.getString(2)%>&typeOfAsset=<%=SchemeType%>')"><%=schemes%></a> </td>
	 				<td class="<%=style%>" style="text-align: center;"> <a href="#" onclick="viewWorks('./rws_abstract_report_his_drill1.jsp?mode=oneMonth&dCode=<%=rs5.getString(1)%>&dName=<%=rs5.getString(2)%>&typeOfAsset=<%=SchemeType%>')"><%=withInOneMonth%></a> </td>
	 				<td class="<%=style%>" style="text-align: center;"> <%=ndf.format(per)%> </td>
	 				<td class="<%=style%>" style="text-align: center;"> <a href="#" onclick="viewWorks('./rws_abstract_report_his_drill1.jsp?mode=twoMonth&dCode=<%=rs5.getString(1)%>&dName=<%=rs5.getString(2)%>&typeOfAsset=<%=SchemeType%>')"><%=oneTwoMonth%></a> </td>
	 				<td class="<%=style%>" style="text-align: center;"> <a href="#" onclick="viewWorks('./rws_abstract_report_his_drill1.jsp?mode=threeMonth&dCode=<%=rs5.getString(1)%>&dName=<%=rs5.getString(2)%>&typeOfAsset=<%=SchemeType%>')"><%=twoThreeMonth%></a> </td>	 				
	 				<td class="<%=style%>" style="text-align: center;"> <a href="#" onclick="viewWorks('./rws_abstract_report_his_drill1.jsp?mode=greaterThan3Month&dCode=<%=rs5.getString(1)%>&dName=<%=rs5.getString(2)%>&typeOfAsset=<%=SchemeType%>')"><%=threeSixMonth%></a> </td>
	 				<td class="<%=style%>" style="text-align: center;"> <a href="#" onclick="viewWorks('./rws_abstract_report_his_drill1.jsp?mode=greaterThan6Month&dCode=<%=rs5.getString(1)%>&dName=<%=rs5.getString(2)%>&typeOfAsset=<%=SchemeType%>')"><%=moreThanSixMonths%></a> </td>
	 				<td class="<%=style%>" style="text-align: center;"> <a href="#" onclick="viewWorks('./rws_abstract_report_his_drill1.jsp?mode=HorizontalTotal&dCode=<%=rs5.getString(1)%>&dName=<%=rs5.getString(2)%>&typeOfAsset=<%=SchemeType%>')"><%=totalHori%></a> </td>
	 				<td class="<%=style%>" style="text-align: center;"> <%=ndf.format(totPer)%> </td>
	 				<td class="<%=style%>" style="text-align: center;"> <a href="#" onclick="viewWorks('./rws_abstract_report_his_drill1.jsp?mode=Functioning&dCode=<%=rs5.getString(1)%>&dName=<%=rs5.getString(2)%>&typeOfAsset=<%=SchemeType%>')"><%=functioning%></a> </td>
	 				<td class="<%=style%>" style="text-align: center;"> <a href="#" onclick="viewWorks('./rws_abstract_report_his_drill1.jsp?mode=NotFunctioning&dCode=<%=rs5.getString(1)%>&dName=<%=rs5.getString(2)%>&typeOfAsset=<%=SchemeType%>')"><%=notFunctioning%></a> </td>
