...
<form>
<%
	String dname1=(String)session.getAttribute("dname1");
	String aname=(String)session.getAttribute("assetname1");
	String dcode1=(String)session.getAttribute("dcode1");
	String asset1=(String)session.getAttribute("asset1");
...
String gqry=" select c.dname,c.mname,c.pname,c.panch_name,c.panch_code,a.asset_code,a.asset_name,decode(a.type_of_asset_code,'01','Pws','02','Mpws','03','Cpws','04','handpumps','09','Direct pumping','06','openwells')atype ,nvl(a.location,'-'),to_char(DATE_CREATION,'dd/mm/yyyy') ,nvl(asset_cost,0),nvl(decode(asset_status,'1','Working','2','Not Working','3','Dried','4','Seasonal','5','Condemed'),'-')   from rws_asset_mast_tbl a,rws_complete_hab_view c,rws_asset_scheme_tbl s where a.asset_code=s.asset_code and c.panch_code=substr(s.scheme_code,1,16) and c.panch_code=a.hab_code and  substr(a.hab_code,1,2)='"+dcode1+"'  and type_of_asset_code='"+asset1+"' and a.asset_code is not null order by pname ,panch_name ";
...
// System.out.println("gqryssssssssssss:"+gqry);
	...
	if(rs8.isBeforeFirst())
   {
	...
	%>
	<tr><td  colspan="11"  class=btext>Panchayat Name:<%=rs8.getString(3)%></td></tr>
	<%}else{}%>
	<tr>
