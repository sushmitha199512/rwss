...
<table border = 1 cellspacing = 0 cellpadding = 0 height=5% width="100%" bordercolor=#000000 style="border-collapse:collapse" bgcolor="#ffffff" align = "center">
	<caption>
		<table  border=0 rules=none style="border-collapse:collapse" align = "right">			
			<tr align="right">
				...
				<td class="bwborder"><a href="./rws_work_proj_abstract_excel.jsp" target=_new">Excel|</a></td>
				<%}%>
			</tr>
		</table>
	</caption>
	...
	////System.out..println("First Drill");
	   ...
	   %>
	   	...
	   	<%@ include file="./rws_work_proj_abstract_label.jsp"%>
	   	<%String qry1=null;
		...
		String view1="Create or Replace view spillworks?  as SELECT work_id,office_code,no_of_habs,programme_code FROM RWS_WORK_ADMN_TBL W where w.category_code=? and w.ADMIN_DATE < ? and work_cancel_dt is null  and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < ?))";
		String view2="Create or Replace view newworks?  as SELECT work_id,office_code,no_of_habs,programme_code FROM RWS_WORK_ADMN_TBL W where w.category_code=? and w.ADMIN_DATE >= ?  and work_cancel_dt is null ";
	   ...
	   //// ////System.out.println("view2"+view2);
		...
		<td class=rptValue title=Click nowrap><b>
		<%=rs1.getString(2)%><BR>
		<a href="rws_work_proj_abstract_rpt1.jsp?district=<%=rs1.getString(1)%>&planNonplan=<%=planCode%>&drill=1&catCode=<%=catCode%>&distname=<%=rs1.getString(2)%>">Division</a>|<a href="rws_work_proj_abstract_rpt1.jsp?district=<%=rs1.getString(1)%>&planNonplan=<%=planCode%>&drill=11&catCode=<%=catCode%>&distname=<%=rs1.getString(2)%>">Mandal</a></td>
	   	...
	   	while(rs21.next()&& rs22.next())
            ...
            totestamt=rs44.getFloat(1) + rs44.getFloat(2)+rs55.getFloat(1) + rs55.getFloat(2);;
      ...
      if(rs44!=null){rs44.close();}
       ...
       if(stmt8!=null){stmt8.close();}
         ...
         String qry9=null;Statement stmt9=null;
            try
            {
            qry9 = "select sum(case when date_of_completion<'"+fixYear+"' then 1 else 0 end),sum(case when date_of_completion>='"+fixYear+"' and date_of_completion<'"+fixNextYear+"' then 1 else 0 end) from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rs1.getString(1)+"' "+planQry+" and w.category_code='"+catCode+"' ";
       ...
       String qry4=null;
			...
			qry4 = "select (select sum(w.no_of_habs) from RWS_ADMN_HAB_LNK_TBL wc, spillworks"+session.getId().substring(1,10)+" w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rs1.getString(1)+"')as a, (select sum(w.no_of_habs) from RWS_ADMN_HAB_LNK_TBL wc, newworks"+session.getId().substring(1,10)+" w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rs1.getString(1)+"')as b from dual";

			qry4 = "select (select nvl(sum(count(distinct hab_code)),0) from RWS_ADMN_HAB_LNK_TBL wc, spillworks"+session.getId().substring(1,10)+" w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rs1.getString(1)+"' group by w.work_id)as a, (select nvl(sum(count(distinct hab_code)),0) from RWS_ADMN_HAB_LNK_TBL wc, newworks"+session.getId().substring(1,10)+" w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rs1.getString(1)+"' group by w.work_id)as b from dual";
            ...
            while(rs4.next())
				...
				ghabsann1=ghabsann1+rs4.getInt(1)+rs4.getInt(2);
	               %>
		                 <!-- <td class=rptValue align=right><%=rs4.getInt(1)%></td> -->
		                 <td class=rptValue align=right><%=rs4.getInt(1)+rs4.getInt(2)%></td>
				   <%
       ...
       if(stmt4!=null){stmt4.close();}
        ...
        String qry10=null; Statement stmt10=null;
           ...
           // qry10="Select count(e.hab_code) from RWS_WORK_COMP_HAB_LNK_TBL e where e.work_id in (select work_id from spillworks"+session.getId().substring(1,10)+" where substr(work_id,5,2)='"+rs1.getString(1)+"' union select work_id from newworks"+session.getId().substring(1,10)+" where substr(work_id,5,2)='"+rs1.getString(1)+"')";
            ...
            qry10="select (Select count(e.hab_code) from RWS_WORK_COMP_HAB_LNK_TBL e where e.work_id in (select w.work_id from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rs1.getString(1)+"' and date_of_completion<'"+fixYear+"' "+planQry+" and w.category_code='"+catCode+"' ))a, (Select count (e.hab_code) from RWS_ADMN_HAB_LNK_TBL e where e.work_id in (select w.work_id from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rs1.getString(1)+"' and date_of_completion>='"+fixYear+"' and date_of_completion<'"+fixNextYear+"' "+planQry+" and w.category_code='"+catCode+"' ))b from dual";
			////// ////System.out.println("qry10:"+qry10);
...
<td class="btext" align="right"><%=ghabcovn1%></td>
       ...
       Statement stmtdv1=null,stmtdv2=null;
		...
		String dropqry2="drop VIEW newworks?";
		 PreparedStatement psv1=conn.prepareStatement(dropqry1);
	       PreparedStatement psv2=conn.prepareStatement(dropqry2);
	        psv1.setString(1,session.getId().substring(1,10));
			psv2.setString(1,session.getId().substring(1,10));
		psv1.executeUpdate();
		psv2.executeUpdate();
		}
	...
	////System.out..println("First Drill");
	   ...
	   %>
		...
		String view1="Create or Replace view spillworks"+session.getId().substring(1,10)+" as SELECT work_id,office_code,no_of_habs FROM RWS_WORK_ADMN_TBL W where w.category_code='"+catCode+"' "+planQry+" and w.ADMIN_DATE < '"+fixYear+"' and SUBSTR (w.work_id,5,2)='"+rDistrict+"' and work_cancel_dt is null and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < '"+fixYear+"'))";
		String view2="Create or Replace view newworks"+session.getId().substring(1,10)+" as SELECT work_id,office_code,no_of_habs FROM RWS_WORK_ADMN_TBL W where w.category_code='"+catCode+"' "+planQry+" and w.ADMIN_DATE >= '"+fixYear+"' and SUBSTR (w.work_id,5,2)='"+rDistrict+"'  and work_cancel_dt is null";
		////// ////System.out.println("View1:"+view1);
		////// ////System.out.println("View2:"+view2);
		Statement stmtv1= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
	   	Statement stmtv2= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
	    PreparedStatement ps1=conn.prepareStatement(view1);
	       PreparedStatement ps2=conn.prepareStatement(view2);
			...
			ps1.setString(2,  catCode+" "+planQry);
			ps1.setString(3,fixYear);
			ps1.setString(4,rDistrict);
			ps1.setString(5,  fixYear);
			ps2.setString(1,session.getId().substring(1,10));
			ps2.setString(2,  catCode+" "+planQry);
			ps2.setString(3,fixYear);
			ps2.setString(4,  rDistrict);
		...
		<tr onmouseover="this.style.background='#E3E4FA'" onmouseout="this.style.background='WHITE'">
        <td class=rptValue><%=++slno%></td>
		<td class=rptValue title=Click><b><a href="rws_work_proj_abstract_rpt1.jsp?district=<%=rDistrict%>&planNonplan=<%=planCode%>&drill=2&distname=<%=districtName%>&catCode=<%=catCode%>&division=<%=rs1.getString(1)%>&divname=<%=rs1.getString(2)%>"><%=rs1.getString(2)%></a></td>
	  ...
	  ////System.out..println("query"+qry22);
	   	...
	   	while(rs21.next()&& rs22.next())
            ...
            qry44 = "select (select sum(a.sanction_amount) from rws_work_admn_tbl a,spillworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and SUBSTR(b.office_code,4,1)='"+rs1.getString(1)+"')a,(select sum(a.sanction_amount) from rws_work_admn_prog_lnk_tbl a,spillworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and substr(office_code,4,1)='"+rs1.getString(1)+"')b from dual";
////// ////System.out.println("qry44bbbbbbbb"+qry44);
            stmt44 = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
            rs44=stmt44.executeQuery(qry44);

  			qry45 = "select (select sum(a.sanction_amount) from rws_work_admn_tbl a,newworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and SUBSTR(b.office_code,4,1)='"+rs1.getString(1)+"')a,(select sum(a.sanction_amount) from rws_work_admn_prog_lnk_tbl a,newworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and substr(office_code,4,1)='"+rs1.getString(1)+"')b from dual";
           ////System.out..println("qry45"+qry45);
            stmt45 = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
            rs55=stmt45.executeQuery(qry45);
            while(rs44.next()&& rs55.next())
       ...
       if(stmt44!=null){stmt44.close();}
         ...
         // ArrayList progs=worksData.getAdminPrograms(reque,fb.getWorkId());
          ...
          String work="";
            ...
            {
            // in drill 1
            //qry52="SELECT work_id,office_code FROM RWS_WORK_ADMN_TBL W where w.plan_code ='"+planCode+"' and w.category_code='"+catCode+"' and w.ADMIN_DATE < '"+fixYear+"' and SUBSTR (w.work_id,5,2)='"+rDistrict+"' and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION >= '"+fixYear+"' or DATE_OF_COMPLETION is null)) union SELECT work_id,office_code FROM RWS_WORK_ADMN_TBL W where w.plan_code ='"+planCode+"' and w.category_code='"+catCode+"' and w.ADMIN_DATE >= '"+fixYear+"' and SUBSTR (w.work_id,5,2)='"+rDistrict+"' ";
...
<%
       ...
       if(stmt8!=null){stmt8.close();}
         ...
         String qry9=null;Statement stmt9=null;
            try
            {
				qry9 = "select sum(case when date_of_completion<'"+fixYear+"' then 1 else 0 end),sum(case when date_of_completion>='"+fixYear+"' and date_of_completion<'"+fixNextYear+"' then 1 else 0 end) from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rDistrict+"' and SUBSTR(office_code,4,1)='"+rs1.getString(1)+"' "+planQry+" and w.category_code='"+catCode+"' ";
       ...
       String qry4=null;
			...
			qry4 = "select (select sum(w.no_of_habs) from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rDistrict+"' and SUBSTR (office_code,4,1) ='"+rs1.getString(1)+"' and date_of_completion<'"+fixYear+"'  "+planQry+" and w.category_code='"+catCode+"' )as a, (select sum(w.no_of_habs) from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rDistrict+"' and SUBSTR (office_code,4,1) ='"+rs1.getString(1)+"' and date_of_completion>='"+fixYear+"' and date_of_completion<'"+fixNextYear+"'  "+planQry+" and w.category_code='"+catCode+"' )as b from dual";
			////System.out..println("qry4"+qry4);
       ...
       if(stmt4!=null){stmt4.close();}
        ...
        String qry10=null; Statement stmt10=null;
          ...
          //  qry10="Select count(e.hab_code) from RWS_WORK_COMP_HAB_LNK_TBL e where e.work_id in (select work_id from spillworks"+session.getId().substring(1,10)+" where SUBSTR(office_code,4,1)='"+rs1.getString(1)+"' union select work_id from newworks"+session.getId().substring(1,10)+" where SUBSTR(office_code,4,1)='"+rs1.getString(1)+"')";
            ////System.out..println("qry10:"+qry10);
			qry10="select (Select count(e.hab_code) from RWS_WORK_COMP_HAB_LNK_TBL e where e.work_id in (select w.work_id from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rDistrict+"' and SUBSTR (office_code,4,1) ='"+rs1.getString(1)+"' and date_of_completion<'"+fixYear+"' "+planQry+" and w.category_code='"+catCode+"' ))a, (Select count (e.hab_code) from RWS_ADMN_HAB_LNK_TBL e where e.work_id in (select w.work_id from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rDistrict+"' and SUBSTR (office_code,4,1) ='"+rs1.getString(1)+"' and date_of_completion>='"+fixYear+"' and date_of_completion<'"+fixNextYear+"'  "+planQry+" and w.category_code='"+catCode+"' ))b from dual";
			////System.out..println("qry10"+qry10);
            stmt10= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
             rs10=stmt10.executeQuery(qry10);
            while(rs10.next())
...
<td class="btext" align="right"><%=ghabcovn1%></td>
       ...
       Statement stmtdv1=null,stmtdv2=null;
		...
		String dropqry1="drop VIEW spillworks?";
		String dropqry2="drop VIEW newworks?";
		 PreparedStatement pss1=conn.prepareStatement(dropqry1);
	       PreparedStatement pss2=conn.prepareStatement(dropqry2);
	        pss1.setString(1,session.getId().substring(1,10));
			pss2.setString(1,session.getId().substring(1,10));
		pss1.executeUpdate();
		pss2.executeUpdate();
		}
		catch(SQLException se)
	...
	////System.out..println("First Drill");
	   ...
	   %>
		...
		qry1="SELECT DISTINCT m.mcode,m.mname from rws_mandal_tbl m,RWS_WORK_ADMN_TBL A,RWS_ADMN_HAB_LNK_TBL AL WHERE A.WORK_ID=AL.WORK_ID AND m.mcode=SUBSTR (AL.HAB_CODE,6,2) and SUBSTR (AL.HAB_CODE,1,2) ='"+rDistrict+"' and SUBSTR (AL.HAB_CODE,1,2)=m.dcode order BY m.mname ";
		////System.out..println("Test Qry is::"+qry1);
		stmt1 = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
		rs1=stmt1.executeQuery(qry1);
		//code modified by pranavi
		String view1="Create or Replace view spillworks? as SELECT DISTINCT W.work_id,office_code,no_of_habs,A.HAB_CODE FROM RWS_WORK_ADMN_TBL W,RWS_ADMN_HAB_LNK_TBL A where W.WORK_ID=A.WORK_ID AND w.category_code=? and w.ADMIN_DATE < ? and SUBSTR (w.work_id,5,2)=? and work_cancel_dt is null and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < ?))";
		String view2="Create or Replace view newworks? as SELECT DISTINCT W.work_id,office_code,no_of_habs,A.HAB_CODE FROM RWS_WORK_ADMN_TBL W,RWS_ADMN_HAB_LNK_TBL A where W.WORK_ID=A.WORK_ID and work_cancel_dt is null AND w.category_code=? and w.ADMIN_DATE >= ? and SUBSTR (w.work_id,5,2)=?";
		//////System.out.println("View1:"+view1);
		//////System.out.println("View2:"+view2);
		Statement stmtv1= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
	   	Statement stmtv2= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
		...
		<tr onmouseover="this.style.background='#E3E4FA'" onmouseout="this.style.background='WHITE'">
        <td class=rptValue><%=++slno%></td>
		<td class=rptValue title=Click nowrap><b><%=session.getAttribute("districtName")%></td>
		...
		String qry21 = "select (select count(*) from spillworks"+session.getId().substring(1,10)+" where substr(hab_code,6,2)='"+rs1.getString(1)+"')a,(select sum(no_of_habs) from spillworks"+session.getId().substring(1,10)+" where substr(hab_code,6,2) ='"+rs1.getString(1)+"')b,(select count(*) from spillworks"+session.getId().substring(1,10)+" a,rws_work_admn_prog_lnk_tbl b where a.work_id=b.work_id and substr(hab_code,6,2)='"+rs1.getString(1)+"')c,(select count(*) from newworks"+session.getId().substring(1,10)+" where substr(hab_code,6,2) ='"+rs1.getString(1)+"')aa,(select sum(no_of_habs) from newworks"+session.getId().substring(1,10)+" where substr (hab_code,6,2) ='"+rs1.getString(1)+"')bb,(select count(*) from newworks"+session.getId().substring(1,10)+" a,rws_work_admn_prog_lnk_tbl b where a.work_id=b.work_id and substr(hab_code,6,2) ='"+rs1.getString(1)+"')cc,(select nvl(sum(a.sanction_amount),0) from rws_work_admn_tbl a,spillworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and substr(hab_code,6,2)='"+rs1.getString(1)+"')aaa,(select nvl(sum (a.sanction_amount),0) from rws_work_admn_prog_lnk_tbl a,spillworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and substr (hab_code,6,2)='"+rs1.getString(1)+"')bbb,(select nvl(sum(a.sanction_amount),0) from rws_work_admn_tbl a,newworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and substr(hab_code,6,2)='"+rs1.getString(1)+"')aaaa,(select nvl(sum(a.sanction_amount),0) from rws_work_admn_prog_lnk_tbl a,newworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and substr(hab_code,6,2)='"+rs1.getString(1)+"')bbbb,(select count(hab_code) from spillworks"+session.getId().substring(1,10)+" where substr(hab_code,6,2)='"+rs1.getString(1)+"')aaaaa,(select count(distinct hab_code) from spillworks"+session.getId().substring(1,10)+" where substr(hab_code,6,2)='"+rs1.getString(1)+"')bbbbb,(select count(hab_code) from newworks"+session.getId().substring(1,10)+" where substr(hab_code,6,2)='"+rs1.getString(1)+"')aaaaaa,(select count(distinct hab_code) from newworks"+session.getId().substring(1,10)+" where substr (hab_code,6,2) ='"+rs1.getString(1)+"')bbbbcc from dual";
		qry21="select (select count(distinct work_id) from spillworks"+session.getId().substring(1,10)+" where substr(hab_code,6,2)='"+rs1.getString(1)+"')a,(select sum(no_of_habs) from spillworks"+session.getId().substring(1,10)+" where substr(hab_code,6,2) ='"+rs1.getString(1)+"')b,(select count(*) from spillworks"+session.getId().substring(1,10)+" a,rws_work_admn_prog_lnk_tbl b where a.work_id=b.work_id and substr(hab_code,6,2)='"+rs1.getString(1)+"')c,(select count(distinct work_id) from newworks"+session.getId().substring(1,10)+" where substr(hab_code,6,2) ='"+rs1.getString(1)+"')aa,(select sum(no_of_habs) from newworks"+session.getId().substring(1,10)+" where substr (hab_code,6,2) ='"+rs1.getString(1)+"')bb,(select count(*) from newworks"+session.getId().substring(1,10)+" a,rws_work_admn_prog_lnk_tbl b where a.work_id=b.work_id and substr(hab_code,6,2) ='"+rs1.getString(1)+"')cc,(select nvl(sum(sum(distinct a.sanction_amount)),0) from rws_work_admn_tbl a,spillworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and substr(hab_code,6,2)='"+rs1.getString(1)+"'  group by a.work_id)aaa,(select nvl(sum(sum(distinct a.sanction_amount)),0) from rws_work_admn_prog_lnk_tbl a,spillworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and substr (hab_code,6,2)='"+rs1.getString(1)+"'  group by a.work_id)bbb,(select nvl(sum(sum(distinct a.sanction_amount)),0) from rws_work_admn_tbl a,newworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and substr(hab_code,6,2)='"+rs1.getString(1)+"' group by a.work_id)aaaa,(select nvl(sum(sum(distinct a.sanction_amount)),0) from rws_work_admn_prog_lnk_tbl a,newworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and substr(hab_code,6,2)='"+rs1.getString(1)+"' group by a.work_id)bbbb,(select count(hab_code) from spillworks"+session.getId().substring(1,10)+" where substr(hab_code,6,2)='"+rs1.getString(1)+"')aaaaa,(select count(distinct hab_code) from spillworks"+session.getId().substring(1,10)+" where substr(hab_code,6,2)='"+rs1.getString(1)+"')bbbbb,(select count(hab_code) from newworks"+session.getId().substring(1,10)+" where substr(hab_code,6,2)='"+rs1.getString(1)+"')aaaaaa,(select count(distinct hab_code) from newworks"+session.getId().substring(1,10)+" where substr (hab_code,6,2) ='"+rs1.getString(1)+"')bbbbcc from dual";
		//////System.out.println("11111111:"+qry21);
		stmt21 = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
		rs21=stmt21.executeQuery(qry21);
		String aa="",bbb="";
		if(rs21.next())
		{
		gspillsn=gspillsn+rs21.getInt(1);
		gnspillsn=gnspillsn+rs21.getInt(4);
		gspillamtn=gspillamtn+spillamt;
		gnspillamtn=gnspillamtn+nspillamt;
		gtspamtn=gtspamtn+tspillamt;
		//tothabsanctioned+=rs21.getInt(2)+rs21.getInt(5);

		estamtn=rs21.getFloat(7) + rs21.getFloat(8);
		estamtn1=estamtn1+rs21.getFloat(7) + rs21.getFloat(8);
		estamts=rs21.getFloat(9) + rs21.getFloat(10);
		estamts1=estamts1+rs21.getFloat(9) + rs21.getFloat(10);
		totestamt=rs21.getFloat(7) + rs21.getFloat(8)+rs21.getFloat(9) + rs21.getFloat(10);
		totestamt1=totestamt1+totestamt;

		ghabcovn=ghabcovn+rs21.getInt(11)+rs21.getInt(13);
		ghabcovn1=ghabcovn1+rs21.getInt(12)+rs21.getInt(14);
		aa = ""+(rs21.getInt(11)+rs21.getInt(13));
		bbb =""+(rs21.getInt(12)+rs21.getInt(14));

		tothabsanctioned+=rs21.getInt(11)+rs21.getInt(13);
		 %>
			   <td class=rptValue align=right><%=rs21.getInt(1)%></td>
				<td class=rptValue align=right><%=rs21.getInt(4)%></td>
				<td class=rptValue align=right><%=rs21.getInt(1) +rs21.getInt(4) %></td>
				<td class=rptValue align=right><%=rs21.getInt(11)+rs21.getInt(13)%></td>
				<td class=rptValue align=right><%if(estamtn==0)out.println("<center>-</center>");else out.println(Math.round(estamtn *100)/100f);%></td>
				<td class=rptValue align=right><%if(estamts==0)out.println("<center>-</center>");else out.println(Math.round(estamts*100)/100f);%></td>
				<td class=rptValue align=right><%if(totestamt==0)out.println("<center>-</center>");else out.println(Math.round(totestamt*100)/100f);%></td>
		<%
          ...
          String work="";
            ...
            try
            {
			String bb = "select sum(case when year<>'"+financialyr+"' then total else 0 end),sum(case when year='"+financialyr+"' then total else 0 end) from rws_exp_view where work_id in (select work_id from spillworks"+session.getId().substring(1,10)+" where substr(hab_code,6,2) ='"+rs1.getString(1)+"' union select work_id from newworks"+session.getId().substring(1,10)+" where  substr(hab_code,6,2) ='"+rs1.getString(1)+"')";
		...
		%>
			...
			<td class=rptValue align=right><%=ndf.format(workData.round(expdur1,2))%></td>
			<td class=rptValue align=right><%=ndf.format((workData.round((preyearexp1+expdur1),2)))%></td>
			 <td class=rptValue align=right><%=aa%></td>
			 <td class=rptValue align=right><%=bbb%></td>
		<%}%>
<tr>
	<td colspan=3 class="btext">GrandTotal(Normal)</td>
	<td class="btext" align="right"><%=gspillsn%></td>
	<td class="btext" align="right"><%=gnspillsn %></td>
	<td class="btext" align="right"><%=gspillsn+gnspillsn%></td>
	<td class="btext" align="right"><%=tothabsanctioned%></td>
	...
	<td class="btext" align="right"><%=ghabcovn1%></td>
       ...
       Statement stmtdv1=null,stmtdv2=null;
		...
		String dropqry1="drop VIEW spillworks?";
		String dropqry2="drop VIEW newworks?";
		 PreparedStatement pst1=conn.prepareStatement(dropqry1);
	       PreparedStatement pst2=conn.prepareStatement(dropqry2);
	        pst1.setString(1,session.getId().substring(1,10));
			pst2.setString(1,session.getId().substring(1,10));
		pst1.executeUpdate();
		pst2.executeUpdate();
		}
		catch(SQLException se)
	...
	////System.out..println("Second Drill");
	   ...
	   %>
		...
		stmt1 = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
		rs1=stmt1.executeQuery(qry1);
		String view1="Create or Replace view spillworks?  as SELECT work_id,office_code,no_of_habs FROM RWS_WORK_ADMN_TBL W where w.category_code=? and w.ADMIN_DATE < ?  and SUBSTR (w.office_code,2,2)=? and SUBSTR(w.office_code,4,1)=? and work_cancel_dt is null and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < ?))";
		String view2="Create or Replace view newworks?  as SELECT work_id,office_code,no_of_habs FROM RWS_WORK_ADMN_TBL W where w.category_code=? and w.ADMIN_DATE >= ? and SUBSTR (w.office_code,2,2)=? and SUBSTR(w.office_code,4,1)=? and work_cancel_dt is null ";
		Statement stmtv1= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
	   	Statement stmtv2= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
       ...
       PreparedStatement ps2=conn.prepareStatement(view2);
		...
		ps1.setString(2,  catCode+" "+planQry);
		ps1.setString(3,fixYear);
		ps1.setString(4,  divisionCode);
		ps1.setString(5,fixYear);
		ps2.setString(1,session.getId().substring(1,10));
		ps2.setString(2,  catCode+" "+planQry);
		ps2.setString(3,fixYear);
		ps2.setString(4,  divisionCode);	
 	   ps1.executeUpdate();
 	   	ps2.executeUpdate();   	
	   	...
	   	while(rs21.next()&& rs22.next())
					{
 					 ...
 					 %>
 					       <td class=rptValue align=right><%=rs21.getInt(1) %></td>
							<td class=rptValue align=right><%=rs22.getInt(1) %></td>
							<% int a=rs21.getInt(1)+rs22.getInt(1) ;
						     if(a>0)
							{%>
							<td class=rptValue align=right><a href="work_list_report.jsp?district=<%=rDistrict%>&planNonplan=<%=planCode%>&drill=2&distname=<%=districtName%>&catCode=<%=catCode%>&division=<%=divisionCode%>&divname=<%=divname%>&subdivision=<%=rs1.getString(1)%>&subdivname=<%=rs1.getString(2)%>"><%=rs21.getInt(1)+rs22.getInt(1) %></a></td>
							<%}else{%>
                          <td class=rptValue align=right><%=rs21.getInt(1)+rs22.getInt(1) %></td>
						  <%}%>
							<td class=rptValue align=right><%=rs21.getInt(2)+rs22.getInt(2)%></td>
            ...
            qry44 = " select  (select sum(a.sanction_amount) from rws_work_admn_tbl a,spillworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and  SUBSTR(b.office_code,5,2)='"+rs1.getString(1)+"')a,(select sum(sanction_amount) from spillworks"+session.getId().substring(1,10)+"  a,rws_work_admn_prog_lnk_tbl b where a.work_id=b.work_id and  substr (office_code,5,2) ='"+rs1.getString(1)+"')b from dual";
           //// ////System.out.println("qry44ssssssssssss"+qry44);
            stmt44 = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
            rs44=stmt44.executeQuery(qry44);

  			qry45 = "select (select sum(a.sanction_amount) from rws_work_admn_tbl a,newworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and SUBSTR(b.office_code,5,2)='"+rs1.getString(1)+"')a,(select sum(sanction_amount) from newworks"+session.getId().substring(1,10)+"  a,rws_work_admn_prog_lnk_tbl b where a.work_id=b.work_id and  substr (office_code,5,2) ='"+rs1.getString(1)+"')b from dual";
            // // ////System.out.println("qry45"+qry45);
            stmt45 = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
            rs55=stmt45.executeQuery(qry45);
            while(rs44.next()&& rs55.next())
...
<%	
       ...
       if(stmt8!=null){stmt8.close();}
         ...
         String qry9=null;Statement stmt9=null;
            ...
            {
            //qry9="SELECT distinct SUM(CASE WHEN WC.work_id IS NOT NULL THEN 1 ELSE 0 END) FROM RWS_WORK_COMPLETION_TBL WC,RWS_WORK_ADMN_TBL W WHERE W.WORK_ID=WC.WORK_ID and wc.work_id in (select work_id from spillworks"+session.getId().substring(1,10)+" where SUBSTR(office_code,4,1)='"+divisionCode+"' and SUBSTR(office_code,5,2)='"+rs1.getString(1)+"' union select work_id from newworks"+session.getId().substring(1,10)+" where SUBSTR(office_code,4,1)='"+divisionCode+"' and SUBSTR(office_code,5,2)='"+rs1.getString(1)+"')";
          qry9 = "select sum(case when date_of_completion<'"+fixYear+"' then 1 else 0 end),sum(case when date_of_completion>='"+fixYear+"'  and date_of_completion<'"+fixNextYear+"' then 1 else 0 end) from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rDistrict+"' and SUBSTR(office_code,4,1)='"+divisionCode+"' and SUBSTR(office_code,5,2)='"+rs1.getString(1)+"' "+planQry+" and w.category_code='"+catCode+"' ";
			 stmt9= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
          // ////// ////System.out.println("qry9"+qry9);   
             rs9=stmt9.executeQuery(qry9);
            while(rs9.next())
       ...
       String qry4=null;
			...
			////System.out..println("qry4"+qry4);
			qry4 = "select (select sum(w.no_of_habs) from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rDistrict+"' and SUBSTR(office_code,4,1)='"+divisionCode+"' and SUBSTR(office_code,5,2)='"+rs1.getString(1)+"' and date_of_completion<'"+fixYear+"' "+planQry+" and w.category_code='"+catCode+"' )as a, (select sum(w.no_of_habs) from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rDistrict+"' and SUBSTR(office_code,4,1)='"+divisionCode+"' and SUBSTR(office_code,5,2)='"+rs1.getString(1)+"' and date_of_completion>='"+fixYear+"'  and date_of_completion<'"+fixNextYear+"' "+planQry+" and w.category_code='"+catCode+"' )as b from dual";
       ...
       if(stmt4!=null){stmt4.close();}
        ...
        String qry10=null; Statement stmt10=null;
           ...
           // ////// ////System.out.println("qry10:"+qry10);
		   qry10="select (Select count(e.hab_code) from RWS_WORK_COMP_HAB_LNK_TBL e where e.work_id in (select w.work_id from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rDistrict+"' and SUBSTR(office_code,4,1)='"+divisionCode+"' and SUBSTR(office_code,5,2)='"+rs1.getString(1)+"' and date_of_completion<'"+fixYear+"' "+planQry+" and w.category_code='"+catCode+"' ))a, (Select count (e.hab_code) from RWS_ADMN_HAB_LNK_TBL e where e.work_id in (select w.work_id from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rDistrict+"' and SUBSTR(office_code,4,1)='"+divisionCode+"' and SUBSTR(office_code,5,2)='"+rs1.getString(1)+"' and date_of_completion>='"+fixYear+"'  and date_of_completion<'"+fixNextYear+"' "+planQry+" and w.category_code='"+catCode+"' ))b from dual";
       ...
       Statement stmt11=null,stmt111=null;ResultSet rs11=null,rs111=null;
           ...
           // ////// ////System.out.println("qry111:"+qry111);
			...
			<%int a=tnspillw;
				if(a>0)
				{%>
			<td class="rptValue" align=right><a href="work_list_report.jsp?district=<%=rDistrict%>&planNonplan=<%=planCode%>&drill=2&distname=<%=districtName%>&catCode=<%=catCode%>&division=<%=divisionCode%>&divname=<%=divname%>&subdivision=00&subdivname="><%=tnspillw%></a></td>
			<%}else{%>
       ...
       String qry44=null;Statement stmt44=null;
            ...
            try
            {
//            qry44="SELECT SUM(CASE WHEN w.ADMIN_DATE <  '"+fixYear+"' and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION >= '01-Apr- 2008' or DATE_OF_COMPLETION is null)) AND E.TS_AMOUNT IS NOT NULL THEN TS_AMOUNT ELSE 0 END) AS EST_AMT FROM RWS_WORK_ADMN_TBL W, RWS_NEW_EST_TBL E where w.plan_code ='"+planCode+"' and w.category_code='"+catCode+"' and substr(w.office_code,2,2)='"+rs1.getString(1)+"' and W.WORK_ID=E.WORK_ID AND W.PROGRAMME_CODE=SUBSTR(E.WORK_ID,1,2)";
            qry44 = "select sum(a.sanction_amount) from rws_work_admn_tbl a,spillworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and  substr(b.office_code,5,2)='00'";
...
//          qry45="SELECT SUM(CASE WHEN w.ADMIN_DATE >= '"+fixYear+"' AND E.TS_AMOUNT IS NOT NULL THEN TS_AMOUNT ELSE 0 END) AS EST_AMT FROM RWS_WORK_ADMN_TBL W, RWS_NEW_EST_TBL E where w.plan_code ='"+planCode+"' and w.category_code='"+catCode+"' and substr(w.office_code,2,2)='"+rs1.getString(1)+"' and W.WORK_ID=E.WORK_ID AND W.PROGRAMME_CODE=SUBSTR(E.WORK_ID,1,2)";
  			qry45 = "select sum(a.sanction_amount) from rws_work_admn_tbl a,newworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and substr(b.office_code,5,2)='00'";
           ////// ////System.out.println("qry45"+qry45);
            stmt45 = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
            rs55=stmt45.executeQuery(qry45);
            while(rs44.next()&& rs55.next())
       ...
       if(stmt8!=null){stmt8.close();}
         ...
         String qry9=null;Statement stmt9=null;
            ...
            {
            //qry9="SELECT distinct SUM(CASE WHEN WC.work_id IS NOT NULL THEN 1 ELSE 0 END) FROM RWS_WORK_COMPLETION_TBL WC,RWS_WORK_ADMN_TBL W WHERE W.WORK_ID=WC.WORK_ID and wc.work_id in (select work_id from spillworks"+session.getId().substring(1,10)+" where SUBSTR(office_code,4,1)='"+divisionCode+"' and SUBSTR(office_code,5,2)='"+rs1.getString(1)+"' union select work_id from newworks"+session.getId().substring(1,10)+" where SUBSTR(office_code,4,1)='"+divisionCode+"' and SUBSTR(office_code,5,2)='"+rs1.getString(1)+"')";
          qry9 = "select sum(case when date_of_completion<'"+fixYear+"' then 1 else 0 end),sum(case when date_of_completion>='"+fixYear+"' then 1 else 0 end) from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rDistrict+"' and SUBSTR(office_code,4,1)='"+divisionCode+"' and substr(office_code,5,2)='00'"+planQry+" and w.category_code='"+catCode+"' ";
			 stmt9= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
          // ////// ////System.out.println("qry9"+qry9);   
             rs9=stmt9.executeQuery(qry9);
            while(rs9.next())
       ...
       String qry4=null;
			...
			//qry4="Select count(e.hab_code) from RWS_ADMN_HAB_LNK_TBL e where e.work_id in (select work_id from spillworks"+session.getId().substring(1,10)+" where SUBSTR(office_code,4,1)='"+divisionCode+"' and SUBSTR(office_code,5,2)='"+rs1.getString(1)+"' union select work_id from newworks"+session.getId().substring(1,10)+" where SUBSTR(office_code,4,1)='"+divisionCode+"' and SUBSTR(office_code,5,2)='"+rs1.getString(1)+"')";
			////System.out..println("qry4"+qry4);
			qry4 = "select (select sum(w.no_of_habs) from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rDistrict+"' and SUBSTR(office_code,4,1)='"+divisionCode+"' and substr(office_code,5,2)='00' and date_of_completion<'"+fixYear+"' "+planQry+" and w.category_code='"+catCode+"' )as a, (select sum(w.no_of_habs) from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rDistrict+"' and SUBSTR(office_code,4,1)='"+divisionCode+"' and substr(office_code,5,2)='00' and date_of_completion>='"+fixYear+"' "+planQry+" and w.category_code='"+catCode+"' )as b from dual";
 ...
 -->		                 <td class=rptValue align=right><%=rs4.getInt(2)%></td>
       ...
       if(stmt4!=null){stmt4.close();}
        ...
        String qry10=null; Statement stmt10=null;
            ...
            //qry10="Select count(e.hab_code) from RWS_WORK_COMP_HAB_LNK_TBL e where e.work_id in (select work_id from spillworks"+session.getId().substring(1,10)+" where SUBSTR(office_code,4,1)='"+divisionCode+"' and SUBSTR(office_code,5,2)='"+rs1.getString(1)+"' union select work_id from newworks"+session.getId().substring(1,10)+" where SUBSTR(office_code,4,1)='"+divisionCode+"' and SUBSTR(office_code,5,2)='"+rs1.getString(1)+"')";
           // ////// ////System.out.println("qry10:"+qry10);
		   qry10="select (Select count(e.hab_code) from RWS_WORK_COMP_HAB_LNK_TBL e where e.work_id in (select w.work_id from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rDistrict+"' and SUBSTR(office_code,4,1)='"+divisionCode+"' and substr(office_code,5,2)='00' and date_of_completion<'"+fixYear+"' "+planQry+" and w.category_code='"+catCode+"' ))a, (Select count (e.hab_code) from RWS_ADMN_HAB_LNK_TBL e where e.work_id in (select w.work_id from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rDistrict+"' and SUBSTR(office_code,4,1)='"+divisionCode+"' and substr(office_code,5,2)='00' and date_of_completion>='"+fixYear+"' "+planQry+" and w.category_code='"+catCode+"' ))b from dual";
