...
if(drill.equals("null") || drill.equals("0"))
	...
	////System.out..println("First Drill");
	   ...
	   %>
		...
		String view1="Create or Replace view spillworks? as SELECT work_id,office_code,no_of_habs FROM RWS_WORK_ADMN_TBL W where w.category_code=? and w.ADMIN_DATE < ? and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < ?))";
		String view2="Create or Replace view newworks?  as SELECT work_id,office_code,no_of_habs FROM RWS_WORK_ADMN_TBL W where w.category_code=? and w.ADMIN_DATE >= ? ";
	   ...
	   //// //System.out.println("view2"+view2);
		...
		<td class=rptValue title=Click><b><a href="rws_work_proj_abstract_rpt1.jsp?district=<%=rs1.getString(1)%>&planNonplan=<%=planCode%>&drill=1&catCode=<%=catCode%>&distname=<%=rs1.getString(2)%>"><%=rs1.getString(2)%></a></td>
      ...
      if(rs44!=null){rs44.close();}
       ...
       String qry4=null;
			...
			qry4 = "select (select sum(w.no_of_habs) from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rs1.getString(1)+"' and date_of_completion<'"+fixYear+"' "+planQry+" and w.category_code='"+catCode+"' )as a, (select sum(w.no_of_habs) from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rs1.getString(1)+"'  and date_of_completion>='"+fixYear+"' and date_of_completion<'"+fixNextYear+"' "+planQry+" and w.category_code='"+catCode+"' )as b from dual";
            ...
            while(rs4.next())
				...
				ghabsann1=ghabsann1+rs4.getInt(2);
	             //System.out.println("hab:"+rs4.getInt(1)+"ddd:"+rs4.getInt(2));
           ...
           // qry10="Select count(e.hab_code) from RWS_WORK_COMP_HAB_LNK_TBL e where e.work_id in (select work_id from spillworks"+session.getId().substring(1,10)+" where substr(work_id,5,2)='"+rs1.getString(1)+"' union select work_id from newworks"+session.getId().substring(1,10)+" where substr(work_id,5,2)='"+rs1.getString(1)+"')";
			...
			%>
			 <!-- <td class=rptValue align=right><%=rs10.getInt(1)%></td> -->
 			 <td class=rptValue align=right><%=rs10.getInt(2)%>bbbbbbb</td>
			<%
			}}catch(Exception e1)
       {
       e1.printStackTrace();
...
<td class="btext" align="right"><%=ghabcovn1%></td>
	...
	////System.out..println("First Drill");
	   ...
	   %>
		...
		String view1="Create or Replace view spillworks?  as SELECT work_id,office_code,no_of_habs FROM RWS_WORK_ADMN_TBL W where w.category_code=? and w.ADMIN_DATE < ? and SUBSTR (w.work_id,5,2)=? and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < ?))";
		String view2="Create or Replace view newworks?  as SELECT work_id,office_code,no_of_habs FROM RWS_WORK_ADMN_TBL W where w.category_code=? and w.ADMIN_DATE >= ? and SUBSTR (w.work_id,5,2)=? ";
		////// //System.out.println("View1:"+view1);
		////// //System.out.println("View2:"+view2);
		Statement stmtv1= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
	   	Statement stmtv2= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
...
<td class="btext" align="right"><%=ghabcovn1%></td>
	...
	////System.out..println("Second Drill");
	   ...
	   %>
		...
		stmt1 = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
		rs1=stmt1.executeQuery(qry1);
		String view1="Create or Replace view spillworks? as SELECT work_id,office_code,no_of_habs FROM RWS_WORK_ADMN_TBL W where w.category_code=? and w.ADMIN_DATE < ?  and SUBSTR (w.office_code,2,2)=? and SUBSTR(w.office_code,4,1)=? and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION < ?))";
		String view2="Create or Replace view newworks?  as SELECT work_id,office_code,no_of_habs FROM RWS_WORK_ADMN_TBL W where w.category_code=? and w.ADMIN_DATE >= ? and SUBSTR (w.office_code,2,2)=? and SUBSTR(w.office_code,4,1)=?";
		Statement stmtv1= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
	   	Statement stmtv2= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
	   	...
	   	while(rs21.next()&& rs22.next())
					{
 					 ...
 					 %>
 					       <td class=rptValue align=right><%=rs21.getInt(1) %></td>
							<td class=rptValue align=right><%=rs22.getInt(1) %></td>
							<td class=rptValue align=right><a href="work_list_report.jsp?district=<%=rDistrict%>&planNonplan=<%=planCode%>&drill=2&distname=<%=districtName%>&catCode=<%=catCode%>&division=<%=divisionCode%>&divname=<%=divname%>&subdivision=<%=rs1.getString(1)%>&subdivname=<%=rs1.getString(2)%>"><%=rs21.getInt(1)+rs22.getInt(1) +  rs21.getInt(3)+  rs22.getInt(3)%></a></td>
							<td class=rptValue align=right><%=rs21.getInt(2)+rs22.getInt(2)%></td>
					<%
	   	}
