...
<table border = 1 cellspacing = 0 cellpadding = 0 height=5% width="100%" bordercolor=#000000 style="border-collapse:collapse" bgcolor="#ffffff" align = "center">
	...
	<input type="hidden" name="planCode" value="<%=planNonplan%>";
	...
	////System.out..println("First Drill");
	   ...
	   %>
		...
		<%if(rs1.getInt(1)==1){%>
		<td class=rptValue title=Click><b><a href="rws_work_proj_abstract_rpt2.jsp?district=<%=rs1.getString(1)%>&planNonplan=<%=planNonplan%>&catCode=1&finyear=<%=financialyr%>&distname=<%=rs1.getString(2)%>"><%=rs1.getString(2)%></a></td>
		<%}
		else if(rs1.getInt(1)==2){%>
		<td class=rptValue title=Click><b><a href="rws_work_proj_abstract_rpt2.jsp?district=<%=rs1.getString(1)%>&planNonplan=<%=planNonplan%>&catCode=2&finyear=<%=financialyr%>&distname=<%=rs1.getString(2)%>"><%=rs1.getString(2)%></a></td>
		<%}
		else if(rs1.getInt(1)==3){%>
		<td class=rptValue title=Click><b><a href="rws_work_proj_abstract_rpt2.jsp?district=<%=rs1.getString(1)%>&planNonplan=<%=planNonplan%>&catCode=3&finyear=<%=financialyr%>&distname=<%=rs1.getString(2)%>"><%=rs1.getString(2)%></a></td>
		<%}
		else%>
	   	<% 
	   	String qry2=null;Statement stmt2=null;
		    ...
		    String qry21="select (select count(*) from spillworks"+session.getId().substring(1,10)+" where category_code='"+rs1.getString(1)+"' "+planQry+")a,(select sum(no_of_habs) from spillworks"+session.getId().substring(1,10)+" where category_code='"+rs1.getString(1)+"' "+planQry+")b from dual";	
			String qry22 = "select (select count(*) from newworks"+session.getId().substring(1,10)+" where category_code='"+rs1.getString(1)+"' "+planQry+")a,(select sum(no_of_habs) from newworks"+session.getId().substring(1,10)+" where category_code='"+rs1.getString(1)+"' "+planQry+")b from dual";
...
//            qry44="SELECT SUM(CASE WHEN w.ADMIN_DATE <  '"+fixYear+"' and w.work_id not in (select work_id from rws_work_completion_tbl where (DATE_OF_COMPLETION >= '01-Apr- 2008' or DATE_OF_COMPLETION is null)) AND E.TS_AMOUNT IS NOT NULL THEN TS_AMOUNT ELSE 0 END) AS EST_AMT FROM RWS_WORK_ADMN_TBL W, RWS_NEW_EST_TBL E where w.plan_code ='1' and w.category_code='2' and substr(w.office_code,2,2)='"+rs1.getString(1)+"' and W.WORK_ID=E.WORK_ID AND W.PROGRAMME_CODE=SUBSTR(E.WORK_ID,1,2)";
            qry44 = "select sum(a.sanction_amount) from rws_work_admn_tbl a,spillworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and a.category_code='"+rs1.getString(1)+"' "+planQry1+"";
...
//          qry45="SELECT SUM(CASE WHEN w.ADMIN_DATE >= '"+fixYear+"' AND E.TS_AMOUNT IS NOT NULL THEN TS_AMOUNT ELSE 0 END) AS EST_AMT FROM RWS_WORK_ADMN_TBL W, RWS_NEW_EST_TBL E where w.plan_code ='1' and w.category_code='2' and substr(w.office_code,2,2)='"+rs1.getString(1)+"' and W.WORK_ID=E.WORK_ID AND W.PROGRAMME_CODE=SUBSTR(E.WORK_ID,1,2)";
  			qry45 = "select sum(a.sanction_amount) from rws_work_admn_tbl a,newworks"+session.getId().substring(1,10)+" b where a.work_id=b.work_id and a.category_code='"+rs1.getString(1)+"' "+planQry1+"";
	...
	////System.out..println("First Drill");
	   ...
	   %>
		...
		Statement stmtv1= conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
	   	...
	   	PreparedStatement ps4=conn.prepareStatement(view2);
	    ps3.setString(1,session.getId().substring(1,10));
	    ps3.setString(fixYear);
	    ps3.setString(rDistrict);
	    ps3.setString( fixYear);
	    ps4.setString(1,session.getId().substring(1,10));
	    ps4.setString(fixYear);
	    ps4.setString(rDistrict);
       ...
       Statement stmtdv1=null,stmtdv2=null;
		...
		String dropqry2="drop VIEW newworks?";
		 ...
		 PreparedStatement ps1=conn.prepareStatement(dropqry1);
		  ...
		  ps1.setString(parameterIndex, session.getId().substring(1,10));
		  ps2.setString(parameterIndex, session.getId().substring(1,10));
		ps1.executeUpdate();
		ps2.executeUpdate();
		}
		catch(SQLException se)
