	...
	<form name=f1 method="post">
		...
		<center>
			<font color="#663899" face="Maiandra GD" size="4px"><b>Physical
					(Coverage of Habitations) and Financial Achievements - (<%=yearStart %>-<%=yearEnd %>) <br/><br/> Programme: <%=programmeName %></b></font> <br />
		...
		<table border=1 cellspacing=0 cellpadding=0 height=5% width="1050px"
			...
			<%
				...
				try {
					...
					String districtQuery="select dcode,INITCAP(dname) as dname from rws_district_tbl order by dcode";
					Statement statement1 = conn.createStatement();
					ResultSet resultSet1 = statement1.executeQuery(districtQuery);
					
					while (resultSet1.next()) {
						dcode=resultSet1.getString("dcode");
						dname=resultSet1.getString("dname");
						...
						String targetHabsQ1 = "select count(b.hab_code) from rws_work_admn_tbl a,RWS_ADMN_HAB_LNK_TBL b,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m  where WORK_CANCEL_DT is null  "
								...
								+ programme
								+ ") and substr(a.work_id,5,2)='"+dcode+"' and c.GROUNDING_DATE is not null  and substr(a.work_id,7,2) in ("
								+ schemes
						...
						String achievementHabsQ1 = "select count(h.hab_code) from rws_work_admn_tbl a,rws_work_completion_tbl b,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m,RWS_WORK_COMP_HAB_LNK_TBL h  where WORK_CANCEL_DT is null "
								...
								+ programme
								+ ") and substr(a.work_id,5,2)='"+dcode+"' and b.DATE_OF_COMPLETION between '01-APR-"
								+ yearStart
								+ "' and '30-JUN-"
								+ yearStart
						...
						String achievementHabsQ2 = "select count(h.hab_code) from rws_work_admn_tbl a,rws_work_completion_tbl b,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m,RWS_WORK_COMP_HAB_LNK_TBL h  where WORK_CANCEL_DT is null "
								+ "   and h.work_id=b.work_id and b.work_id=a.work_id and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id=a.work_id and m.work_id=i.work_id  and a.programme_code in ("
								+ programme
								+ ") and substr(a.work_id,5,2)='"+dcode+"' and b.DATE_OF_COMPLETION between '01-JUL-"
								+ yearStart
								+ "' and '30-SEP-"
								+ yearStart
								+ "' and b.DATE_OF_COMPLETION is not null  and c.CORE_DASHBOARD_STATUS<>'N'";
						...
						String achievementHabsQ3 = "select count(h.hab_code) from rws_work_admn_tbl a,rws_work_completion_tbl b,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m,RWS_WORK_COMP_HAB_LNK_TBL h  where WORK_CANCEL_DT is null "
								+ "   and h.work_id=b.work_id and b.work_id=a.work_id and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id=a.work_id and m.work_id=i.work_id  and a.programme_code in ("
								+ programme
								+ ") and substr(a.work_id,5,2)='"+dcode+"' and b.DATE_OF_COMPLETION between '01-OCT-"
								+ yearStart
								+ "' and '31-DEC-"
								+ yearStart
								+ "' and b.DATE_OF_COMPLETION is not null  and c.CORE_DASHBOARD_STATUS<>'N'";
						...
						String achievementHabsQ4 = "select count(h.hab_code) from rws_work_admn_tbl a,rws_work_completion_tbl b,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m,RWS_WORK_COMP_HAB_LNK_TBL h  where WORK_CANCEL_DT is null "
								+ "   and h.work_id=b.work_id and b.work_id=a.work_id and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id=a.work_id and m.work_id=i.work_id  and a.programme_code in ("
								+ programme
								+ ") and substr(a.work_id,5,2)='"+dcode+"' and b.DATE_OF_COMPLETION between '01-JAN-"
								+ yearEnd
								+ "' and '31-MAR-"
								+ yearEnd
								+ "' and b.DATE_OF_COMPLETION is not null  and c.CORE_DASHBOARD_STATUS<>'N'";
						...
						//****************Total Exp FY Wise***************//
						String totExpBankDurQuery="select sum(BILL_AMOUNT_CENTRAL) from RWS_BILL_BANK_BRIDGE_DET_TBL a,rws_work_admn_tbl b WHERE a.work_id=b.work_id and substr(a.work_id,5,2)='"+dcode+"' and BANK_SEND_DATE BETWEEN '01-APR-"+yearStart+"' AND '31-MAR-"+yearEnd+"'  and b.programme_code in ("+ programme+ ")";
						stmt1 = conn.createStatement();
						...
						String totExpPaoDurQuery="select sum(BILL_AMOUT_STATE) from RWS_BILL_PAO_BRIDGE_DET_TBL a,rws_work_admn_tbl b WHERE a.work_id=b.work_id and substr(a.work_id,5,2)='"+dcode+"' and STATE_SEND_DATE BETWEEN '01-APR-"+yearStart+"' AND '31-MAR-"+yearEnd+"'  and b.programme_code in ("+ programme+ ")";
						stmt1 = conn.createStatement();
						...
						String totExpBankPreQuery="select sum(BILL_AMOUNT_CENTRAL) from RWS_BILL_BANK_BRIDGE_DET_TBL a,rws_work_admn_tbl b WHERE a.work_id=b.work_id and substr(a.work_id,5,2)='"+dcode+"' and BANK_SEND_DATE BETWEEN '01-APR-"+(yearStart-1)+"' AND '"+pDate+"'  and b.programme_code in ("+ programme+ ")";
						stmt1 = conn.createStatement();
						...
						String totExpPaoPreQuery="select sum(BILL_AMOUT_STATE) from RWS_BILL_PAO_BRIDGE_DET_TBL a,rws_work_admn_tbl b WHERE a.work_id=b.work_id and substr(a.work_id,5,2)='"+dcode+"' and STATE_SEND_DATE BETWEEN '01-APR-"+(yearStart-1)+"' AND '"+pDate+"'  and b.programme_code in ("+ programme+ ")";
						stmt1 = conn.createStatement();
			...
			<tr>
				...
				<td class="<%=style%>" style="text-align: left;" align="left" ><%=dname%></td>
				<% if(habsSacQ1>0){%>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_habs_covered_drill_rpt.jsp?mode=sHabs&programme=<%=programme%>&yearStart=01-APR-<%=yearStart%>&yearEnd=30-JUN-<%=yearStart%>&message=<%=dname%> District <%=programmeName%> Quarter-1 Target&dcode=<%=dcode%>')"><%=habsSacQ1%></a></td>
				<%}else{ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><%=habsSacQ1%></td>
				<%} if(habsCovQ1>0){%>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_habs_covered_drill_rpt.jsp?mode=cHabs&programme=<%=programme%>&yearStart=01-APR-<%=yearStart%>&yearEnd=30-JUN-<%=yearStart%>&message=<%=dname%> District <%=programmeName%> Quarter-1 Achievement&dcode=<%=dcode%>')"><%=habsCovQ1%></a></td>
				<%}else{ %>
				...
				<% if(prehabsCovQ1>0){%>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_habs_covered_drill_rpt.jsp?mode=preHabs&programme=<%=programme%>&yearStart=01-APR-<%=(yearStart-1)%>&yearEnd=30-JUN-<%=(yearStart-1)%>&message=<%=dname%> District <%=programmeName%> Quarter-1 Previous FY Achievement&dcode=<%=dcode%>')"><%=prehabsCovQ1%></a></td>
				<%}else{ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><%=prehabsCovQ1%></td>
				<%} if(habsSacQ2>0){%>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_habs_covered_drill_rpt.jsp?mode=sHabs&programme=<%=programme%>&yearStart=01-JUL-<%=yearStart%>&yearEnd=30-SEP-<%=yearStart%>&message=<%=dname%> District <%=programmeName%> Quarter-2 Target&dcode=<%=dcode%>')"><%=habsSacQ2%></a></td>
				<%}else{ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><%=habsSacQ2%></td>
				<%} if(habsCovQ2>0){%>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_habs_covered_drill_rpt.jsp?mode=cHabs&programme=<%=programme%>&yearStart=01-JUL-<%=yearStart%>&yearEnd=30-SEP-<%=yearStart%>&message=<%=dname%> District <%=programmeName%> Quarter-2 Achievement&dcode=<%=dcode%>')"><%=habsCovQ2%></a></td>
				<%}else{ %>
				...
				<% if(prehabsCovQ2>0){%>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_habs_covered_drill_rpt.jsp?mode=preHabs&programme=<%=programme%>&yearStart=01-JUL-<%=(yearStart-1)%>&yearEnd=30-SEP-<%=(yearStart-1)%>&message=<%=dname%> District <%=programmeName%> Quarter-2 Previous FY Achievement&dcode=<%=dcode%>')"><%=prehabsCovQ2%></a></td>
				<%}else{ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><%=prehabsCovQ2%></td>
				<%} if(habsSacQ3>0){%>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_habs_covered_drill_rpt.jsp?mode=sHabs&programme=<%=programme%>&yearStart=01-OCT-<%=yearStart%>&yearEnd=31-DEC-<%=yearStart%>&message=<%=dname%> District <%=programmeName%> Quarter-3 Target&dcode=<%=dcode%>')"><%=habsSacQ3%></a></td>
				<%}else{ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><%=habsSacQ3%></td>
				<%} if(habsCovQ3>0){%>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_habs_covered_drill_rpt.jsp?mode=cHabs&programme=<%=programme%>&yearStart=01-OCT-<%=yearStart%>&yearEnd=31-DEC-<%=yearStart%>&message=<%=dname%> District <%=programmeName%> Quarter-3 Achievement&dcode=<%=dcode%>')"><%=habsCovQ3%></a></td>
				<%}else{ %>
				...
				<% if(prehabsCovQ3>0){%>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_habs_covered_drill_rpt.jsp?mode=preHabs&programme=<%=programme%>&yearStart=01-OCT-<%=(yearStart-1)%>&yearEnd=31-DEC-<%=(yearStart-1)%>&message=<%=dname%> District <%=programmeName%> Quarter-3 Previous FY Achievement&dcode=<%=dcode%>')"><%=prehabsCovQ3%></a></td>
				<%}else{ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><%=prehabsCovQ3%></td>
				<%} if(habsSacQ4>0){%>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_habs_covered_drill_rpt.jsp?mode=sHabs&programme=<%=programme%>&yearStart=01-JAN-<%=yearEnd%>&yearEnd=31-MAR-<%=yearEnd%>&message=<%=dname%> District <%=programmeName%> Quarter-4 Target&dcode=<%=dcode%>')"><%=habsSacQ4%></a></td>
				<%}else{ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><%=habsSacQ4%></td>
				<%} if(habsCovQ4>0){%>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_habs_covered_drill_rpt.jsp?mode=cHabs&programme=<%=programme%>&yearStart=01-JAN-<%=yearEnd%>&yearEnd=31-MAR-<%=yearEnd%>&message=<%=dname%> District <%=programmeName%> Quarter-4 Achievement&dcode=<%=dcode%>')"><%=habsCovQ4%></a></td>
				<%}else{ %>
				...
				<% if(prehabsCovQ4>0){%>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_habs_covered_drill_rpt.jsp?mode=preHabs&programme=<%=programme%>&yearStart=01-JAN-<%=(yearEnd-1)%>&yearEnd=31-MAR-<%=(yearEnd-1)%>&message=<%=dname%> District <%=programmeName%> Quarter-4 Previous FY Achievement&dcode=<%=dcode%>')"><%=prehabsCovQ4%></a></td>
				<%}else{ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><%=prehabsCovQ4%></td>
				<%} if(habsSacTot>0){%>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_habs_covered_drill_rpt.jsp?mode=sHabs&programme=<%=programme%>&yearStart=01-APR-<%=yearStart%>&yearEnd=31-MAR-<%=yearEnd%>&message=<%=dname%> District <%=programmeName%> Total Target&dcode=<%=dcode%>')"><%=habsSacTot%></a></td>
				<%}else{ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><%=habsSacTot%></td>
				<%} if(habsCovTot>0){%>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_habs_covered_drill_rpt.jsp?mode=cHabs&programme=<%=programme%>&yearStart=01-APR-<%=yearStart%>&yearEnd=31-MAR-<%=yearEnd%>&message=<%=dname%> District <%=programmeName%> Total Achievement&dcode=<%=dcode%>')"><%=habsCovTot%></a></td>
				<%}else{ %>
				...
				<% if(prehabsCovTot>0){%>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_habs_covered_drill_rpt.jsp?mode=preHabs&programme=<%=programme%>&yearStart=01-APR-<%=(yearStart-1)%>&yearEnd=31-MAR-<%=(yearEnd-1)%>&message=<%=dname%> District <%=programmeName%> Total Previous FY Achievement&dcode=<%=dcode%>')"><%=prehabsCovTot%></a></td>
				<%}else{ %>
