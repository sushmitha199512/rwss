...
String actionTaken=(String)session.getAttribute("actionTaken");
String yn="";
...
<td class=gridhdbg><b>Message Sent</b></td>
	...
	try{
		...
		if(actionTaken.equals("ALL")){
			 query="select COMPLAINT_NO,to_char(COMPLAINT_DATE,'dd/mm/yyyy'),COMPLAINT_DESC,NAME,ADDRESS,PHONE_NO,ATTENDED_BY,to_char(ATTENDED_DATE,'dd/mm/yyyy'),ACTION_DESC,MESSAGE_SENT,nvl(GRIEVANCE_RELATED_TO,'-'),nvl(GRIEVANCE_CATEGORY,'-'),nvl(GRIEVANCE_ENTRY_CATEGORY,'-'),nvl(EMAILID,'-') from rws_generic_complaints_reg_tbl where ACTION_TAKEN in('transportation','Hiring of Source','Flushing/Deepening','Scheme Repaired','Scheme Repair pending','Proposal Stage','No DW Problem','HPs repaired','Pipelines rectified','Complaint not factual','GP For Maintenance') and substr(HABITATION_CODE,1,2)='"+dcode+"' and COMPLAINT_DATE between to_date('"+fromDate+"','dd/mm/yyyy')  and  to_date('"+toDate+"','dd/mm/yyyy') and ATTENDED_DATE is NOT NULL";
		}
		else{
		 query="select COMPLAINT_NO,to_char(COMPLAINT_DATE,'dd/mm/yyyy'),COMPLAINT_DESC,NAME,ADDRESS,PHONE_NO,ATTENDED_BY,to_char(ATTENDED_DATE,'dd/mm/yyyy'),ACTION_DESC,MESSAGE_SENT,nvl(GRIEVANCE_RELATED_TO,'-'),nvl(GRIEVANCE_CATEGORY,'-'),nvl(GRIEVANCE_ENTRY_CATEGORY,'-'),nvl(EMAILID,'-') from rws_generic_complaints_reg_tbl where ACTION_TAKEN='"+actionTaken+"' and substr(HABITATION_CODE,1,2)='"+dcode+"' and COMPLAINT_DATE between to_date('"+fromDate+"','dd/mm/yyyy')  and  to_date('"+toDate+"','dd/mm/yyyy') and ATTENDED_DATE is NOT NULL";
		}
...
<td class="<%=style %>" style="text-align: left;" align="right"><%=resultSet.getString(11) %></td>
...
<td class="<%=style %>" style="text-align: left;" align="right"><%=resultSet.getString(4) %>/<br><%=resultSet.getString(5) %>/<br><%=resultSet.getString(6) %>/<br><%=resultSet.getString(14) %></td>
...
<%
				...
				else if(resultSet.getString(10).equals("Y")){
					yn="Yes";
				}
				else if(resultSet.getString(10).equals("N")){
					yn="No";
				}
