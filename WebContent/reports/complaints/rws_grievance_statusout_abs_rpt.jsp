...
<tr><td align="right" class="bwborder"><a href="javascript:onclick=history.go(-1)">Back&nbsp;|</a></td>
			...
			<td><a href="./rws_grievance_status_excel.jsp">Excel</a></td> </tr>
...
try
{
  ...
  if(rs3.next())
   ...
   %>
     <td class=rptValue><a href="#" onclick="viewWorks('rws_grievance_statusout_drill.jsp?dcode=<%=dcode1%>&dname=<%=rs8.getString(2)%>&date=<%=rs2.getString(1)%>')"><%=compaintno%></a></td>
     <td class=rptValue><a href="#" onclick="viewWorks('rws_grievance_statusout_drill1.jsp?dcode=<%=dcode1%>&dname=<%=rs8.getString(2)%>&date=<%=rs2.getString(1)%>')"><%=attendedno%></a></td>
       <%
     }
  else{
	  compaintno=0;
...
<%			
  ...
  attendedno1+=attendedno;
	...
	<td class=rptValue><a href="#" onclick="viewWorks('rws_grievance_statusout_drill2.jsp?date=<%=rs2.getString(1)%>')"><%=gcompaintno1%></a></td>
     <td class=rptValue><a href="#" onclick="viewWorks('rws_grievance_statusout_drill3.jsp?date=<%=rs2.getString(1)%>')"><%=attendedno1%></a></td>   
	</tr>
