...
String qu1="SELECT DISTINCT m.mcode,m.mname as MANDAL,sum(Case When SUBSTR(S.SCHEME_CODE,17,3) LIKE '6%' then 1 else 0 END)AS PWS,sum(Case When SUBSTR(S.SCHEME_CODE,17,3) LIKE '8%' then 1 else 0 END)AS CPWS,sum(Case When SUBSTR(S.SCHEME_CODE,17,3) LIKE '4%' then 1 else 0 END)AS MPWS,sum(Case When SUBSTR(S.SCHEME_CODE,17,3) LIKE '7%' then 1 else 0 END)AS PONDS,sum(Case When SUBSTR(S.SCHEME_CODE,17,3) LIKE '2%' then 1 else 0 END)AS OPENWELLS,sum(Case When SUBSTR(S.SCHEME_CODE,17,3) LIKE '0%' then 1 else 0 END)AS HANDPUMPS,sum(Case When SUBSTR(S.SCHEME_CODE,17,3) LIKE '3%' then 1 else 0 END)AS SHALLOWHNDPUMPS  FROM RWS_ASSET_MAST_TBL M,RWS_ASSET_SCHEME_TBL S,rws_mandal_tbl m WHERE M.ASSET_CODE=S.ASSET_CODE and substr(M.hab_code,1,2)=m.dcode and substr(M.hab_code,6,2)=m.mcode and substr(M.hab_code,1,2)='"+dcode+"' group by  m.mcode,m.mname";
...
%>
		...
		<td class=rptValue>
			<a href="rws_panch_asset_status_no_rpt.jsp?mcode=<%=rs6.getString(1)%>&mname=<%=rs6.getString(2)%>&dcode=<%=dcode%>"><%=rs6.getString(2)%></a></td>
