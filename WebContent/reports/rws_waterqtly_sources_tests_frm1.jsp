...
function fnSubmit()
	{   
		...
		else if(document.f1.typeofsource.value==" ")
		{
		...
		{
			document.f1.action="./rws_newwaterqtly_sources_rpt.jsp";
			document.f1.submit();
		}
	}
function fnRefresh(str) {
	UnTip();
	document.forms[0].elements['refresh'].value="Refreshing Please Wait.....";
	document.forms[0].elements['refresh'].disabled=true;
	  ...
	  xhttp.onreadystatechange = function() {
	    if (xhttp.readyState == 4 && xhttp.status == 200) {
				...
				res=removeNL(res);
				alert(res);
	    	 	document.forms[0].elements['refresh'].value="Refresh";
				document.forms[0].elements['refresh'].disabled=false;
			   
	    //document.getElementById("txtHint").innerHTML = xhttp.responseText;
	    }
	  };
	  xhttp.open("GET", "rws_refresh_wq_data.jsp?status="+str, true);
	  xhttp.send();
	}
	...
	<table  border=0 rules=none style="border-collapse:collapse" align = "right" bgcolor="#edf2f8">
		...
		<tr bgcolor="#8A9FCD" align="center">
			<td align="center" class="textborder" colspan="2">
				<font color="#FFFFFF">WaterQuality Sources Report</font>
			</td>	
		</tr>
<%
	String rDist = null;
	...
	if(loggedUser.substring(0,3).equals("col")){
		rDist=(String)session.getAttribute("district");
	}
	//System.out.println("district:"+rDist);
...
//	System.out.println("mcode:"+rDiv);
	...
	source=request.getParameter("typeofsource");
%>
...
<%
				...
				stmt = conn.createStatement();
				 ...
				 query = "SELECT distinct sc.circle_office_code, c.circle_office_name from rws_circle_office_tbl c, rws_water_sample_collect_tbl sc  where c.circle_OFFICE_CODE = sc.circle_OFFICE_CODE  ";
				 if(loggedUser.substring(0,3).equals("col")){
					 query+=" and sc.circle_office_code='"+rDist+"'" ;
				 }
				 query+=" order by c.circle_OFFICE_name";
...
<tr>
 <td class = btext ><font color=navy><b>Month</b> </font>
     ...
     String qry1=" select distinct to_char(TEST_DATE ,'MON'),to_number(to_char(test_date,'mm')) from rws_ftk_testing_tbl where TEST_DATE is not null union select distinct to_char (samp_collect_date,'MON'),to_number(to_char(samp_collect_date,'mm')) from rws_water_sample_collect_tbl where samp_collect_date is not null order by 2 ";
  //System.out.println("Date"+RwsUtils.getCurrentDate("dd/MMM/yy"));  
//23/Nov/12
...
<td class = btext ><font color=navy><b>Type Of Sources</b> </font>
		
		</td>
		<td class = btext colspan=3><SELECT name="typeofsource" style="width:150px" class="mycombo" >

		<!-- <option value="00">SELECT...</option> -->
	 	<option value="51"> ALL</option>  
	 	<option value="01"> Scheme Sources</option>  
	...
	<tr>
		<td colspan=2 align=center>
			...
			<input type=button  id="refresh" onclick="fnRefresh('lab')" value="Refresh" title="Refresh Data" class = btext onmouseover="Tip(this.value,TITLE,'Click Here To Refresh Water Quality Data')"  onmouseout="UnTip()">
