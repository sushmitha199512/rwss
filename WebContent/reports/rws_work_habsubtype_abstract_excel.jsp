...
try
{
	...
	if(per==null)per="ALL";
	String perQry = "";
	if(per!=null && !per.equals("ALL"))perQry = " and habitation_sub_type='"+per+"'";
%>
<html>
<head>
<!-- 
...
//	   System.out.println("query1"+qry21);
	   //System.out.println("query2"+qry22);
	   	...
	   	while(rs21.next()&& rs22.next())
			...
			if(!per.equals("ALL"))
			{
				...
				String qqqq = "select sum(case when habitation_sub_type='"+per+"' then census_plain_popu+census_sc_popu+census_st_popu else 0 end) as scpop,sum(census_plain_popu+census_sc_popu+census_st_popu) from spillworks"+session.getId().substring(1,10)+" a,rws_admn_hab_lnk_tbl b,rws_habitation_directory_tbl c where a.work_id=b.work_id and b.hab_code=c.hab_code and substr(a.work_id,5,2)='"+rs1.getString(1)+"'";
				String qqqq1 = "select sum(case when habitation_sub_type='"+per+"' then census_plain_popu+census_sc_popu+census_st_popu else 0 end) as scpop,sum(census_plain_popu+census_sc_popu+census_st_popu) from newworks"+session.getId().substring(1,10)+" a,rws_admn_hab_lnk_tbl b,rws_habitation_directory_tbl c where a.work_id=b.work_id and b.hab_code=c.hab_code and substr(a.work_id,5,2)='"+rs1.getString(1)+"'";
       ...
       if(stmt8!=null){stmt8.close();}
         ...
         String qry9=null;Statement stmt9=null;
			...
			qry9 = "select sum(case when date_of_completion<'"+fixYear+"' then 1 else 0 end),sum(case when date_of_completion>='"+fixYear+"' and date_of_completion<'"+fixNextYear+"' then 1 else 0 end) from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rs1.getString(1)+"' "+planQry1+"";
       ...
       if(stmt4!=null){stmt4.close();}
        ...
        String qry10=null; Statement stmt10=null;
           ...
           // qry10="Select count(e.hab_code) from RWS_WORK_COMP_HAB_LNK_TBL e where e.work_id in (select work_id from spillworks"+session.getId().substring(1,10)+" where substr(work_id,5,2)='"+rs1.getString(1)+"' union select work_id from newworks"+session.getId().substring(1,10)+" where substr(work_id,5,2)='"+rs1.getString(1)+"')";
            ...
            qry10="select (Select count(e.hab_code) from RWS_WORK_COMP_HAB_LNK_TBL e where e.work_id in (select w.work_id from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rs1.getString(1)+"' and date_of_completion<'"+fixYear+"' "+planQry1+"))a, (Select count (e.hab_code) from RWS_ADMN_HAB_LNK_TBL e where e.work_id in (select w.work_id from rws_work_completion_tbl wc, rws_work_admn_tbl w where w.work_id=wc.work_id and SUBSTR (w.work_id,5,2)='"+rs1.getString(1)+"' and date_of_completion>='"+fixYear+"' and date_of_completion<'"+fixNextYear+"' "+planQry1+"))b from dual";
			////System.out.println("qry10:"+qry10);
...
<td class="btext" align="right"><%=ghabcovn1%></td>
       ...
       Statement stmtdv1=null,stmtdv2=null;
		...
		PreparedStatement psm11=conn.prepareStatement(dropqry1);
	       PreparedStatement psm12=conn.prepareStatement(dropqry2);
	        psm11.setString(1,session.getId().substring(1,10));
			psm12.setString(2, session.getId().substring(1,10));
		psm11.executeUpdate();
		psm12.executeUpdate();
		}
