...
try
{
	...
	String q = "select sum(OP_CL_BAL),substr(office_code,2,3) from RWS_GRANT_OP_CL_BAL_TBL group by substr(office_code,2,3)";
	...
	String q1 = "select nvl(sum(b.LOC_AMOUNT),0),substr(office_to_code,2,3) from rws_loc_master_tbl a,rws_loc_details_tbl b,rws_bro_master_tbl c where a.loc_id=b.loc_id and a.bro_id=b.bro_id and b.bro_id=c.bro_id and   a.bro_id=c.bro_id and fin_year='2010-2011' group by substr(office_to_code,2,3)";
	...
	String q5 = "select nvl(sum(BILL_AMT_CONTRACTOR),0),nvl(sum(BILL_AMT_ADMISSIBLE),0),substr(b.office_code,2,3) from rws_work_bill_tbl a,rws_work_admn_tbl b where a.work_id=b.work_id and bill_sl_no<>'0' group by substr(b.office_code,2,3)";
...
<tr  class="alternateRow">
	...
	<%
		...
		<!-- OB -->
		<%if(ob!=null && !ob.equals("0")){%>
		<td class="rptValue" align="right"><a href="#" onClick="wopen('rws_ob_releases_rpt.jsp?dcode=<%=rs.getString(1)%>&divcode=<%=rs.getString(2)%>&dnamee=<%=rs.getString(3)%>&divname=<%=rs.getString(4)%>','popup', 900, 300)"><%=ndf.format(Double.parseDouble(ob))%></a></td>
		<%}else{%>
		<td class="rptValue" align="right"><%=ndf.format(Double.parseDouble(ob))%></td>
		<%}%>
		<!-- Releases -->
		<%if(release!=null && !release.equals("0")){%>
		<td class="rptValue" align="right"><a href="#" onClick="wopen('rws_releases_rpt.jsp?dcode=<%=rs.getString(1)%>&divcode=<%=rs.getString(2)%>&dnamee=<%=rs.getString(3)%>&divname=<%=rs.getString(4)%>','popup', 900, 300)"><%=ndf.format(Double.parseDouble(release)*100000)%></a></td>
		<%}else{%>
		...
		<td class="rptValue" align="right"><a href="#" onClick="wopen('rws_exp_vouhcers_rpt.jsp?dcode=<%=rs.getString(1)%>&divcode=<%=rs.getString(2)%>&dnamee=<%=rs.getString(3)%>&divname=<%=rs.getString(4)%>','popup', 900, 300)"><%=ndf.format(Double.parseDouble(exp))%></a></td>
		<%}else{%>
		...
		<td class="rptValue" align="right"><a href="#" onClick="wopen('rws_bill_rpt.jsp?dcode=<%=rs.getString(1)%>&divcode=<%=rs.getString(2)%>&dnamee=<%=rs.getString(3)%>&divname=<%=rs.getString(4)%>','popup', 900, 300)"><%=ndf.format(Double.parseDouble(bill))%></a></td>
		<%}else{%>
