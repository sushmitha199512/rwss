...
<%    DecimalFormat ndf = new DecimalFormat("####.00");
    ...
    String nrdwpSubQuery = "select PROGRAMME_CODE,PROGRAMME_NAME from  rws_programme_tbl where NRDWP_RELATED='N' and CM_CORE_DB='Y' and PROGRAMME_NAME like '%NRDWP%' order by 1";
    ...
    String CmDBQuery = "select PROGRAMME_CODE,PROGRAMME_NAME from  rws_programme_tbl where CM_CORE_DB='Y' and NRDWP_RELATED='N' and PROGRAMME_NAME not like '%NRDWP%' order by 1";
    ...
    <form name=f1 id="f1" method="post" action="rws_work_progress_abstract_list1.jsp">
        ...
        <table border=1 cellspacing=0 cellpadding=0 height=5% width="1000px" class="baseTable"
            ...
            <tr align=center align=center class="thead">
                ...
                <td class=btext rowspan=3>Sub Programme</td>
                <%}%>
            ...
            <%
                ...
                try {
                    ...
                    for (int i = 1; i <= programmeList.size(); i++){                    	 
					    ...
					    if (programme.length() > 2) {
					        programmeNam = "NRDWP&nbsp;(SVS&nbsp;AND&nbsp;MVS)";
			           	} else {
                   ...
                   if (mode.equals("initial")) {
                        ...
                        query = "select sum(a.SANCTION_AMOUNT),count(distinct a.work_id),cir.CIRCLE_OFFICE_NAME,cir.CIRCLE_OFFICE_CODE from rws_work_admn_tbl a,rws_programme_tbl p,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,"
                                + "rws_contractor_selection_tbl m, rws_circle_office_tbl cir where WORK_CANCEL_DT is null and   a.programme_code=p.PROGRAMME_CODE and a.programme_code in (" + programme + ") "
                                + "and a.work_id not in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and DATE_OF_COMPLETION<'01-APR-" + yearStart + "') "
                                + "and target_date_comp>='01-APR-" + yearStart + "' and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id  "
                                + "and a.TYPE_OF_ASSET in (" + schemes + ") and a.CORE_DASHBOARD_STATUS='Y' and SUBSTR(a.OFFICE_CODE,2,2)=cir.CIRCLE_OFFICE_CODE group by cir.CIRCLE_OFFICE_NAME,cir.CIRCLE_OFFICE_CODE  order by cir.CIRCLE_OFFICE_CODE";
                    } else if (mode.equals("div")) {
                        query = "select sum(a.SANCTION_AMOUNT),count(distinct a.work_id),div.DIVISION_OFFICE_NAME,div.DIVISION_OFFICE_CODE from rws_work_admn_tbl a,rws_programme_tbl p,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,"
                                + "rws_contractor_selection_tbl m, rws_circle_office_tbl cir, rws_division_office_tbl div where WORK_CANCEL_DT is null and  a.programme_code=p.PROGRAMME_CODE and a.programme_code in (" + programme + ") "
                                + "and a.work_id not in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and DATE_OF_COMPLETION<'01-APR-" + yearStart + "') "
                                + "and target_date_comp>='01-APR-" + yearStart + "' and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id "
                                + "and a.TYPE_OF_ASSET in (" + schemes + ") and a.CORE_DASHBOARD_STATUS='Y' and SUBSTR(a.OFFICE_CODE,2,2)=cir.CIRCLE_OFFICE_CODE  and SUBSTR(a.OFFICE_CODE,4,1)=div.DIVISION_OFFICE_CODE "
                        ...
                        query = "select sum(a.SANCTION_AMOUNT),count(distinct a.work_id),sdiv.SUBDIVISION_OFFICE_NAME,sdiv.SUBDIVISION_OFFICE_CODE from rws_work_admn_tbl a,rws_programme_tbl p,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,"
                                + "rws_contractor_selection_tbl m, rws_circle_office_tbl cir, rws_division_office_tbl div, rws_subdivision_office_tbl sdiv where WORK_CANCEL_DT is null and  a.programme_code=p.PROGRAMME_CODE and a.programme_code in (" + programme + ") "
                                + "and a.work_id not in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and DATE_OF_COMPLETION<'01-APR-" + yearStart + "') "
                                + "and target_date_comp>='01-APR-" + yearStart + "' and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id "
                                + "and a.TYPE_OF_ASSET in (" + schemes + ") and a.CORE_DASHBOARD_STATUS='Y' and SUBSTR(a.OFFICE_CODE,2,2)=cir.CIRCLE_OFFICE_CODE  and SUBSTR(a.OFFICE_CODE,4,1)=div.DIVISION_OFFICE_CODE "
               ...
               //    System.out.println(" work test query is :" +query);
                   ...
                   if (rs.next()) {                    	
                    ...
                    if (mode.equals("initial")) {
                       ...
                       // System.out.println("sanctioned habs1"+sacHabs);
                        ...
                        sacHabs = "select count(distinct b.hab_code),SUM(nvl(CENSUS_PLAIN_POPU,0)+nvl(CENSUS_SC_POPU,0)+nvl(CENSUS_ST_POPU,0)+nvl(CENSUS_MINORITY_POPU,0)) from rws_work_admn_tbl a,RWS_ADMN_HAB_LNK_TBL b, RWS_WORK_COMMENCEMENT_TBL c "
                                + ", rws_habitation_directory_tbl hd where hd.hab_code=b.hab_code and WORK_CANCEL_DT is null and a.work_id not in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and DATE_OF_COMPLETION<'01-APR-" + yearStart + "') "
                                + "and target_date_comp>='01-APR-" + yearStart + "' and  b.work_id=a.work_id and COVERAGE_STATUS<>'UI' and c.work_id(+)=a.work_id and a.programme_code in (" + programme + ") "
                                + " and a.TYPE_OF_ASSET in (" + schemes + ") and  a.CORE_DASHBOARD_STATUS='Y' and SUBSTR(a.OFFICE_CODE,2,2)='" + programme + "'";
                       // System.out.println("sanctioned habs2"+sacHabs);
                    } else if (mode.equals("div")) {
                        sacHabs = "select count(distinct b.hab_code),SUM(nvl(CENSUS_PLAIN_POPU,0)+nvl(CENSUS_SC_POPU,0)+nvl(CENSUS_ST_POPU,0)+nvl(CENSUS_MINORITY_POPU,0)) from rws_work_admn_tbl a,RWS_ADMN_HAB_LNK_TBL b,RWS_WORK_COMMENCEMENT_TBL c "
                                + ", rws_habitation_directory_tbl hd where hd.hab_code=b.hab_code and WORK_CANCEL_DT is null and a.work_id not in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and DATE_OF_COMPLETION<'01-APR-" + yearStart + "') "
                                + "and target_date_comp>='01-APR-" + yearStart + "' and  b.work_id=a.work_id and COVERAGE_STATUS<>'UI' and c.work_id(+)=a.work_id and a.programme_code in (" + programe + ") "
                                + " and a.TYPE_OF_ASSET in (" + schemes + ") and  a.CORE_DASHBOARD_STATUS='Y' and SUBSTR(a.OFFICE_CODE,2,2)='" + request.getParameter("coc") + "'  and SUBSTR(a.OFFICE_CODE,4,1)='" + programme + "'";
                        //System.out.println("sanctioned habs3"+sacHabs);
                    } else if (mode.equals("sdiv")) {
                        sacHabs = "select count(distinct b.hab_code),SUM(nvl(CENSUS_PLAIN_POPU,0)+nvl(CENSUS_SC_POPU,0)+nvl(CENSUS_ST_POPU,0)+nvl(CENSUS_MINORITY_POPU,0)) from rws_work_admn_tbl a,RWS_ADMN_HAB_LNK_TBL b ,RWS_WORK_COMMENCEMENT_TBL c  "
                                + ", rws_habitation_directory_tbl hd where hd.hab_code=b.hab_code and WORK_CANCEL_DT is null and a.work_id not in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and DATE_OF_COMPLETION<'01-APR-" + yearStart + "') "
                                + "and target_date_comp>='01-APR-" + yearStart + "' and  b.work_id=a.work_id and COVERAGE_STATUS<>'UI' and c.work_id(+)=a.work_id and  a.programme_code in (" + programe + ") "
                                + "and a.TYPE_OF_ASSET in (" + schemes + ") and  a.CORE_DASHBOARD_STATUS='Y' and SUBSTR(a.OFFICE_CODE,2,2)='" + request.getParameter("coc") + "'  and SUBSTR(a.OFFICE_CODE,4,1)='" + request.getParameter("doc") + "' and SUBSTR(a.OFFICE_CODE,5,2)='" + programme + "'";
                      //  System.out.println("sanctioned habs4"+sacHabs);
                    }
                ...
                if (mode.equals("initial")) {
                	...
                	compWorks = "select count(distinct a.work_id),count(h.hab_code),SUM(nvl(CENSUS_PLAIN_POPU,0)+nvl(CENSUS_SC_POPU,0)+nvl(CENSUS_ST_POPU,0)+nvl(CENSUS_MINORITY_POPU,0)) from RWS_WORK_COMP_HAB_LNK_TBL h,rws_work_admn_tbl a,rws_work_completion_tbl b,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m"
                            + ", rws_habitation_directory_tbl hd where WORK_CANCEL_DT is null and b.work_id=a.work_id and h.work_id=a.work_id and hd.hab_code=h.hab_code and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id  and "
                            + "a.programme_code in (" + programe + ")  and b.DATE_OF_COMPLETION >= '01-APR-" + yearStart + "'  and b.DATE_OF_COMPLETION is not null and "
                            + " target_date_comp>='01-APR-" + yearStart + "' and a.TYPE_OF_ASSET in (" + schemes + ") and  a.CORE_DASHBOARD_STATUS='Y' and SUBSTR(a.OFFICE_CODE,2,2)='" + programme + "'";
                } else if (mode.equals("div")) {
                	compWorks = "select count(distinct a.work_id),count(h.hab_code),SUM(nvl(CENSUS_PLAIN_POPU,0)+nvl(CENSUS_SC_POPU,0)+nvl(CENSUS_ST_POPU,0)+nvl(CENSUS_MINORITY_POPU,0)) from RWS_WORK_COMP_HAB_LNK_TBL h,rws_work_admn_tbl a,rws_work_completion_tbl b,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m"
                            + ", rws_habitation_directory_tbl hd where WORK_CANCEL_DT is null and b.work_id=a.work_id and h.work_id=a.work_id and hd.hab_code=h.hab_code and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id  and "
                            + "a.programme_code in (" + programe + ")  and b.DATE_OF_COMPLETION >= '01-APR-" + yearStart + "'  and b.DATE_OF_COMPLETION is not null and "
                            + " target_date_comp>='01-APR-" + yearStart + "' and a.TYPE_OF_ASSET in (" + schemes + ") and  a.CORE_DASHBOARD_STATUS='Y' and SUBSTR(a.OFFICE_CODE,2,2)='" + request.getParameter("coc") + "'  and SUBSTR(a.OFFICE_CODE,4,1)='" + programme + "'";
                } else if (mode.equals("sdiv")) {
                	compWorks = "select count(distinct a.work_id),count(h.hab_code),SUM(nvl(CENSUS_PLAIN_POPU,0)+nvl(CENSUS_SC_POPU,0)+nvl(CENSUS_ST_POPU,0)+nvl(CENSUS_MINORITY_POPU,0)) from RWS_WORK_COMP_HAB_LNK_TBL h,rws_work_admn_tbl a,rws_work_completion_tbl b,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m"
                            + ", rws_habitation_directory_tbl hd where WORK_CANCEL_DT is null and b.work_id=a.work_id and h.work_id=a.work_id and hd.hab_code=h.hab_code and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id  and "
                            + "a.programme_code in (" + programe + ")  and b.DATE_OF_COMPLETION >= '01-APR-" + yearStart + "'  and b.DATE_OF_COMPLETION is not null and "
                            + " target_date_comp>='01-APR-" + yearStart + "' and a.TYPE_OF_ASSET in (" + schemes + ") and  a.CORE_DASHBOARD_STATUS='Y' and SUBSTR(a.OFFICE_CODE,2,2)='" + request.getParameter("coc") + "'  and SUBSTR(a.OFFICE_CODE,4,1)='" + request.getParameter("doc") + "' and SUBSTR(a.OFFICE_CODE,5,2)='" + programme + "'";
                }                   
                ...
                if (mode.equals("initial")) {
                	...
                	comphabs = "select count(distinct h.hab_code),SUM(nvl(CENSUS_PLAIN_POPU,0)+nvl(CENSUS_SC_POPU,0)+nvl(CENSUS_ST_POPU,0)+nvl(CENSUS_MINORITY_POPU,0)) from RWS_WORK_COMP_HAB_LNK_TBL h,rws_work_admn_tbl a,rws_work_completion_tbl b,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m"
                            + ", rws_habitation_directory_tbl hd where WORK_CANCEL_DT is null and b.work_id=a.work_id and h.work_id=a.work_id and hd.hab_code=h.hab_code and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id  and "
                            + "a.programme_code in (" + programe + ")  and  ((b.DATE_OF_COMPLETION >= '01-APR-" + yearStart +"' and b.DATE_OF_COMPLETION is not null) OR (b.partial_date_of_completion >= '01-APR-" + yearStart +"' and b.DATE_OF_COMPLETION is null)) and "
                            + " target_date_comp>='01-APR-" + yearStart + "' and a.TYPE_OF_ASSET in (" + schemes + ") and  a.CORE_DASHBOARD_STATUS='Y' and SUBSTR(a.OFFICE_CODE,2,2)='" + programme + "'";
                } else if (mode.equals("div")) {
                	comphabs = "select count(distinct h.hab_code),SUM(nvl(CENSUS_PLAIN_POPU,0)+nvl(CENSUS_SC_POPU,0)+nvl(CENSUS_ST_POPU,0)+nvl(CENSUS_MINORITY_POPU,0)) from RWS_WORK_COMP_HAB_LNK_TBL h,rws_work_admn_tbl a,rws_work_completion_tbl b,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m"
                            + ", rws_habitation_directory_tbl hd where WORK_CANCEL_DT is null and b.work_id=a.work_id and h.work_id=a.work_id and hd.hab_code=h.hab_code and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id  and "
              ...
              String paoLFI = "";
                    ...
                    if (mode.equals("initial")) {
                        ...
                        bankLM = "select sum(nvl(BILL_AMOUNT_CENTRAL,0)) from RWS_BILL_BANK_BRIDGE_DET_TBL b,rws_work_admn_tbl a,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m "
                                        + "where WORK_CANCEL_DT is null and a.programme_code in ("+programe+") and a.work_id not in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and DATE_OF_COMPLETION<'01-APR-" + yearStart + "') "
                                        + "and  b.work_id=a.work_id and i.work_id(+)=a.work_id and target_date_comp>='01-APR-" + yearStart + "' and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id and BANK_SEND_DATE>'31-MAR-" + yearStart + "' "
                                        + "and BANK_SEND_DATE<'01-" + currentMonth + "-" + cyear + "' and a.TYPE_OF_ASSET in (" + schemes + ") and  a.CORE_DASHBOARD_STATUS='Y' and SUBSTR(a.OFFICE_CODE,2,2)='" + programme + "'";
                    } else if (mode.equals("div")) {
                        bankLM = "select sum(nvl(BILL_AMOUNT_CENTRAL,0)) from RWS_BILL_BANK_BRIDGE_DET_TBL b,rws_work_admn_tbl a,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m "
                                        + "where WORK_CANCEL_DT is null and a.programme_code in ("+programe+") and a.work_id not in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and DATE_OF_COMPLETION<'01-APR-" + yearStart + "') "
                                        + "and  b.work_id=a.work_id and i.work_id(+)=a.work_id and target_date_comp>='01-APR-" + yearStart + "' and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id and BANK_SEND_DATE>'31-MAR-" + yearStart + "' "
                                        + "and BANK_SEND_DATE<'01-" + currentMonth + "-" + cyear + "' and a.TYPE_OF_ASSET in (" + schemes + ") and  a.CORE_DASHBOARD_STATUS='Y'  and SUBSTR(a.OFFICE_CODE,2,2)='" + request.getParameter("coc") + "'  and SUBSTR(a.OFFICE_CODE,4,1)='" + programme + "'";
                    } else if (mode.equals("sdiv")) {
                        bankLM = "select sum(nvl(BILL_AMOUNT_CENTRAL,0)) from RWS_BILL_BANK_BRIDGE_DET_TBL b,rws_work_admn_tbl a,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m "
                                        + "where WORK_CANCEL_DT is null and a.programme_code in ("+programe+") and a.work_id not in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and DATE_OF_COMPLETION<'01-APR-" + yearStart + "') "
                                        + "and  b.work_id=a.work_id and i.work_id(+)=a.work_id and target_date_comp>='01-APR-" + yearStart + "' and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id and BANK_SEND_DATE>'31-MAR-" + yearStart + "' "
                                        + "and BANK_SEND_DATE<'01-" + currentMonth + "-" + cyear + "' and a.TYPE_OF_ASSET in (" + schemes + ") and  a.CORE_DASHBOARD_STATUS='Y' and SUBSTR(a.OFFICE_CODE,2,2)='" + request.getParameter("coc") + "'  and SUBSTR(a.OFFICE_CODE,4,1)='" + request.getParameter("doc") + "' and SUBSTR(a.OFFICE_CODE,5,2)='" + programme + "'";
                    }
                  ...
                  String paoLM = "";
                    ...
                    if (mode.equals("initial")) {
                        ...
                        paoLM = "select sum(nvl(BILL_AMOUT_STATE,0)) from RWS_BILL_PAO_BRIDGE_DET_TBL b,rws_work_admn_tbl a,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m "
                            + "where WORK_CANCEL_DT is null and a.programme_code in ("+programe+") and a.work_id not in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and DATE_OF_COMPLETION<'01-APR-" + yearStart + "') "
                            + "and  b.work_id=a.work_id and i.work_id(+)=a.work_id and target_date_comp>='01-APR-" + yearStart + "' and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id and STATE_SEND_DATE>'31-MAR-" + yearStart + "' "
                            + "and STATE_SEND_DATE<'01-" + currentMonth + "-" + cyear + "' and a.TYPE_OF_ASSET in (" + schemes + ") and  a.CORE_DASHBOARD_STATUS='Y' and SUBSTR(a.OFFICE_CODE,2,2)='" + programme + "'";
                    } else if (mode.equals("div")) {
                        paoLM = "select sum(nvl(BILL_AMOUT_STATE,0)) from RWS_BILL_PAO_BRIDGE_DET_TBL b,rws_work_admn_tbl a,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m "
                            + "where WORK_CANCEL_DT is null and a.programme_code in ("+programe+") and a.work_id not in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and DATE_OF_COMPLETION<'01-APR-" + yearStart + "') "
                            + "and  b.work_id=a.work_id and i.work_id(+)=a.work_id and target_date_comp>='01-APR-" + yearStart + "' and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id and STATE_SEND_DATE>'31-MAR-" + yearStart + "' "
                            + "and STATE_SEND_DATE<'01-" + currentMonth + "-" + cyear + "' and a.TYPE_OF_ASSET in (" + schemes + ") and  a.CORE_DASHBOARD_STATUS='Y' and SUBSTR(a.OFFICE_CODE,2,2)='" + request.getParameter("coc") + "'  and SUBSTR(a.OFFICE_CODE,4,1)='" + programme + "'";
                    } else if (mode.equals("sdiv")) {
                        paoLM = "select sum(nvl(BILL_AMOUT_STATE,0)) from RWS_BILL_PAO_BRIDGE_DET_TBL b,rws_work_admn_tbl a,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m "
                            + "where WORK_CANCEL_DT is null and a.programme_code in ("+programe+") and a.work_id not in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and DATE_OF_COMPLETION<'01-APR-" + yearStart + "') "
                            + "and  b.work_id=a.work_id and i.work_id(+)=a.work_id and target_date_comp>='01-APR-" + yearStart + "' and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id and STATE_SEND_DATE>'31-MAR-" + yearStart + "' "
                            + "and STATE_SEND_DATE<'01-" + currentMonth + "-" + cyear + "' and a.TYPE_OF_ASSET in (" + schemes + ") and  a.CORE_DASHBOARD_STATUS='Y' and SUBSTR(a.OFFICE_CODE,2,2)='" + request.getParameter("coc") + "'  and SUBSTR(a.OFFICE_CODE,4,1)='" + request.getParameter("doc") + "' and SUBSTR(a.OFFICE_CODE,5,2)='" + programme + "'";
                    }                    
                    stmt1 = conn.createStatement();
               ...
               String paoCM = "";
                    ...
                    if (mode.equals("cir")) {
                        phyCompNotFinBank += " and SUBSTR(a.OFFICE_CODE,2,2)='" + programme + "'";
                    }
                    else if (mode.equals("div")) {
                        phyCompNotFinBank += " and SUBSTR(a.OFFICE_CODE,2,2)='" + request.getParameter("coc") + "'  and SUBSTR(a.OFFICE_CODE,4,1)='" + programme + "'";
                    }
                    else if (mode.equals("sdiv")) {
                        phyCompNotFinBank += " and SUBSTR(a.OFFICE_CODE,2,2)='" + request.getParameter("coc") + "'  and SUBSTR(a.OFFICE_CODE,4,1)='" + request.getParameter("doc") + "' and SUBSTR(a.OFFICE_CODE,5,2)='" + programme + "'";
                    }
                    ...
                    if (mode.equals("cir")) {
                        phyCompNotFinPAO += " and SUBSTR(a.OFFICE_CODE,2,2)='" + programme + "'";
                    }
                    else if (mode.equals("div")) {
                        phyCompNotFinPAO += " and SUBSTR(a.OFFICE_CODE,2,2)='" + request.getParameter("coc") + "'  and SUBSTR(a.OFFICE_CODE,4,1)='" + programme + "'";
                    }
                    else if (mode.equals("sdiv")) {
                        phyCompNotFinPAO += " and SUBSTR(a.OFFICE_CODE,2,2)='" + request.getParameter("coc") + "'  and SUBSTR(a.OFFICE_CODE,4,1)='" + request.getParameter("doc") + "' and SUBSTR(a.OFFICE_CODE,5,2)='" + programme + "'";
                    }
                   ...
                   popBenf==0 && bankLastYear==0.0 && paoLastYear==0.0 && bankLastMonth ==0.0 && paoLastMonth==0.0 &&
                     bankCurrentMonth ==0.0 && paoCurrentMonth==0.0 && worksPhyCompNotFin==0.0)
                 {}else{
                ...
                %>
                    ...
                    <%if(!mainProgrammeName.equals(dupeProgrammeName)){ %>
                             <td class="<%=style%>" style="text-align: right;" align="right"><%=++slno%></td>
                              <td class="<%=style%>" style="text-align: left;" align="right"><%=mainProgrammeName%></td>
                             <%}else{ %>
                             <td class="<%=style%>" style="text-align: right;" align="right"></td>
                              <td class="<%=style%>" style="text-align: right;" align="right"></td>
                             <%} %>
                             <td class="<%=style%>" style="text-align: left;" align="right"><a href="rws_work_progress_CM_div_sudiv.jsp?mode=circles&programme=<%=programme%>&programmeName=<%=programmeNam%>"><%=programmeNam%></a></td>
                             <td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_work_progress_CM_abstract_drill.jsp?mode=tWorks&pc=<%=programme%>&pn=<%=programmeNam%>')"><%=noOfWorks%></a></td>
                              <td class="<%=style%>" style="text-align: right;" align="right"><%=ndf.format(estCost) %></td>
                             <td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_work_progress_CM_abstract_drill.jsp?mode=sHabs&pc=<%=programme%>&pn=<%=programmeNam%>')"><%=habsSanctioned%></a></td>
                             <td class="<%=style%>" style="text-align: right;" align="right"><%=popBenf%></td>
                             <td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_work_progress_CM_abstract_drill.jsp?mode=cWorks&pc=<%=programme%>&pn=<%=programmeNam%>')"><%=worksComp%></a></td>
                              	
                       		
                          <td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_work_progress_CM_abstract_drill.jsp?mode=cHabs&pc=<%=programme%>&pn=<%=programmeNam%>')"><%=habsCov%></a></td>
			                ...
			                <td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_work_progress_CM_abstract_drill.jsp?mode=fWorks&pc=<%=programme%>&pn=<%=programmeNam%>')"><%=noOfWorks - worksComp%></a></td>
                ...
                dupeProgrammeName=mainProgrammeName;
                    ...
                    estimatedCost.put(programme, estCost);
                	expenditure.put(programme, (bankLastYear + paoLastYear + bankLastMonth + paoLastMonth + bankCurrentMonth + paoCurrentMonth) / 100000);
                	balanceEstimatedCost.put(programme, arfc);
                      ...
                      /*   if (NRDWPCodes.indexOf(programme) != -1 && mode.equals("initial")) {
                        	
                        	
                        	
                            codesToSend += programme + ",";
            ...
            <%
              ...
              /*  if (mode.equals("initial")) {
                	...
                	if (programe.length() >= NRDWPCodes.length()) {
                 	    String sacHabs1 = "select count(distinct b.hab_code),SUM(nvl(CENSUS_PLAIN_POPU,0)+nvl(CENSUS_SC_POPU,0)+nvl(CENSUS_ST_POPU,0)+nvl(CENSUS_MINORITY_POPU,0)) from rws_work_admn_tbl a,RWS_ADMN_HAB_LNK_TBL b,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m "
                                 ...
                                 + "and a.programme_code in (" + NRDWPCodes1 + ") and a.TYPE_OF_ASSET in (" + schemes + ") and a.CORE_DASHBOARD_STATUS='Y' and COVERAGE_STATUS<>'UI' and b.hab_code=habs.PANCH_CODE and dist.dcode=SUBSTR(habs.PANCH_CODE,1,2) and mdl.mcode=SUBSTR(habs.PANCH_CODE,6,2) "
                                ...
                                //System.out.println("sacHabs1"+sacHabs1);
                                 stmt1 = conn.createStatement();
                                 rs1 = stmt1.executeQuery(sacHabs1);
                                 if (rs1.next()) {NRDWPhabsSanctioned12 = rs1.getInt(1);
                                 nrdwpPopBenf = rs1.getInt(2);}
                   }	
                	 */
            %>
          ...
          <%--   <tr>
                ...
                <td class="<%=style%>" style="text-align: right;"><a href="#" onclick="viewWorks('./rws_work_progress_CM_abstract_drill.jsp?mode=sHabs&pc=<%=NRDWPCodes%>&pn=NRDWP')"><%=NRDWPhabsSanctioned12%></td>
