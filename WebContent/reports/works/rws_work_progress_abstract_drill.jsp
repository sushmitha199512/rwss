...
<table width="70%" border=1 style="border-collapse: collapse"
	...
	<tr>
		...
		<td class="<%=style%>" style="text-align: left;" align="right"><a
			href="#"
			onclick="viewWorks('./rws_work_progress_abstract_drill.jsp?mode=sHabs&mode1=work&workid=<%=rs.getString(2)%>&dcode=<%=dcode%>&programmeCode=<%=programmeCode%>&dname=<%=dname%>')"><%=noofHabs%></a></td>
		<%
			}
							} else {
								if (typeofasset.equals("11")
		...
		<td class="<%=style%>" style="text-align: left;" align="right"><a
			href="#"
			onclick="viewWorks('./rws_work_progress_abstract_drill.jsp?mode=cHabs&mode1=work&workid=<%=rs.getString(2)%>&dcode=<%=dcode%>&programmeCode=<%=programmeCode%>&dname=<%=dname%>')"><%=noofHabs%></a></td>
		<%
			}
							}
		%>
				...
				try {
					...
					while (rs1.next()) {
						...
						String details = "select dname,mname,pname,vname,panch_code,panch_name  from rws_district_tbl d,rws_mandal_tbl m,rws_panchayat_tbl p,rws_village_tbl v,rws_panchayat_raj_tbl pr"
								+ " where d.dcode=m.dcode and m.mcode=substr(panch_code,6,2) and d.dcode=p.dcode and m.mcode=p.mcode and p.pcode=substr(panch_code,13,2)"
								+ " and d.dcode=v.dcode and m.mcode=v.mcode and p.pcode=v.pcode and v.vcode=substr(panch_code,8,3) and panch_code='"
								+ hab_code + "'";
						statement = conn.createStatement();
