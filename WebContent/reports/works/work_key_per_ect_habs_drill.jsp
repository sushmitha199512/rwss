...
<br />
<center>
	<font color="#663899" face="Maiandra GD" size="4px"><b> <br />
			<%if(mode.equals("completed")){ %> Habitations for Completed Works
			During <%=ys%>-<%=ye%> in Programme :<%=programmeName%> <%if(ccode!=null){ %>
			Circle:<%=cname%> <%}else if(divcode!=null){ %> And Division:<%=divname %><br>
			<%}else if(subdivcode!=null){ %> And SubDivision :<%=subdivname %> <%}
            }else if(mode.equals("total")){ %> Habitations for Total
			Works During <%=ys%>-<%=ye%> in Programme :<%=programmeName%> <%if(ccode!=null){ %>
			Circle:<%=cname%> <%}else if(divcode!=null){ %> And Division:<%=divname %><br>
			<%}else if(subdivcode!=null){ %> And SubDivision :<%=subdivname %> <%}
            }else if(mode.equals("targeted")){ %> Target Habitations
			During <%=ys%>-<%=ye%> in Programme :<%=programmeName%> <%if(ccode!=null){ %>
			Circle:<%=cname%> <%}else if(divcode!=null){ %> And Division:<%=divname %><br>
			<%}else if(subdivcode!=null){ %> And SubDivision :<%=subdivname %> <%}
            }
            if(workid==null){}
            else{%> And Work Id : <%=workid%> <%}  %> <br />
	</b></font>
        ...
        String schemes = "'01','02','03','10'";
            ...
            try {
              if(mode.equals("total")){ 
             	  ...
             	  query="select distinct b.hab_code from rws_work_admn_tbl a,RWS_WORK_COMP_HAB_LNK_TBL b,rws_programme_tbl p,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c,rws_contractor_selection_tbl m,rws_habitation_directory_tbl h "
							+"where  WORK_CANCEL_DT is null and a.programme_code in ("+programme+") and SUBSTR(a.work_id,7,2) in ("+ schemes +")  and a.PROGRAMME_CODE=p.PROGRAMME_CODE and "
