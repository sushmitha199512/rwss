               ...
               /*  if (mode.equals("tWorks")) {
                ...
                if (mode.equals("tWorks")) {
                    /* query = "select a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP ,'dd/mm/yyyy'),'--') from "
                            ...
                            + "and target_date_comp>='01-APR-" + yearStart + "' and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id=a.work_id and m.work_id=i.work_id "
                            + "and a.TYPE_OF_ASSET in (" + schemes + ") and c.CORE_DASHBOARD_STATUS<>'N'"; */
                	query = "select a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP ,'dd/mm/yyyy'),'--'),ADMIN_NO,dist.dname as dname,nvl(lead_habitation,'--') as lead_habitation from "
                            + " rws_work_admn_tbl a,rws_programme_tbl p,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c, rws_contractor_selection_tbl m,rws_district_tbl dist where "
                            + " substr(a.work_id,5,2)=dist.dcode and WORK_CANCEL_DT is null and  a.programme_code=p.PROGRAMME_CODE and a.programme_code in (" + pc + ") "
                            + " and a.work_id not in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and DATE_OF_COMPLETION<'01-APR-" + yearStart + "') "
                    ...
                    query = "select distinct a.SANCTION_AMOUNT, a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP ,'dd/mm/yyyy'),'--'),to_char(b.DATE_OF_COMPLETION,'dd/mm/yyyy'),ADMIN_NO,dist.dname as dname,nvl(lead_habitation,'--') as lead_habitation "
                            + " from RWS_WORK_COMP_HAB_LNK_TBL h,rws_habitation_directory_tbl hd,rws_work_admn_tbl a,rws_work_completion_tbl b,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m,rws_district_tbl dist where "
                            + "  substr(a.work_id,5,2)=dist.dcode and WORK_CANCEL_DT is null and b.work_id=a.work_id and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id "
                            + " and h.work_id=a.work_id and hd.hab_code=h.hab_code and a.programme_code in ("+pc+")  and b.DATE_OF_COMPLETION >= '01-APR-"+yearStart+"' and b.DATE_OF_COMPLETION is not null and "
                   ...
                   /*  query = "select a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP ,'dd/mm/yyyy'),'--') from "
                            ...
                            + "and a.TYPE_OF_ASSET in (" + schemes + ") and c.CORE_DASHBOARD_STATUS<>'N'"; */
                            query = "select a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP ,'dd/mm/yyyy'),'--'),ADMIN_NO,dist.dname as dname,nvl(lead_habitation,'--') as lead_habitation from "
                                    + " rws_work_admn_tbl a,rws_programme_tbl p,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c, rws_contractor_selection_tbl m,rws_district_tbl dist " 
                                    + " where substr(a.work_id,5,2)=dist.dcode and WORK_CANCEL_DT is null and   a.programme_code=p.PROGRAMME_CODE and a.programme_code in (" + pc + ") and "
                                    + " a.work_id not in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null)"
                	                + " and target_date_comp>='01-APR-" + yearStart + "' and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id and a.TYPE_OF_ASSET in (" + schemes + ") and a.CORE_DASHBOARD_STATUS='Y'";
                }
...
//                }
        ...
        <tr>
            ...
            <td class="<%=style%>" style="text-align: right;" align="right"><%=physicalStage%></td>
                ...
                if (mode.equals("sHabs")) {
                 ...
                 //System.out.println(query);
                    ...
                    query = "select distinct dist.dname,mdl.mname,panch.pname,vill.vname,habs.PANCH_NAME,b.hab_code,SUM(nvl(CENSUS_PLAIN_POPU,0)+nvl(CENSUS_SC_POPU,0)+nvl(CENSUS_ST_POPU,0)+nvl(CENSUS_MINORITY_POPU,0)) as pop,hd.coverage_status "
                           ...
                           + "WORK_CANCEL_DT is null and a.work_id=comp.work_id and ((comp.DATE_OF_COMPLETION >= '01-APR-"+yearStart+"'  and comp.DATE_OF_COMPLETION is not null) OR (comp.partial_date_of_completion >= '01-APR-"+yearStart+"' and comp.DATE_OF_COMPLETION is null))"
