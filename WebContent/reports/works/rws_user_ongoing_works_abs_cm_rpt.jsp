	...
	<form name=f1 method="post">
		...
		<table border=1 cellspacing=0 cellpadding=0 height=5% width="100%"
			...
			<tr align=center align=center>
				...
				<td class=gridhdbg rowspan=2>Exp. Upto Last Financial Year <br>(<%=(yearStart - 1)%>-<%=(yearEnd - 1)%>)
				</td>
				<td colspan=3 class=gridhdbg align="center">Exp. During the
					Financial Year <br>(<%=yearStart%>-<%=yearEnd%>)
				</td>
				...
				<td class=gridhdbg rowspan=1 colspan=2
					style="width: 250px !important;">Physically Completed But<br />Not
					Financially Completed<br />(<%=yearStart - 1%>-<%=yearEnd - 1%>)
				</td>
			...
			<%
				...
				try {
					...
					if (mode.equals("divisions")) {
						...
						query += " and d.dcode='"
								+ rDistrict
								+ "' and substr(a.work_id,7,2) not in ('11','12') and d.dcode=div.circle_office_code and div.division_office_code=substr(a.office_code,4,1)";
						...
						query += " and d.dcode='"
								+ rDistrict
								+ "' and substr(a.work_id,7,2) not in ('11','12') and d.dcode=div.circle_office_code and div.division_office_code=substr(a.office_code,4,1) and div.circle_office_code=sdiv.circle_office_code and div.division_office_code=sdiv.division_office_code and sdiv.division_office_code='"
								+ divCode
			...
			<tr>
				...
				<td class="<%=style%>" style="text-align: left;" align="right"><a
					href="./rws_user_ongoing_works_abs_cm_rpt.jsp?mode=subdivisions&district=<%=rs.getString(4)%>&program=<%=programe%>&dname=<%=rs.getString(3)%>&finyear=<%=finyear%>&divCode=<%=rs.getString(5)%>&finyear=<%=finyear%>"><%=rs.getString(6)%></a></td>
				<td class="<%=style%>" style="text-align: right;" align="right"><a
					href="#"
					onclick="viewWorks('./rws_user_work_progress_abstract_drill.jsp?mode=tWorks&dcode=<%=rs.getString(4)%>&programmeCode=<%=programe%>&dname=<%=rs.getString(3)%>&divCode=<%=rs.getString(5)%>&finyear=<%=finyear%>')"><%=noOfWorks%></a></td>
				<%
					} else if (mode.equals("subdivisions")) {
				%>
				...
				<td class="<%=style%>" style="text-align: right;" align="right"><a
					href="#"
					onclick="viewWorks('./rws_user_work_progress_abstract_drill.jsp?mode=tWorks&dcode=<%=rs.getString(4)%>&programmeCode=<%=programe%>&dname=<%=rs.getString(3)%>&divCode=<%=rs.getString(5)%>&subdivCode=<%=rs.getString(7)%>&finyear=<%=finyear%>')"><%=noOfWorks%></a></td>
				<%
					} else {
				%>
				<td class="<%=style%>" style="text-align: left;" align="right"><a
					href="./rws_user_ongoing_works_abs_cm_rpt.jsp?mode=divisions&district=<%=rs.getString(4)%>&program=<%=programe%>&dname=<%=rs.getString(3)%>&finyear=<%=finyear%>"><%=rs.getString(3)%></a></td>
				<td class="<%=style%>" style="text-align: right;" align="right"><a
					href="#"
					onclick="viewWorks('./rws_user_work_progress_abstract_drill.jsp?mode=tWorks&dcode=<%=rs.getString(4)%>&programmeCode=<%=programe%>&dname=<%=rs.getString(3)%>&finyear=<%=finyear%>')"><%=noOfWorks%></a></td>
				<%
					}
				%>
				...
				<%
					String sacHabs = "select count(b.hab_code) from rws_work_admn_tbl a,RWS_ADMN_HAB_LNK_TBL b,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m  where WORK_CANCEL_DT is null  and substr(a.WORK_ID,5,2)='"
							...
							sacHabs += " and a.ADMIN_DATE >to_date('01/04/2011','dd/mm/yyyy') and c.GROUNDING_DATE is not null";
				...
				<td class="<%=style%>" style="text-align: right;" align="right"><a
					href="#"
					onclick="viewWorks('./rws_user_work_progress_abstract_drill.jsp?mode=sHabs&mode1=normal&dcode=<%=rs.getString(4)%>&programmeCode=<%=programe%>&dname=<%=rs.getString(3)%>&divCode=<%=rs.getString(5)%>&finyear=<%=finyear%>')"><%=habsSanctioned%></a></td>
				<%
					} else if (mode.equals("subdivisions")) {
				%>
				<td class="<%=style%>" style="text-align: right;" align="right"><a
					href="#"
					onclick="viewWorks('./rws_user_work_progress_abstract_drill.jsp?mode=sHabs&mode1=normal&dcode=<%=rs.getString(4)%>&programmeCode=<%=programe%>&dname=<%=rs.getString(3)%>&divCode=<%=rs.getString(5)%>&subdivCode=<%=rs.getString(7)%>&finyear=<%=finyear%>')"><%=habsSanctioned%></a></td>
				<%
					} else {
				%>
				<td class="<%=style%>" style="text-align: right;" align="right"><a
					href="#"
					onclick="viewWorks('./rws_user_work_progress_abstract_drill.jsp?mode=sHabs&mode1=normal&dcode=<%=rs.getString(4)%>&programmeCode=<%=programe%>&dname=<%=rs.getString(3)%>&finyear=<%=finyear%>')"><%=habsSanctioned%></a></td>
				<%
					}
							}
							stmt1.close();
				...
				<%
					String bankLFY = "select sum(BILL_AMOUNT_CENTRAL) from RWS_BILL_BANK_BRIDGE_DET_TBL b,rws_work_admn_tbl a,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m where WORK_CANCEL_DT is null and substr(a.WORK_ID,5,2)='"
							...
							bankLFY += " and a.ADMIN_DATE >to_date('01/04/2011','dd/mm/yyyy') and c.GROUNDING_DATE is not null";
							stmt1 = conn.createStatement();
							...
							paoFLY += " and a.ADMIN_DATE >to_date('01/04/2011','dd/mm/yyyy') and c.GROUNDING_DATE is not null";
							stmt1 = conn.createStatement();
							...
							refundFLY+=" and a.ADMIN_DATE >to_date('01/04/2011','dd/mm/yyyy') and c.GROUNDING_DATE is not null";
							stmt1 = conn.createStatement();
							rs1 = stmt1.executeQuery(paoFLY);
				...
				<%
					String bankUptoLastMonth = "select sum(BILL_AMOUNT_CENTRAL) from RWS_BILL_BANK_BRIDGE_DET_TBL b,rws_work_admn_tbl a,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m where WORK_CANCEL_DT is null and substr(a.WORK_ID,5,2)='"
							...
							bankUptoLastMonth += " and a.ADMIN_DATE >to_date('01/04/2011','dd/mm/yyyy') and c.GROUNDING_DATE is not null";
							stmt1 = conn.createStatement();
							...
							paoUptoLastMonth += " and a.ADMIN_DATE >to_date('01/04/2011','dd/mm/yyyy') and c.GROUNDING_DATE is not null";
							stmt1 = conn.createStatement();
				...
				<%
					String bankcurrMonth = "select sum(BILL_AMOUNT_CENTRAL) from RWS_BILL_BANK_BRIDGE_DET_TBL b,rws_work_admn_tbl a,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m where WORK_CANCEL_DT is null and substr(a.WORK_ID,5,2)='"
							...
							bankcurrMonth += " and a.ADMIN_DATE >to_date('01/04/2011','dd/mm/yyyy') and c.GROUNDING_DATE is not null";
							stmt1 = conn.createStatement();
							...
							paocurrMonth += " and a.ADMIN_DATE >to_date('01/04/2011','dd/mm/yyyy') and c.GROUNDING_DATE is not null";
							stmt1 = conn.createStatement();
				...
				<td class="<%=style%>" style="text-align: right;" align="right"><a
					href="#"
					onclick="viewWorks('./rws_user_work_progress_abstract_drill.jsp?mode=cWorks&dcode=<%=rs.getString(4)%>&programmeCode=<%=programe%>&dname=<%=rs.getString(3)%>&divCode=<%=rs.getString(5)%>&finyear=<%=finyear%>')"><%=worksComp%></a></td>
				<%
					} else if (mode.equals("subdivisions")) {
				%>
				<td class="<%=style%>" style="text-align: right;" align="right"><a
					href="#"
					onclick="viewWorks('./rws_user_work_progress_abstract_drill.jsp?mode=cWorks&dcode=<%=rs.getString(4)%>&programmeCode=<%=programe%>&dname=<%=rs.getString(3)%>&divCode=<%=rs.getString(5)%>&subdivCode=<%=rs.getString(7)%>&finyear=<%=finyear%>')"><%=worksComp%></a></td>
				<%
					} else {
				%>
				<td class="<%=style%>" style="text-align: right;" align="right"><a
					href="#"
					onclick="viewWorks('./rws_user_work_progress_abstract_drill.jsp?mode=cWorks&dcode=<%=rs.getString(4)%>&programmeCode=<%=programe%>&dname=<%=rs.getString(3)%>&finyear=<%=finyear%>')"><%=worksComp%></a></td>
				<%
					}
							}
							stmt1.close();
				...
				<td class="<%=style%>" style="text-align: right;" align="right"><a
					href="#"
					onclick="viewWorks('./rws_user_work_progress_abstract_drill.jsp?mode=cHabs&mode1=normal&dcode=<%=rs.getString(4)%>&programmeCode=<%=programe%>&dname=<%=rs.getString(3)%>&divCode=<%=rs.getString(5)%>&finyear=<%=finyear%>')"><%=habsCov%></a></td>
				<%
					} else if (mode.equals("subdivisions")) {
				%>
				<td class="<%=style%>" style="text-align: right;" align="right"><a
					href="#"
					onclick="viewWorks('./rws_user_work_progress_abstract_drill.jsp?mode=cHabs&mode1=normal&dcode=<%=rs.getString(4)%>&programmeCode=<%=programe%>&dname=<%=rs.getString(3)%>&divCode=<%=rs.getString(5)%>&subdivCode=<%=rs.getString(7)%>&finyear=<%=finyear%>')"><%=habsCov%></a></td>
				<%
					} else {
				%>
				...
				<td class="<%=style%>" style="text-align: right;" align="right"><a
					href="#"
					onclick="viewWorks('./rws_user_work_progress_abstract_drill.jsp?mode=cHabs&mode1=normal&dcode=<%=rs.getString(4)%>&programmeCode=<%=programe%>&dname=<%=rs.getString(3)%>&finyear=<%=finyear%>')"><%=habsCov%></a></td>
				<%
					}
							}
							stmt1.close();
				...
				<%
					String phyCompNotFinWorks = "select count(a.work_id) from rws_work_admn_tbl a,rws_work_completion_tbl b,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m,rws_programme_tbl p  where WORK_CANCEL_DT is null and substr(a.WORK_ID,5,2)='"
									+ rs.getString(4)
									+ "' and SUBSTR(a.work_id,7,2) not in ('11','12') and b.work_id=a.work_id and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id=a.work_id and m.work_id=i.work_id and a.ADMIN_DATE >to_date('01/04/2011','dd/mm/yyyy') and a.PROGRAMME_CODE=p.PROGRAMME_CODE  and a.programme_code in ("
									+ programe
				...
				<td class="<%=style%>" style="text-align: right;" align="right"><a
					href="#"
					onclick="viewWorks('./rws_user_work_progress_abstract_drill.jsp?mode=pnfWorks&dcode=<%=rs.getString(4)%>&programmeCode=<%=programe%>&dname=<%=rs.getString(3)%>&divCode=<%=rs.getString(5)%>&finyear=<%=finyear%>')"><%=worksPhyCompNotFin%></a></td>
				<%
					} else if (mode.equals("subdivisions")) {
				%>
				<td class="<%=style%>" style="text-align: right;" align="right"><a
					href="#"
					onclick="viewWorks('./rws_user_work_progress_abstract_drill.jsp?mode=pnfWorks&dcode=<%=rs.getString(4)%>&programmeCode=<%=programe%>&dname=<%=rs.getString(3)%>&divCode=<%=rs.getString(5)%>&subdivCode=<%=rs.getString(7)%>&finyear=<%=finyear%>')"><%=worksPhyCompNotFin%></a></td>
				<%
					} else {
				%>
				<td class="<%=style%>" style="text-align: right;" align="right"><a
					href="#"
					onclick="viewWorks('./rws_user_work_progress_abstract_drill.jsp?mode=pnfWorks&dcode=<%=rs.getString(4)%>&programmeCode=<%=programe%>&dname=<%=rs.getString(3)%>&finyear=<%=finyear%>')"><%=worksPhyCompNotFin%></a></td>
				<%
					}
							}
							stmt1.close();
