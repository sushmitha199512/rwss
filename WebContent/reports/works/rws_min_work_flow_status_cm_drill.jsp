...
left: -350px; /*position where enlarged image should offset horizontally */
    ...
    <br/><br/>
        <center>
            <font color="#663899" face="Maiandra GD" size="4px"><b>Works Progress - Work flow <br/><br/> <%=message %></b></font>
    ...
    <br/>
          ...
          <%
              ...
              try {
                  if(mode.equals("admin_sactioned")){
                      query = "select distinct a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP,'dd/mm/yyyy'),'--') as TARGET_DATE_COMP,a.PROGRAMME_CODE,a.SUBPROGRAMME_CODE,nvl(a.LEAD_HABITATION,'--') as lead_habitation, d.DNAME as dname from RWS_WORK_ADMN_TBL a"
                    		  +" , RWS_DISTRICT_TBL d where d.DCODE=SUBSTR(a.work_id,5,2) and WORK_CANCEL_DT is null and a.work_id not in "
                              +"(select work_id from rws_work_completion_tbl where  DATE_OF_COMPLETION is not null and DATE_OF_COMPLETION<'"+yearStart+"') " 
                              +"and substr(a.work_id,7,2) in ("	+ schemes+ ") and PROGRAMME_CODE in ("+programme+") and TARGET_DATE_COMP>='"+ yearStart+"' "
                              +" and ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '') and a.CORE_DASHBOARD_STATUS='Y'";
                              }
                   else if(mode.equals("grounded")){
                       query="select distinct a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP,'dd/mm/yyyy'),'--') as TARGET_DATE_COMP,a.PROGRAMME_CODE,a.SUBPROGRAMME_CODE,nvl(a.LEAD_HABITATION,'--') as lead_habitation, d.DNAME as dname "
                    		   +" from RWS_WORK_COMMENCEMENT_TBL b,RWS_WORK_ADMN_TBL a,rws_new_est_tbl e,RWS_CONTRACTOR_SELECTION_TBL c,RWS_SOURCE_IDEN_FIN_TBL s , RWS_DISTRICT_TBL d where d.DCODE=SUBSTR(a.work_id,5,2) and a.work_id not in "
		                       +" (select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and DATE_OF_COMPLETION<'"+yearStart+"') " 
		                       +" and WORK_CANCEL_DT is null and substr(a.work_id,7,2) in ("+ schemes+ ") and a.CORE_DASHBOARD_STATUS='Y' and a.work_id=b.work_id "
		                       +" and a.work_id=e.work_id  and c.work_id=a.work_id and a.work_id=s.work_id and a.PROGRAMME_CODE in ("+programme+") and TARGET_DATE_COMP>='"+ yearStart+"'"
		                       +" and ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '')";
                   }
                   else if(mode.equals("completed")){
                    /*   query="select distinct a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP,'dd/mm/yyyy'),'--') as TARGET_DATE_COMP"
                    		  +" from RWS_WORK_ADMN_TBL a,RWS_WORK_COMMENCEMENT_TBL b,rws_new_est_tbl e,RWS_CONTRACTOR_SELECTION_TBL c,RWS_SOURCE_IDEN_FIN_TBL s,RWS_WORK_COMP_HAB_LNK_TBL h,rws_habitation_directory_tbl hd where a.work_id in "
	                          +"(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and DATE_OF_COMPLETION>'31-MAR-"+ys+ "') " 
	                          +" and h.work_id=a.work_id and hd.hab_code=h.hab_code and substr(a.work_id,7,2) in ("+ schemes+ ") and a.work_id=b.work_id and a.work_id=e.work_id and a.work_id=c.work_id and a.CORE_DASHBOARD_STATUS='Y'"
	                          +" and a.work_id=s.work_id and a.PROGRAMME_CODE in ("+ programme+") and TARGET_DATE_COMP>='"+ yearStart+"' and ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '')"; */	
	                          
                	  query="select distinct a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP,'dd/mm/yyyy'),'--') as TARGET_DATE_COMP,a.PROGRAMME_CODE,a.SUBPROGRAMME_CODE,nvl(a.LEAD_HABITATION,'--') as lead_habitation, d.DNAME as dname "
                    		  +" from RWS_WORK_ADMN_TBL a,RWS_WORK_COMMENCEMENT_TBL b,rws_new_est_tbl e,RWS_CONTRACTOR_SELECTION_TBL c,RWS_SOURCE_IDEN_FIN_TBL s , RWS_DISTRICT_TBL d where d.DCODE=SUBSTR(a.work_id,5,2) and a.work_id in "
	                          +"(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and DATE_OF_COMPLETION>='01-APR-"+ys+ "') " 
	                          +" and substr(a.work_id,7,2) in ("+ schemes+ ") and a.work_id=b.work_id and a.work_id=e.work_id and a.work_id=c.work_id and a.CORE_DASHBOARD_STATUS='Y'"
	                          +" and a.work_id=s.work_id and a.PROGRAMME_CODE in ("+ programme+") and TARGET_DATE_COMP>='"+ yearStart+"' and ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '')";
                  }
                   else if(mode.equals("not_grounded")){
                       query="select distinct a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP,'dd/mm/yyyy'),'--') as TARGET_DATE_COMP,a.PROGRAMME_CODE,a.SUBPROGRAMME_CODE,nvl(a.LEAD_HABITATION,'--') as lead_habitation, d.DNAME as dname "
                    		   +" from RWS_WORK_ADMN_TBL a , RWS_DISTRICT_TBL d where d.DCODE=SUBSTR(a.work_id,5,2) and a.work_id not in "
							   +"(select e.work_id from RWS_WORK_COMMENCEMENT_TBL e,rws_source_iden_fin_tbl b,rws_new_est_tbl c,RWS_CONTRACTOR_SELECTION_TBL d where e.work_id = b.work_id and e.work_id = c.work_id and e.work_id = d.work_id and GROUNDING_DATE is not null ) and a.work_id not in " 
                               ...
                               +"  a.PROGRAMME_CODE in ("+programme+") and TARGET_DATE_COMP>='01-APR-"+ ys+"' "
                               +" and ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '') ";
                      ...
                      /*  String Not_Grounded_Query="select count (distinct a.work_id) from RWS_WORK_ADMN_TBL a where a.work_id not in "
							   +"(select a.work_id from RWS_WORK_COMMENCEMENT_TBL a,rws_source_iden_fin_tbl b,rws_new_est_tbl c,RWS_CONTRACTOR_SELECTION_TBL d where a.work_id = b.work_id and a.work_id = c.work_id and a.work_id = d.work_id and GROUNDING_DATE is not null) and a.work_id not in " 
                                ...
                                +" and ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '') "; */
                   } else if(mode.equals("not_ground")){
                           query="select distinct SANCTION_AMOUNT,work_id,work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP,'dd/mm/yyyy'),'--') as TARGET_DATE_COMP,PROGRAMME_CODE,SUBPROGRAMME_CODE,nvl(LEAD_HABITATION,'--') as lead_habitation, d.DNAME as dname"
                        		   +" from RWS_WORK_ADMN_TBL w, RWS_DISTRICT_TBL d where d.DCODE=SUBSTR(w.work_id,5,2) and work_id not in (select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null)"
                   ...
                   } else if(mode.equals("commissioned")){
                       query="select distinct a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP,'dd/mm/yyyy'),'--') as TARGET_DATE_COMP,a.PROGRAMME_CODE,a.SUBPROGRAMME_CODE,nvl(a.LEAD_HABITATION,'--') as lead_habitation, d.DNAME as dname"
                    		   +" from RWS_WORK_ADMN_TBL a,RWS_WORK_COMMENCEMENT_TBL b,rws_new_est_tbl e,RWS_CONTRACTOR_SELECTION_TBL c,RWS_SOURCE_IDEN_FIN_TBL s , RWS_DISTRICT_TBL d where d.DCODE=SUBSTR(a.work_id,5,2) and a.work_id in "
                               +"(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and DATE_OF_COMM is not null and DATE_OF_COMM>='01-APR-"+ys+ "') and b.GROUNDING_DATE is not null " 
                               ...
                               +" s.work_id=a.work_id and a.PROGRAMME_CODE in ("+ programme+") and TARGET_DATE_COMP>='01-APR-"+ ys+"' and ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '')";
                   }
                   else if(mode.equals("underprogress")){
                       query="select distinct a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP,'dd/mm/yyyy'),'--') as TARGET_DATE_COMP,a.PROGRAMME_CODE,a.SUBPROGRAMME_CODE,nvl(a.LEAD_HABITATION,'--') as lead_habitation, d.DNAME as dname"
                    		   +" from RWS_WORK_COMMENCEMENT_TBL b,RWS_WORK_ADMN_TBL a,rws_new_est_tbl e,RWS_CONTRACTOR_SELECTION_TBL c,RWS_SOURCE_IDEN_FIN_TBL s , RWS_DISTRICT_TBL d where d.DCODE=SUBSTR(a.work_id,5,2) and a.work_id not in "
                               +"(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null) and b.GROUNDING_DATE is not null " 
                               +"and WORK_CANCEL_DT is null and substr(a.work_id,7,2) in ("+ schemes+ ") and a.CORE_DASHBOARD_STATUS='Y' and a.work_id=b.work_id and "
                               +" a.work_id=e.work_id  and c.work_id=a.work_id and a.work_id=s.work_id and a.PROGRAMME_CODE in ("+programme+") and TARGET_DATE_COMP>='01-APR-"+ ys+"' "
                               +" and ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '') ";
                    }
                  if(!mode.equals("not_ground")){
                  if(ccode!=null){
                  		query+=" and substr(a.office_code,2,2)='"+ccode+"'";
                  	}
                  	if(divcode!=null){
          ...
          %>
          <tr>
              <td class="btext" style="text-align: right;" colspan=8>Total:</td>
