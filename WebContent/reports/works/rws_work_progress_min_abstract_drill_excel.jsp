...
response.setHeader("Content-Disposition", "inline; filename="
		+ "rws_works_abstract_rpt_excel.xls");
%>
...
<html:html>
  ...
  /*   String schemeName = request.getParameter("schemeName"); */
   String schemeName="";
    if(session.getAttribute("schemeName").toString()!=null){
    	schemeName = session.getAttribute("schemeName").toString();
    }
    ...
    DecimalFormat ndf = new DecimalFormat("####.00");
        ...
        String coc="";
        if(session.getAttribute("coc").toString()!=null){
        	coc = session.getAttribute("coc").toString();
        }
        
        String con="";
        if(session.getAttribute("con").toString()!=null){
        	con = session.getAttribute("con").toString();
        }
       ...
       ///// System.out.println("222");
        ...
        String doc="";
        if(session.getAttribute("doc").toString()!=null){
         doc = session.getAttribute("doc").toString();
        }
        
        
        String don="";
        if(session.getAttribute("don").toString()!=null){
         don = session.getAttribute("don").toString();
        }
        
        String soc="";
        if(session.getAttribute("soc").toString()!=null){
         soc = session.getAttribute("soc").toString();
        }
        
        String son="";
        if(session.getAttribute("son").toString()!=null){
         son = session.getAttribute("son").toString();
        }
        String mode1="";
        if(session.getAttribute("mode1").toString()!=null){
        	mode1 = session.getAttribute("mode1").toString();
        }
        
        String spc="";
        if(session.getAttribute("spc").toString()!=null){
        	spc = session.getAttribute("spc").toString();
        }
        String spn="";
        if(session.getAttribute("spn").toString()!=null){
        	spn = session.getAttribute("spn").toString();
        }
        
        
        
       // String don = session.getAttribute("don").toString();
       // String soc = session.getAttribute("soc").toString();
       ///// String son = session.getAttribute("son").toString();
        String pc = session.getAttribute("pc").toString();
        String pn = session.getAttribute("pn").toString();
       ...
       // System.out.println(pc+"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"+schemeName);
        ...
        int yearStart = 0;
        int yearEnd = 0;
        yearStart=Integer.parseInt(session.getAttribute("yearStart")+"");
        yearEnd=Integer.parseInt(session.getAttribute("yearEnd")+"");
      /*   if (month <= 3) {
            yearStart = cyear - 1;
            yearEnd = cyear;
        } else {
        ...
        String schemes = session.getAttribute("schemes").toString();
        String sub = "ALL";
        if (session.getAttribute("sub").toString() != null) {
        ...
        <caption>
            <table  border=0 rules=none style="border-collapse:collapse" align = "right">			
                ...
                <a href="#" onClick="window.close();">Close</a>&nbsp;|&nbsp;
                 <a href="./rws_work_progress_min_abstract_drill_excel1.jsp" target=_new>Excel</a> 
            </table>
        </caption>
        <tr  align="center">
...
{query = "select a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP ,'dd/mm/yyyy'),'--'),ADMIN_NO,dis.dname,nvl(a.lead_habitation,'--') as lead_habitation,nvl(a.office_code,'--') as office_code from "
        ...
        leadHab= "'" + leadHab + "'";
        String rps = "&nbsp;";
        String ss1= leadHab.replaceAll("'",rps);
        
        String work_id="'" + rs.getString(2) + "'";
        String ss2= work_id.replaceAll("'",rps);
        ...
        <tr>
            ...
            <td class="<%=style%>" style="text-align: left;" align="right"><%=mandName%></td>
             <td class="<%=style%>" style="text-align: left;" align="right"><%=ss1%></td>
              <td class="<%=style%>" style="text-align: left;" align="right"><%=leadhabName%></td>
            <td class="<%=style%>" style="text-align: right;" align="right"><%=ss2%></td>
