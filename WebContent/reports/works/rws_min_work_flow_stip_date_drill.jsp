          ...
          <%
              ...
              try {
                	...
                	if(mode.equals("yearsg3")){
                      query = "select distinct a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP,'dd/mm/yyyy'),'--') as TARGET_DATE_COMP,a.PROGRAMME_CODE,a.SUBPROGRAMME_CODE,nvl(a.LEAD_HABITATION,'--') as lead_habitation,d.DNAME as dname FROM RWS_WORK_COMMENCEMENT_TBL b,RWS_WORK_ADMN_TBL a,rws_new_est_tbl e,RWS_CONTRACTOR_SELECTION_TBL c,RWS_SOURCE_IDEN_FIN_TBL s, RWS_DISTRICT_TBL d where d.DCODE=SUBSTR(a.work_id,5,2) and a.work_id not in "
                              ...
                              +" and ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '')  and MONTHS_BETWEEN(To_Date('"+cDate+"','DD-MM-YYYY'),C.Date_Of_Completion) > 36";
                	}else if(mode.equals("yearsg2")){
                        query = "select distinct a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP,'dd/mm/yyyy'),'--') as TARGET_DATE_COMP,a.PROGRAMME_CODE,a.SUBPROGRAMME_CODE,nvl(a.LEAD_HABITATION,'--') as lead_habitation,d.DNAME as dname FROM RWS_WORK_COMMENCEMENT_TBL b,RWS_WORK_ADMN_TBL a,rws_new_est_tbl e,RWS_CONTRACTOR_SELECTION_TBL c,RWS_SOURCE_IDEN_FIN_TBL s, RWS_DISTRICT_TBL d where d.DCODE=SUBSTR(a.work_id,5,2) and a.work_id not in "
                                +"(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null ) and b.GROUNDING_DATE is not null " 
                                +"and WORK_CANCEL_DT is null and substr(a.work_id,7,2) in ("+ schemes+ ") and a.CORE_DASHBOARD_STATUS='Y' and a.work_id=b.work_id and "
                                +" a.work_id=e.work_id  and c.work_id=a.work_id and a.work_id=s.work_id and a.PROGRAMME_CODE in ("+programme+") and TARGET_DATE_COMP>='01-APR-"+ ys+"' "
                                +" and ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '')  and MONTHS_BETWEEN(To_Date('"+cDate+"','DD-MM-YYYY'),C.Date_Of_Completion) > 24 and MONTHS_BETWEEN(To_Date('"+cDate+"','DD-MM-YYYY'),C.Date_Of_Completion)<=36";
                  	}
                        ...
                        query = "select distinct a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP,'dd/mm/yyyy'),'--') as TARGET_DATE_COMP,a.PROGRAMME_CODE,a.SUBPROGRAMME_CODE,nvl(a.LEAD_HABITATION,'--') as lead_habitation,d.DNAME as dname FROM RWS_WORK_COMMENCEMENT_TBL b,RWS_WORK_ADMN_TBL a,rws_new_est_tbl e,RWS_CONTRACTOR_SELECTION_TBL c,RWS_SOURCE_IDEN_FIN_TBL s, RWS_DISTRICT_TBL d where d.DCODE=SUBSTR(a.work_id,5,2) and a.work_id not in "
                                ...
                                +"and WORK_CANCEL_DT is null and substr(a.work_id,7,2) in ("+ schemes+ ") and a.CORE_DASHBOARD_STATUS='Y' and a.work_id=b.work_id and "
                                +" a.work_id=e.work_id  and c.work_id=a.work_id and a.work_id=s.work_id and a.PROGRAMME_CODE in ("+programme+") and TARGET_DATE_COMP>='01-APR-"+ ys+"' "
                                +" and ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '')  and MONTHS_BETWEEN(To_Date('"+cDate+"','DD-MM-YYYY'),C.Date_Of_Completion) > 12 and MONTHS_BETWEEN(To_Date('"+cDate+"','DD-MM-YYYY'),C.Date_Of_Completion)<=24";
                  	}
                        ...
                        query = "select distinct a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP,'dd/mm/yyyy'),'--') as TARGET_DATE_COMP,a.PROGRAMME_CODE,a.SUBPROGRAMME_CODE,nvl(a.LEAD_HABITATION,'--') as lead_habitation,d.DNAME as dname FROM RWS_WORK_COMMENCEMENT_TBL b,RWS_WORK_ADMN_TBL a,rws_new_est_tbl e,RWS_CONTRACTOR_SELECTION_TBL c,RWS_SOURCE_IDEN_FIN_TBL s, RWS_DISTRICT_TBL d where d.DCODE=SUBSTR(a.work_id,5,2) and a.work_id not in "
                                ...
                                +"and WORK_CANCEL_DT is null and substr(a.work_id,7,2) in ("+ schemes+ ") and a.CORE_DASHBOARD_STATUS='Y' and a.work_id=b.work_id and "
                                +" a.work_id=e.work_id  and c.work_id=a.work_id and a.work_id=s.work_id and a.PROGRAMME_CODE in ("+programme+") and TARGET_DATE_COMP>='01-APR-"+ ys+"' "
                                +" and ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '')  and MONTHS_BETWEEN(To_Date('"+cDate+"','DD-MM-YYYY'),C.Date_Of_Completion) > 6 and MONTHS_BETWEEN(To_Date('"+cDate+"','DD-MM-YYYY'),C.Date_Of_Completion)<=12";
                  	}
                        ...
                        query = "select distinct a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP,'dd/mm/yyyy'),'--') as TARGET_DATE_COMP,a.PROGRAMME_CODE,a.SUBPROGRAMME_CODE,nvl(a.LEAD_HABITATION,'--') as lead_habitation,d.DNAME as dname FROM RWS_WORK_COMMENCEMENT_TBL b,RWS_WORK_ADMN_TBL a,rws_new_est_tbl e,RWS_CONTRACTOR_SELECTION_TBL c,RWS_SOURCE_IDEN_FIN_TBL s, RWS_DISTRICT_TBL d where d.DCODE=SUBSTR(a.work_id,5,2) and a.work_id not in "
                                ...
                                +"and WORK_CANCEL_DT is null and substr(a.work_id,7,2) in ("+ schemes+ ") and a.CORE_DASHBOARD_STATUS='Y' and a.work_id=b.work_id and "
                                +" a.work_id=e.work_id  and c.work_id=a.work_id and a.work_id=s.work_id and a.PROGRAMME_CODE in ("+programme+") and TARGET_DATE_COMP>='01-APR-"+ ys+"' "
                                +" and ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '')  and MONTHS_BETWEEN(To_Date('"+cDate+"','DD-MM-YYYY'),C.Date_Of_Completion) > 0 and MONTHS_BETWEEN(To_Date('"+cDate+"','DD-MM-YYYY'),C.Date_Of_Completion)<=6";
                  	}
                	else if(mode.equals("days0")){
                        query = "select distinct a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP,'dd/mm/yyyy'),'--') as TARGET_DATE_COMP,a.PROGRAMME_CODE,a.SUBPROGRAMME_CODE,nvl(a.LEAD_HABITATION,'--') as lead_habitation,d.DNAME as dname FROM RWS_WORK_COMMENCEMENT_TBL b,RWS_WORK_ADMN_TBL a,rws_new_est_tbl e,RWS_CONTRACTOR_SELECTION_TBL c,RWS_SOURCE_IDEN_FIN_TBL s, RWS_DISTRICT_TBL d where d.DCODE=SUBSTR(a.work_id,5,2) and a.work_id not in "
                                +"(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null ) and b.GROUNDING_DATE is not null " 
                                +"and WORK_CANCEL_DT is null and substr(a.work_id,7,2) in ("+ schemes+ ") and a.CORE_DASHBOARD_STATUS='Y' and a.work_id=b.work_id and "
                                +" a.work_id=e.work_id  and c.work_id=a.work_id and a.work_id=s.work_id and a.PROGRAMME_CODE in ("+programme+") and TARGET_DATE_COMP>='01-APR-"+ ys+"' "
                                +" and ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '')  and MONTHS_BETWEEN(To_Date('"+cDate+"','DD-MM-YYYY'),C.Date_Of_Completion) <= 0 ";
                  	}
                     if(ccode!=null){
                  ...
                  while (rs.next()) {              	       	  
                     ...
                     String compDate = "";
                      ...
                      if(!mode.equals("days0")){
	                      ...
	                      String resonQry = "select b.DELAY_REASON_NAME from RWS_CONTRACTOR_SELECTION_TBL a,RWS_WORK_DELAY_REASONS b where a.DELAY_REASON_CODE=b.DELAY_REASON_CODE and a.WORK_ID='"
	                                  + rs.getString(2) + "'";
