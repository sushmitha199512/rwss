   ...
   String jhcprCode="91";
    ...
    while (resultSet.next()) {
    	if (!progCode.equals("")) {
    		...
    		System.out.println("test1-----"+progCode);
    		progCodeNRDWP=progCodeNRDWP + "," + resultSet.getString(1);
    	} else {
    		progCode = resultSet.getString(1);//1
    		progCodeNRDWP=resultSet.getString(1);//1
    	}
    ...
    programmeList.put(x, progCode+"@NRDWP RELATED");//1@NRDWP RELATED x=1
    ...
    while (resultSetn.next()) {
    	x++;
    	programmeList.put(x,resultSetn.getString(1)+"@NRDWP RELATED");//50@NRDWP RELATED  x=2
    	progCodeNRDWP+=","+resultSetn.getString(1);//1,50
    	System.out.println("test2-----"+progCodeNRDWP);
    }
    resultSetn.close();
    statementn.close();
    ...
    while (rsCmDB.next()) {
    	x++;
    	programmeList.put(x,rsCmDB.getString(1)+"@"+rsCmDB.getString(2));//x=3 04@NABARD
    	System.out.println("test3-----"+rsCmDB.getString(1)+"@"+rsCmDB.getString(2));			
    }
    x++;
    programmeList.put(x,scode+"@"+"JJM");
    ///System.out.println("programme list NRDWP"+programmeList);
    x++;
    programmeList.put(x,jhcprCode+"@"+"JHC");
        ...
        <table border=1 cellspacing=0 cellpadding=0 height=5% width="800px" class="baseTable"
            ...
            <%
            	Connection connection = null;
            				...
            				System.out.println("test4----- " + mainProgrammeName + " schemeName:" + schemeName);
            				String programmeNam = "";
            				if (programme.length() > 2) {
            					programmeNam = "NRDWP";
            					System.out.println("test5----- " + programmeNam);
            				} else {
            					...
            					if (resultSet1.next()) {
            						...
            						System.out.println("test6----- " + programmeNam);
            					}
            					resultSet.close();
            					statement.close();
            				}
            				if (programme.equals(scode) || programme.equals(jhcprCode)) {
            					if (schemesSizeTemp.equals("below5")) {
            						System.out.println("test7----- " + programme);
            						...
            						query = "select sum(a.SANCTION_AMOUNT),count(distinct a.work_id) from rws_work_admn_tbl a,rws_programme_tbl p,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,"
            								+ "rws_contractor_selection_tbl m where WORK_CANCEL_DT is null and  a.programme_code=p.PROGRAMME_CODE and a.programme_code in ("
            								+ programme + ") "
            								+ "and a.work_id not in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and TO_DATE(DATE_OF_COMPLETION)<'01-APR-"
            								+ yearStart + "') " + "and TO_DATE(TARGET_DATE_COMP)>='01-APR-" + yearStart
            								+ "' and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id "
            								+ "and a.TYPE_OF_ASSET in (" + schemes
            								+ ") and a.CORE_DASHBOARD_STATUS='Y' and a.SANCTION_AMOUNT <=5";
            					} else {
            						System.out.println("test8----- " + programme);
            					...
            					if (programme.equals(jhcprCode))
            						schemesSizeTemp = "below5";
            				...
            				if (programme.equals(scode) || programme.equals(jhcprCode)) {
            					if (schemesSizeTemp.equals("below5")) {
            						...
            						fhtcsForHouseHolds = "select sum(hd.totalhouseholds)  from rws_work_admn_tbl a,RWS_ADMN_HAB_LNK_TBL b,RWS_WORK_COMMENCEMENT_TBL c "
            								+ ", imismasterdata_tbl hd where hd.habcode=b.hab_code and WORK_CANCEL_DT is null and a.work_id not in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and TO_DATE(DATE_OF_COMPLETION)<'01-APR-"
            								+ yearStart + "') " + "and TO_DATE(TARGET_DATE_COMP)>='01-APR-" + yearStart
            								+ "' and  b.work_id=a.work_id and  c.work_id(+)=a.work_id "
            								+ "and a.programme_code in (" + programme + ")  and a.TYPE_OF_ASSET in ("
            								+ schemes + ")  and a.CORE_DASHBOARD_STATUS='Y' and a.SANCTION_AMOUNT <=5";
            					} else {
            						//schemeName +=" (Above 5 laks)";
            						fhtcsForHouseHolds = "select sum(hd.totalhouseholds)  from rws_work_admn_tbl a,RWS_ADMN_HAB_LNK_TBL b,RWS_WORK_COMMENCEMENT_TBL c "
            								+ ", imismasterdata_tbl hd where hd.habcode=b.hab_code and WORK_CANCEL_DT is null and a.work_id not in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and TO_DATE(DATE_OF_COMPLETION)<'01-APR-"
            					...
            					fhtcsForHouseHolds = "select sum(hd.totalhouseholds)  from rws_work_admn_tbl a,RWS_ADMN_HAB_LNK_TBL b,RWS_WORK_COMMENCEMENT_TBL c "
            							...
            							+ yearStart + "') " + "and TO_DATE(TARGET_DATE_COMP)>='01-APR-" + yearStart
            							+ "' and  b.work_id=a.work_id and  c.work_id(+)=a.work_id "
            							+ "and a.programme_code in (" + programme + ")  and a.TYPE_OF_ASSET in (" + schemes
            				...
            				if (programme.equals(scode) || programme.equals(jhcprCode)) {
            					if (schemesSizeTemp.equals("below5")) {
            						...
            						sacHabs = "select count(distinct b.hab_code),SUM(nvl(CENSUS_PLAIN_POPU,0)+nvl(CENSUS_SC_POPU,0)+nvl(CENSUS_ST_POPU,0)+nvl(CENSUS_MINORITY_POPU,0)) from rws_work_admn_tbl a,RWS_ADMN_HAB_LNK_TBL b,RWS_WORK_COMMENCEMENT_TBL c "
            								+ ", rws_habitation_directory_tbl hd where hd.hab_code=b.hab_code and WORK_CANCEL_DT is null and a.work_id not in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and TO_DATE(DATE_OF_COMPLETION)<'01-APR-"
            								+ yearStart + "') " + "and TO_DATE(TARGET_DATE_COMP)>='01-APR-" + yearStart
            								+ "' and  b.work_id=a.work_id and COVERAGE_STATUS<>'UI' and  c.work_id(+)=a.work_id "
            								+ "and a.programme_code in (" + programme + ")  and a.TYPE_OF_ASSET in ("
            								+ schemes + ")  and a.CORE_DASHBOARD_STATUS='Y' and a.SANCTION_AMOUNT <=5";
            					} else {
            						//schemeName +=" (Above 5 laks)";
            				...
            				if (programme.equals(scode) || programme.equals(jhcprCode)) {
            					if (schemesSizeTemp.equals("below5")) {
            						...
            						Not_Grounded_Query = "select count (distinct a.work_id) from RWS_WORK_ADMN_TBL a,rws_programme_tbl p where a.work_id not in "
            								...
            								+ "and WORK_CANCEL_DT is null and a.programme_code=p.PROGRAMME_CODE and substr(a.work_id,7,2) in ("
            								+ schemes + ") and a.CORE_DASHBOARD_STATUS='Y'  and  "
            								+ "  a.PROGRAMME_CODE in (" + programme
            								+ ") and TO_DATE(TARGET_DATE_COMP)>='01-APR-" + yearStart + "' "
            					...
            					Not_Grounded_Query = "select count (distinct a.work_id) from RWS_WORK_ADMN_TBL a,rws_programme_tbl p where a.work_id not in "
            							...
            							+ "(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null)  "
            							+ "and WORK_CANCEL_DT is null and a.programme_code=p.PROGRAMME_CODE and substr(a.work_id,7,2) in ("
            							+ schemes + ") and a.CORE_DASHBOARD_STATUS='Y'  and  " + "  a.PROGRAMME_CODE in ("
            							+ programme + ") and TO_DATE(TARGET_DATE_COMP)>='01-APR-" + yearStart + "' " + " ";//and ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '') 
            				}
            				...
            				if (programme.equals(scode) || programme.equals(jhcprCode)) {
            					if (schemesSizeTemp.equals("below5")) {
            						...
            						billsamtquery = "select sum(wb.BILL_AMT_ADMISSIBLE) from rws_work_admn_tbl a,rws_programme_tbl p,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c , "//rws_new_est_tbl t ,
            								+ "rws_contractor_selection_tbl m,RWS_WORK_BILL_TBL wb where WORK_CANCEL_DT is null and  a.programme_code=p.PROGRAMME_CODE and a.programme_code in ("
            								+ programme + ") "
            								+ "and a.work_id not in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and TO_DATE(DATE_OF_COMPLETION)<'01-APR-"
            								+ yearStart + "') " + "and TO_DATE(TARGET_DATE_COMP)>='01-APR-" + yearStart
            								+ "' and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id  "//and t.work_id=a.work_id
            								+ "and a.TYPE_OF_ASSET in (" + schemes
            								+ ") and a.CORE_DASHBOARD_STATUS='Y' and wb.EE_FWD_DATE is not null and wb.work_id=a.work_id and  not EXISTS  (select 1 from rws_bill_bank_bridge_tbl b where b.work_id=wb.work_Id and b.bill_sl_no=wb.bill_sl_no ) and a.SANCTION_AMOUNT <=5";
            					} else {
            						//schemeName +=" (Above 5 laks)";
            						billsamtquery = "select sum(wb.BILL_AMT_ADMISSIBLE) from rws_work_admn_tbl a,rws_programme_tbl p,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c , "//rws_new_est_tbl t ,
            								+ "rws_contractor_selection_tbl m,RWS_WORK_BILL_TBL wb where WORK_CANCEL_DT is null and  a.programme_code=p.PROGRAMME_CODE and a.programme_code in ("
            					...
            					billsamtquery = "select sum(wb.BILL_AMT_ADMISSIBLE) from rws_work_admn_tbl a,rws_programme_tbl p,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c , "//rws_new_est_tbl t ,
            							...
            							+ "' and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id  "//and t.work_id=a.work_id
            							+ "and a.TYPE_OF_ASSET in (" + schemes
            							+ ") and a.CORE_DASHBOARD_STATUS='Y' and wb.EE_FWD_DATE is not null and wb.work_id=a.work_id and  not EXISTS  (select 1 from rws_bill_bank_bridge_tbl b where b.work_id=wb.work_Id and b.bill_sl_no=wb.bill_sl_no )";
            				...
            				if (programme.equals(scode) || programme.equals(jhcprCode)) {
            					if (schemesSizeTemp.equals("below5")) {
            						...
            						compWorks = "select count(distinct a.work_id),count(h.hab_code),SUM(nvl(CENSUS_PLAIN_POPU,0)+nvl(CENSUS_SC_POPU,0)+nvl(CENSUS_ST_POPU,0)+nvl(CENSUS_MINORITY_POPU,0)) from RWS_WORK_COMP_HAB_LNK_TBL h,rws_work_admn_tbl a,rws_work_completion_tbl b,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m"
            								...
            								+ " a.programme_code in (" + programme
            								+ ")   and TO_DATE(b.DATE_OF_COMPLETION) >= '01-APR-" + yearStart
            								+ "' and TO_DATE(b.DATE_OF_COMPLETION)<='31-MAR-" + yearEnd
            								+ "' and b.DATE_OF_COMPLETION is not null  and "
            								+ " TO_DATE(TARGET_DATE_COMP)>='01-APR-" + yearStart
            								+ "' and a.TYPE_OF_ASSET in (" + schemes
            								+ ") and  a.CORE_DASHBOARD_STATUS='Y' and a.SANCTION_AMOUNT <=5";
            					} else {
            						//schemeName +=" (Above 5 laks)";
            					...
            					compWorks = "select count(distinct a.work_id),count(h.hab_code),SUM(nvl(CENSUS_PLAIN_POPU,0)+nvl(CENSUS_SC_POPU,0)+nvl(CENSUS_ST_POPU,0)+nvl(CENSUS_MINORITY_POPU,0)) from RWS_WORK_COMP_HAB_LNK_TBL h,rws_work_admn_tbl a,rws_work_completion_tbl b,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m"
            							...
            							+ "' and TO_DATE(b.DATE_OF_COMPLETION)<='31-MAR-" + yearEnd
            							+ "' and b.DATE_OF_COMPLETION is not null  and "
            							+ " TO_DATE(TARGET_DATE_COMP)>='01-APR-" + yearStart + "' and a.TYPE_OF_ASSET in ("
            				...
            				if (programme.equals(scode) || programme.equals(jhcprCode)) {
            					if (schemesSizeTemp.equals("below5")) {
            						...
            						commWorks = "select count(distinct a.work_id) from RWS_WORK_COMP_HAB_LNK_TBL h,rws_work_admn_tbl a,rws_work_completion_tbl b,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m"
            								+ ", rws_habitation_directory_tbl hd where WORK_CANCEL_DT is null and b.work_id=a.work_id and h.work_id=a.work_id and hd.hab_code=h.hab_code and i.work_id=a.work_id  and c.work_id=a.work_id and m.work_id=a.work_id  and "
            								+ " a.programme_code in (" + programme
            								+ ") and  b.DATE_OF_COMPLETION is not null and b.DATE_OF_COMM is not null and TO_DATE(b.DATE_OF_COMM) >= '01-APR-"
            								+ yearStart + "' and TO_DATE(b.DATE_OF_COMM)<='31-MAR-" + yearEnd + "' and"
            								+ " TO_DATE(TARGET_DATE_COMP)>='01-APR-" + yearStart
            								+ "' and a.TYPE_OF_ASSET in (" + schemes
            								+ ") and  a.CORE_DASHBOARD_STATUS='Y' and a.SANCTION_AMOUNT <=5";
            					} else {
            				...
            				if (programme.equals(scode)) {
            					if (schemesSizeTemp.equals("below5") || programme.equals(jhcprCode)) {
            						//schemeName +=" (Below 5 laks)";
            						comphabs = "select count(distinct h.hab_code),SUM(nvl(CENSUS_PLAIN_POPU,0)+nvl(CENSUS_SC_POPU,0)+nvl(CENSUS_ST_POPU,0)+nvl(CENSUS_MINORITY_POPU,0)) from RWS_WORK_COMP_HAB_LNK_TBL h,rws_work_admn_tbl a,rws_work_completion_tbl b,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m"
            								...
            								+ ")  and ((TO_DATE(b.DATE_OF_COMPLETION) >= '01-APR-" + yearStart
            								+ "' and TO_DATE(b.DATE_OF_COMPLETION) <= '31-MAR-" + yearEnd
            								+ "' and b.DATE_OF_COMPLETION is not null) OR (TO_DATE(b.partial_date_of_completion) >= '01-APR-"
            								+ yearStart + "' and TO_DATE(b.partial_date_of_completion) <= '31-MAR-"
            								+ yearEnd + "' and b.DATE_OF_COMPLETION is null)) and "
            								+ " TO_DATE(TARGET_DATE_COMP)>='01-APR-" + yearStart
            								+ "' and a.TYPE_OF_ASSET in (" + schemes
            								+ ") and  a.CORE_DASHBOARD_STATUS='Y' and a.SANCTION_AMOUNT <=5";
            					} else {
            					...
            					comphabs = "select count(distinct h.hab_code),SUM(nvl(CENSUS_PLAIN_POPU,0)+nvl(CENSUS_SC_POPU,0)+nvl(CENSUS_ST_POPU,0)+nvl(CENSUS_MINORITY_POPU,0)) from RWS_WORK_COMP_HAB_LNK_TBL h,rws_work_admn_tbl a,rws_work_completion_tbl b,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m"
            							...
            							+ "' and TO_DATE(b.DATE_OF_COMPLETION) <= '31-MAR-" + yearEnd
            							+ "' and b.DATE_OF_COMPLETION is not null) OR (TO_DATE(b.partial_date_of_completion) >= '01-APR-"
            							+ yearStart + "' and TO_DATE(b.partial_date_of_completion) <= '31-MAR-" + yearEnd
            							+ "' and b.DATE_OF_COMPLETION is null)) and "
            							+ " TO_DATE(TARGET_DATE_COMP)>='01-APR-" + yearStart + "' and a.TYPE_OF_ASSET in ("
            							+ schemes + ") and  a.CORE_DASHBOARD_STATUS='Y'";
            				}
            				...
            				if (programme.equals(scode) || programme.equals(jhcprCode)) {
            					if (schemesSizeTemp.equals("below5")) {
            						...
            						bankCum = "select sum(nvl(BILL_AMOUNT_CENTRAL,0)) from RWS_BILL_BANK_BRIDGE_DET_TBL b,rws_work_admn_tbl a,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m "
            								...
            								+ ")  and a.work_id not in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and TO_DATE(DATE_OF_COMPLETION)<'01-APR-"
            								+ yearStart + "') "
            								+ "and  b.work_id=a.work_id and i.work_id(+)=a.work_id and to_date(target_date_comp)>='01-APR-"
            								+ yearStart + "' and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id  "
            								+ "and a.TYPE_OF_ASSET in (" + schemes
            								+ ") and  a.CORE_DASHBOARD_STATUS='Y' and to_date(BANK_SEND_DATE)<='31-MAR-"
            								+ yearEnd + "' and a.SANCTION_AMOUNT <=5";
            					} else {
            						//schemeName +=" (Above 5 laks)";
            						bankCum = "select sum(nvl(BILL_AMOUNT_CENTRAL,0)) from RWS_BILL_BANK_BRIDGE_DET_TBL b,rws_work_admn_tbl a,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m "
            								+ "where WORK_CANCEL_DT is null and  a.programme_code in (" + programme
            				...
            				if (programme.equals(scode) || programme.equals(jhcprCode)) {
            					if (schemesSizeTemp.equals("below5")) {
            						...
            						paoCum = "select sum(nvl(BILL_AMOUT_STATE,0)) from RWS_BILL_PAO_BRIDGE_DET_TBL b,rws_work_admn_tbl a,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m "
            								...
            								+ yearStart + "' and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id  "
            								+ "and a.TYPE_OF_ASSET in (" + schemes
            								+ ") and  a.CORE_DASHBOARD_STATUS='Y' and to_date(STATE_SEND_DATE)<='31-MAR-"
            								+ yearEnd + "' and a.SANCTION_AMOUNT <=5";
            					} else {
            						//schemeName +=" (Above 5 laks)";
            						paoCum = "select sum(nvl(BILL_AMOUT_STATE,0)) from RWS_BILL_PAO_BRIDGE_DET_TBL b,rws_work_admn_tbl a,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m "
            				...
            				if (programme.equals(scode) || programme.equals(jhcprCode)) {
            					if (schemesSizeTemp.equals("below5")) {
            						...
            						bankTot = "select sum(nvl(BILL_AMOUNT_CENTRAL,0)) from RWS_BILL_BANK_BRIDGE_DET_TBL b,rws_work_admn_tbl a,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m "
            								...
            								+ "and  b.work_id=a.work_id and i.work_id(+)=a.work_id and TO_DATE(TARGET_DATE_COMP)>='01-APR-"
            								+ yearStart
            								+ "' and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id and to_date(BANK_SEND_DATE)>='01-APR-"
            								+ yearStart + "' " + "and to_date(BANK_SEND_DATE)<='31-MAR-" + yearEnd
            								+ "' and a.TYPE_OF_ASSET in (" + schemes
            								+ ") and  a.CORE_DASHBOARD_STATUS='Y' and a.SANCTION_AMOUNT <=5";
            					} else {
            						//schemeName +=" (Above 5 laks)";
            				...
            				if (programme.equals(scode) || programme.equals(jhcprCode)) {
            					if (schemesSizeTemp.equals("below5")) {
            						...
            						paoTot = "select sum(nvl(BILL_AMOUT_STATE,0)) from RWS_BILL_PAO_BRIDGE_DET_TBL b,rws_work_admn_tbl a,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m "
            								...
            								+ "and  b.work_id=a.work_id and i.work_id(+)=a.work_id and TO_DATE(TARGET_DATE_COMP)>='01-APR-"
            								+ yearStart
            								+ "' and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id and to_date(STATE_SEND_DATE)>='01-APR-"
            								+ yearStart + "' " + "and to_date(STATE_SEND_DATE)<='31-MAR-" + yearEnd
            								+ "' and a.TYPE_OF_ASSET in (" + schemes
            								+ ") and  a.CORE_DASHBOARD_STATUS='Y' and a.SANCTION_AMOUNT <=5";
            					} else {
            						//schemeName +=" (Above 5 laks)";
            				...
            				String amountTot = ndf
            						.format((bankLastMonth + paoLastMonth + bankCurrentMonth + paoCurrentMonth) / 100000);
            				...
            				if (programme.equals(scode) || programme.equals(jhcprCode)) {
            					if (schemesSizeTemp.equals("below5")) {
            						...
            						phyCompNotFinBank = "select sum(nvl(BILL_AMOUNT_CENTRAL,0)) as BILL_AMOUNT_CENTRAL from RWS_BILL_BANK_BRIDGE_DET_TBL b,rws_work_admn_tbl a,"
            								...
            								+ " and a.TYPE_OF_ASSET in (" + schemes
            								+ ") and to_date(BANK_SEND_DATE)>='01-APR-" + yearStart
            								...
            								+ " i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id and b.work_id=a.work_id and a.work_id in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and to_date(DATE_OF_COMPLETION)<='31-MAR-"
            								+ yearStart + "') and a.SANCTION_AMOUNT <=5";
            					} else {
            						//schemeName +=" (Above 5 laks)";
            						phyCompNotFinBank = "select sum(nvl(BILL_AMOUNT_CENTRAL,0)) as BILL_AMOUNT_CENTRAL from RWS_BILL_BANK_BRIDGE_DET_TBL b,rws_work_admn_tbl a,"
            								+ " rws_programme_tbl p,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m where WORK_CANCEL_DT is null "
            					...
            					phyCompNotFinBank = "select sum(nvl(BILL_AMOUNT_CENTRAL,0)) as BILL_AMOUNT_CENTRAL from RWS_BILL_BANK_BRIDGE_DET_TBL b,rws_work_admn_tbl a,"
            							+ " rws_programme_tbl p,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m where WORK_CANCEL_DT is null "
            							+ " and a.TYPE_OF_ASSET in (" + schemes + ") and to_date(BANK_SEND_DATE)>='01-APR-"
            							+ yearStart + "' and to_date(BANK_SEND_DATE)<='31-MAR-" + yearEnd + "' "
            							+ " and a.programme_code=p.PROGRAMME_CODE and a.programme_code in (" + programme
            							+ ") AND a.CORE_DASHBOARD_STATUS='Y' and ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '') and "
            							+ " i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id and b.work_id=a.work_id and a.work_id in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and to_date(DATE_OF_COMPLETION)<='31-MAR-"
            				...
            				if (programme.equals(scode) || programme.equals(jhcprCode)) {
            					if (schemesSizeTemp.equals("below5")) {
            						...
            						phyCompNotFinPAO = "select sum(nvl(BILL_AMOUT_STATE,0)) as BILL_AMOUT_STATE from RWS_BILL_PAO_BRIDGE_DET_TBL b,rws_work_admn_tbl a,"
            								+ " rws_programme_tbl p,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m where WORK_CANCEL_DT is null "
            								+ " and a.TYPE_OF_ASSET in (" + schemes
            								+ ") and to_date(STATE_SEND_DATE)>='01-APR-" + yearStart
            					...
            					phyCompNotFinPAO = "select sum(nvl(BILL_AMOUT_STATE,0)) as BILL_AMOUT_STATE from RWS_BILL_PAO_BRIDGE_DET_TBL b,rws_work_admn_tbl a,"
            							+ " rws_programme_tbl p,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m where WORK_CANCEL_DT is null "
            							+ " and a.TYPE_OF_ASSET in (" + schemes + ") and to_date(STATE_SEND_DATE)>='01-APR-"
            							+ yearStart + "'  and to_date(STATE_SEND_DATE)<='31-MAR-" + yearEnd + "'"
            							+ " and a.programme_code=p.PROGRAMME_CODE and a.programme_code in (" + programme
            							+ ") AND a.CORE_DASHBOARD_STATUS='Y' and ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '') and "
            							+ " i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id and b.work_id=a.work_id and a.work_id in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null and to_date(DATE_OF_COMPLETION)<='31-MAR-"
            				...
            				stmt1.close();
            				rs1.close();
            				String amountExpIncPhyCompNotFin = ndf
            						.format((expIncBankPhyCompNotFin + expIncPhyCompNotFin) / 100000);
            				// arfc = estCost - (bankLastYear + paoLastYear + bankLastMonth + paoLastMonth + bankCurrentMonth + paoCurrentMonth) / 100000;
            				if (noOfWorks == 0 && estCost == 0 && habsSanctioned == 0 && worksComp == 0 && habsCov == 0
            						&& popCov == 0 && popBenf == 0 && bankYear == 0.0 && paoYear == 0.0
            						&& bankLastMonth == 0.0 && paoLastMonth == 0.0 && bankCurrentMonth == 0.0
            						&& paoCurrentMonth == 0.0 && worksPhyCompNotFin == 0.0 && paoPhyCompNotFin == 0
            						&& bankPhyCompNotFin == 0) {
            				} else {
            					if (styleCount % 2 == 0) {
            						style = "gridbg1";
            ...
            %>
                    <tr>
                    <% 
                    if(mainProgrammeName.equals("91")){
                    	mainProgrammeName="JHC";
                    } 
                    System.out.println("test8---->> "+mainProgrammeName+"===dup>>> "+dupeProgrammeName);
                    ...
                    %>
                             ...
                             <%if(mainProgrammeName.equals("NRDWP RELATED")){ 
                            	 System.out.println("test10-----"+mainProgrammeName);
                            	 %>
                              ...
                              <%}else{System.out.println("test12-----"+mainProgrammeName);
                            	  %>
                              <%if(programme.equals(scode)||programme.equals(jhcprCode)){ 
                            	  String abc="";
                            	  System.out.println("test9-----"+programme); %>
...
<%--                                 <td class="<%=style%>" style="text-align: left;" align="right" width="100px"><%=mainProgrammeName%></td>
 --%>                              
                             ...
                             <%}else{ 
                            	 ...
                            	 System.out.println("test11-----"+programe);
                            	 %>
                             ...
                             <%} %>
                             <%}}else{  System.out.println("insideelse1167----"+mainProgrammeName);%>
                ...
                <%
	            System.out.println("dupeProgrammeName1209>>  "+dupeProgrammeName);    
