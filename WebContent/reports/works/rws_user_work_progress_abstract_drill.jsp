    ...
    <table width="70%" border=1 style="border-collapse: collapse"
            ...
            try {
                ...
                if (mode.equals("tWorks")) {
                   ...
                   // System.out.println("@@@@@@@@@query: " + query);
                    ...
                    query = "select a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(PROBABLE_DATE,'dd/mm/yyyy'),'--'),p.PROGRAMME_NAME,to_char(b.DATE_OF_COMPLETION,'dd/mm/yyyy') from rws_work_admn_tbl a,rws_work_completion_tbl b,rws_programme_tbl p,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m  where WORK_CANCEL_DT is null and substr(a.WORK_ID,5,2)='" + dcode + "' and SUBSTR(a.work_id,7,2) not in ('11','12') and b.work_id=a.work_id and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id=a.work_id and a.ADMIN_DATE >to_date('01/04/2011','dd/mm/yyyy') and a.PROGRAMME_CODE=p.PROGRAMME_CODE and m.work_id=i.work_id  and a.programme_code in (" + programmeCode + ") and b.DATE_OF_COMPLETION is not null and b.DATE_OF_COMPLETION between '01-APR-" + (yearStart-1) + "' and '31-MAR-" + (yearEnd-1) + "' and b.DT_FIN_COMPLETION is null";
                    if (request.getParameter("divCode") != null) {
                        query += " and substr(a.office_code,4,1)='"
                                + request.getParameter("divCode") + "'";
                    }
        ...
        <tr>
            ...
            <td class="<%=style%>" style="text-align: left;" align="right"><a
                    href="#"
                    onclick="viewWorks('./rws_user_work_progress_abstract_drill.jsp?mode=sHabs&mode1=work&workid=<%=rs.getString(2)%>&dcode=<%=dcode%>&programmeCode=<%=programmeCode%>&dname=<%=dname%>&finyear=<%=yearStart%>-<%=yearEnd%>')"><%=noofHabs%></a></td>
                <%
                    }
            ...
            <td class="<%=style%>" style="text-align: left;" align="right"><a
                    href="#"
                    onclick="viewWorks('./rws_user_work_progress_abstract_drill.jsp?mode=cHabs&mode1=work&workid=<%=rs.getString(2)%>&dcode=<%=dcode%>&programmeCode=<%=programmeCode%>&dname=<%=dname%>&finyear=<%=yearStart%>-<%=yearEnd%>')"><%=noofHabs%></a></td>
                <%
                    }
                %>
            ...
            try {
                ...
                if (mode.equals("sHabs")) {
                    query = "select b.hab_code from rws_work_admn_tbl a,RWS_ADMN_HAB_LNK_TBL b,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m  where WORK_CANCEL_DT is null  and substr(a.WORK_ID,5,2)='"
                            + dcode
                            + "' and SUBSTR(a.work_id,7,2) not in ('11','12') and a.work_id not in(select work_id from rws_work_completion_tbl where DATE_OF_COMPLETION is not null) and  b.work_id=a.work_id and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id=a.work_id and m.work_id=i.work_id and a.ADMIN_DATE >to_date('01/04/2011','dd/mm/yyyy') and c.GROUNDING_DATE is not null  and a.programme_code in ("
                            + programmeCode + ")";
                    ...
                    query = "select h.hab_code from rws_work_admn_tbl a,rws_work_completion_tbl b,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m,RWS_WORK_COMP_HAB_LNK_TBL h  where WORK_CANCEL_DT is null and substr(a.WORK_ID,5,2)='"
                            + dcode
                            + "' and SUBSTR(a.work_id,7,2) not in ('11','12') and h.work_id=b.work_id and b.work_id=a.work_id and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id=a.work_id and m.work_id=i.work_id  and a.programme_code in ("
                            + programmeCode
                            + ") and b.DATE_OF_COMPLETION is not null";
