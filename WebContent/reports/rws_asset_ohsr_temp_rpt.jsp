...
<%
	...
	int CountNo = 0;	
	ArrayList AROhsr = new ArrayList();
	AROhsr = (ArrayList) session.getAttribute("ohsr");
	int limitPerPage = 20;
	String [] S1 = new String [AROhsr.size()];
%>
</head>
<body bgcolor="#edf2f8">
...
%>
	...
	<tr align=center>
		...
		<td class=rptLabel colspan=10>OHSR</td>		
	</tr>
	<tr>
		<td class=rptLabel>No <br> Lts</td>		
		<td class=rptLabel>Location</td>
		<td class=rptLabel>Capacity</td>
		<td class=rptLabel>Made</td>
		<td class=rptLabel>Height</td>
		<td class=rptLabel>Condition</td>
		<td class=rptLabel>Fill <br> Empty <br> In Hours</td>		
		<td class=rptLabel>Hygine <br> Fencing</td>		
		<td class=rptLabel>No Of Fillings</td>
	</tr>

<%
	int totalpgs = Math.round(AROhsr.size() / limitPerPage);
	...
	if(request.getParameter("pr") != null)
	{
	...
	else
		{
			...
			//System.out.println("AR Size = " + AROhsr.size());				
					for(int i = pr; i<limitPerPage + pg && i<AROhsr.size(); i++)	
					{					
					S1 = (String[])AROhsr.get(i);					
%>	
	<tr>
...
%>
			...
			<a href="rws_asset_ohsr_temp_rpt.jsp?pr=0">[First Page]</a> 
			<a href="rws_asset_ohsr_temp_rpt.jsp?pr=<%=pr-limitPerPage%>">[Previous Page]</a> 
<%
		}
			if (pr < (totalpgs * limitPerPage) - limitPerPage) 
			{
%>
				<a href="rws_asset_ohsr_temp_rpt.jsp?pr=<%=pr+limitPerPage%>">[Next Page]</a> 
				<a href="rws_asset_ohsr_temp_rpt.jsp?pr=<%=(totalpgs * limitPerPage) - limitPerPage%>">[Last Page]</a> 
<%
			}
...
{
	//System.out.println("The Error in assetohsrrpt.jsp = " + e);
}
%>
		</td>
	</tr>
