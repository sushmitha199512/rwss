...
ResultSet rs1=null,rs3=null,rs4=null,rs5=null,rs6=null,rs7=null;
 ...
 String agree="";
  if(dcode!=null)
  {
	  agree="SELECT substr(office_code,2,2),substr(office_code,4,1),substr(office_code,5,2),sum(PERCENT_COMPLETED_VAL/100000) FROM RWS_WORK_ADMN_TBL W,rws_milestone_target_ach_tbl  m   where substr(office_code,5,2)is not null and w.work_id=m.work_id and work_cancel_dt is null and substr(work_id,5,2)='"+dcode+"' and w.programme_code in ("+programe+") and PERCENT_COMPLETED_VAL is not null and  w.work_id not in (select work_id from rws_work_completion_tbl where date_of_completion is not null and (to_date(DATE_OF_COMPLETION) < '01-Apr-"+startyr+"'))  ";
...
///for component
  ...
  if(dcode!=null)
  {
	  com="SELECT substr(office_code,2,2),sum(PERCENT_COMPLETED_WORK_VAL/100000)  FROM RWS_WORK_ADMN_TBL W,rws_milestone_target_ach_tbl m   where substr(office_code,5,2)is not null and w.work_id=m.work_id and work_cancel_dt is null and substr(work_id,5,2)='"+dcode+"' and w.programme_code in ("+programe+")  and  w.work_id not in (select work_id from rws_work_completion_tbl where date_of_completion is not null and (to_date(DATE_OF_COMPLETION) < '01-Apr-"+startyr+"'))  ";
   if(scheme!=null && !scheme.equals("")){
