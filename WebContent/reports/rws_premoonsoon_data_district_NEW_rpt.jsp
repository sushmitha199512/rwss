...
<caption>
			<table  border=0 rules=none style="border-collapse:collapse" align = "right">		
				<tr>
					...
					<a href="../home.jsp">Home</a> |<a href="./rws_premoonsoon1_data_district_rpt_NEW_excel.jsp?District:&nbsp;&nbsp;<%=dName%> ">Excel</a></td>
				</tr>
			</table>
			</caption>
</table>
<table border = 0 cellspacing = 0 cellpadding = 0  width=100%  style="border-collapse:collapse">
<tr><td  align="center" class=gridhdbg>Water Quality Ground Data  Pre-Monsoon (01-Jan-<%=year %> to 31-June-<%=year %>)</td></tr>
</table>
<table border = 1 cellspacing = 0 cellpadding = 0  width=100%   style="border-collapse:collapse" class="striped">
<tr><td colspan=24 align="center" class=gridhdbg>District:&nbsp;&nbsp;<%=dName%> </td></tr>
<tr>
...
<td class=gridhdbg colspan=9  align="center">Element-wise ground water quality parameters(Pre-monsoon(Jan<%=year %> to June<%=year %>))</td>
...
//String qry="select distinct  hp_code,LOCATION,nvl(LATITUDE,0),nvl(LONGITUDE,0) ,nvl(DEPTH,0) ,STATIC_WL   from rws_hp_subcomp_param_tbl where  substr(HP_CODE,1,2)='"+dcode+"'";
//System.out.println("qry"+qry);

String qry="select distinct  hp_code,LOCATION,nvl(LATITUDE,0),nvl(LONGITUDE,0),nvl(DEPTH,0) ,STATIC_WL,h.MNAME,h.PNAME,h.VNAME,PANCH_NAME,PANCH_CODE  from rws_hp_subcomp_param_tbl a, rws_complete_hab_view h where  substr(a.hp_code,1,16)=h.panch_code and substr(HP_CODE,1,2)='"+dcode+"'";
...
while(rs1.next())
{
	...
	String qry11="select distinct  a.test_id,c.testing_parameter_value,c.testing_parameter_code,to_char(a.PREPARED_ON,'DD/MM/YYYY'),a.PREPARED_ON  from rws_wq_test_results_tbl a , rws_wq_test_results_lnk_tbl c where a.source_code='"+rs1.getString(1)+"'  and a.test_id=c.test_id and to_char(a.PREPARED_ON,'yyyy')='"+year+"' and to_char(a.PREPARED_ON,'MON') in ('JAN','FEB','MAR','APR','MAY','JUN') order by a.PREPARED_ON desc";
...
String qry3="select distinct OPENWELL_CODE ,LOCATION,nvl(LATITUDE,0),nvl(LONGITUDE,0),nvl(DEPTH,0),h.MNAME,h.PNAME,h.VNAME,PANCH_NAME,PANCH_CODE  from rws_open_well_mast_tbl a, rws_complete_hab_view h where  substr(a.OPENWELL_CODE,1,16)=h.panch_code and substr(OPENWELL_CODE,1,2)='"+dcode+"'";  
if(mcode !=null && !mcode.equals("00"))
{
...
while(rs3.next())
{
	...
	String qry13="select distinct a.test_id,c.testing_parameter_value,c.testing_parameter_code,to_char(a.PREPARED_ON,'DD/MM/YYYY'),SOURCE_CODE,a.PREPARED_ON   from rws_wq_test_results_tbl a , rws_wq_test_results_lnk_tbl c where a.source_code='"+rs3.getString(1)+"' and a.prepared_on=(select max(b.prepared_on) from rws_wq_test_results_tbl b  where b.source_code=a.source_code) and a.test_id=c.test_id and to_char(a.PREPARED_ON,'yyyy')='"+year+"' and to_char(a.PREPARED_ON,'MON') in ('JAN','FEB','MAR','APR','MAY','JUN') order by a.PREPARED_ON";
...
<%
  ...
  temphabcode=rs3.getString(10);
    ...
    //adding panch code
    String qry4="select  distinct SOURCE_CODE,nvl(LOCATION,'-'),nvl(LATITUDE,0),nvl(LONGITUDE,0),nvl(DEPTH,0) ,nvl(STATIC_WATER_LEVEL,0),h.MNAME,h.PNAME,h.VNAME,PANCH_NAME,PANCH_CODE  from rws_source_tbl a, rws_complete_hab_view h where  substr(a.Source_code,1,16)=h.panch_code and  substr(SOURCE_CODE ,1,2)='"+dcode+"'";
   ...
   //BEFORE WORKING 
    	...
    	String qry12="select distinct  a.test_id,c.testing_parameter_value,c.testing_parameter_code,to_char(a.PREPARED_ON,'DD/MM/YYYY'),SOURCE_CODE,a.PREPARED_ON  from rws_wq_test_results_tbl a , rws_wq_test_results_lnk_tbl c where a.source_code='"+rs5.getString(1)+"' and  a.test_id=c.test_id and  to_char(a.PREPARED_ON,'yyyy')='"+year+"' and to_char(a.PREPARED_ON,'MON') in ('JAN','FEB','MAR','APR','MAY','JUNE') order by a.PREPARED_ON";
 ...
 if(cnt==0){
    	...
    	%>
    	<jsp:forward page="rws_prepost_data_district_NEW_frm.jsp"></jsp:forward>
    	<%
