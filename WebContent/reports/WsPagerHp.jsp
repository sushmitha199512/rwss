...
function goSearch(last,dist,rmandal,panchayat,village,hab,assetcode)
{
	...
	{
		var url = "WsPagerHp.jsp?pager.offset="+document.forms[0].go.value+"&district="+dist+"&mandal="+rmandal+"&panchayat="+panchayat+"&village="+village+"&hab="+hab+"&asset="+assetcode;
...
<caption>
 ...
 <tr bgcolor="#8A9FCD" align="center">
		 <td align="center" class="rptHeading" colspan=7><font color="white">Asset Check List Report - Hand Pumps</font></td>	
   </tr>
    <tr bgcolor="#8A9FCD" align="center">
   ...
   <td align="center" class="rptHeading"><b>HAND PUMP</b></td>
	</tr>
	<%
...
String query="select * from (select a.*, rownum rnum from (select distinct nvl(a.asset_name,'- '),a.hab_code,a.asset_code,a.TYPE_OF_ASSET_CODE,d.dcode,d.dname,m.mcode,m.mname,p.pcode,p.pname,v.vcode,v.vname,h.panch_code,h.panch_name,a. division_office_code dc,a.subdivision_office_code sdc,nvl(do.division_office_name,'-')dn,nvl (sdo.subdivision_office_name,'-') sdn,s.scheme_code,nvl(a.LOCATION,'-')LOCATION,nvl(a.ASSET_COST,'0')ASSET_COST,nvl(to_char(a.DATE_CREATION,'dd-mm-yyyy'),'-') as DATE_CREATION,POPU_BENEFITED from rws_district_tbl d,rws_mandal_tbl m,rws_panchayat_tbl p,rws_village_tbl v,rws_panchayat_raj_tbl h,rws_habitation_directory_tbl hd,rws_asset_mast_tbl a,RWS_HANDPUMP_ASSET_VIEW s,rws_division_office_tbl do,rws_subdivision_office_tbl sdo where a.asset_code=s.asset_code and d.dcode=m.dcode and m.dcode=p.dcode and m.mcode =p.mcode and p.dcode=v.dcode and p.mcode=v.mcode and p.pcode=v.pcode and substr (h.panch_code,1,2) = v.dcode and substr (h.panch_code,6,2) = v.mcode and substr(h.panch_code,13,2) = v.pcode and substr (h.panch_code,8,3) = v.vcode and h.panch_code=hd.hab_code and type_of_asset_code='"+assetcode+"' and d.dcode='"+rDist+"'";
   ...
   <td class=rptvalue><font size=2><%=rs.getString(1).toUpperCase()%>/<BR><%=rs.getString("scheme_code")%>/<BR><%=rs.getString("ASSET_code")%></font></td>
   <td class=rptvalue nowrap><font size=2><%=rs.getString("dname")%>/<br><%=rs.getString("DN")%>/<br><%=rs.getString("SDN")%>/<br><%=rs.getString("mname")%>/<br><%=rs.getString("pname")%>/<br><%=rs.getString("vname")%>/<br><%=rs.getString("panch_name")%></font> </td>  
     <% query="select p.hab_code,(census_plain_popu + census_st_popu + census_st_popu+FLOAT_POP)as totpop  from rws_habitation_directory_tbl p,RWS_HANDPUMP_ASSET_VIEW a where p.hab_code=SUBSTR(a.SCHEME_code,1,16) and asset_code='"+rs.getString(3)+"'";  
...
////System.out.println("aa:"+aa);
	     String[] aaa = aa.split("/");
		 ////System.out.println("aaaaaaaaaadddd:"+aaa);
			 %>
...
////System.out.println("aaaaaaaaaaddddddddddddslno:"+totCount);
...
<%}%>
 ...
 <%
if(totCount!=0){
String firstpgurl = "WsPagerHp.jsp?pager.offset=0&district="+rDist+"&mandal="+rMandal+"&asset="+assetcode+"";
String prevpgurl = "WsPagerHp.jsp?pager.offset="+prevpgno+"&district="+rDist+"&mandal="+rMandal+"&asset="+assetcode;
String nextpgurl = "WsPagerHp.jsp?pager.offset="+nextpgno+"&district="+rDist+"&mandal="+rMandal+"&asset="+assetcode;
String lastpgurl = "WsPagerHp.jsp?pager.offset="+totalpgno+"&district="+rDist+"&mandal="+rMandal+"&asset="+assetcode;
...
if(i<=totalpgno)
	{
	String currenturl = "WsPagerHp.jsp?pager.offset="+i+"&district="+rDist+"&mandal="+rMandal+"&asset="+assetcode;
	if(currentpageno==i){%>
...
<%}
	...
	{
		%>
		<%String currenturl = "WsPagerHp.jsp?pager.offset="+j+"&district="+rDist+"&mandal="+rMandal+"&asset="+assetcode;
		if(currentpageno==j){%>
...
<!-- &nbsp;&nbsp;&nbsp;Go to Page:<input type="text" name="go" maxlength="4" size="2"><input type="submit" name="search" value="GO" onclick="goSearch('<%=lastpgno%>','<%=rDist%>','<%=assetcode%>')"> -->
</td></tr>
 </table>
