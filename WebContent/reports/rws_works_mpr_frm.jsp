...
{
	var zURL = "../reports/rws_works_mpr_hab_frm.jsp?workId="+workId;
...
String qq = "select distinct work_id,month from rws_ddws_works_mpr_tbl where (TO_NUMBER(month)>=4 and year=2010) or (TO_NUMBER(month)<=3) and year=2011";
...
//rs.first();
if(rs!=null)rs.close();
rs = stmt.executeQuery("select a.work_id,to_char(b.date_of_completion) as doc,b.date_of_completion,w.work_name,to_char(w.admin_date,'dd/mm/yyyy'),w.sanction_amount,dname from rws_work_admn_tbl w,rws_ddws_scheme_data_port a,rws_work_completion_tbl b,rws_district_tbl c where a.work_id=b.work_id(+) and w.work_id=a.work_id and work_asset='W' and fin_year='2010-2011' and substr(w.work_id,5,2) =c.dcode order by DATE_OF_COMPLETION desc");
workCount = 1;
while(rs.next())
{
	String cdate = rs.getString(2);
