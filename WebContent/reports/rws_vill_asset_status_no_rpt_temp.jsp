...
<%	
	...
	{		
		...
		//String panchname="";
		//String villagename="";
		String villcode[]=new String[labelNames.length];	
		 if(request.getParameter("gopg") !=  null) 
			{
				GOPG = Integer.parseInt(request.getParameter("gopg"));
		...
		{	
			...
			{		
				...
				//String villcode=record1[1];

	%>
		<tr align=left >
		<td  class="mystyle1"><%= sno%></td>
		<td class="mystyle1"><a href="rws_hab_asset_status_no_rpt.jsp?village=<%=record1[0]%>&vName=<%=record1[1]%>">
		<%=record1[1]%></a></td>
<%		
...
<% 
	...
	if((start+30)<=myList1.size())
		{
		%>
		<a href="rws_vill_asset_status_no_rpt_temp.jsp?start=<%=r1%>&slno=<%=slno%>&pgcnt=<%=s1%>"><font   color=#FF00CC>Next </font></a>
		<%}
	%>
</td>
</table>
