<%
 qurysdd ="";
	...
	<tr>
		<td align="left" class="rptHeading" colspan=10>III. Technical Sanction Details</td>	
	</tr>
...
<%	
	...
	%>	
	    <tr bgcolor="#ffffff">
			...
			<td class="btext"><font face="verdana" size="2" color="black"></b><%=rs.getFloat(4)%></b></font></td>
		</tr>
	    <tr bgcolor="#ffffff">
	    <td class="btext">2. Tech.SancNo/Tech.SancDate</td>
		<td class="btext"><font face="verdana" size="2" color="black"></b><%=rs.getString(5)%>/<br><%=rs.getString(6)%></font></td>			
		</tr>
    	<tr bgcolor="#ffffff">
    	<td class="btext">3. SSRYear</td>
    	<td class=btext><font face="verdana" size="2" color="black"></b><%=rs.getString(7)%></font></td>
    	</tr>
		<tr bgcolor="#ffffff">
		<% 
		...
		<td class=btext><font face="verdana" size="2" color="black"></b><%=rs.getString(9)%></b></font></td>
		</tr>
		<%}else  if(rs.getString(8).equals("12")){%>
		<td class="btext">4. No Of Labs</td>
		<td class=btext><font face="verdana" size="2" color="black"></b><%=rs.getString(10)%></font></td>
		</tr>
		<%}else{%>
		<td class="btext">4. No Of Habs</td>
		<td class=btext><font face="verdana" size="2" color="black"></b><%=rs.getString(3)%></font></b></td>
		</tr>
		<%}%>
	 ...
	 if(rs.getString(8).equals("11"))
		...
		%>
		 ...
		 <h2 class="expand">Technical Sanction Habitations (<%=rs.getString(3)%>)</h2>
            <div class="collapse">
                <p>
		--><table>	
...
<tr></tr> 	
		<tr>
			<td style="text-align: left;" class="rptHeading" colspan=8>&nbsp;IV&nbsp;Technical Sanction Habitations (<%=rs.getString(3)%>)</td>	
		</tr>  	
		<tr bgcolor="#ffffff">
          <%if(rs.getString(8).equals("11")){%>
          <td class="btext" >Habitation/School Name</td>
		  <%}else if(rs.getString(8).equals("12")){%>
        <td class="btext" >Division/Lab Name</td>
		<%}else{%>
		...
		<tr bgcolor="#ffffff">	
		 <%if(rs.getString(8).equals("11")){%>
			 <td class=<%=style %> style="text-align: left;" ><%=++sno%>.&nbsp;<%=rs1.getString(2)%>/<%=rs1.getString(5)%></td>
			 <td class=<%=style %> align="right"><%=rs1.getString(3)%></td>
			 <td class=<%=style %> align="center"><%=rs1.getString(4)%></td>
      <%}else if(rs.getString(8).equals("12")){%>
			<td class=<%=style %> style="text-align: left;" ><%=++sno%>.&nbsp;<%=rs1.getString(2)%>/<%=rs1.getString(3)%></td>
			<td class=rptValue align="center">-</td>
			<td class=rptValue align="center">-</td>
  <%}else{%>
			<td class=<%=style %> style="text-align: left;" ><%=++sno%>.&nbsp;<%=rs1.getString(2)%></td>
...
if(tCounts!=0)
	...
	while(r.next())
	    ...
	    <tr bgcolor="#ffffff">
	    <td class="btext2">2. Tech.SancNo/Tech.SancDate</td>
		<td class="rptValue" colspan="2"><%=r.getString(5)%>/<br><%=r.getString(6)%></td>		
		</tr>
    	<tr bgcolor="#ffffff">
    	<td class="btext2">3. SSRYear</td>
		...
		{
		%>
		<td class="btext2">4. No Of Schools</td>
		<%}else  if(r.getString(8).equals("12")){%>
		<td class="btext2">4. No Of Labs</td>
		<%}else{%>
		<td class="btext2" >4. No Of Habs</td>
		<%}%>
    ...
    //    System.out.println("query in revised::4:"+sln);
		...
		<tr bgcolor="#ffffff">
		 <%if(rs.getString(8).equals("11")){//System.out.println("query in revised::5:"+rs.getString(8));%>
		  ...
		  <%}else if(rs.getString(8).equals("12")){%>
        <td class="btext2" >5. Division/Lab Name</td>
		<%}else{%>
			<td class="btext2" >5. Habitations</td>
			<%}%>
		<td class="btext2">Total Population</td>
