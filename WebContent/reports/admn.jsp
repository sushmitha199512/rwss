...
<%	
	...
	Statement st1=null,stwork=null;
	ResultSet rs1=null,rs2=null;
	String qury1=null;
	Statement ss=null,stmt1=null;
    ResultSet rr=null,rsw=null,rswork=null;
...
//System.out.println("scheme in admin:"+schName);
	String aName = String.valueOf(session.getAttribute("augName"));	
	if( aName !=null)
	{
		session.setAttribute("aName",aName);
	}
		//System.out.println("aName in admin:"+aName);
	...
	//System.out.println("planName in admin:"+planName);
...
//String query="div.division_office_code,div.division_office_name,sd.subdivision_office_code,sd.subdivision_office_name,a.type_of_asset_code,nvl(a.type_of_asset_name,'-') as asset_name from rws_circle_office_tbl cir,rws_division_office_tbl div,rws_subdivision_office_tbl sd,RWS_WORK_ADMN_TBL w,rws_asset_type_tbl a  where cir.circle_office_code=div.circle_office_code and cir.circle_office_code=sd.circle_office_code and div.division_office_code=sd.division_office_code and substr(w.office_code,2,2)=cir.circle_office_code and substr(w.office_code,4,1)=div.division_office_code and substr(w.office_code,5,2)=sd.subdivision_office_code and substr(w.work_id,7,2)=a.type_of_asset_code \";
	...
	<caption>
		<table  border=0 rules=none style="border-collapse:collapse" align = "right">			
			<tr align="right">
				...
				<td class="bwborder"><a href="./rws_admn_status_xls.jsp?district=<%=dCode%>&division=<%=divCode%>&subdivision=<%=subdivCode%>&work=<%=workccode%>&augmentation=<%=aCode%>&plan=<%=planCode%>&scheme=<%=schCode%>&programme=<%=pCode%>&subprogramme=<%=spCode%>&year=<%=yearCode%>&dName=<%=dName%>&divName=<%=divName%>&subdivName=<%=subdivName%>&workcName=<%=workcName%>&augName=<%=aName%>&PlanName=<%=planName%>&schName=<%=schName%>&progName=<%=pName%>&sprogName=<%=spName%>&yrName=<%=year%>" target="_new">Excel</a></td> 
			</tr>			
		</table>
	</caption>

	<tr bgcolor="#8A9FCD" align="center">
		<td align="center" class="rptHeading" colspan="17">Administrative Sanction Report</td>	
	</tr>
