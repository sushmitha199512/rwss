...
<td align="center" class=btext>7</td><td align="center" class=btext>8</td><td align="center" class=btext>9</td>
 ...
 //  select DNAME ,DIVISION_OFFICE_NAME  from rws_district_tbl a,rws_division_office_tbl b where CIRCLE_OFFICE_CODE =DCODE  and STATUS <>'UI' ORDER BY 1 ;
 
    
   query="select dname,DIVISION_OFFICE_NAME,a.WORK_ID,b.WORK_NAME  ,"+field+" ,b.SANCTION_AMOUNT,nvl("+chargeField+",0),nvl("+dateField+",'-') from RWS_NOTGROUNDWORKS_STATUS_TBL a,rws_work_admn_tbl b,rws_district_tbl c,rws_division_office_tbl d,rws_programme_tbl p where a.work_id=b.work_id and d.CIRCLE_OFFICE_CODE =c.DCODE  and STATUS <>'UI'and c.dcode=substr(b.office_code,2,2) and "+field+"='Y' ";
      if(dateField!=null){
      ...
      if(sname!=null && !sname.equals("")){
    	  query+=" and b.type_of_asset in("+sname+")";
    	}
      query+= " and b.PROGRAMME_CODE =p.PROGRAMME_CODE and p.programme_code in ("+pname+") ";

   query+="and substr(b.office_code,2,2)='"+dcode+"' and substr(b.office_code,4,1)=d.DIVISION_OFFICE_CODE  and substr(b.office_code,4,1)='"+divcode+"'";
