...
<%
	...
	//System.out.println("*****REPORT BEGINING*****"+distcode.trim());
...
<tr bgcolor="#8A9FCD" align="center">
		<td align="center" class="rptHeading" colspan=5>DivisionWise-ModuleWise-Report</td>	
	</tr>
...
<%	
	...
	query2="SELECT distinct division_office_code,division_office_name from rws_division_office_tbl where circle_office_code='"+distcode.trim()+"' order by  division_office_code,division_office_name";
	  ...
	  while(rs.next())
	   {
		...
		<td class=rptValue><a href="./rws_subdiv_udate_count1.jsp?dcode=<%=distcode%>&dname=<%=distname%>&divcode=<%=rs.getString(1)%>&divname=<%=rs.getString(2)%>&date=<%=date%>&date1=<%=date1%>&type=<%=type%>">
		<%=rs.getString(2)%></a>
		</td>
