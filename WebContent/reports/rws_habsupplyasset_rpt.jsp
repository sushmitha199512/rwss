...
String typename="";
	if(type.equals("1"))
	{
		typename="No UI Habitations";
	}
	else
	{
		typename="UI Habitations";
	}
	//
	...
	<td align="center" class="rptHeading" > Schemes Gap Report (Difference in Assets and Water Supply data)</td>
	</tr>
	<tr bgcolor="#8A9FCD" align="center">
	<td class=btext>District:<font color=red><%=dname%></font>&nbsp;&nbsp;Type:<font color=red><%=typename%></font></td>
	</tr>
	</table>
   <table   bgcolor="#FFFFFF" bordercolor= "#8A9FCD"  width="100%" border=1 style="border-collapse:collapse"  ALIGN=CENTER >
    ...
    <td class=btext  nowrap rowspan=2>District</td>
...
String sample="";
    if(type.equals("1"))
    {
    sample="select count(*),m.circle_office_code FROM RWS_ASSET_MAST_TBL M, RWS_ASSET_SCHEME_TBL S,rws_shallowhandpumps_tbl h WHERE h.asset_code=m.asset_code and  M.ASSET_CODE=S.ASSET_CODE  AND M.TYPE_OF_ASSET_CODE='05'  and m.hab_code in(select panch_code from rws_complete_hab_view) group by circle_office_code";
    }
    else if(type.equals("2"))
    {
    sample="select count(*),m.circle_office_code FROM RWS_ASSET_MAST_TBL M, RWS_ASSET_SCHEME_TBL S,rws_shallowhandpumps_tbl h WHERE h.asset_code=m.asset_code and  M.ASSET_CODE=S.ASSET_CODE  AND M.TYPE_OF_ASSET_CODE='05' and m.hab_code not in(select panch_code from rws_complete_hab_view)  group by circle_office_code";
    }
...
if(type.equals("1"))
{
 sample1="select sum(shp_no_safe),substr(c.panch_code,1,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and shp_no_safe is not null and h.hab_code in(select panch_code from rws_complete_hab_view) group by substr(c.panch_code,1,2)";
}
else if(type.equals("2"))
{
sample1="select sum(shp_no_safe),substr(c.panch_code,1,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and shp_no_safe is not null and h.hab_code not in(select panch_code from rws_complete_hab_view )  group by substr(c.panch_code,1,2)";
}
    ...
    if(type.equals("1"))
    {
     sample11="select sum(shp_no),substr(c.panch_code,1,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and shp_no is not null and h.hab_code in(select panch_code from rws_complete_hab_view) group by substr(c.panch_code,1,2)";
   }
   else if(type.equals("2"))
{
  sample11="select sum(shp_no),substr(c.panch_code,1,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and shp_no is not null and h.hab_code not in(select panch_code from rws_complete_hab_view)  group by substr(c.panch_code,1,2)";
}
...
String locality="";
    if(type.equals("1"))
    { 
    locality="select count(*),m.circle_office_code FROM RWS_ASSET_MAST_TBL M, RWS_ASSET_SCHEME_TBL S,rws_hp_subcomp_param_tbl h WHERE h.asset_code=m.asset_code and  M.ASSET_CODE=S.ASSET_CODE  AND M.TYPE_OF_ASSET_CODE='04'  and hab_code in(select panch_code from rws_complete_hab_view) group by circle_office_code";
    }
    else
    {
     locality="select count(*),m.circle_office_code FROM RWS_ASSET_MAST_TBL M, RWS_ASSET_SCHEME_TBL S,rws_hp_subcomp_param_tbl h WHERE h.asset_code=m.asset_code and  M.ASSET_CODE=S.ASSET_CODE  AND M.TYPE_OF_ASSET_CODE='04'  and m.hab_code not in(select panch_code from rws_complete_hab_view )  group by circle_office_code";
    }
    ...
    if(type.equals("1"))
{
    locality1="select sum(hps_no),substr(c.panch_code,1,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and hps_no is not null and h.hab_code in(select panch_code from rws_complete_hab_view) group by substr(c.panch_code,1,2)";
}
else
{
    locality1="select sum(hps_no),substr(c.panch_code,1,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and hps_no is not null and h.hab_code not in(select panch_code from rws_complete_hab_view )  group by substr(c.panch_code,1,2)";
}
...
if(type.equals("1"))
{
	 locality11="select sum(hps_no_unsafe),substr(c.panch_code,1,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and hps_no_unsafe is not null and h.hab_code in(select panch_code from rws_complete_hab_view) group by substr(c.panch_code,1,2)";
}
else
{
	 locality11="select sum(hps_no_unsafe),substr(c.panch_code,1,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and hps_no_unsafe is not null and h.hab_code not in(select panch_code from rws_complete_hab_view)  group by substr(c.panch_code,1,2)";
}
   ...
   if(type.equals("1"))
   {  
    asset="select count(*),m.circle_office_code FROM RWS_ASSET_MAST_TBL M, RWS_ASSET_SCHEME_TBL S WHERE  M.ASSET_CODE=S.ASSET_CODE  AND M.TYPE_OF_ASSET_CODE='02'  and hab_code in(select panch_code from rws_complete_hab_view)  group by circle_office_code";
   }
  else
{
    asset="select count(*),m.circle_office_code FROM RWS_ASSET_MAST_TBL M, RWS_ASSET_SCHEME_TBL S WHERE  M.ASSET_CODE=S.ASSET_CODE  AND M.TYPE_OF_ASSET_CODE='02'  and m.hab_code not in(select panch_code from rws_complete_hab_view )   group by circle_office_code";
}
    ...
    if(type.equals("1"))
    {
     asset1="select sum(mpws_no),substr(c.panch_code,1,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and mpws_no is not null and h.hab_code in(select panch_code from rws_complete_hab_view) group by substr(c.panch_code,1,2)";
    }
    else
    {
   asset1="select sum(mpws_no),substr(c.panch_code,1,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and mpws_no is not null and h.hab_code not in(select panch_code from rws_complete_hab_view )  group by substr(c.panch_code,1,2)";
    }
    ...
    if(type.equals("1"))
     {
      asset11="select sum(mpws_unsafe_no),substr(c.panch_code,1,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and mpws_unsafe_no is not null  and h.hab_code in(select panch_code from rws_complete_hab_view) group by substr(c.panch_code,1,2)";
     }
     else
     {
           asset11="select sum(mpws_unsafe_no),substr(c.panch_code,1,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and mpws_unsafe_no is not null  and h.hab_code not in(select panch_code from rws_complete_hab_view)  group by substr(c.panch_code,1,2)";
    ...
    if(type.equals("1"))
    {
    hh="select count(*),m.circle_office_code FROM RWS_ASSET_MAST_TBL M, RWS_ASSET_SCHEME_TBL S where M.ASSET_CODE=S.ASSET_CODE  AND M.TYPE_OF_ASSET_CODE='01'  and hab_code in(select panch_code from rws_complete_hab_view)  group by m.circle_office_code";
    }
    else if(type.equals("2"))
    {
         hh="select count(*),m.circle_office_code FROM RWS_ASSET_MAST_TBL M, RWS_ASSET_SCHEME_TBL S where M.ASSET_CODE=S.ASSET_CODE  AND M.TYPE_OF_ASSET_CODE='01'  and substr(m.hab_code,1,16) not in (select panch_code from rws_complete_hab_view)   group by m.circle_office_code";
    }
    ...
    if(type.equals("1"))
    {
    cpwsqry="select count(*),substr(m.hab_code,1,2) FROM RWS_ASSET_MAST_TBL M, RWS_ASSET_SCHEME_TBL S WHERE  M.ASSET_CODE=S.ASSET_CODE  AND M.TYPE_OF_ASSET_CODE='03'  and hab_code in(select panch_code from rws_complete_hab_view) group by substr(m.hab_code,1,2)";
    }
    else
    {
     cpwsqry="select count(*),substr(m.hab_code,1,2) FROM RWS_ASSET_MAST_TBL M, RWS_ASSET_SCHEME_TBL S WHERE  M.ASSET_CODE=S.ASSET_CODE  AND M.TYPE_OF_ASSET_CODE='03' and m.hab_code not in(select panch_code from rws_complete_hab_view)   group by substr(m.hab_code,1,2)";
    }
    ...
    if(type.equals("1"))
    {
    cpwsqry1="select sum(cpws_no),substr(c.panch_code,1,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and cpws_no is not null and h.hab_code in(select panch_code from rws_complete_hab_view) group by substr(c.panch_code,1,2)";
    }
    else
   {
   cpwsqry1="select sum(cpws_no),substr(c.panch_code,1,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and cpws_no is not null  and h.hab_code not in(select panch_code from rws_complete_hab_view ) group by substr(c.panch_code,1,2)";
    ...
    String hh1="";
     if(type.equals("1"))
     {
     hh1="select sum(pws_no),substr(c.panch_code,1,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and pws_no is not null and h.hab_code in(select panch_code from rws_complete_hab_view) group by substr(c.panch_code,1,2)";
     }
     else
     {
     hh1="select sum(pws_no),substr(c.panch_code,1,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and pws_no is not null and h.hab_code not in(select panch_code from rws_complete_hab_view )  group by substr(c.panch_code,1,2)";
     }
    ...
    //
     ...
     if(type.equals("1"))
     {
      pwsunsafe="select sum(pws_unsafe_no),substr(c.panch_code,1,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and pws_unsafe_no is not null and h.hab_code in(select panch_code from rws_complete_hab_view)  group by substr(c.panch_code,1,2)";
      }
     else
     {
     pwsunsafe="select sum(pws_unsafe_no),substr(c.panch_code,1,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and pws_unsafe_no is not null and h.hab_code not in(select panch_code from rws_complete_hab_view) group by substr(c.panch_code,1,2)";
     }
    ...
    //
       ...
       if(dcode.equals("ALL"))
       {
       qry="select  dname,dcode   from rws_district_tbl  where dcode<>'16'   ";
      }
       else
       {
        qry="select  dname,dcode   from rws_district_tbl  where dcode='"+dcode+"'  and dcode<>'16' ";
       }
...
shtotal5=shtotal5+shcount;
    ...
    <td class="rptValue"><a href="rws_habsupplyassetmand_rpt.jsp?type=<%=type%>&district=<%=rs8.getString(2)%>&dname=<%=rs8.getString(1)%> "><%=rs8.getString(1)%></a></td>
