 ...
 */
/*
String completedWorks="select d.dname,b.division_office_name,p.programme_name,w.work_id,w.work_name,nvl(to_char(w.ADMIN_DATE),0),  FROM RWS_WORK_ADMN_TBL W ,rws_division_office_tbl b,rws_programme_tbl p ,rws_district_tbl d where b.CIRCLE_OFFICE_CODE =d.DCODE and to_date(w.ADMIN_DATE) <'01-Apr-"+yr+"' and w.programme_code=p.programme_code  and p.programme_code  in("+programe+") and work_cancel_dt is null and substr(w.office_code,2,2)=b.circle_office_code and substr(w.office_code,4,1)=b.division_office_code and substr(w.office_code,2,2)='"+dcode+"' and substr(w.office_code,4,1)='"+divCode+"' ";
 if(scheme!=null && !scheme.equals("")){
          completedWorks+="and w.type_of_asset in("+scheme+")";
}
completedWorks+=" and w.programme_code in ("+programe+") and  w.work_id  in (select work_id from rws_work_completion_tbl where (to_date(DATE_OF_COMPLETION) >= '01-Apr-"+yr+"'))  ";

*/

String schoolsCompQuery="select s.dname,b.division_office_name,w.work_id,w.work_name,d.SCHOOL_NAME,d.SCHOOL_CODE,nvl(d.SCHOOL_LOCATION,'-'),d.SCHOOL_CATEGORY_CODE FROM RWS_WORK_ADMN_TBL W ,rws_division_office_tbl b,RWS_WORK_SCHOOL_LNK_TBL c,RWS_SCHOOL_MASTER_TBL d ,rws_district_tbl s where b.CIRCLE_OFFICE_CODE =s.DCODE and substr(w.work_id,7,2)='11' and W.work_id in(select work_id from rws_work_completion_tbl where date_of_completion is not null) and work_cancel_dt is null and substr(w.office_code,2,2)=b.circle_office_code and substr(w.office_code,4,1)=b.division_office_code and c.work_id=w.work_id and c.hab_code=d.hab_code and d.SCHOOL_CATEGORY_CODE <>'01' and c.school_code=d.school_code   and substr(w.office_code,2,2)='"+dcode+"' and substr(w.office_code,4,1)='"+divCode+"' ";
...
System.out.println("schoolsCompQuery****"+schoolsCompQuery);
   rs1=stmt1.executeQuery(schoolsCompQuery);
