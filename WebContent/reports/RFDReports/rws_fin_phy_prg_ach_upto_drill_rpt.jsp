...
String sanctionedHabs="select d.dname,b.division_office_name,p.programme_name,w.work_id,w.work_name,nvl(to_char(w.ADMIN_DATE),0),w.NO_OF_HABS,HAB_CODE FROM RWS_WORK_ADMN_TBL W ,rws_division_office_tbl b,rws_programme_tbl p,rws_work_comp_hab_lnk_tbl c,rws_district_tbl d where  b.CIRCLE_OFFICE_CODE =d.DCODE and substr(w.office_code,2,2)='"+dcode+"' and substr(w.office_code,4,1)='"+divCode+"' and to_date(w.ADMIN_DATE) <'01-Apr-"+yr+"' and c.work_id=w.work_id and w.programme_code=p.programme_code   and work_cancel_dt is null and substr(w.office_code,2,2)=b.circle_office_code and substr(w.office_code,4,1)=b.division_office_code ";
if(scheme!=null && !scheme.equals(""))
{
  sanctionedHabs+="and w.type_of_asset in("+scheme+")";
}
sanctionedHabs+=" and w.programme_code in ("+programe+") and  w.work_id  in (select work_id from rws_work_completion_tbl where (to_date(DATE_OF_COMPLETION) >= '01-Apr-"+yr+"'))   group by  d.dname,b.division_office_name,p.programme_name,w.work_id,w.work_name,nvl(to_char(w.ADMIN_DATE),0),w.NO_OF_HABS,HAB_CODE order by 1";
...
sanctionedHabs1+=" and w.programme_code in ("+programe+") and  w.work_id  in (select work_id from rws_work_completion_tbl where (to_date(DATE_OF_COMPLETION) >= '01-Apr-"+yr+"'))   group by  d.dname,b.division_office_name,p.programme_name,w.work_id,w.work_name,nvl(to_char(w.ADMIN_DATE),0),w.NO_OF_HABS,HAB_CODE order by 1";
