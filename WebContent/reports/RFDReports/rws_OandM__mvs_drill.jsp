...
try
{
   ...
   Double toiletcost=0.0;
	...
	qry="select distinct count(b.Hab_code),substr(a.hab_code,6,2) from rws_asset_mast_tbl a,rws_asset_hab_tbl b where a.type_of_asset_code='03' and  substr(a.hab_code,1,2)='"+dcode+"' and a.asset_code=b.asset_code group by substr(a.hab_code,6,2)";
		...
		while(rs81.next())
	{
		...
		qry="select count(SAFE_LPCD),substr(a.hab_code,6,2) from rws_habitation_directory_tbl a,rws_asset_mast_tbl b where substr(a.hab_code,1,2)='"+dcode+"' and  b.hab_code=a.hab_code group by substr(a.hab_code,6,2) order by  substr(a.hab_code,6,2)";
 ...
 qry="select  sum(case when asset_status='01' or asset_status='2' then 1 else 0 end),substr(hab_code,6,2) from rws_asset_mast_tbl  where type_of_asset_code='03' and  substr(hab_code,1,2)='"+dcode+"'  group by substr(hab_code,6,2) order by substr(hab_code,6,2)";
	...
	while(rs83.next())
{
 ...
 qry="select mcode,mname,count(*) from rws_asset_mast_tbl a,rws_mandal_tbl b  where type_of_asset_code='03' and  substr(hab_code,1,2)='"+dcode+"'  and b.dcode=substr(a.hab_code,1,2) and b.mcode=substr(a.hab_code,6,2) group by mname,substr(hab_code,6,2),mcode order by mcode";
...
%>
 <tr>
	<td class="<%=style%> "style="text-align: middle;"><%=sno++%></td>
      <td class="<%=style%> "style="text-align: left;"><a href="#" onclick="viewWorks('rws_OandM__mvs_panchayat_drill.jsp?dcode=<%=dcode%>&dname=<%=dname%>&mcode=<%=rs8.getString(1)%>&mname=<%=rs8.getString(2)%>')"><%=rs8.getString(2)%></a></td>
