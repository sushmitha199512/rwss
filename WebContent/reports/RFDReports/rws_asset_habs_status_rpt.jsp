...
{ %>
			<tr><td align="right" class="bwborder"><a href="/pred/home.jsp?loginMode=watersoft">Home|</a></td>
				<td class="bwborder"><a href="javascript:onclick=history.go(-1)">Back|</a><a href="./rws_asset_habs_status_excel.jsp" target=_new>Excel</a></td></tr>
<%  } else {%>
...
<tr bgcolor="#8A9FCD" ">
		<td align="center" class="rptHeading" colspan=14>Status Of MVS(CPWS) Schemes</td>	
	</tr>
		<tr >
...
String totalMVSSchemesQuery="select CIRCLE_OFFICE_CODE,count(*) from rws_asset_mast_tbl where TYPE_OF_ASSET_CODE='03' and ASSET_STATUS is not null group by CIRCLE_OFFICE_CODE order by CIRCLE_OFFICE_CODE";
...
String schemeHabs="select CIRCLE_OFFICE_CODE,count(*) from rws_asset_mast_tbl  a,rws_asset_hab_tbl b where a.TYPE_OF_ASSET_CODE='03' and a.ASSET_CODE =b.ASSET_CODE and ASSET_STATUS is not null  group by CIRCLE_OFFICE_CODE order by CIRCLE_OFFICE_CODE";
...
String workingSchemes="select CIRCLE_OFFICE_CODE,count(*) from rws_asset_mast_tbl where TYPE_OF_ASSET_CODE='03' and ASSET_STATUS ='01' group by CIRCLE_OFFICE_CODE order by CIRCLE_OFFICE_CODE";
...
String wschemeHabs="select CIRCLE_OFFICE_CODE,count(*) from rws_asset_mast_tbl  a,rws_asset_hab_tbl b ,rws_complete_hab_view c where a.TYPE_OF_ASSET_CODE='03'  and ASSET_STATUS ='01'  and a.ASSET_CODE =b.ASSET_CODE  and c.panch_code=b.hab_code and c.coverage_status<>'UI' group by CIRCLE_OFFICE_CODE order by CIRCLE_OFFICE_CODE";
...
String coveredHabsALL="select c.CIRCLE_OFFICE_CODE,sum(case when a.coverage_status='FC' then 1 else 0 end)fc,sum(case when a.coverage_status='PC4' then 1 else 0 end) PC4,sum(case when a.coverage_status='PC3' then 1 else 0 end)PC3,sum(case when a.coverage_status='PC2' then 1 else 0 end) PC2,sum(case when a.coverage_status='PC1' then 1 else 0 end)PC1,sum(case when a.coverage_status='NSS' then 1 else 0 end) NSS,sum(case when a.coverage_status='NC' then 1 else 0 end) NC,sum(case when a.coverage_status='UI' then 1 else 0 end) UI  from rws_asset_mast_tbl  c,rws_asset_hab_tbl b,rws_complete_hab_view a where c.TYPE_OF_ASSET_CODE='03'  and ASSET_STATUS ='01'  and c.ASSET_CODE =b.ASSET_CODE  and b.hab_code=a.PANCH_CODE group by c.CIRCLE_OFFICE_CODE order by c.CIRCLE_OFFICE_CODE";
...
String NotMVSSchemesQuery="select CIRCLE_OFFICE_CODE,count(*) from rws_asset_mast_tbl where TYPE_OF_ASSET_CODE='03' and ASSET_STATUS <>'01' group by CIRCLE_OFFICE_CODE order by CIRCLE_OFFICE_CODE";
...
String schemeNotHabs="select CIRCLE_OFFICE_CODE,count(*) from rws_asset_mast_tbl  a,rws_asset_hab_tbl b where a.TYPE_OF_ASSET_CODE='03'  and ASSET_STATUS <>'01'  and a.ASSET_CODE =b.ASSET_CODE  group by CIRCLE_OFFICE_CODE order by CIRCLE_OFFICE_CODE";
...
%>
<td  class=rptValue><a href="#" onclick="viewWorks('rws_asset_habs_status_drill.jsp?type=3&dcode=<%=rs1.getString(2) %> ')"><%=mvsSchems %></a></td>
<td  class=rptValue><a href="#" onclick="viewWorks('rws_asset_habs_status_drill.jsp?type=4&dcode=<%=rs1.getString(2) %> ')"><%=mvsSchemHabs %></a></td>
...
<td  class=rptValue><a href="#" onclick="viewWorks('rws_asset_habs_status_drill.jsp?type=1&dcode=<%=rs1.getString(2) %> ')"><%=nmvsSchems %></a></td>
<td  class=rptValue><a href="#" onclick="viewWorks('rws_asset_habs_status_drill.jsp?type=2&dcode=<%=rs1.getString(2) %> ')"><%=nmvsSchemHabs %></a></td>
