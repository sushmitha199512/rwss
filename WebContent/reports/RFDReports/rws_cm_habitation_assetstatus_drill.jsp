...
District:</td><td class="rptvalue"><%=dname%></td>
	...
	<tr >
	<td align="center" class="rptHeading" >Mandal Wise Habitation Status Report</td>
	</tr>
...
try
{
	  ...
	  ///////////query for mvs///
	  qry="select count(*),substr(hab_code,6,2) from rws_asset_mast_tbl  where type_of_asset_code='03' and  substr(hab_code,1,2)='"+dcode+"'  group by substr(hab_code,6,2) order by substr(hab_code,6,2)";
	   Statement	stmt80=conn.createStatement();
			...
			while(rs80.next())
		{
			...
			qry="select count(*),substr(hab_code,6,2) from rws_asset_mast_tbl  where type_of_asset_code='01' and  substr(hab_code,1,2)='"+dcode+"'  group by substr(hab_code,6,2) order by substr(hab_code,6,2)";
			...
			// qry="select sum(case when type_of_asset_code='01' or type_of_asset_code='02' then 1 else 0 end),substr(a.hab_code,1,2) from rws_asset_mast_tbl a,rws_asset_hab_tbl b where  a.asset_code=b.asset_code group by substr(a.hab_code,1,2) ";
			   Statement	stmt81=conn.createStatement();
					...
					while(rs81.next())
				{
					...
					qry="select count(*),substr(hab_code,6,2) from rws_asset_mast_tbl  where type_of_asset_code='02' and  substr(hab_code,1,2)='"+dcode+"'  group by substr(hab_code,6,2) order by substr(hab_code,6,2)";
			...
			// qry="select sum(case when type_of_asset_code='01' or type_of_asset_code='02' then 1 else 0 end),substr(a.hab_code,1,2) from rws_asset_mast_tbl a,rws_asset_hab_tbl b where  a.asset_code=b.asset_code group by substr(a.hab_code,1,2) ";
			   Statement	stmt90=conn.createStatement();
					...
					while(rs90.next())
				{
					...
					qry="select count(*),substr(hab_code,6,2) from rws_asset_mast_tbl  where type_of_asset_code='05' and  substr(hab_code,1,2)='"+dcode+"'  group by substr(hab_code,6,2) order by substr(hab_code,6,2)";
		...
		//////query for handpumps///					
			...
			qry="select count(*),substr(hab_code,6,2) from rws_asset_mast_tbl  where type_of_asset_code='04' and  substr(hab_code,1,2)='"+dcode+"'  group by substr(hab_code,6,2) order by substr(hab_code,6,2)";
					// qry="select sum(case when type_of_asset_code='04' or type_of_asset_code='05' or type_of_asset_code='09' then 1 else 0 end),substr(a.hab_code,1,2) from rws_asset_mast_tbl a,rws_asset_hab_tbl b where  a.asset_code=b.asset_code group by substr(a.hab_code,1,2)";
	...
	/////query for diret pumping ///////////
			...
			qry="select count(*),substr(hab_code,6,2) from rws_asset_mast_tbl  where type_of_asset_code='09' and  substr(hab_code,1,2)='"+dcode+"'  group by substr(hab_code,6,2) order by substr(hab_code,6,2)";
					// qry="select sum(case when type_of_asset_code='04' or type_of_asset_code='05' or type_of_asset_code='09' then 1 else 0 end),substr(a.hab_code,1,2) from rws_asset_mast_tbl a,rws_asset_hab_tbl b where  a.asset_code=b.asset_code group by substr(a.hab_code,1,2)";
					...
					//	System.out.println("schoolcount0:"+schoolcount0);
							...
							while(rs93.next())
						{
							 ...
							 qry="select count(*),substr(hab_code,6,2) from rws_asset_mast_tbl  where type_of_asset_code='08' and  substr(hab_code,1,2)='"+dcode+"'  group by substr(hab_code,6,2) order by substr(hab_code,6,2)";
		...
		qry="select distinct count(b.Hab_code),substr(a.hab_code,6,2) from rws_asset_mast_tbl a,rws_asset_hab_tbl b where a.type_of_asset_code='03' and a.asset_code=b.asset_code and substr(a.hab_code,1,2)='"+dcode+"' group by substr(a.hab_code,6,2)";
		...
		qry="select distinct sum(case when a.type_of_asset_code='01' or a.type_of_asset_code='02' then 1 else 0 end),substr(a.hab_code,6,2) from rws_asset_mast_tbl a,rws_asset_hab_tbl b where a.asset_code=b.asset_code  and substr(a.hab_code,1,2)='"+dcode+"' group by substr(a.hab_code,6,2)";
			...
			qry="select count(distinct a.hab_code),substr(a.hab_code,6,2) from rws_asset_mast_tbl a,rws_asset_hab_tbl b where a.asset_code=b.asset_code and  a.type_of_asset_code in ('09','04','05') and substr(a.hab_code,1,2)='"+dcode+"' group by substr(a.hab_code,6,2)";
			...
			qry="select distinct sum(case when a.type_of_asset_code='08' then 1 else 0 end),substr(a.hab_code,6,2) from rws_asset_mast_tbl a,rws_asset_hab_tbl b where a.asset_code=b.asset_code and substr(a.hab_code,1,2)='"+dcode+"' group by substr(a.hab_code,6,2)";
	...
	qry="select distinct mcode,mname,SUM(Case When b.coverage_status='FC' then 1 Else 0 End) as FC,SUM(Case When b.coverage_status='PC4' then 1 Else 0 End) AS PC4,SUM(Case When b.coverage_status='PC3' then 1 Else 0 End) AS PC3,SUM (Case When b.coverage_status='PC2' then 1 Else 0 End) as PC2,SUM(Case When b.coverage_status='PC1' then 1 Else 0 End) as PC1,sum(case when b.coverage_status='NSS' then 1 else 0 end) as QA from rws_complete_hab_view a,rws_habitation_directory_tbl b WHERE b.HAB_CODE=a.panch_code and dcode='"+dcode+"' group by mcode,mname order by dcode";
	...
	%>
	 <tr>
	    ...
	    <td class="<%=style%> "style="text-align: left;"><a href="#" onclick="viewWorks('rws_cm_habitation_assetstatus_panchayat_drill.jsp?dcode=<%=dcode%>&dname=<%=dname%>&mcode=<%=rs8.getString(1)%>&mname=<%=rs8.getString(2)%>')"><%=rs8.getString(2)%></a></td>
