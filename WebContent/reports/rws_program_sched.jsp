...
try
{
     query ="select distinct nvl(w.work_id,'-'),nvl(w.work_name,'-') from RWS_WORK_ADMN_TBL w,RWS_WORK_COMMENCEMENT_TBL con where w.work_id=con.work_id  and substr(w.office_code,2,2)='"+dCode+"'";
...
<table border = 1 cellspacing = 0 cellpadding = 0   bordercolor=#000000 style="border-collapse:collapse" bgcolor="#ffffff" align = "center">
	<caption>
		<table  border=0 rules=none style="border-collapse:collapse" align = "right">			
			<tr align="right">
				<td class="bwborder"><a href="./rws_program_sched_frm.jsp">Back |</a></td>
...
<tr bgcolor="#8A9FCD" align="center">
		<td align="center" class="rptHeading" colspan=11>Programme Schedule Report Details</font></td>	
	</tr>

	<tr bgcolor="#edf2f8" align="center">
		<td align="center" class="rptValue" colspan=11> District: <%=dName%></font></td>	
	</tr>	
		...
		<td class="btext" rowspan=2>Engineer<br>Assigned to</td>
		<td class="btext" rowspan=2>Work Start<br> Date</td>
...
<%	
	...
	while(rs.next())
	{
		...
		//out.println(qury1);
		String qury2="select distinct m.milestone_name, st.milestone_stage_name,NVL(TO_CHAR(mil.TARGET_DATE,'DD-MON-YYYY'),'-'),NVL(TO_CHAR(mil.ACHIEVE_DATE,'DD-MON-YYYYY'),'-'),mil.PERCENT_COMPLETED from rws_milestone_tbl m,RWS_MILESTONE_TARGET_ACH_TBL mil,RWS_MILESTONE_STAGE_TBL st where m.milestone_code=mil.milestone_code and st.milestone_code=mil.milestone_code and st.milestone_stage_code=mil.milestone_stage_code and mil.work_id='"+rs.getString(1)+"'";
