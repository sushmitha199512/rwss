...
//System.out.println("dcode"+dcode);
    ...
    String sample="",sample1="",sample11="",locality="",locality1="",locality11="",asset="",hh="",cpwsqry="",cpwsqry1="",hh1="",pwsunsafe="",asset1="",asset11="";
	String typename="";
	if(type.equals("1"))
	{
   ...
   <table   bgcolor="#FFFFFF" bordercolor= "#8A9FCD"  width="100%" border=1 style="border-collapse:collapse"  ALIGN=CENTER >
    ...
    <td class=btext  nowrap rowspan=2>Mandal</td>
...
try
{
    ...
    if(type.equals("1"))
    {
    sample1="select sum(shp_no_safe),substr(c.panch_code,6,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and shp_no_safe is not null and substr(c.panch_code,1,2)='"+dcode+"' and h.hab_code in(select panch_code from rws_complete_hab_view) group by substr(c.panch_code,6,2)";
    }
    else
    {
        sample1="select sum(shp_no_safe),substr(c.panch_code,6,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and shp_no_safe is not null and substr(c.panch_code,1,2)='"+dcode+"' and h.hab_code not in(select panch_code from rws_complete_hab_view)  group by substr(c.panch_code,6,2)";
    }
  ...
  if(type.equals("1"))
  {
  sample11="select sum(shp_no),substr(c.panch_code,6,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and shp_no is not null and substr(c.panch_code,1,2)='"+dcode+"' and h.hab_code in(select panch_code from rws_complete_hab_view) group by substr(c.panch_code,6,2)";
  }
  else
  {
  sample11="select sum(shp_no),substr(c.panch_code,6,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and shp_no is not null and substr(c.panch_code,1,2)='"+dcode+"' and h.hab_code not in(select panch_code from rws_complete_hab_view)  group by substr(c.panch_code,6,2)";
   }
    ...
    if(type.equals("1"))
    {
    locality="select count(*),substr(m.hab_code,6,2) FROM RWS_ASSET_MAST_TBL M, RWS_ASSET_SCHEME_TBL S,rws_hp_subcomp_param_tbl h WHERE h.asset_code=m.asset_code and  M.ASSET_CODE=S.ASSET_CODE  AND M.TYPE_OF_ASSET_CODE='04' and substr(m.hab_code,1,2)='"+dcode+"' and hab_code in(select panch_code from rws_complete_hab_view)  group by substr(m.hab_code,6,2)";
    }
    else
    {
        locality="select count(*),substr(m.hab_code,6,2) FROM RWS_ASSET_MAST_TBL M, RWS_ASSET_SCHEME_TBL S,rws_hp_subcomp_param_tbl h WHERE h.asset_code=m.asset_code and  M.ASSET_CODE=S.ASSET_CODE  AND M.TYPE_OF_ASSET_CODE='04' and substr(m.hab_code,1,2)='"+dcode+"' and m.hab_code not in(select panch_code from rws_complete_hab_view)  group by substr(m.hab_code,6,2)";
    } 
    ...
    if(type.equals("1"))
    {  
    locality1="select sum(hps_no),substr(c.panch_code,6,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and hps_no is not null and substr(c.panch_code,1,2)='"+dcode+"' and h.hab_code in(select panch_code from rws_complete_hab_view) group by substr(c.panch_code,6,2)";
    }
    else
    {
      locality1="select sum(hps_no),substr(c.panch_code,6,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and hps_no is not null and substr(c.panch_code,1,2)='"+dcode+"' and h.hab_code not in(select panch_code from rws_complete_hab_view) group by substr(c.panch_code,6,2)";
	...
	if(type.equals("1"))
    {
	locality11="select sum(hps_no_unsafe),substr(c.panch_code,6,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and hps_no_unsafe is not null and substr(c.panch_code,1,2)='"+dcode+"' and h.hab_code in(select panch_code from rws_complete_hab_view)  group by substr(c.panch_code,6,2)";
    }
    else
    {
    	locality11="select sum(hps_no_unsafe),substr(c.panch_code,6,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and hps_no_unsafe is not null and substr(c.panch_code,1,2)='"+dcode+"' and h.hab_code not in(select panch_code from rws_complete_hab_view)  group by substr(c.panch_code,6,2)";
    ...
    if(type.equals("1"))
   {
     asset="select count(*),substr(m.hab_code,6,2) FROM RWS_ASSET_MAST_TBL M, RWS_ASSET_SCHEME_TBL S WHERE  M.ASSET_CODE=S.ASSET_CODE  AND M.TYPE_OF_ASSET_CODE='02' and substr(m.hab_code,1,2)='"+dcode+"'  and hab_code in(select panch_code from rws_complete_hab_view) group by substr(m.hab_code,6,2)";
   }
   else
   {
     asset="select count(*),substr(m.hab_code,6,2) FROM RWS_ASSET_MAST_TBL M, RWS_ASSET_SCHEME_TBL S WHERE  M.ASSET_CODE=S.ASSET_CODE  AND M.TYPE_OF_ASSET_CODE='02' and substr(m.hab_code,1,2)='"+dcode+"' and m.hab_code not in(select panch_code from rws_complete_hab_view)  group by substr(m.hab_code,6,2)";
   }
    ...
    //
     if(type.equals("1"))
     { 
     asset1="select sum(mpws_no),substr(c.panch_code,6,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and mpws_no is not null and substr(c.panch_code,1,2)='"+dcode+"' and h.hab_code in(select panch_code from rws_complete_hab_view)  group by substr(c.panch_code,6,2)";
     }
     else
     {
     asset1="select sum(mpws_no),substr(c.panch_code,6,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and mpws_no is not null and substr(c.panch_code,1,2)='"+dcode+"'  and h.hab_code not in(select panch_code from rws_complete_hab_view) group by substr(c.panch_code,6,2)";
     }
	...
	if(stsh1 != null)
	{
     ...
     if(type.equals("1"))
     {
     asset11="select sum(mpws_unsafe_no),substr(c.panch_code,6,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and mpws_unsafe_no is not null and substr(c.panch_code,1,2)='"+dcode+"'  and h.hab_code in(select panch_code from rws_complete_hab_view)  group by substr(c.panch_code,6,2)";
     }
     else
     {
     asset11="select sum(mpws_unsafe_no),substr(c.panch_code,6,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and mpws_unsafe_no is not null and substr(c.panch_code,1,2)='"+dcode+"' and h.hab_code not in (select panch_code from rws_complete_hab_view)   group by substr(c.panch_code,6,2)";
    ...
    if(type.equals("1"))
    {
    hh="select count(*),substr(m.hab_code,6,2) FROM RWS_ASSET_MAST_TBL M, RWS_ASSET_SCHEME_TBL S WHERE  M.ASSET_CODE=S.ASSET_CODE  AND M.TYPE_OF_ASSET_CODE='01' and substr(m.hab_code,1,2)='"+dcode+"' and hab_code in(select panch_code from rws_complete_hab_view) group by substr(m.hab_code,6,2)";
    }
    else
    {
        hh="select count(*),substr(m.hab_code,6,2) FROM RWS_ASSET_MAST_TBL M, RWS_ASSET_SCHEME_TBL S WHERE  M.ASSET_CODE=S.ASSET_CODE  AND M.TYPE_OF_ASSET_CODE='01' and substr(m.hab_code,1,2)='"+dcode+"' and m.hab_code not in(select panch_code from rws_complete_hab_view) group by substr(m.hab_code,6,2)";
    }
...
if(type.equals("1"))
{
      cpwsqry="select count(*),substr(m.hab_code,6,2) FROM RWS_ASSET_MAST_TBL M, RWS_ASSET_SCHEME_TBL S WHERE  M.ASSET_CODE=S.ASSET_CODE  AND M.TYPE_OF_ASSET_CODE='03' and substr(m.hab_code,1,2)='"+dcode+"' and hab_code in(select panch_code from rws_complete_hab_view) group by substr(m.hab_code,6,2)";
}
else
{
      cpwsqry="select count(*),substr(m.hab_code,6,2) FROM RWS_ASSET_MAST_TBL M, RWS_ASSET_SCHEME_TBL S WHERE  M.ASSET_CODE=S.ASSET_CODE  AND M.TYPE_OF_ASSET_CODE='03' and substr(m.hab_code,1,2)='"+dcode+"' and m.hab_code not in(select panch_code from rws_complete_hab_view) group by substr(m.hab_code,6,2)";
}
    ...
    //
     if(type.equals("1"))
     {
     cpwsqry1="select sum(cpws_no),substr(c.panch_code,6,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and cpws_no is not null and substr(c.panch_code,1,2)='"+dcode+"' and h.hab_code in(select panch_code from rws_complete_hab_view) group by substr(c.panch_code,6,2)";
     }
     else
     {
     cpwsqry1="select sum(cpws_no),substr(c.panch_code,6,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and cpws_no is not null and substr(c.panch_code,1,2)='"+dcode+"'  and h.hab_code not in(select panch_code from rws_complete_hab_view) group by substr(c.panch_code,6,2)";
     }
    ...
    if(type.equals("1"))
     {
     hh1="select sum(pws_no),substr(c.panch_code,6,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and pws_no is not null and substr(c.panch_code,1,2)='"+dcode+"' and h.hab_code in(select panch_code from rws_complete_hab_view) group by substr(c.panch_code,6,2)";
}
else
{
     hh1="select sum(pws_no),substr(c.panch_code,6,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and pws_no is not null and substr(c.panch_code,1,2)='"+dcode+"'  and h.hab_code not in (select panch_code from rws_complete_hab_view) group by substr(c.panch_code,6,2)";
}
...
if(type.equals("1"))
   {
      pwsunsafe="select sum(pws_unsafe_no),substr(c.panch_code,6,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and pws_unsafe_no is not null and substr(c.panch_code,1,2)='"+dcode+"' and h.hab_code in(select panch_code from rws_complete_hab_view) group by substr(c.panch_code,6,2)";
}
else
{
     pwsunsafe="select sum(pws_unsafe_no),substr(c.panch_code,6,2) FROM rws_hab_supply_status_tbl h,rws_panchayat_raj_tbl c WHERE h.hab_code=c.panch_code and pws_unsafe_no is not null and substr(c.panch_code,1,2)='"+dcode+"'  and h.hab_code not in(select panch_code from rws_complete_hab_view) group by substr(c.panch_code,6,2)";
    ...
    //
       ...
       if(dcode.equals("ALL"))
       {
       qry="select  mname,mcode   from rws_mandal_tbl where dcode='"+dcode+"'   ";
      }
       else
       {
        qry="select  mname,mcode   from rws_mandal_tbl where   dcode='"+dcode+"'   ";
...
shtotal5=shtotal5+shcount;
    ...
    <td class="rptValue"><a href="rws_habsupplyassetpanch_rpt.jsp?type=<%=type%>&district=<%=dcode%>&dname=<%=dname%>&mcode=<%=rs8.getString(2)%>&mname=<%=rs8.getString(1)%>"><%=rs8.getString(1)%></a></td>
