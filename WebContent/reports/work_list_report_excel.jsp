...
%>
	...
	<%
		...
		qry4="select nvl(to_char (GROUNDING_DATE,'dd-mm-yyyy'),'-') from RWS_WORK_COMMENCEMENT_TBL where work_id='"+rs1.getString(1)+"'";
		//System.out.println("THe 4nd Query is::"+qry4);		
if(stmt4!=null)stmt4.close();
		stmt4 = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
		rs4=stmt4.executeQuery(qry4);
		...
		{
%>
  		<td class=rptValue nowrap><%if(rs4.getString(1)==null) out.println("<center>-</center>"); else out.println(rs4.getString(1));%></td>
	<%
		}
		else
		{
...
if(stmt5!=null)stmt5.close();
		...
		qry6 = "select distinct nvl(remarks,'-'),NVL(TO_CHAR (DATE_OF_COMPLETION,'DD-MON-YYYY'),'-'),NVL(TO_CHAR(DATE_OF_COMM,'DD- MON-YYYY'),'-') from RWS_WORK_COMPLETION_TBL where WORK_ID='"+rs1.getString(1)+"'";
...
if(stmt6!=null)stmt6.close();
		...
		{
		%>
	  		<td class=rptValue align="center"  nowrap><%=rs6.getString(1)%></td>
...
<%
	...
	%>
	<td class=rptValue  align="center" title="Click to Get Habitation Details"><b><a href="javascript: void(0)"  onclick="window.open('work_list_report.jsp?hab=2&work_id=<%=rs1.getString(1)%>',  'windowname1',  'width=500, height=550,resizable=true'); return false;"><%=rs1.getString(5)%></a></b></td>
	<%
...
//	String qry8 = "select hl.hab_code from rws_new_est_tbl est,rws_est_new_hab_lnk_tbl hl where est.work_id=hl.work_id and hl.work_id="+rs1.getString(1)+"";
	...
	%>
			...
			<td class=rptValue  align="center" title="Click to Get Habitation Details"><b><a href="javascript: void(0)"  onclick="window.open('work_list_report.jsp?hab=1&work_id=<%=rs1.getString(1)%>',  'windowname1',  'width=500, height=550,resizable=true'); return false;"><%=rs7.getString(1)%></a></b></td>
	<%
	}
	else
	{
