...
<caption>
	<table  border=0 rules=none style="border-collapse:collapse" align = "right" width="600px">
		<tr align="right">
			<td class="bwborder"><a href="javascript:onclick=history.go(-1)">Back</a>|<a href="../home.jsp">Home</a> 
			</td>
		</tr>
		</table>
</caption>
...
<tr><td class=rptvalue align="center"><b><u>Procs. No. BI(1)/  831 / <%=prevYear%>, Dated :<font face="verdana" size=2 color="green"></font></u></b> </td></tr>
<tr><td>&nbsp;</td></tr>
...
<%String qry="select distinct division_office_name,office_code,sum(bill_amount_central),circle_office_code,division_office_code  from rws_bill_bank_bridge_temp_tbl a,rws_complete_office_view v where substr (office_code,2,2) =v.circle_office_code and substr(office_code,4,1)=v.division_office_code ";

 if(cir!=null && !cir.equals("") && div!=null && !div.equals(""))
 {
	 qry+= "  and  substr (office_code,2,2) ='"+cir+"' and  substr (office_code,4,1)='"+div+"' "; 
 }
qry+="   group by  division_office_name,office_code,circle_office_code,division_office_code ";
...
<tr><td class="btext"><%=recCount++ %> .<%=rs.getString(1) %></td></tr>
		<tr>
		<td>
		...
		<td class=rptvalue   rowspan = 2 width="100px" cellspacing=0><font color="red">Work Name</font></td>
...
qry1="select a.work_id,work_name,bill_no,bill_amount_central,bill_activation_flag,c.subprogramme_code,s.subprogramme_name ,p.programme_name,no_of_habs from rws_bill_bank_bridge_temp_tbl a,rws_work_bill_tbl b,rws_work_admn_tbl c,rws_subprogramme_tbl s ,rws_programme_tbl p where a.work_id=b.work_id and a.work_id=c.work_id and c.work_id=b.work_id and a.bill_sl_no=b.bill_sl_no  and substr(a.office_code,2,2)='"+rs.getString(4)+"' and substr(a.office_code,4,1)='"+rs.getString(5)+"' and c.programme_code=s.programme_code and c.subprogramme_code=s.subprogramme_code and p.programme_code=c.programme_code  ";
...
while(rs2.next())
{
	if(rs2.getInt(1)>1)
	{
	qry3="select  sum(nvl(CENSUS_SC_POPU,0)), sum(nvl(CENSUS_ST_POPU,0)) ,sum(nvl(CENSUS_PLAIN_POPU,0)) from RWS_EST_NEW_HAB_LNK_TBL a, "+tname+" b   where work_id='"+rs1.getString(1)+"' and a.hab_code=b.hab_code";
...
%>
    <tr>
	   <td class=rptvalue align="center" width="20px"><%=sno++%></td>
		...
		{
		qry4="select  nvl(CENSUS_SC_POPU,0) , nvl(CENSUS_ST_POPU,0) ,nvl(CENSUS_PLAIN_POPU,0) from RWS_EST_NEW_HAB_LNK_TBL a, rws_habitation_directory_tbl b   where work_id='"+rs1.getString(1)+"' and a.hab_code=b.hab_code";
