    ...
    <br/>
         ...
         <%if(mode1!=null && mode1!="" && mode1.equals("works")){%>
          ...
          <%
              ...
              try {
                     query = "select a.SANCTION_AMOUNT,a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(PROBABLE_DATE,'dd/mm/yyyy'),'--'),p.PROGRAMME_NAME,nvl(to_char(TARGET_DATE_COMP,'dd/mm/yyyy'),'--') as TARGET_DATE_COMP,dist.dname as dname,nvl(lead_habitation,'--') as lead_habitation from rws_work_admn_tbl a,rws_programme_tbl p,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c,rws_contractor_selection_tbl m,rws_district_tbl dist "
  							+" where substr(a.work_id,5,2)=dist.dcode and  WORK_CANCEL_DT is null and a.programme_code in ("+programme+") and SUBSTR(a.work_id,7,2) in ("+ schemes +")  and a.PROGRAMME_CODE=p.PROGRAMME_CODE and "
