...
try
{
	...
	if(tbl_check!=null)
	{
     // System.out.println(" i am getting status years list"+"status loop end"+(start-1)+"status loop start"+fyear1);
      ...
      conn.commit();
		...
		qry4="create table rws_slippage_temp_status_tbl(hab_code VARCHAR2(16) not null,hab_name VARCHAR2(50),cur_status VARCHAR2(3),prev_status VARCHAR2(3),?,PRIMARY KEY (hab_code))";
		stmt_tbl=conn.createStatement();
		 PreparedStatement ps3=conn.prepareStatement(qry4);
         ps3.setString(1, finalstr);
		ps3.executeUpdate();
    ...
    qry4="create table rws_slippage_temp_status_tbl(hab_code VARCHAR2(16) not null,hab_name VARCHAR2(50),cur_status VARCHAR2(3),prev_status VARCHAR2(3),?,PRIMARY KEY (hab_code))";
	stmt_tbl=conn.createStatement();
	PreparedStatement pst=conn.prepareStatement(qry4);
    pst.setString(1, finalstr);
	pst.executeUpdate();
    ...
    cov_rs=cov_stmt.executeQuery(cov_query);
	 ...
	 {  hcode=cov_rs.getString(1);
        ...
        if(hcode!=null && !hcode.equals(""))
         {
          query="update rws_slippage_temp_status_tbl set status"+j+"=? where hab_code=?"; 
          PreparedStatement pst=conn.prepareStatement(query);
          pst.setString(1,cov_status);
          pst.setString(2,sel_rs.getString(1));
          pst.executeUpdate();
