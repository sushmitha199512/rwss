...
qry="select dname,substr(AP_HAB_CODE,1,2),count(BENIF_ID),sum(TOTALTOILETCOST)/10000,sum(case when CONSTRUCTED_YEAR is not null then 1 else 0 end) as concount from rws_ihhl_tbl a,rws_district_tbl b where substr(AP_HAB_CODE,1,2)=dcode group by dname,substr(AP_HAB_CODE,1,2) order by substr(AP_HAB_CODE,1,2)";
