...
<%  
    ...
    { 
       response.setContentType("text/html");
		...
		//System.out.println(e+"Couldn't create Pool");
		}
...
while(r6.next())
{
   ...
   if(count!=0){
	 ...
	 if(x.equals("l"))
    {
     XYChart ch = new XYChart(570, 186);
    
	// ch.addTitle("Scheme Wise Representative Proposal", "times new roman Bold Italic",12);
...
%>
  ...
  for(j=0;j<count;j++){
	...
	if(x.equals("b"))
   {
	...
	ch.xAxis().setTitle("Scheme Wises");
...
if(x.equals("p"))
 {
    ...
    // ch.addTitle2(Chart.Top, "Asset Type Representative Proposal Graph Report", "Times New Roman Bold Italic",13);
	ch.set3D();
...
<%	 
	   ...
	   for(int m=0;m<count;m++) {%>
			...
			<td class="mystyle1"><a href="./rws_sche_pro_list_rpt.jsp?district=<%=district%>&mandal=<%=mandal%>&panchayat=<%=panchayat%>&scheme[]=<%=districts[m]%>&FDate=<%=fdate%>"><%=data[m]%></a></td>
		</tr>
  <%
		   total=total+data[m];
   }%>
