	...
	<caption>
		<table  border=0 rules=none style="border-collapse:collapse" align = "right">			
			<tr align="right">
				...
				<td class="bwborder"><a href="./rws_excel3_prnt.jsp" target="_new">Print |</a></td>
			    <td class="bwborder"><a href="./rws_district_update_date_xls.jsp" target="_new">Excel</a></td>
			</tr>
		</table>
	</caption>
   ...
   if(dcode != null && dName!=null && rDistrict.equals("00") && updateDate==null)
	{
		...
		query="SELECT DISTINCT D.DCODE, D.DNAME, PR.PANCH_NAME,PR.PANCH_CODE,nvl(HD.COVERAGE_STATUS,'--'),nvl(TO_CHAR(update_date,'DD-MON-YYYY'),'-') FROM RWS_DISTRICT_TBL D, RWS_HABITATION_DIRECTORY_TBL HD,RWS_PANCHAYAT_RAJ_TBL PR WHERE SUBSTR(HD.HAB_CODE,1,2) = D.DCODE AND PR.PANCH_CODE = HD.HAB_CODE and update_date is null ORDER BY D.DCODE";

		
	}
	if(dcode != null && dName!=null && rDistrict.equals("00"))
	{
		//System.out.println("IN DISTRICT SELECTION...");
		stmt = conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);
		
        query="SELECT distinct d.dcode,D.DNAME,PR.PANCH_NAME,PR.PANCH_CODE,nvl(HD.COVERAGE_STATUS,'--'),nvl(TO_CHAR(update_date,'DD-MON-YYYY'),'-') from RWS_DISTRICT_TBL D, RWS_HABITATION_DIRECTORY_TBL HD,RWS_PANCHAYAT_RAJ_TBL PR where update_date is null and SUBSTR(HD.HAB_CODE,1,2) = D.DCODE AND PR.PANCH_CODE = HD.HAB_CODE AND SUBSTR(HD.HAB_CODE,1,2) = '"+dcode+"' and d.dname='"+dName+"' order by d.dcode";
	}
