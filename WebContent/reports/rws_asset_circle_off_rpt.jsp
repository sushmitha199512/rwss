...
/*if(!assetType.equals("00")  && district.equals("00")  &&  mandal.equals("-1")))
 {

	query1 = "insert into TEMP_ASSET_HAB_TBL (select hab_code, asset_Code from RWS_ASSET_HAB_TBL where  substr(asset_code,7,2)='" +assetType + "'  union select hab_code, asset_code from RWS_ASSET_MAST_TBL where substr(asset_code,7,2)='" +assetType + "') ";

	System.out.println("panchayat="+  query1);
 }
 else
 {
	query1 = "insert into TEMP_ASSET_HAB_TBL (select hab_code, asset_Code from RWS_ASSET_HAB_TBL   union select hab_code, asset_code from RWS_ASSET_MAST_TBL) ";
 }
 ...
 if(b>0)
 {
	 nic.watersoft.commons.println("TABLE CREATED");
 }
 else
 {
...
if(!assetType.equals("00")  && dCode.equals("00"))
{
	...
	query = "SELECT  distinct c.circle_office_code , c.circle_office_name as \"Circle Name\", nvl(sum(asset_cost),0) as \"Total Asset Cost (in Lakhs)\", count(am.asset_code) as \"Total No. of Assets\", count(am.hab_code) as \"Total No. of Habitations\" from  RWS_ASSET_MAST_TBL am, rws_circle_office_tbl c where c.circle_office_code=am.circle_office_code and substr(am.asset_code,7,2) = '" +assetType + "' group by c.circle_office_code , c.circle_office_name";
   
}
else if(assetType.equals("00") && dCode.equals("00"))
	{
		query = "SELECT distinct c.circle_office_code , c.circle_office_name as \"Circle Name\", nvl(sum(asset_cost),0) as \"Total Asset Cost (in Lakhs)\", count(am.asset_code) as \"Total No. of Assets\", count(am.hab_code) as \"Total No. of Habitations\" from  RWS_ASSET_MAST_TBL am, rws_circle_office_tbl c where c.circle_office_code=am.circle_office_code  group by c.circle_office_code , c.circle_office_name";
   }
