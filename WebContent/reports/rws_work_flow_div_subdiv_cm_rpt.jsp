...
<BODY>
	  ...
	  entrustedQuery="select count(distinct a.work_id) from RWS_CONTRACTOR_SELECTION_TBL b,RWS_WORK_ADMN_TBL a,rws_new_est_tbl e  where "
	                ...
	                +" TARGET_DATE_COMP>='01-APR-"+ yearStart+"' and ( TARGET_DATE_COMP is not null OR TARGET_DATE_COMP <> '') and a.CORE_DASHBOARD_STATUS='Y'";
	 ...
	 /*  completedQuery="select count(distinct a.work_id) from RWS_WORK_ADMN_TBL a,RWS_WORK_COMMENCEMENT_TBL b,RWS_CONTRACTOR_SELECTION_TBL c,RWS_SOURCE_IDEN_FIN_TBL s,rws_new_est_tbl e,RWS_WORK_COMP_HAB_LNK_TBL h,rws_habitation_directory_tbl hd where a.work_id in "
		...
		if(mode.equals("circles")){
			...
			techworksQuery+="  and substr(a.office_code,2,2)='"+res.getString("circle_office_code")+"'";
			entrustedQuery+=" and substr(a.office_code,2,2)='"+res.getString("circle_office_code")+"'";
			groundedQuery+=" and substr(b.office_code,2,2)='"+res.getString("circle_office_code")+"'";
			underprogresQuery+=" and substr(a.office_code,2,2)='"+res.getString("circle_office_code")+"'";
			...
			techworksQuery+=" and substr(a.office_code,2,2)='"+ccode+"' and substr(a.office_code,4,1)='"+res.getString("division_office_code")+"'";
			entrustedQuery+=" and substr(a.office_code,2,2)='"+ccode+"' and substr(a.office_code,4,1)='"+res.getString("division_office_code")+"'";
			groundedQuery+=" and substr(b.office_code,2,2)='"+ccode+"' and substr(b.office_code,4,1)='"+res.getString("division_office_code")+"'";
			underprogresQuery+=" and substr(a.office_code,2,2)='"+ccode+"' and substr(a.office_code,4,1)='"+res.getString("division_office_code")+"'";
			...
			techworksQuery+=" and substr(a.office_code,2,2)='"+ccode+"' and substr(a.office_code,4,1)='"+divCode+"' and substr(a.office_code,5,2)='"+res.getString("subdivision_office_code")+"'";
			entrustedQuery+=" and substr(a.office_code,2,2)='"+ccode+"' and substr(a.office_code,4,1)='"+divCode+"' and substr(a.office_code,5,2)='"+res.getString("subdivision_office_code")+"'";
			...
			underprogresQuery+=" and substr(a.office_code,2,2)='"+ccode+"' and substr(a.office_code,4,1)='"+divCode+"' and substr(a.office_code,5,2)='"+res.getString("subdivision_office_code")+"'";
        	...
        	<tr>
				...
				<%if(mode.equals("circles")){ %>
				<td class="<%=style%>" style="text-align: left;" align="left" ><a href="./rws_work_flow_div_subdiv_cm_rpt.jsp?mode=divisions&programme=<%=programme%>&finyear=<%=finyear%>&programmeName=<%=programmeName%>&ccode=<%=ccode%>&cname=<%=cname%>"><%=cname%></a></td>
				<%}else if(mode.equals("divisions")){ %>
				<td class="<%=style%>" style="text-align: left;" align="left" ><a href="./rws_work_flow_div_subdiv_cm_rpt.jsp?mode=subdivisions&programme=<%=programme%>&finyear=<%=finyear%>&programmeName=<%=programmeName%>&ccode=<%=ccode%>&cname=<%=cname%>&divCode=<%=divCode%>&divname=<%=divName%>"><%=divName%></a></td>
				<%}else if(mode.equals("subdivisions")){ %>
				<td class="<%=style%>" style="text-align: left;" align="left" ><%=subdivName%></td>
				<%} %>                                              
				<%if(mode.equals("circles")){ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_work_flow_status_cm_drill.jsp?mode=admin_sactioned&programme=<%=programme%>&yearStart=01-APR-<%=yearStart%>&yearEnd=31-MAR-<%=yearEnd%>&message=<%=cname%> Circle <%=programmeName%> Admin Sanctioned&ccode=<%=ccode%>&year=<%=yearStart%>')"><%=admin_sactioned %></a></td>
				<%}else if(mode.equals("divisions")){ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_work_flow_status_cm_drill.jsp?mode=admin_sactioned&programme=<%=programme%>&yearStart=01-APR-<%=yearStart%>&yearEnd=31-MAR-<%=yearEnd%>&message=<%=cname%> Circle, <%=divName%> Division <%=programmeName%> Admin Sanctioned&ccode=<%=ccode%>&divcode=<%=divCode%>&year=<%=yearStart%>')"><%=admin_sactioned %></a></td>
				<%}else if(mode.equals("subdivisions")){ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_work_flow_status_cm_drill.jsp?mode=admin_sactioned&programme=<%=programme%>&yearStart=01-APR-<%=yearStart%>&yearEnd=31-MAR-<%=yearEnd%>&message=<%=cname%> Circle, <%=divName%> Division, <%=subdivName%> SubDivision <%=programmeName%> Admin Sanctioned&ccode=<%=ccode%>&divcode=<%=divCode%>&subdivcode=<%=subdivCode%>&year=<%=yearStart%>')"><%=admin_sactioned %></a></td>
				<%} %>                                                
				<%if(mode.equals("circles")){ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_work_flow_status_cm_drill.jsp?mode=tech_sanctioned&programme=<%=programme%>&yearStart=01-APR-<%=yearStart%>&yearEnd=31-MAR-<%=yearEnd%>&message=<%=cname%> Circle <%=programmeName%> Tech. Sanctioned&ccode=<%=ccode%>&year=<%=yearStart%>')"><%=tech_sanctioned %></a></td>
				<%}else if(mode.equals("divisions")){ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_work_flow_status_cm_drill.jsp?mode=tech_sanctioned&programme=<%=programme%>&yearStart=01-APR-<%=yearStart%>&yearEnd=31-MAR-<%=yearEnd%>&message=<%=cname%> Circle, <%=divName%> Division <%=programmeName%> Tech. Sanctioned&ccode=<%=ccode%>&divcode=<%=divCode%>&year=<%=yearStart%>')"><%=tech_sanctioned %></a></td>
				<%}else if(mode.equals("subdivisions")){ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_work_flow_status_cm_drill.jsp?mode=tech_sanctioned&programme=<%=programme%>&yearStart=01-APR-<%=yearStart%>&yearEnd=31-MAR-<%=yearEnd%>&message=<%=cname%> Circle, <%=divName%> Division, <%=subdivName%> SubDivision <%=programmeName%> Tech. Sanctioned&ccode=<%=ccode%>&divcode=<%=divCode%>&subdivcode=<%=subdivCode%>&year=<%=yearStart%>')"><%=tech_sanctioned %></a></td>
				<%} %>                                            
				<%if(mode.equals("circles")){ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_work_flow_status_cm_drill.jsp?mode=entrusted&programme=<%=programme%>&yearStart=01-APR-<%=yearStart%>&yearEnd=31-MAR-<%=yearEnd%>&message=<%=cname%> Circle <%=programmeName%> Entrusted&ccode=<%=ccode%>&year=<%=yearStart%>')"><%=entrusted %></a></td>
				<%}else if(mode.equals("divisions")){ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_work_flow_status_cm_drill.jsp?mode=entrusted&programme=<%=programme%>&yearStart=01-APR-<%=yearStart%>&yearEnd=31-MAR-<%=yearEnd%>&message=<%=cname%> Circle, <%=divName%> Division <%=programmeName%> Entrusted&ccode=<%=ccode%>&divcode=<%=divCode%>&year=<%=yearStart%>')"><%=entrusted %></a></td>
				<%}else if(mode.equals("subdivisions")){ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_work_flow_status_cm_drill.jsp?mode=entrusted&programme=<%=programme%>&yearStart=01-APR-<%=yearStart%>&yearEnd=31-MAR-<%=yearEnd%>&message=<%=cname%> Circle, <%=divName%> Division, <%=subdivName%> SubDivision <%=programmeName%> Entrusted&ccode=<%=ccode%>&divcode=<%=divCode%>&subdivcode=<%=subdivCode%>&year=<%=yearStart%>')"><%=entrusted %></a></td>
				<%} %>                                             
				<%if(mode.equals("circles")){ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_work_flow_status_cm_drill.jsp?mode=grounded&programme=<%=programme%>&yearStart=01-APR-<%=yearStart%>&yearEnd=31-MAR-<%=yearEnd%>&message=<%=cname%> Circle <%=programmeName%> Grounded&ccode=<%=ccode%>&year=<%=yearStart%>')"><%=grounded %></a></td>
				<%}else if(mode.equals("divisions")){ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_work_flow_status_cm_drill.jsp?mode=grounded&programme=<%=programme%>&yearStart=01-APR-<%=yearStart%>&yearEnd=31-MAR-<%=yearEnd%>&message=<%=cname%> Circle, <%=divName%> Division <%=programmeName%> Grounded&ccode=<%=ccode%>&divcode=<%=divCode%>&year=<%=yearStart%>')"><%=grounded %></a></td>
				<%}else if(mode.equals("subdivisions")){ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_work_flow_status_cm_drill.jsp?mode=grounded&programme=<%=programme%>&yearStart=01-APR-<%=yearStart%>&yearEnd=31-MAR-<%=yearEnd%>&message=<%=cname%> Circle, <%=divName%> Division, <%=subdivName%> SubDivision <%=programmeName%> Grounded&ccode=<%=ccode%>&divcode=<%=divCode%>&subdivcode=<%=subdivCode%>&year=<%=yearStart%>')"><%=grounded %></a></td>
				<%} %>                                                
				<%if(mode.equals("circles")){ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_work_flow_status_cm_drill.jsp?mode=under_progress&programme=<%=programme%>&yearStart=01-APR-<%=yearStart%>&yearEnd=31-MAR-<%=yearEnd%>&message=<%=cname%> Circle <%=programmeName%> Under Progress&ccode=<%=ccode%>&year=<%=yearStart%>')"><%=under_progress %></a></td>
				<%}else if(mode.equals("divisions")){ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_work_flow_status_cm_drill.jsp?mode=under_progress&programme=<%=programme%>&yearStart=01-APR-<%=yearStart%>&yearEnd=31-MAR-<%=yearEnd%>&message=<%=cname%> Circle, <%=divName%> Division <%=programmeName%> Under Progress&ccode=<%=ccode%>&divcode=<%=divCode%>&year=<%=yearStart%>')"><%=under_progress %></a></td>
				<%}else if(mode.equals("subdivisions")){ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_work_flow_status_cm_drill.jsp?mode=under_progress&programme=<%=programme%>&yearStart=01-APR-<%=yearStart%>&yearEnd=31-MAR-<%=yearEnd%>&message=<%=cname%> Circle, <%=divName%> Division, <%=subdivName%> SubDivision <%=programmeName%> Under Progress&ccode=<%=ccode%>&divcode=<%=divCode%>&subdivcode=<%=subdivCode%>&year=<%=yearStart%>')"><%=under_progress %></a></td>
				<%} %>                                                              
				<%if(mode.equals("circles")){ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_work_flow_status_cm_drill.jsp?mode=completed&programme=<%=programme%>&yearStart=01-APR-<%=yearStart%>&yearEnd=31-MAR-<%=yearEnd%>&message=<%=cname%> Circle <%=programmeName%> Completed&ccode=<%=ccode%>&year=<%=yearStart%>')"><%=completed %></a></td>
				<%}else if(mode.equals("divisions")){ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_work_flow_status_cm_drill.jsp?mode=completed&programme=<%=programme%>&yearStart=01-APR-<%=yearStart%>&yearEnd=31-MAR-<%=yearEnd%>&message=<%=cname%> Circle, <%=divName%> Division <%=programmeName%> Completed&ccode=<%=ccode%>&divcode=<%=divCode%>&year=<%=yearStart%>')"><%=completed %></a></td>
				<%}else if(mode.equals("subdivisions")){ %>
				<td class="<%=style%>" style="text-align: right;" align="right"><a href="#" onclick="viewWorks('./rws_work_flow_status_cm_drill.jsp?mode=completed&programme=<%=programme%>&yearStart=01-APR-<%=yearStart%>&yearEnd=31-MAR-<%=yearEnd%>&message=<%=cname%> Circle, <%=divName%> Division, <%=subdivName%> SubDivision <%=programmeName%> Completed&ccode=<%=ccode%>&divcode=<%=divCode%>&subdivcode=<%=subdivCode%>&year=<%=yearStart%>')"><%=completed %></a></td>
				<%} %>
                <%if(mode.equals("circles")){ %>
