	   ...
	   while(rs.next())
	   {
		   ...
		   %>
 		   <tr>     
			  ...
			  <td class=label><a href="physicalprogressdisprosubdivisionreport.jsp?circle=<%=circlecode%>&division=<%=divisioncode%>&subdivision=<%=rs.getString(7)%>&AssetType=<%=AssetType%>&cmbYear=<%=year%>&cmbMonth=<%=month%>" ><%=rs.getString(1)%></a></td>
