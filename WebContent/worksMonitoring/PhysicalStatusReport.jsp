...
<tr>
<td colspan=20 align=center class=label bgcolor=#A899FB>Physical Status Report</b></center></td>
</tr>
<tr>
	   ...
	   while(rs.next())
	   {
		   ...
		   %>
 		   ...
 		   <tr>     
			  ...
			  <td class=label><a href="PhysicalStatusDivisionReport.jsp?circle=<%=rs.getString(6)%>&division=<%=rs.getInt(7)%>&subdivision=<%=subdivisioncode%>&AssetType=<%=AssetType%>&cmbYear=<%=year%>&cmbMonth=<%=month%>" ><%=rs.getString(1)%></a></td> 
...
if (!circlecode.equals("00") && !AssetType.equals("") && !year.equals("") && divisioncode==0 && subdivisioncode.equals("0") && month.equals("0"))
	{
 query="select circle_office_name,sum(NO_NEW_WORKS_SANC_UPTO_MON ) as NO_NEW_WORKS_SANC_UPTO_MON  ,sum(NO_NEW_WORKS_SANC_DUR_MON ) as NO_NEW_WORKS_SANC_DUR_MON ,sum(TOT_NO_WORKS_COMP_DUR_MON ) as TOT_NO_WORKS_COMP_DUR_MON ,sum(TOT_NO_WORKS_COMP_UPTO_MON ) as TOT_NO_WORKS_COMP_UPTO_MON,substr(pp.office_code,4,1) from rws_circle_office_tbl co,rws_physical_progress_tbl pp where co.circle_office_code=substr(pp.office_code,2,2) and substr(pp.office_code,2,2)=? and year= ? and asset_type = ? group by circle_office_name, substr(pp.office_code,4,1)";
...
if (!circlecode.equals("00") && !AssetType.equals("") && !year.equals("") && divisioncode!=0 && subdivisioncode.equals("0") && month.equals("0"))
	{
	System.out.println("circle code"+circlecode);
	query="select division_office_name,sum(NO_NEW_WORKS_SANC_UPTO_MON ) as NO_NEW_WORKS_SANC_UPTO_MON  ,sum(NO_NEW_WORKS_SANC_DUR_MON ) as NO_NEW_WORKS_SANC_DUR_MON ,sum(TOT_NO_WORKS_COMP_DUR_MON ) as TOT_NO_WORKS_COMP_DUR_MON ,sum(TOT_NO_WORKS_COMP_UPTO_MON ) as TOT_NO_WORKS_COMP_UPTO_MON from rws_division_office_tbl co,rws_physical_progress_tbl pp where co.circle_office_code = substr(pp.office_code,2,2) and co.division_office_code = substr(pp.office_code,4,1) and substr(pp.office_code,2,2)=? and year= ? and asset_type = ? and substr(pp.office_code,4,1)=? group by division_office_name, substr(pp.office_code,4,1)";
	pstmt=conn.prepareStatement(query);
	   pstmt.setString(1,circlecode);
	    pstmt.setString(2,year);
	    pstmt.setString(3,AssetType);
...
sln=1;
	...
	{
	System.out.println("circle code"+circlecode);
	query="select division_office_name,NO_NEW_WORKS_SANC_UPTO_MON,NO_NEW_WORKS_SANC_DUR_MON,TOT_NO_WORKS_COMP_DUR_MON ,TOT_NO_WORKS_COMP_UPTO_MON from rws_division_office_tbl co,rws_physical_progress_tbl pp where co.circle_office_code = substr(pp.office_code,2,2) and co.division_office_code = substr(pp.office_code,4,1) and substr(pp.office_code,2,2)=? and substr(pp.office_code,4,1)=? and year= ? and asset_type = ? and month=? ";
	pstmt=conn.prepareStatement(query);
	   pstmt.setString(1,circlecode);
	   pstmt.setInt(2,divisioncode);
