	    ...
	    while(rs.next())
	   {
		   ...
		   %>
 		   ...
 		   <tr>     
			  ...
			  <td class=label><a href="ReleasesExpenditureSubdivisionReport.jsp?circle=<%=circlecode%>&division=<%=divisioncode%>&subdivision=<%=rs.getString(20)%>&AssetType=<%=AssetType%>&cmbYear=<%=year%>&cmbMonth=<%=month%>" ><%=rs.getString(1)%></a></td> 
