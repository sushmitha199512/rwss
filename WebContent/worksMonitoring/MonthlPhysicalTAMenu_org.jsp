...
function fnsubmit()
{
	document.f1.action="MonthlyPhysicalTAReport.jsp";
	document.f1.submit();
}
...
<table cellpadding="0" cellspacing="0">
		<tr>
			...
			<td background="resources/images/top_center_b.gif" width="210" class="heading" valign="middle" align="center" class id ="mainheading">Monthly Physical Targets & Achievements Report</td>
...
<form name=f1 method="post" action="MonthlPhysicalTAMenu.jsp">
...
<%
	 ...
	 {   
		...
		<td class=label>:</td>
		 <td><select name=circle onChange="getDivisions()" class="comboFont"> 
...
%>
	...
	<td class=label>:</td>
	<td><select name=division onChange="getSubDivisions()" class="comboFont">
...
%>
	...
	<td class=label>:</td>
	<td><select name=subdivision class="comboFont">
	 <option value=0></option>
<%
...
%>
	...
	<td class=label>:</td>
	<td><select name=AssetType  onChange="assettypesubmit()" class="comboFont">
...
dtr2="select distinct asset_type,type_of_asset_name from rws_physical_progress_tbl pp,rws_asset_type_tbl at where pp.asset_type=at.TYPE_OF_ASSET_CODE and substr(pp.office_code,2,2) = '" + circlecode + "'";
	...
	<td class=label>:</td>
	<td><select name=cmbYear  onChange="yearsubmit()" class="comboFont">
	<!-- <option value=0>Please Select...</option> -->
<%
	for (i=0; i < 5; i++)
	{
...
if(circlecode.equals("00"))
	...
	dtr3="select distinct year from rws_physical_progress_tbl pp,rws_asset_type_tbl at where pp.asset_type=at.TYPE_OF_ASSET_CODE";
		 }
		 else
		 {
			 dtr3="select distinct year from rws_physical_progress_tbl where substr(office_code,2,2) = '" + circlecode + "'";
		 }
//nyear=request.getParameter("cmbYear");
pstmt=conn.prepareStatement(dtr3);
	...
	<td class=label>:</td>
	<td><select name=cmbMonth class="comboFont">
	<!-- <option value=0></option> -->
<%
	//str="select month_code,month_name from rws_month_mast_tbl order by month_code";
if(!circlecode.equals("00"))
	str="select distinct month,month_name from rws_physical_progress_tbl pp,rws_month_mast_tbl mm where pp.month=mm.month_code and substr(office_code,2,2) = '" + circlecode + "'";
	else
		str="select distinct month,month_name from rws_physical_progress_tbl pp,rws_month_mast_tbl mm where pp.month=mm.month_code";
    pstmt=conn.prepareStatement(str);
	rs=pstmt.executeQuery();
...
%>
    ...
    <%
		...
		<tr>	
			...
			<td background="resources/images/bottom_center.gif" width="230" class="heading" valign="middle"	align="center"></td>
