...
<tr>
<td colspan=20 align=center class=label bgcolor=#A899FB>Physical Progress During the Year Report</b></center></td>
</tr>
<tr>
		   ...
		   %>
 		   ...
 		   <tr>     
			  <td class=label><%=sln%></td>
			   <td class=label><a href="PhysicalProgressDuringYearDivisionReport.jsp?circle=<%=rs.getString(14)%>&division=<%=rs.getInt(15)%>&subdivision=<%=subdivisioncode%>&AssetType=<%=AssetType%>&cmbYear=<%=year%>&cmbMonth=<%=month%>" ><%=rs.getString(1)%></a></td>
...
sln=1;
	...
	{
		System.out.println("circle code"+circlecode);
		System.out.println(request.getParameter("circle"));
		System.out.println(request.getParameter("division"));
		System.out.println(request.getParameter("subdivision"));
