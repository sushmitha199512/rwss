    		...
    		rst = stmt1.executeQuery(qry1); 
    			...
    			while(rst.next())
    			{
    				...
    				str="select count(*) from RWS_RESIDUAL_CHLORINE_SMS_TBL B,RWS_MOBILE_SMS_REGISTER_TBL c  WHERE  to_char(b.sms_mobile) =c.MOBILE and PPM_VALUE<0.2  and substr(c.habitation_code,1,2)='"+rst.getString(1)+"' and (ACTION_ENTERED_DATE is null or ACTION_ENTERED_DATE='')  and TEST_DATE>=to_date('"+fromdate+"','dd/mm/yyyy') and TEST_DATE<=to_date('"+todate+"','dd/mm/yyyy')+1";
    				 ...
    				 stmt2.close();
    				 rs.close();
    				 str="select count(*) from RWS_RESIDUAL_CHLORINE_SMS_TBL B,RWS_MOBILE_SMS_REGISTER_TBL c  WHERE  to_char(b.sms_mobile) =c.MOBILE  and substr(c.habitation_code,1,2)='"+rst.getString(1)+"'  and TEST_DATE>=to_date('"+fromdate+"','dd/mm/yyyy') and TEST_DATE<=to_date('"+todate+"','dd/mm/yyyy')+1";
    				 stmt2=conn.createStatement();
    				 rs=stmt2.executeQuery(str);
    				 if(rs.next())
    	    		   {
    			   ...
    			   %>
    				  ...
    				  <td class="<%=style%> "style="text-align: right;"><%=rcA%></td>
    				  <td class="<%=style%> "style="text-align: right;"><%=rcN%></td>
    				  <td class="<%=style%> "style="text-align: right;"><%=rcTot%></td>
    				  
    				  <td class="<%=style%> "style="text-align: right;"><%=dfA%></td>
    				  <td class="<%=style%> "style="text-align: right;"><%=dfN%></td>
    				  <td class="<%=style%> "style="text-align: right;"><%=drA%></td>
    				  <td class="<%=style%> "style="text-align: right;"><%=drN%></td>
    				  
    				  <td class="<%=style%> "style="text-align: right;"><%=ohA%></td>
    				  <td class="<%=style%> "style="text-align: right;"><%=ohN%></td>
    				  <td class="<%=style%> "style="text-align: right;"><%=glA%></td>
    				  <td class="<%=style%> "style="text-align: right;"><%=glN%></td>
