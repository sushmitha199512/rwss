...
window.opener.document.forms[0].elements["chlorides["+<%=value%>+"].assetCode"].value=astCode;
window.opener.document.forms[0].elements["chlorides["+<%=value%>+"].assetName"].value=assetName;
window.opener.document.forms[0].elements["chlorides["+<%=value%>+"].sourceCode"].value=sourceCode;
window.opener.document.forms[0].elements["chlorides["+<%=value%>+"].sourceName"].value=sourceName;
<%--window.opener.document.forms[0].elements["chlorides["+<%=value%>+"].astType"].value=astCode.substring(6,8);
 window.opener.document.forms[0].elements["chlorides["+<%=value%>+"].repairs"].value=sourceName; --%>
...
window.opener.document.forms[0].action="switch.do?prefix=/smsmobile&page=/ResidualChlorine.do&mode=chloridesourceprint";
...
<table border= 0 cellspacing = 0 cellpadding = 0   bordercolor=#000000 style="border-collapse:collapse" bgcolor="">	
	...
	<tr>
		...
		<html:option value=""><font class="myfontclr1">SELECT...</font></html:option>
       	...
       	<%String qry="select ASSET_CODE,ASSET_NAME FROM RWS_ASSET_MAST_TBL WHERE substr(HAB_CODE,1,2)='"+habitation.substring(0,2)+"' AND  substr(HAB_CODE,6,2)='"+habitation.substring(5, 7)+"' and substr(HAB_CODE,13,2)='"+habitation.substring(12, 14)+"' AND TYPE_OF_ASSET_CODE IN ('01','02','03','04','05','08','06','09')";  
