...
<script language="JavaScript">
	  ...
	  function getParameters(testcode,s) 
	  {
	     // alert(" in get parameters function " +testcode);
	   var testId = '<%=request.getParameter("testId")%>';
	  // alert(testId);
		 // var testval=that.value;
		 var testval=testcode;
	     /// alert("value of test :"+testval);
		if(testcode!=0)
		{
		  // alert("in if checked" +s);
		  // 
		 //  url = "switch.do?prefix=/wquality&page=/WaterSampleRes.do&mode=test&testCode="+testval+"&testId="+<%=request.getParameter("testId")%>+"&sourcecode="+s;
		  // alert(url);
		  
		  url = "/pred/switch.do?prefix=/wquality&page=/WaterSampleRes.do&mode=test&testCode="+testval+"&testId="+testId+"&sourceCode="+s;
		  // alert(url);
...
String testcode=(String)samples.get(0); %>

<FONT class=myfontclr1>
<input type="radio" checked="true" name="testCode<%=(String)samples.get(0)%>" value="<%=(String)samples.get(0)%>" onClick=getParameters('<%=testcode%>','<%=s%>')></FONT>
</td>
