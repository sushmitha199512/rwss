...
function validateSubmit(frm)
{
	...
	{
		var url = "switch.do?prefix=/estimates&page=/OMEstSanctionAction.do&mode=Submit";
		frm.action = url;
		frm.submit();
	}
}
...
{
	if(validateOMEstTechForm(frm))
	{
		if(!isSelected(elem,id,'Please select atleast one estimate to sanction.'))
			return false;
		else
		return true;
	}
...
<html:javascript formName="OMEstTechForm" />
...
<c:set var="submittedForAEE" value="${requestScope.submittedEstimates[0]}" />
<c:set var="submittedForDEE" value="${requestScope.submittedEstimates[1]}" />
<c:set var="submittedForEE" value="${requestScope.submittedEstimates[2]}" />
<c:set var="submittedForSE" value="${requestScope.submittedEstimates[3]}" />
<c:set var="submittedForCE" value="${requestScope.submittedEstimates[4]}" />

<c:set var="showSanPanel" value="false" />
<c:set var="pending" value="false" />

<html:form action="OMEstSanctionAction">
...
<td>
	<jsp:include page="/commons/TableHeader.jsp">
		<jsp:param name="TableName" value="O&M Estimates - Technical Sanctions" />
...
<td>
	<fieldset>
	<legend>Technical Sanction Details</legend>
	<label>
		<table>
		<tbody class="formTBody">
		<tr>
			<td class="column1"><bean:message key="app.tsOffice" /><span class="mandatory">*</span></td>
			<td class="column2">
				<html:text property="tsOffice" styleClass="thin-text-box" readonly="true" />
			</td>
			<td class="column3"><bean:message key="app.tsGivenBy" /><span class="mandatory">*</span></td>
			<td class="column4">
		...
		<tr>
			<td class="column1"><bean:message key="app.tsNo" /><span class="mandatory">*</span></td>
			<td class="column2">
				<html:text property="tsEndtNo" styleClass="thin-text-box" 
						   onkeypress="upperOnly()" />
			</td>
			<td class="column3"><bean:message key="app.tsDate" /><span class="mandatory">*</span></td>
			<td class="column4">
				<html:text property="tsEndtDate" styleClass="thin-text-box" maxlength="10" size="14"/>
				<a href="javascript: showCalendar(document.OMEstTechForm.tsEndtDate);">
					<img src="<rws:context page='/images/calendar.gif'/>" border="0" align="absmiddle">
				</a>
			</td>
		</tr>
		<tr>
			<td class="column1"><bean:message key="app.tsAmount" /><span class="mandatory">*</span></td>
			<td class="column2">
				<html:text property="tsAmount" styleClass="thin-text-box" 
						   onkeypress="upperOnly()" />
			</td>
			<td class="column3"><bean:message key="app.ssrYear" /><span class="mandatory">*</span></td>
			<td class="column4">
...
<label>
<table width="775" cellpadding="0" cellspacing="0" border="1"
...
<c:set var="index" value="${status.index}" />
	<tr class='<c:out value="row${index%2}" />'>
		...
		<td align="center" width="110" nowrap><c:out value="${item.workId}" /></td>
		<td width="80"><c:out value="${item.sourceCode}" /></td>
		<td align="center" width="60" nowrap><c:out value="${item.financialYear}" /></td>
		<td align="center" width="80"><c:out value="${item.totalComponents}" /> 
			<input type="button" value="..." name="<c:out value='${item.workId}'/>"
			class="btext" onclick="javascript: fnShowWorkComponents(this, 'om')">
		</td>
		...
		<td align="center" width="45">
			<input type="button" class="btext" value="Edit" 
				   onclick="fnEditEstimate(this.form,'<c:out value='${item.workId}'/>','om')"
...
<c:set var="index" value="${status.index}" />
	<tr class='<c:out value="row${index%2}" />'>
		<td align="center" width="40"><input type="checkbox"
			name="sel" id="ee" value='<c:out value="${item.workId}"/>'>
		</td>
		<td align="center" width="110" nowrap><c:out value="${item.workId}" /></td>
		<td width="80"><c:out value="${item.sourceCode}" /></td>
		<td align="center" width="60" nowrap><c:out value="${item.financialYear}" /></td>
...
<tbody class="label">
<c:forEach items="${estimatesForSE}" var="item" varStatus="status">
<c:set var="index" value="${status.index}" />
	<tr class='<c:out value="row${index%2}" />'>
		<td align="center" width="40"><input type="checkbox"
...
<tfoot>
	<tr bgcolor="#8A9FCD">
		...
		<tr>
			<th align="left">&nbsp;
				<a href="javascript: checkAll('sel','se')" id="white"><bean:message key="app.checkAll"/></a>
				<a href="javascript: clearAll('sel','se')" id="white"><bean:message key="app.clearAll"/></a>
			</th>
			<th align="right" class="label">
				<b>No. of Estimates:&nbsp;<c:out value="${index+1}" />&nbsp;</b>
			</th>
...
<table>
	<tr>
		<td>
			<input type="button" class="btext" name="cmbShowHideSubmit" value="Submit" style="width:75px"
				   onclick="javascript:fnShowHideSubmit('sel','se')"
...
<tfoot>
	<tr bgcolor="#8A9FCD">
		...
		<tr>
			<th align="left">&nbsp;
				<a href="javascript: checkAll('sel','ce')" id="white"><bean:message key="app.checkAll"/></a>
				<a href="javascript: clearAll('sel','ce')" id="white"><bean:message key="app.clearAll"/></a>
			</th>
			<th align="right" class="label">
				<b>No. of Estimates:&nbsp;<c:out value="${index+1}" />&nbsp;</b>
			</th>
...
<c:set var="officeCode" value="" />
	...
	<tr class='<c:out value="row${index%2}" />'>
		...
		<td align="center" width="110" nowrap style="display:block;cursor:hand"
...
<tbody class="label">
<c:forEach items="${submittedForSE}" var="item" varStatus="status">
<c:set var="index" value="${status.index}" />
<c:set var="officeCode" value="" />
	<c:if test="${officeCode != item.officeCode}">
		<c:set var="officeCode" value="${item.officeCode}" />
...
<tfoot>
	...
	<td colspan="10">
		...
		<tr>
			<th align="left">&nbsp;
				 <a href="javascript: checkAll('sel','sub_se')" id="white"><bean:message key="app.checkAll"/></a>
...
<table width="100%">
	<c:if test="${RWS_USER.officeName != 'Circle Office'}">
		<tr>
			<td align="left">
				<input type="button" class="btext"	name="cmbShowHideSubmit" value="Submit" 
					style="width:75px" onclick="javascript:fnShowHideSubmit('sel','sub_se')"
				   <c:if test="${RWS_USER.headOfOffice == 'false'}">
				   disabled = "disabled"
				   </c:if>
				>
...
<c:set var="officeCode" value="" />
	...
	<tr class='<c:out value="row${index%2}" />'>
		<td align="center" width="40">
			<input type="checkbox" name="sel" id="sub_ce" value='<c:out value="${item.workId}"/>'>
		</td>
		<td align="center" width="110" nowrap style="display:block;cursor:hand"
			onmouseover='<c:out value="dddrivetip(\"${item.message}\")" escapeXml="false" />'
			onmouseout="hideddrivetip()"> 
...
<table width="100%">
	<c:if test="${RWS_USER.officeName != 'Head Office'}">
		<tr>
			<td align="left">
				<input type="button" class="btext"	name="cmbShowHideSubmit" value="Submit" 
					style="width:75px" onclick="javascript:fnShowHideSubmit('sel','sub_ce')"
				   <c:if test="${RWS_USER.headOfOffice == 'false'}">
				   disabled = "disabled"
				   </c:if>
				>
...
<table align="left">
<tbody class="formTBody">
	<logic:notEmpty name="OMEstTechForm" property="submitTo">
	<tr>
		<td><bean:message key="app.submitTo" /></td>
		<td>:</td>
		<td colspan="4">
			<bean:write name="OMEstTechForm" property="submitOffice" />
			 <html:hidden property="submitTo" /> </span>
		</td>
	</tr>
	</logic:notEmpty>
	<tr>
	...
	<tr>
		...
		<td>:</td>
		<td>
			<html:text property="submitDate" maxlength="10" styleClass="thin-text-box" size="14" /> 
			<a href="javascript: showCalendar(document.OMEstTechForm.submitDate);">
				<img src="<rws:context page='/images/calendar.gif'/>" border="0" align="absmiddle">
			</a>
		</td>
	</tr>
	...
	<tr>
		<td colspan="3" align="center">
			<html:button property="mode" styleClass="btext" onclick="validateSubmit(this.form)">
...
<iframe src="" name="rws_temp" border="0" frameborder="0" width="200" height="100"
