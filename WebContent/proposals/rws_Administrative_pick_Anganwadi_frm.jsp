...
<script language="JavaScript">
	function carrySchools(){
		var subdivisionOfficeCode = '<%=subdivisionOfficeCode%>';
		var prooved = '<%=prooved%>';
		var assetcode = '<%=assetcode%>';
		var txt = document.getElementById("schools");
		var selectSchools = "false";
		for(i=0;i<txt.length;i++){
			if(txt.options[i].selected==true){
				selectSchools="true";	
			}
		}
		if(selectSchools=="true"){
			var s ="";
			var count = 0;
			for(i=0;i<txt.length;i++){
				if(txt.options[i].selected==true){
			...
			if(confirm("Are you sure to Add Selected Anganwadis("+count+") .\nPress OK to continue...")){
			...
			alert("Select Anganwadi to Add Anganwadis");
		}
	}
	...
	function AjaxFun1(destinationname){
		...
		if(selectHab=="true"){
			...
			var postData = "";
			var openRequest = getAjaxRequest();
			var urll = "/pred/proposals/AnganwadiesGet.jsp?mandals="+s;       
			openRequest.open("POST",urll,false);
			postData=s;
			openRequest.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
			openRequest.send(postData);
			document.getElementById("schools1").innerHTML="<select name=schools multiple size=10 class=hightlongcombo>"+openRequest.responseText+"</select>";
		}
		else{
			alert("Select Habitation to Get Anganwadis");
		}
	}
...
<body method="post"leftmargin="0" topmargin="0" bgcolor="#edf2f8">
	<html:form action="SaveAdminHabitation" onsubmit="return fnAdd(this)">
		...
		<logic:notEmpty name="mandals">
					...
					<td>
						<table width="100%" cellspacing="0" cellpadding="2" border="1" bordercolor="#000000">
							<tr>
								...
								<td width="5%">&nbsp;</td>
								<td width="70%"  align=center><FONT size="2" face="verdana"><b>Anganwadis : Anganwadi Code-Anganwadi Name-Location</b></FONT></td>
							</tr>
							<%int i=0; %>
							<tr>
								<td>
								...
								<td>
									<input type="button" value="Get Anganwadis>>>" onClick="AjaxFun1('schools')">
								</td>
								<td id="schools1">
									<select name="schools" multiple size="10" class="hightlongcombo"></select>
								</td>
							</tr>
						</table>
			...
			<br/>
			<center>
				<html:button property="mode" value="Add Anganwadis" styleClass="btext" onclick="carrySchools()"/>
