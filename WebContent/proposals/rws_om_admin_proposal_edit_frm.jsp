...
{
	var flag = false;
	if(validateEditOMAdminProposalForm(EditOMAdminProposalForm))
	{
...
{
	var frm = that.form;
	var url = "switch.do?prefix=/proposals&page=/OMAdminProposalData.do";
	frm.action = url;
	frm.submit();
}
//-->
...
<html:javascript formName="EditOMAdminProposalForm" />

<%@ include file="/commons/rws_header2.jsp" %>

<html:form action="/EditOMAdminProposal" method="post" enctype="multipart/form-data">
...
<label>
	...
	<tr>
		...
		<td class="column2">
			<html:text property="proposalId" readonly="true" styleClass="thin-text-box" />
		</td>
	...
	<tr>
		...
		<td colspan="3" class="column2">
			<html:text property="others" disabled="true" onkeypress="upperOnly()"
	...
	<tr>
		<td class="column1"><bean:message key="app.sanctionedAmount"/><span class="mandatory">*</span></td>
		<td class="column2">
			<html:text property="sanctionedAmount" styleClass="thin-text-box" onkeypress="return decimalsOnly(event)"/>
		</td>
		<td class="column3"><bean:message key="app.remarks" /></td>
		<td class="column4">
