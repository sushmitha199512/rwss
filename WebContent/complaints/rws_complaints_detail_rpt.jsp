...
{
		 qry="select   complaint_no,to_char(complaint_date,'dd/mm/yyyy'),complaint_desc,decode(category_code,'01','LEAKING','02','PUMP DAMAGED')result,nvl(to_char(attended_date,'dd/mm/yyyy'),'-'),nvl(attended_by,'-'),nvl(complaint_status,'-'),nvl(a.remarks,'-') from rws_complaints_register_tbl a,rws_asset_mast_tbl b where b.asset_code=a.asset_code and  substr(source_code,1,2)='"+dcode+"' and attended_date is null order by complaint_no  ";
}
if(request.getParameter("attend").equals("2"))
{
         qry="select   complaint_no,to_char(complaint_date,'dd/mm/yyyy'),complaint_desc,decode(category_code,'01','LEAKING','02','PUMP DAMAGED')result,nvl(to_char(attended_date,'dd/mm/yyyy'),'-'),nvl(attended_by,'-'),nvl(complaint_status,'-'),nvl(a.remarks,'-') from rws_complaints_register_tbl a,rws_asset_mast_tbl b where b.asset_code=a.asset_code and  substr(source_code,1,2)='"+dcode+"' and attended_date is not  null order by complaint_no  ";
}
		...
		%>
				...
				<td class=rptValue NOWRAP><%=rs2.getString(7)%></td>
