...
function openDeleteWindow(url)
{
	 ...
	 var left = parseInt((screen.availWidth/2)-(width/2));
	 var top = parseInt((screen.availHeight/2)-(height/2));
	 	 //var url = "/pred/complaints/Complaints.do?mode=cdelete&complntno="+complntno;
     var properties = "width="+width+",height="+height+",screenX="+left+",screenY="+top+",toolbar=no,"
				   + "status=no,menubar=no,location=no,directories=no,scrollbars=yes,copyhistory=no,"
				   + "resizable=yes,status=yes";
		newWindow = window.open(url, "_New", properties);
...
function fnDelete(complntno,typeofAsset,dcode)
	...
	{
	document.forms[0].action="switch.do?prefix=/complaints&page=/Complaints.do?mode=cdelete&complntno="+complntno+"&typeofAsset="+typeofAsset+"&dcode="+dcode;
	document.forms[0].submit();
	}
	}
function fnEdit(complntno,typeofAsset,dcode)
{
 document.forms[0].action="switch.do?prefix=/complaints&page=/Complaints.do?mode=get&complntno="+complntno+"&typeofAsset="+typeofAsset+"&dcode="+dcode;
 document.forms[0].submit();
}
</script>
<table width="100%">
...
<a href="#1" onclick="openEditWindow('switch.do?prefix=/complaints&page=/Complaints.do?mode=get&complntno=<bean:write name='COMPLAINTSLIST' property='complntno' />')">Edit&nbsp;|</a>&nbsp;
...
<a href="#1" onclick="openDeleteWindow('switch.do?prefix=/complaints&page=/Complaints.do?mode=cdelete&complntno=<bean:write name='COMPLAINTSLIST' property='complntno' />')">
