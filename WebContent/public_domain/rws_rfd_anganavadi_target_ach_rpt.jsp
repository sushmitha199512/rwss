...
<tr><td align="right" class="bwborder" colspan="2">
                    <a href="rws_rfd_anganavadi_target_ach_excel.jsp" target="_new">Excel</a>|<a href="javascript:onclick=history.go(-1)">Back</a></td></tr>       
...
<!-- Grid Table Starts -->                                      
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
                                        ...
                                        <tr >
		<td align="center" class="gridhdbg" colspan=11><font color="white">Drinking Water  In Anganwadi Centers. <%=cYear %>-<%=nYear %></font></td>	
	</tr>
		<tr >
...
<td class="gridhdbg" align=center >Achievement&nbsp;UpTo (<%=prevmonth %>-<%=cYear %>)</td>
...
int count=1;
  try{
	  ...
	  Query.delete(0,Query.length());
	   Query.append("select *from rws_rfd_cs_rpt_tmp_tbl where rpt_type='ANG' order by HAB_ACH_PER desc ");
		 ...
		 count=1;
		  ...
		  <tr><td style="text-align: left;" class=<%=style %>><%=count++ %></td><td style="text-align: left;" class=<%=style %> ><%=rst.getString(2) %></td><td style="text-align: right;" class=<%=style %> ><%=rst.getString(3) %></td><td style="text-align: right;" class=<%=style %> > <a href="#" onclick="viewWorks('rws_rfd_sch_ang_target_ach_rpt_drill.jsp?dcode=<%=rst.getString(1) %>&type=ang')"><%=rst.getString(5) %></a></td>
	  ...
	  %>
	  <tr><td class="gridhdbg" style="text-align: right;"  colspan="2">Total </td><td style="text-align: right;"  class="gridhdbg"><%=(int)svsT %></td><td style="text-align: right;" class="gridhdbg" ><%=(int)svsH %></td>
 ...
 %>
				   ...
				   Please Click Here to View Graph<a href="<rws:context page='/public_domain/rws_rfd_ang_target_ach_graph.jsp' />" target="_new"><img src="<rws:context page='/images/chart.png' />"  width="40" height="40"/></a></td>
				</tr>
</table>
