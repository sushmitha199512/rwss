...
var popFeatures = "width=900,height=650,toolbar=0,location=0,directories=0,status=1,menuBar=0,scrollBars=1,resizable=1,screenX=10,screenY=10,left=10,top=30";
  ...
  <tr><td align="right" class="bwborder" colspan="2">
                    <a href="./rws_rfd_mvs_target_ach_rel_excel.jsp" target="_new">Excel</a>|<a href="javascript:onclick=history.go(-1)" >Back</a></td></tr>  
			</table>
...
int count=1;
  try{
	  ...
	  Query.append("select d.dcode,d.dname ,count(*) from rws_work_admn_tbl a,rws_work_comp_hab_lnk_tbl b,rws_district_tbl d,rws_complete_hab_view ch  where ch.panch_code=b.hab_code and a.work_id in(select work_id from rws_work_completion_tbl where date_of_completion is not null and to_date(date_of_completion,'dd/mm/yy')>='01-APR-"+cYear.toString()+"' ) and a.WORK_CANCEL_DT is null and a.category_code='1' and  a.type_of_asset in('01','02','03')   and a.work_id=b.work_id and d.dcode=substr(a.work_id,5,2)  and substr(a.work_id,1,2) in ("+selectedProgramsPhy.toString()+") group by d.dcode,d.dname order by d.dcode");
	  ...
	  Query.delete(0,Query.length());
	  Query.append("select d.dcode,d.dname ,(sum(BILL_AMOUNT_CENTRAL)/100000)  from rws_work_admn_tbl a,rws_district_tbl d, RWS_bill_bank_bridge_det_TBL v  where  a.WORK_CANCEL_DT is null and a.category_code='1'    and v.work_id=a.work_id and d.dcode=substr(a.work_id,5,2)  and substr(a.work_id,1,2) in ("+selectedProgramsFin.toString()+") and to_date(v.BANK_SEND_DATE,'dd/mm/yy')>='01-APR-"+cYear.toString()+"' and  to_date(v.BANK_SEND_DATE,'dd/mm/yy')<'01-"+currentMonth.toString()+"-"+cYear.toString()+"'  group by d.dcode,d.dname order by d.dcode");
...
/**********************************************/
	  ...
	  Query.delete(0,Query.length());
	  Query.append("select d.dcode,d.dname ,(sum(BILL_AMOUT_STATE)/100000)  from rws_work_admn_tbl a,rws_district_tbl d, rws_bill_pao_bridge_det_tbl v  where  a.WORK_CANCEL_DT is null and a.category_code='1'    and v.work_id=a.work_id and d.dcode=substr(a.work_id,5,2)  and substr(a.work_id,1,2) in ("+selectedProgramsFin.toString()+") and to_date(v.STATE_SEND_DATE,'dd/mm/yy')>='01-APR-"+cYear.toString()+"' and  to_date(v.STATE_SEND_DATE,'dd/mm/yy')<'01-"+currentMonth.toString()+"-"+cYear.toString()+"'  group by d.dcode,d.dname order by d.dcode");
...
/**********************************************/
	  ...
	  Query.delete(0,Query.length());
	  Query.append("select d.dcode,d.dname ,(sum(BILL_AMOUNT_CENTRAL)/100000)  from rws_work_admn_tbl a,rws_district_tbl d, RWS_bill_bank_bridge_det_TBL v  where a.WORK_CANCEL_DT is null and a.category_code='1'     and v.work_id=a.work_id and d.dcode=substr(a.work_id,5,2)  and substr(a.work_id,1,2) in ("+selectedProgramsFin.toString()+") and to_date(v.BANK_SEND_DATE,'dd/mm/yy')>='01-"+currentMonth.toString()+"-"+cYear.toString()+"' and  to_date(v.BANK_SEND_DATE,'dd/mm/yy')<'01-"+nextMonth.toString()+"-"+cYear.toString()+"'  group by d.dcode,d.dname order by d.dcode");
	  ...
	  Query.append(" and  w.programme_code in ("+selectedProgramsPhy.toString()+") group by d.dcode");
	  	...
	  	//System.out.println("spillOverWorks qry"+Query.toString());
	  	 rs2=stmt2.executeQuery(Query.toString());
	  ...
	  Query.delete(0,Query.length());
	  Query.append("select d.dcode,d.dname ,(sum(BILL_AMOUT_STATE)/100000)  from rws_work_admn_tbl a,rws_district_tbl d, rws_bill_pao_bridge_det_tbl v  where a.WORK_CANCEL_DT is null and a.category_code='1'     and v.work_id=a.work_id and d.dcode=substr(a.work_id,5,2)  and substr(a.work_id,1,2) in ("+selectedProgramsFin.toString()+") and to_date(v.STATE_SEND_DATE,'dd/mm/yy')>='01-"+currentMonth.toString()+"-"+cYear.toString()+"' and  to_date(v.STATE_SEND_DATE,'dd/mm/yy')<'01-"+nextMonth.toString()+"-"+cYear.toString()+"'  group by d.dcode,d.dname order by d.dcode");
...
/**********************************************/
	  ...
	  Query.delete(0,Query.length());
	  Query.append("select d.dcode,d.dname,t.MVS_HABS,(t.MVS_EXP*100)  from rws_district_tbl d , rws_work_sch_target_tbl t where t.dcode=d.dcode and FIN_YEAR='2013-2014'");
	  ...
	  while(rs3.next()){
		  ...
		  expStr.delete(0,expStr.length());
 			if(spillOverGroundedWorksExpHash.get(key.toString())!=null){
 			 ...
 			 expStr.append((String.valueOf(spillOverGroundedWorksExpHash.get(key.toString()))));
...
/******************************************************/
		...
		style.delete(0,style.length());
		 ...
		 styleCount++;
		  ...
		  <td class="<%=style %>" style="text-align: right;"><a href="#" onclick="viewWorks('rws_rfd_svs_target_ach_rpt_drill.jsp?dcode=<%=rs3.getString(1) %>&type=1')"><%=svsTargetHabs %></a></td><td class="<%=style %>" style="text-align: right;" ><%=ndf.format(bankpaoupto) %></td><td class="<%=style %>" style="text-align: right;" ><%=ndf.format(bankpaocurrent) %></td><td class="<%=style %>" style="text-align: right;" ><%=ndf.format(bankpaotot) %></td></tr>
	  ...
	  Query.delete(0,Query.length());
	  Query.append("select programme_name ,programme_code from rws_programme_tbl where programme_code in ("+selectedProgramsPhy+") order by 2 ");
