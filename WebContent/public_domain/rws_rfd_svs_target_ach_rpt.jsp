...
<tr><td align="right" class="bwborder" colspan="2">
                    <a href="./rws_rfd_svs_target_ach_excel.jsp" target="_new">Excel</a>|<a href="javascript:onclick=history.go(-1)" >Back</a></td></tr>       
...
<td class="gridhdbg" colspan=4 align=center >Achievement (<%=cYear %>-<%=nYear %>) <br><font color="red"><b>Amount In Lakhs.</b></font></br></td>
 <td class="gridhdbg" colspan=3 align=center >Percentage Achieved</td>
</tr>
<tr>
...
int count=1;
  try{
	  ...
	  String selectedProgramsFin="'01','02','18','23','32','48','49','51','58','59','60','46','50'";
	 ...
	 System.out.println("Q2:"+svsExpLastMonthQuery);
	  ...
	  String svsExpLastMonthDirectQuery="select d.dcode,d.dname ,(sum(VOUCHER_AMT)/100000)  from rws_work_admn_tbl a,rws_district_tbl d, rws_work_direct_voucher_tbl v  where   a.WORK_CANCEL_DT is null and a.category_code='2'    and v.work_id=a.work_id and d.dcode=substr(a.work_id,5,2)  and substr(a.work_id,1,2) in ("+selectedProgramsDirectVouchersFin+") and to_date(v.VOUCHER_DT,'dd/mm/yy')>='01-APR-"+cYear+"' and  to_date(v.VOUCHER_DT,'dd/mm/yy')<'01-"+currentMonth+"-"+nYearStr+"'  group by d.dcode,d.dname order by d.dcode";
...
/**********************************************/
	  ...
	  String svsExpCurrentMonthDirectQuery="select d.dcode,d.dname ,(sum(VOUCHER_AMT)/100000)  from rws_work_admn_tbl a,rws_district_tbl d, rws_work_direct_voucher_tbl v  where    a.WORK_CANCEL_DT is null and a.category_code='2'     and v.work_id=a.work_id and d.dcode=substr(a.work_id,5,2)  and substr(a.work_id,1,2) in ("+selectedProgramsDirectVouchersFin+") and to_date(v.VOUCHER_DT,'dd/mm/yy')>='01-"+currentMonth+"-"+nYearStr+"' and  to_date(v.VOUCHER_DT,'dd/mm/yy')<'01-"+nextMonth+"-"+nYearStr+"'  group by d.dcode,d.dname order by d.dcode";
...
stmt4=conn.createStatement();
	  ...
	  String svsTotExpDirectQuery="select d.dcode,d.dname ,(sum(VOUCHER_AMT)/100000)  from rws_work_admn_tbl a,rws_district_tbl d, rws_work_direct_voucher_tbl v  where   a.WORK_CANCEL_DT is null and a.category_code='2'   and v.work_id=a.work_id and d.dcode=substr(a.work_id,5,2)  and substr(a.work_id,1,2) in ("+selectedProgramsDirectVouchersFin+") and to_date(v.VOUCHER_DT,'dd/mm/yy')>='01-APR-"+cYear+"' and  to_date(v.VOUCHER_DT,'dd/mm/yy')<'01-APR-"+nYearStr+"'  group by d.dcode,d.dname order by d.dcode";
	  ...
	  String distQuery="select d.dcode,d.dname,t.SVS_HABS,(t.SVS_EXP*100)  from rws_district_tbl d , rws_work_sch_target_tbl t where t.dcode=d.dcode and FIN_YEAR='2013-2014' and d.dcode<>'16' ";
...
/******************************************************/
	 ...
	 count=1;
	     while(rst.next()){
		  ...
		  <td class="<%=style %>" style="text-align: right;"><a href="#" onclick="viewWorks('rws_rfd_svs_target_ach_rpt_drill.jsp?dcode=<%=rst.getString(1) %>&type=2')"><%=rst.getString(5) %></a></td>
 ...
 String colorValueHab="",colorValueExp="";
	  ...
	  %>
	  <tr><td  style="text-align: right;"  class="gridhdbg" colspan="2">Total </td><td style="text-align: left;"  class="gridhdbg"  ><%=(int)svsT %></td><td class="gridhdbg" style="text-align: right;"><%=ndf.format(expT)+".00" %></td><td class="gridhdbg"  style="text-align: right;"><%=(int)svsH %></td><td class="gridhdbg" style="text-align: right;" ><%=ndf.format(expUpto) %></td><td class="gridhdbg" style="text-align: right;" ><%=ndf.format(expCurrent) %></td><td class="gridhdbg" style="text-align: right;" > <%=ndf.format(expTot) %></td>
