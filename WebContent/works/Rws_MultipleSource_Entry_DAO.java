...
public class Rws_MultipleSource_Entry_DAO
{
	...
	public ArrayList getWorkDetails(String Workid,HttpServletRequest request)
	{
		...
		try{
	         ...
	         String qry=" SELECT distinct a.work_id,e.work_name,mname,pname,HAB_CODE, c.SOURCE_TYPE_NAME,d.SUBSOURCE_TYPE_NAME,SOURCE_CODE ,a.SOURCE_NAME from rws_source_iden_fin_tbl a,rws_mandal_tbl b , RWS_SOURCE_type_TBL c, RWS_subSOURCE_type_TBL d ,rws_work_admn_tbl e,rws_panchayat_tbl f  where a.work_id=e.work_id and a.WORK_ID =? and b.dcode=substr(a.hab_Code,1,2) and b.mcode=substr(a.hab_Code,6,2) and f.dcode=substr(a.hab_Code,1,2) and f.mcode=substr(a.hab_Code,6,2) and f.pcode=substr(a.hab_Code,13,2) and c. SOURCE_TYPE_CODE= a. SOURCE_TYPE_CODE and c.SOURCE_TYPE_CODE=d.SOURCE_TYPE_CODE  and  d.SUBSOURCE_TYPE_CODE=a.SUBSOURCE_TYPE_CODE";
...
public ArrayList getSourceCodes(String sourceType,String habCode) throws Exception
{
	...
	try {
		...
		pst.setString(2, sourceType.substring(4, 5));
		pst.setString(3, sourceType.substring(5, 6));
		rset = stmt.executeQuery(query);
		while (rset.next()) 
		{
...
{   
	...
	try {
		...
		if (comp.equals("newSource"))
		{
			...
			pstat = conn.prepareStatement(insQuery);
			pstat.setString(1,ValidtateString(sourceCode));
			pstat.setString(2,ValidtateString(sourceName));
			pstat.setString(3,ValidtateString(worksBean.getSubcompType());
			pstat.setString(4,ValidtateString(worksBean.getSubSourceType());
			pstat.setString(5,ValidtateString(worksBean.getSitehab()+","+workCode));
			pstat.executeUpdate();
		}
		query = "insert into rws_source_iden_fin_tbl(HAB_CODE,SOURCE_CODE,SOURCE_TYPE_CODE,SUBSOURCE_TYPE_CODE,SITE_HAND_OVER_DATE,REMARKS,"
				...
				+ "?,?,?,?,to_date('30/04/2013','dd/mm/yyyy'),?,?,?,?,?,?,SYSDATE)";
		
		Debug.println("insert source identification query:" + query);
		assetCode = worksBean.getSchemeCode();
		//assetCode = workCode.substring(6, 8);
		pstat = conn.prepareStatement(query);
		pstat.setString(1,ValidtateString(worksBean.getSitehab()));
		pstat.setString(2, ValidtateString(sourceCode));
		pstat.setString(3, ValidtateString(worksBean.getSubcompType()));
		pstat.setString(4, ValidtateString(worksBean.getSubSourceType());
		pstat.setString(5,  ValidtateString(worksBean.getRecommend());
		pstat.setString(6, ValidtateString(worksBean.getSourceIdentby());
		pstat.setString(7, ValidtateString(officeCode));
		pstat.setString(8, ValidtateString(workCode));
		pstat.setString(9, ValidtateString(worksBean.getSchemeCode()));
		pstst.setString(10, ValidtateString(sourceName));
