...
<html:html>
    ...
    <table    width="70%" border=1 style="border-collapse:collapse"  ALIGN=CENTER >
        <%if (mode.equals("qcWorks") ||mode.equals("cWorks") || mode.equals("gWorks") || mode.equals("tWorks") || mode.equals("fWorks") || mode.equals("uWorks") || mode.equals("comWorks") || mode.equals("ngWorks")) {%>
        <tr align=center>
            ...
            <%if(mode.equals("qcWorks") ||mode.equals("cWorks") || mode.equals("tWorks") || mode.equals("comWorks")) {%>
        ...
        <%
            ...
            try {
                ...
                if (mode.equals("tWorks")) {
                	...
                	{ */query = "select distinct a.SANCTION_AMOUNT, a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP ,'dd/mm/yyyy'),'--'),to_char(b.DATE_OF_COMPLETION,'dd/mm/yyyy'),ADMIN_NO,dis.dname,nvl(a.lead_habitation,'--') as lead_habitation,nvl(a.office_code,'--') as office_code "
                	        + "from RWS_WORK_COMP_HAB_LNK_TBL h,rws_habitation_directory_tbl hd,rws_work_admn_tbl a,rws_work_completion_tbl b,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m,rws_district_tbl dis,rws_quality_control_tbl qc  where dis.dcode=substr(a.work_id,5,2) and "
                	        + "WORK_CANCEL_DT is null and b.work_id=a.work_id and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id "
                	        + "and h.work_id=a.work_id and hd.hab_code=h.hab_code and a.programme_code in ("+pc+")   and b.DATE_OF_COMPLETION is not null and "
                	        + " TARGET_DATE_COMP is not null  and  a.CORE_DASHBOARD_STATUS='Y' and b.work_id=qc.work_id and to_date(to_char(qc.inspection_date,'dd/mm/yyyy'),'dd/mm/yyyy') >=  to_date('"+fromDate+"','dd/mm/yyyy') and to_date(to_char(qc.inspection_date,'dd/mm/yyyy'),'dd/mm/yyyy') <= to_date('"+toDate+"','dd/mm/yyyy') ";
                	//}
...
{ */ 
	/* query = "select distinct a.SANCTION_AMOUNT, a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP ,'dd/mm/yyyy'),'--'),to_char(b.DATE_OF_COMPLETION,'dd/mm/yyyy'),ADMIN_NO,dis.dname,nvl(a.lead_habitation,'--') as lead_habitation,to_char(b.DATE_OF_COMM,'dd/mm/yyyy') as DATE_OF_COMM,nvl(a.office_code,'--') as office_code "
        ...
        query = "select distinct a.SANCTION_AMOUNT, a.work_id,a.work_name,nvl(to_char(admin_date,'dd/mm/yyyy'),'--'),nvl(to_char(TARGET_DATE_COMP ,'dd/mm/yyyy'),'--'),to_char(b.DATE_OF_COMPLETION,'dd/mm/yyyy'),ADMIN_NO,dis.dname,nvl(a.lead_habitation,'--') as lead_habitation,to_char(b.DATE_OF_COMM,'dd/mm/yyyy') as DATE_OF_COMM,nvl(a.office_code,'--') as office_code "
                + "from RWS_WORK_COMP_HAB_LNK_TBL h,rws_habitation_directory_tbl hd,rws_work_admn_tbl a,rws_work_completion_tbl b,RWS_SOURCE_IDEN_FIN_TBL i,RWS_WORK_COMMENCEMENT_TBL c ,rws_contractor_selection_tbl m,rws_district_tbl dis  where dis.dcode=substr(a.work_id,5,2) and "
                + "WORK_CANCEL_DT is null and b.work_id=a.work_id and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id "
                + "and h.work_id=a.work_id and hd.hab_code=h.hab_code and a.programme_code in ("+pc+") and b.DATE_OF_COMPLETION is not null and b.DATE_OF_COMM is not null and "
                + " TARGET_DATE_COMP is not null  and  a.CORE_DASHBOARD_STATUS='Y'";
        //}
        ...
        + "and TO_DATE(TARGET_DATE_COMP)>='01-APR-" + yearStart + "' and i.work_id(+)=a.work_id  and c.work_id(+)=a.work_id and m.work_id(+)=a.work_id and a.TYPE_OF_ASSET in (" + schemes + ") and a.CORE_DASHBOARD_STATUS='Y'";
               ...
               //System.out.println("WOrk Related :-:::::: "+ query);
                ...
                while (rs.next()) {
                    ...
                    String inspection_date_query = "select nvl(to_char(inspection_date,'dd/mm/yyyy'),'--')  from rws_quality_control_tbl where work_id='" + rs.getString(2) + "' ";
                  // System.out.println("%%%"+inspection_date_query);
                    statement = conn.createStatement();
                    resultSet = statement.executeQuery(inspection_date_query);
                    if (resultSet.next()) {
                    	inspection_date = resultSet.getString(1);
                    } else {
                    	inspection_date = "--";
                    }
                    statement.close();
 ...
 --%>                  <td class="<%=style%>" style="text-align: left;" align="right"><a href="#" onclick="viewWorks('./rws_work_progress_min_abstract_drill.jsp?mode=fhtcforsacdrill&wid=<%=rs.getString(2)%>&pc=<%=pc%>&pn=<%=pn%>&yearStart=<%=yearStart%>&yearEnd=<%=yearEnd%>&schemeName=<%=schemeName %>')"><%=noofFHTSSanc%></a></td>
            ...
            <%
                ...
                <td class="<%=style%>" style="text-align: left;" align="right"><%=ndf.format(billAmount/ 100000)%></td>
                 <% if(mode.equals("qcWorks")){%>
            <%-- <td class=gridhdbg ><%=inspection_date%></td> --%>
            <td class="<%=style%>" style="text-align: left;" align="right"><%=inspection_date%></td>
            <%} %>
                <%
                statement.close();
                resultSet.close();
