...
//alert("selectedHabs"+selectedHabs);
	var len;
	var isAllowed = false;
	if(selectedHabs)
	{
			...
			if(confirm("Do you want add the selected habitations?"))
			{
				...
				var openerDoc = opener.document;
				 var sourcecode=document.getElementById('sourcecode').value;
		 	 	...
		 	 	document.forms[0].action="./masters/rws_save_selected_habs_crash.jsp?sourcecode="+sourcecode;
...
System.out.println("sourceCode in pick habs nuuuss"+sourcecode);
...
<html:form action="HabiYield"  method="POST"  >
<html:hidden property="sourceCode" value="<%=request.getParameter(\"sourceCode\")%>"/>
<html:hidden property="type" value="<%=request.getParameter(\"type\")%>"/>
<logic:notEmpty name="habitations21">
...
<c:forEach items="${habitations21}" var="item" varStatus="status">
...
<logic:empty name="habitations21">
