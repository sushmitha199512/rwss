...
if(aaa!=null)
	System.out.println("habs size in jsp.:"+aaa.size());
 %>
<script language="JavaScript">
<!--
function getData()
{
	document.forms[0].action="switch.do?prefix=/masters&page=/MuncipalSubdivision.do&mode=showMandals";
...
//		var index = that.value;
	...
	for (var i=0; i < document.forms[0].selectedHabCode.length; i++)
  	{
   		if (document.forms[0].selectedHabCode[i].checked)
    	{
       		//rad_val = document.forms[0].selectedHabCode[i].value;
       		//rad_val_text = document.forms[0].selectedHabCode[i].text;
       		var val = document.forms[0].selectedHabCode[i].value;
       		//alert(val);
   ...
   if(rad_val=='')
   {
   	...
   	else
		{
			...
			frm.action = "switch.do?prefix=/masters&page=/MuncipalSubdivision.do?mode=populateVillage&vcode="+rad_val+"&vname="+rad_text;
			frm.submit();
			window.close();
		}
	}
