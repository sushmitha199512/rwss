...
<%
//System.out.println("masters/rws_habitation_abstract_rpt1.jsp");
...
<table border = 0 cellspacing = 0 cellpadding = 0 height=5% width="100%" bordercolor=#000000 style="border-collapse:collapse" bgcolor="#ffffff" align = "center">	
		...
		<td class=rptLabel colspan=20>Select Type:<font color="red">*</font><SELECT name="poptype" id="poptype" style="width:140px" onchange="this.form.submit()" >
			...
			<option value="PLAIN">PLAIN</option>
				 ...
				 <TR align="center">
		<td class=rptLabel colspan=20>As Per LPCD:<font color="red">*</font><SELECT name="lpcdrange" id="lpcdrange" style="width:140px" onchange="this.form.submit()" >
...
try
{
	...
	String scstplain="";
		...
		while(rs2.next())
		{
			...
			pQuery.delete(0, pQuery.length());
			pQuery.append("select distinct substr(h.panch_code,1,2)||substr(h.panch_code,6,2)||substr(h.panch_code,13,2) from rws_district_tbl d,rws_mandal_tbl m,rws_panchayat_tbl p,rws_village_tbl v,rws_complete_hab_view h,"+habTableName+" hd where d.dcode=m.dcode and m.dcode=p.dcode and m.mcode =p.mcode and p.dcode=v.dcode and p.mcode=v.mcode and p.pcode=v.pcode and substr (h.panch_code,1,2) = v.dcode and substr(h.panch_code,6,2) = v.mcode and substr(h.panch_code,13,2) = v.pcode and substr (h.panch_code,8,3) = v.vcode and h.panch_code=hd.hab_code and hd.coverage_status is not null and hd.coverage_status <> 'UI'  and d.dcode='"+rs2.getString(1)+"' ");
			vQuery.delete(0, vQuery.length());
			vQuery.append("select distinct substr(h.panch_code,1,2)||substr(h.panch_code,6,2)||substr(h.panch_code,13,2)||substr(h.panch_code,8,3) from rws_district_tbl d,rws_mandal_tbl m,rws_panchayat_tbl p,rws_village_tbl v,rws_complete_hab_view h,"+habTableName+" hd where d.dcode=m.dcode and m.dcode=p.dcode and m.mcode =p.mcode and p.dcode=v.dcode and p.mcode=v.mcode and p.pcode=v.pcode and substr (h.panch_code,1,2) = v.dcode and substr(h.panch_code,6,2) = v.mcode and substr(h.panch_code,13,2) = v.pcode and substr (h.panch_code,8,3) = v.vcode and h.panch_code=hd.hab_code and hd.coverage_status is not null and hd.coverage_status <> 'UI'  and d.dcode='"+rs2.getString(1)+"' ");
			//Query modified by pranavi
			hQuery.delete(0, hQuery.length());
		...
		if(poptype.toString()!=null && !poptype.toString().equals("") && !poptype.toString().equals("ALL") ){
				...
				{
			hQuery.append(" and (0.4)*(nvl(hd.CENSUS_PLAIN_POPU,'0')+nvl(hd.CENSUS_SC_POPU,'0')+nvl(hd.CENSUS_ST_POPU,'0')+nvl(hd.CENSUS_MINORITY_POPU,'0'))<= hd.CENSUS_SC_POPU  ");
				}
			...
			hquery1.delete(0, hquery1.length());
			hquery1.append("select distinct h.panch_code,nvl(hd.CENSUS_PLAIN_POPU,0)+nvl(hd.CENSUS_SC_POPU,0)+nvl(hd.CENSUS_ST_POPU,0),nvl(hd.total_water_supply,0),nvl(hd.safe_lpcd,0),nvl(hd.unsafe_lpcd,0) from rws_district_tbl d,rws_mandal_tbl m,rws_panchayat_tbl p,rws_village_tbl v,rws_complete_hab_view h,"+habTableName+" hd where d.dcode=m.dcode and m.dcode=p.dcode and m.mcode =p.mcode and p.dcode=v.dcode and p.mcode=v.mcode and p.pcode=v.pcode and substr (h.panch_code,1,2) = v.dcode and substr(h.panch_code,6,2) = v.mcode and substr(h.panch_code,13,2) = v.pcode and substr (h.panch_code,8,3) = v.vcode and h.panch_code=hd.hab_code and hd.coverage_status is not null and hd.coverage_status <> 'UI'  and d.dcode='"+rs2.getString(1)+"' ");
			if(poptype.toString()!=null && !poptype.toString().equals("") && !poptype.toString().equals("ALL")){
				
				if(poptype.toString().equals("SC"))
				{
					hquery1.append(" and (0.4)*(nvl(hd.CENSUS_PLAIN_POPU,'0')+nvl(hd.CENSUS_SC_POPU,'0')+nvl(hd.CENSUS_ST_POPU,'0')+nvl(hd.CENSUS_MINORITY_POPU,'0'))<= hd.CENSUS_SC_POPU  ");
				}
				else if(poptype.toString().equals("ST"))
				{
...
}else
{
	...
	else if(safelpcd>=13.75 && safelpcd<27.50){count3++; PC2pop=PC2pop+total_pop;}
	else if(safelpcd>=27.50 && safelpcd<41.25){count4++; PC3pop=PC3pop+total_pop;}
	...
	if(safelpcd==0){count7++;  NCpop=NCpop+total_pop;}
		...
		//date = new Date();
			...
			<tr>
				...
				<td class="<%=style%>" style="text-align: right;"><a href="#" onclick="viewWorks('/pred12052015/reports/rws_habitation_abstract_drill_rpt.jsp?rDistrict=<%=rs2.getString(1)%>&rDistrictName=<%=rs2.getString(2)%>&finyear=<%= finYear%>&lpcdrange=<%=lpcdrange%>&poptype=<%=poptype%>&coverageStatus=<%=FC%>')"><%=count1%></a></td>
				<td class="<%=style%>" style="text-align: right;"><%=FCpop%></td>
				<td class="<%=style%>" style="text-align: right;"><a href="#" onclick="viewWorks('/pred12052015/reports/rws_habitation_abstract_drill_rpt.jsp?rDistrict=<%=rs2.getString(1)%>&rDistrictName=<%=rs2.getString(2)%>&finyear=<%= finYear%>&lpcdrange=<%=lpcdrange%>&poptype=<%=poptype%>&coverageStatus=<%=PC4%>')"><%=count5%></a></td>
				<td class="<%=style%>" style="text-align: right;"><%=PC4pop%></td>
				<td class="<%=style%>" style="text-align: right;"><a href="#" onclick="viewWorks('/pred12052015/reports/rws_habitation_abstract_drill_rpt.jsp?rDistrict=<%=rs2.getString(1)%>&rDistrictName=<%=rs2.getString(2)%>&finyear=<%= finYear%>&lpcdrange=<%=lpcdrange%>&poptype=<%=poptype%>&coverageStatus=<%=PC3%>')"><%=count4%></a></td>
				<td class="<%=style%>" style="text-align: right;"><%=PC3pop%></td>
				<td class="<%=style%>" style="text-align: right;"><a href="#" onclick="viewWorks('/pred12052015/reports/rws_habitation_abstract_drill_rpt.jsp?rDistrict=<%=rs2.getString(1)%>&rDistrictName=<%=rs2.getString(2)%>&finyear=<%= finYear%>&lpcdrange=<%=lpcdrange%>&poptype=<%=poptype%>&coverageStatus=<%=PC2%>')"><%=count3%></a></td>
				<td class="<%=style%>" style="text-align: right;"><%=PC2pop%></td>
				<td class="<%=style%>" style="text-align: right;"><a href="#" onclick="viewWorks('/pred12052015/reports/rws_habitation_abstract_drill_rpt.jsp?rDistrict=<%=rs2.getString(1)%>&rDistrictName=<%=rs2.getString(2)%>&finyear=<%= finYear%>&lpcdrange=<%=lpcdrange%>&poptype=<%=poptype%>&coverageStatus=<%=PC1%>')"><%=count2%></a></td>
				<td class="<%=style%>" style="text-align: right;"><%=PC1pop%></td>
				<td class="<%=style%>" style="text-align: right;"><a href="#" onclick="viewWorks('/pred12052015/reports/rws_habitation_abstract_drill_rpt.jsp?rDistrict=<%=rs2.getString(1)%>&rDistrictName=<%=rs2.getString(2)%>&finyear=<%= finYear%>&lpcdrange=<%=lpcdrange%>&poptype=<%=poptype%>&coverageStatus=<%=NSS%>')"><%=count6%></a></td>
				<td class="<%=style%>" style="text-align: right;"><%=NSSpop%></td>
				<td class="<%=style%>" style="text-align: right;"><a href="#" onclick="viewWorks('/pred12052015/reports/rws_habitation_abstract_drill_rpt.jsp?rDistrict=<%=rs2.getString(1)%>&rDistrictName=<%=rs2.getString(2)%>&finyear=<%= finYear%>&lpcdrange=<%=lpcdrange%>&poptype=<%=poptype%>&coverageStatus=<%=NC%>')"><%=count7%></a></td>
...
}else
{
	if(safelpcd==0){count7++;  NCpop=NCpop+total_pop;}
		...
		<tr>
			<td class="<%=style%>"><%=sno++%></td>
			<td class="rptValue"  style="text-align: left;"><a href="rws_habitation_abstract_rpt1.jsp?drill=next1&rDistrict=<%=rDistrict%>&rDistrictName=<%=rDistrictName%>&rMandal=<%=rs2.getString(1)%>&rMandalName=<%=rs2.getString(2)%>&finyear=<%= finYear%>&poptype=<%=poptype%>&lpcdrange=<%=lpcdrange%>"><%=rs2.getString(2)%></a></td>
			<td class="<%=style%>" style="text-align: right;"><%=pcount%></td>
			<td class="<%=style%>" style="text-align: right;"><%=vcount%></td>
			<td class="<%=style%>" style="text-align: right;"><%=hcount%></td>
			<td class="<%=style%>" style="text-align: right;"><%=totalpop%></td>
		...
		<%
		System.out.println("masters/rws_habitation_abstract_rpt1.jsp columnTotal4:"+columnTotal4);
		columnTotal1 += pcount;columnTotal2 += vcount;columnTotal3 += hcount;columnTotal4 += count1;
		columnTotal5 += count5;columnTotal6 += count4;columnTotal7 += count3;columnTotal8 += count2;
		columnTotal9 += count6;columnTotal10 += count7;FCpopTotal +=FCpop;PC4popTotal += PC4pop;
		Gtotalpop+=totalpop;
...
function fncheck(){
	alert(1);
	 ...
	 window.location.replace("rws_habitation_abstract_rpt1.jsp?lpcdrange="+lpcdrange);
	alert("lpcdrange"+lpcdrange);
...
try
{
	...
	getDnameQuery.append("select mcode,mname,dname from rws_mandal_tbl m,rws_district_tbl d where d.dcode = m.dcode and d.dcode = '"+rDistrict.toString()+"' order by m.mcode");
		...
		%>
		<tr>
			<td class="btext" colspan="2">District:<%=dnamee.toString()%></td><td class="btext" colspan="1">Type:<%=poptype%></td><td class="btext" colspan="17">LPCDRange:<%=lpcdrange2%></td>
		</tr>
		<%
		rs2.previous();
		int sno = 1;double Lpcd=0.0;
